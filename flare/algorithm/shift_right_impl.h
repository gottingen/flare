...
IteratorType shift_right_exespace_impl(
  ...
  /*
    ...
    step 1:
      we create a temporary tensor with extent = distance(first, last-n)
      and *move* assign the elements from [first, last-n) to tmp tensor, such that
      tmp tensor becomes:

      | 0  | 1  |  2 | 1  | 2  | 1  | 2  | 2  | 10 |

    step 2:
      move elements of tmp tensor back to range starting at first+n.
   */
  ...
  tmp_tensor_type tmp_tensor("shift_right_impl", num_elements_to_move);
  ...
  ::flare::parallel_for(label,
                         ...
                         step2_func_type(begin(tmp_tensor), first + n));
