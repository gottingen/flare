...
#ifndef FLARE_ALGORITHM_SEARCH_H_
#define FLARE_ALGORITHM_SEARCH_H_

#include <flare/algorithm/search_impl.h>
...
IteratorType1 search(const ExecutionSpace& ex, IteratorType1 first,
  ...
  return detail::search_exespace_impl("flare::search_iterator_api_default", ex,
...
IteratorType1 search(const std::string& label, const ExecutionSpace& ex,
  ...
  return detail::search_exespace_impl(label, ex, first, last, s_first, s_last);
}
...
auto search(const ExecutionSpace& ex,
  ...
  return detail::search_exespace_impl("flare::search_tensor_api_default", ex,
                                    KE::begin(tensor), KE::end(tensor),
                                    KE::begin(s_tensor), KE::end(s_tensor));
}
...
auto search(const std::string& label, const ExecutionSpace& ex,
  ...
  namespace KE = ::flare::experimental;
  return detail::search_exespace_impl(label, ex, KE::begin(tensor), KE::end(tensor),
                                    KE::begin(s_tensor), KE::end(s_tensor));
}
...
auto search(const ExecutionSpace& ex,
  ...
  return detail::search_exespace_impl("flare::search_tensor_api_default", ex,
                                    KE::begin(tensor), KE::end(tensor),
                                    KE::begin(s_tensor), KE::end(s_tensor), pred);
}
...
FLARE_FUNCTION IteratorType1 search(const TeamHandleType& teamHandle,
                                     IteratorType1 first, IteratorType1 last,
                                     IteratorType2 s_first,
                                     IteratorType2 s_last) {
  return detail::search_team_impl(teamHandle, first, last, s_first, s_last);
}

template <typename TeamHandleType, typename DataType1, typename... Properties1,
          typename DataType2, typename... Properties2,
          std::enable_if_t<::flare::is_team_handle_v<TeamHandleType>, int> = 0>
FLARE_FUNCTION auto search(
  ...
  namespace KE = ::flare::experimental;
  return detail::search_team_impl(teamHandle, KE::begin(tensor), KE::end(tensor),
                                KE::begin(s_tensor), KE::end(s_tensor));
}
...
FLARE_FUNCTION IteratorType1 search(const TeamHandleType& teamHandle,
                                     ...
                                     IteratorType2 s_last,
                                     const BinaryPredicateType& pred) {
  return detail::search_team_impl(teamHandle, first, last, s_first, s_last, pred);
}
...
#endif  // FLARE_ALGORITHM_SEARCH_H_
