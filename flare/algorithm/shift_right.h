...
auto shift_right(const ExecutionSpace& ex,
  ...
  return detail::shift_right_exespace_impl("flare::shift_right_tensor_api_default",
...
auto shift_right(const std::string& label, const ExecutionSpace& ex,
                 ...
                 typename decltype(begin(tensor))::difference_type n) {
  detail::static_assert_is_admissible_to_flare_std_algorithms(tensor);
  return detail::shift_right_exespace_impl(label, ex, begin(tensor), end(tensor), n);
}
...
FLARE_FUNCTION auto shift_right(
    ...
    typename decltype(begin(tensor))::difference_type n) {
  detail::static_assert_is_admissible_to_flare_std_algorithms(tensor);
  return detail::shift_right_team_impl(teamHandle, begin(tensor), end(tensor), n);
}
