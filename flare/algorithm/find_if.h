...
#ifndef FLARE_ALGORITHM_FIND_IF_H_
#define FLARE_ALGORITHM_FIND_IF_H_
...
IteratorType find_if(const ExecutionSpace& ex, IteratorType first,
  ...
  return detail::find_if_or_not_exespace_impl<true>(
      "flare::find_if_iterator_api_default", ex, first, last,
      std::move(predicate));
}
...
IteratorType find_if(const std::string& label, const ExecutionSpace& ex,
  ...
  return detail::find_if_or_not_exespace_impl<true>(label, ex, first, last,
                                                  std::move(predicate));
}
...
FLARE_FUNCTION IteratorType find_if(const TeamHandleType& teamHandle,
                                     IteratorType first, IteratorType last,
                                     PredicateType predicate) {
  return detail::find_if_or_not_team_impl<true>(teamHandle, first, last,
                                              std::move(predicate));
}
...
FLARE_FUNCTION auto find_if(const TeamHandleType& teamHandle,
  ...
  detail::static_assert_is_admissible_to_flare_std_algorithms(v);
  namespace KE = ::flare::experimental;
  return detail::find_if_or_not_team_impl<true>(teamHandle, KE::begin(v),
...
#endif  // FLARE_ALGORITHM_FIND_IF_H_
