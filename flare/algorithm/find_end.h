...
#ifndef FLARE_ALGORITHM_FIND_END_H_
#define FLARE_ALGORITHM_FIND_END_H_

#include <flare/algorithm/find_end_impl.h>
...
IteratorType1 find_end(const ExecutionSpace& ex, IteratorType1 first,
  ...
  return detail::find_end_exespace_impl("flare::find_end_iterator_api_default",
...
IteratorType1 find_end(const std::string& label, const ExecutionSpace& ex,
  ...
  return detail::find_end_exespace_impl(label, ex, first, last, s_first, s_last);
}
...
auto find_end(const ExecutionSpace& ex,
  ...
  return detail::find_end_exespace_impl("flare::find_end_tensor_api_default", ex,
...
auto find_end(const std::string& label, const ExecutionSpace& ex,
  ...
  namespace KE = ::flare::experimental;
  return detail::find_end_exespace_impl(label, ex, KE::begin(tensor), KE::end(tensor),
                                      KE::begin(s_tensor), KE::end(s_tensor));
}
...
FLARE_FUNCTION IteratorType1 find_end(const TeamHandleType& teamHandle,
                                       IteratorType1 first, IteratorType1 last,
                                       IteratorType2 s_first,
                                       IteratorType2 s_last) {
  return detail::find_end_team_impl(teamHandle, first, last, s_first, s_last);
}

template <typename TeamHandleType, typename DataType1, typename... Properties1,
          typename DataType2, typename... Properties2,
          std::enable_if_t<::flare::is_team_handle_v<TeamHandleType>, int> = 0>
FLARE_FUNCTION auto find_end(
  ...
  namespace KE = ::flare::experimental;
  return detail::find_end_team_impl(teamHandle, KE::begin(tensor), KE::end(tensor),
                                  KE::begin(s_tensor), KE::end(s_tensor));
}
...
#endif  // FLARE_ALGORITHM_FIND_END_H_
