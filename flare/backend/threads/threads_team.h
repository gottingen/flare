...
#ifndef FLARE_BACKEND_THREADS_THREADS_TEAM_H_
#define FLARE_BACKEND_THREADS_THREADS_TEAM_H_
...
class ThreadsExecTeamMember {
 ...
 public:
  using execution_space      = flare::Threads;
 ...
 public:
  ...
  FLARE_INLINE_FUNCTION bool team_fan_in() const {
    int n, j;
    ...
    for (n = 1;
         ...
         n <<= 1) {
      detail::spinwait_while_equal<int>(m_team_base[j]->state(),
                                      ThreadsExec::Active);
    }
    ...
    if (m_team_rank_rev) {
      ...
      detail::spinwait_while_equal<int>(m_exec->state(), ThreadsExec::Rendezvous);
    }

    return !m_team_rank_rev;
  }

  FLARE_INLINE_FUNCTION void team_fan_out() const {
    int n, j;
    for (n = 1;
         (!(m_team_rank_rev & n)) && ((j = m_team_rank_rev + n) < m_team_size);
         n <<= 1) {
 ...
 public:
  FLARE_INLINE_FUNCTION static int team_reduce_size() {
  ...
  FLARE_INLINE_FUNCTION int team_rank() const { return m_team_rank; }
  FLARE_INLINE_FUNCTION int team_size() const { return m_team_size; }
  ...
  FLARE_INLINE_FUNCTION void team_broadcast(ValueType& value,
                                             const int& thread_id) const {
    FLARE_IF_ON_DEVICE(((void)value; (void)thread_id;))
  ...
  FLARE_INLINE_FUNCTION void team_broadcast(Closure const& f, ValueType& value,
                                             const int& thread_id) const {
    FLARE_IF_ON_DEVICE(((void)f; (void)value; (void)thread_id;))
  ...
  FLARE_INLINE_FUNCTION
      std::enable_if_t<!flare::is_reducer<Type>::value, Type>
      team_reduce(const Type& value) const {
    FLARE_IF_ON_DEVICE((return value;))
  ...
  template <typename ReducerType>
  FLARE_INLINE_FUNCTION
      std::enable_if_t<flare::is_reducer<ReducerType>::value>
      team_reduce(const ReducerType& reducer,
                  const typename ReducerType::value_type contribution) const {
    FLARE_IF_ON_DEVICE(((void)reducer; (void)contribution;))
  ...
  FLARE_INLINE_FUNCTION ArgType team_scan(const ArgType& value,
                                           ArgType* const global_accum) const {
    FLARE_IF_ON_DEVICE(((void)global_accum; return value;))

    FLARE_IF_ON_HOST((  // Make sure there is enough scratch space:
   ...
   */
  template <typename ArgType>
  FLARE_INLINE_FUNCTION ArgType team_scan(const ArgType& value) const {
  ...
  ThreadsExecTeamMember(
      detail::ThreadsExec* exec,
      const TeamPolicyInternal<flare::Threads, Properties...>& team,
    ...
    if (team.league_size()) {
      ...
      if (std::is_same<typename TeamPolicyInternal<
                           flare::Threads, Properties...>::schedule_type::type,
                       flare::Dynamic>::value) {
        m_exec->barrier();
      }
    } else {
...
namespace detail {
template <class... Properties>
class TeamPolicyInternal<flare::Threads, Properties...>
 ...
 private:
  ...
  inline void init(const int league_size_request, const int team_size_request) {
    ...
    const int max_host_team_size = detail::HostThreadTeamData::max_team_members;
    const int team_max =
 ...
 public:
  ...
  TeamPolicyInternal(
      const TeamPolicyInternal<flare::Threads, OtherProperties...>& p) {
  ...
  int team_size_max(const FunctorType&, const ParallelForTag&) const {
    ...
    int max_host_team_size = detail::HostThreadTeamData::max_team_members;
 ...
 private:
  ...
  inline void set_auto_chunk_size() {
    ...
    if (m_chunk_size > 0) {
      if (!detail::is_integral_power_of_two(m_chunk_size))
        flare::abort("TeamPolicy blocking granularity must be power of two");
    }
 ...
 public:
  using member_type = detail::ThreadsExecTeamMember;

  friend class detail::ThreadsExecTeamMember;
};
...
FLARE_INLINE_FUNCTION
    detail::TeamThreadRangeBoundariesStruct<iType, detail::ThreadsExecTeamMember>
    TeamThreadRange(const detail::ThreadsExecTeamMember& thread,
                    const iType& count) {
  return detail::TeamThreadRangeBoundariesStruct<iType,
                                               detail::ThreadsExecTeamMember>(
      thread, count);
}

template <typename iType1, typename iType2>
FLARE_INLINE_FUNCTION detail::TeamThreadRangeBoundariesStruct<
    std::common_type_t<iType1, iType2>, detail::ThreadsExecTeamMember>
TeamThreadRange(const detail::ThreadsExecTeamMember& thread, const iType1& begin,
                const iType2& end) {
...
FLARE_INLINE_FUNCTION
    detail::TeamThreadRangeBoundariesStruct<iType, detail::ThreadsExecTeamMember>
    TeamVectorRange(const detail::ThreadsExecTeamMember& thread,
                    const iType& count) {
  return detail::TeamThreadRangeBoundariesStruct<iType,
                                               detail::ThreadsExecTeamMember>(
      thread, count);
...
FLARE_INLINE_FUNCTION detail::TeamThreadRangeBoundariesStruct<
    std::common_type_t<iType1, iType2>, detail::ThreadsExecTeamMember>
TeamVectorRange(const detail::ThreadsExecTeamMember& thread, const iType1& begin,
                const iType2& end) {
  using iType = std::common_type_t<iType1, iType2>;
  return detail::TeamThreadRangeBoundariesStruct<iType,
                                               detail::ThreadsExecTeamMember>(
...
template <typename iType>
FLARE_INLINE_FUNCTION
    detail::ThreadVectorRangeBoundariesStruct<iType, detail::ThreadsExecTeamMember>
    ThreadVectorRange(const detail::ThreadsExecTeamMember& thread,
                      const iType& count) {
  return detail::ThreadVectorRangeBoundariesStruct<iType,
                                                 detail::ThreadsExecTeamMember>(
      thread, count);
}
...
FLARE_INLINE_FUNCTION
detail::ThreadSingleStruct<detail::ThreadsExecTeamMember> PerTeam(
    const detail::ThreadsExecTeamMember& thread) {
  return detail::ThreadSingleStruct<detail::ThreadsExecTeamMember>(thread);
}

FLARE_INLINE_FUNCTION
detail::VectorSingleStruct<detail::ThreadsExecTeamMember> PerThread(
    const detail::ThreadsExecTeamMember& thread) {
  return detail::VectorSingleStruct<detail::ThreadsExecTeamMember>(thread);
}
}  // namespace flare
...
FLARE_INLINE_FUNCTION void parallel_for(
    const detail::TeamThreadRangeBoundariesStruct<
        iType, detail::ThreadsExecTeamMember>& loop_boundaries,
    const Lambda& lambda) {
...
template <typename iType, class Lambda>
FLARE_INLINE_FUNCTION void parallel_for(
    const detail::ThreadVectorRangeBoundariesStruct<
        iType, detail::ThreadsExecTeamMember>& loop_boundaries,
    const Lambda& lambda) {
...
template <typename iType, class Lambda, typename ValueType>
FLARE_INLINE_FUNCTION std::enable_if_t<!flare::is_reducer<ValueType>::value>
parallel_reduce(const detail::ThreadVectorRangeBoundariesStruct<
                    iType, detail::ThreadsExecTeamMember>& loop_boundaries,
                const Lambda& lambda, ValueType& result) {
  result = ValueType();
  for (iType i = loop_boundaries.start; i < loop_boundaries.end;
...
FLARE_INLINE_FUNCTION void parallel_scan(
    const detail::TeamThreadRangeBoundariesStruct<
        iType, detail::ThreadsExecTeamMember>& loop_bounds,
...
FLARE_INLINE_FUNCTION void parallel_scan(
    ...
    const FunctorType& lambda) {
  using value_type =
      typename detail::FunctorAnalysis<detail::FunctorPatternInterface::SCAN,
...
FLARE_INLINE_FUNCTION void single(
    const detail::VectorSingleStruct<
        detail::ThreadsExecTeamMember>& /*single_struct*/,
    const FunctorType& lambda) {
  lambda();
}
...
template <class FunctorType>
FLARE_INLINE_FUNCTION void single(
    const detail::ThreadSingleStruct<detail::ThreadsExecTeamMember>& single_struct,
    const FunctorType& lambda) {
...
#endif  // FLARE_BACKEND_THREADS_THREADS_TEAM_H_
