...
#ifndef FLARE_BACKEND_THREADS_THREADS_EXEC_H_
#define FLARE_BACKEND_THREADS_THREADS_EXEC_H_
...
class ThreadsExec {
 ...
 private:
  ...
  alignas(16) flare::pair<long, long> m_work_range;
 ...
 public:
  FLARE_INLINE_FUNCTION int pool_size() const { return m_pool_size; }
  FLARE_INLINE_FUNCTION int pool_rank() const { return m_pool_rank; }
  FLARE_INLINE_FUNCTION int numa_rank() const { return m_numa_rank; }
  FLARE_INLINE_FUNCTION int numa_core_rank() const { return m_numa_core_rank; }
  ...
  FLARE_INLINE_FUNCTION void *scratch_memory() const {
    return reinterpret_cast<unsigned char *>(m_scratch) + m_scratch_reduce_end;
  }

  FLARE_INLINE_FUNCTION int volatile &state() { return m_pool_state; }
  FLARE_INLINE_FUNCTION ThreadsExec *const *pool_base() const {
    return m_pool_base;
  }
  ...
  inline int all_reduce(const int value) {
    ...
    *static_cast<volatile int *>(reduce_memory()) = value;
    ...
    if (rev_rank) {
      ...
      detail::spinwait_while_equal<int>(m_pool_state, ThreadsExec::Rendezvous);
    } else {
  ...
  inline void fan_in_reduce(const FunctorType &f) const {
    ...
    for (int i = 0; i < m_pool_fan_size; ++i) {
      ...
      detail::spinwait_while_equal<int>(fan.m_pool_state, ThreadsExec::Active);
  ...
  inline void scan_large(const FunctorType &f) {
    ...
    if (rev_rank) {
      ...
      if ((1 << m_pool_fan_size) < (m_pool_rank + 1)) {
        ...
        detail::spinwait_while_equal<int>(th.m_pool_state, ThreadsExec::Active);
        detail::spinwait_while_equal<int>(th.m_pool_state,
      ...
      detail::spinwait_while_equal<int>(m_pool_state, ThreadsExec::ScanAvailable);
    }
    ...
    for (int i = 0; i < m_pool_fan_size; ++i) {
      ThreadsExec &fan = *m_pool_base[rev_rank + (1 << i)];
      // Wait: ReductionAvailable -> ScanAvailable
      detail::spinwait_while_equal<int>(fan.m_pool_state,
                                      ThreadsExec::ReductionAvailable);
    ...
    if (rev_rank) {
      ...
      detail::spinwait_while_equal<int>(m_pool_state, ThreadsExec::ScanCompleted);
    }
  ...
  static void internal_fence(
      detail::fence_is_static is_static = detail::fence_is_static::yes);
  static void internal_fence(
      const std::string &,
      detail::fence_is_static is_static = detail::fence_is_static::yes);
  static bool sleep();
  ...
  inline long get_work_index_begin() {
    flare::pair<long, long> work_range_new = m_work_range;
    flare::pair<long, long> work_range_old = work_range_new;
    ...
    bool success = false;
    while (!success) {
      work_range_new = flare::atomic_compare_exchange(
...
inline int Threads::in_parallel() { return detail::ThreadsExec::in_parallel(); }

inline int Threads::impl_is_initialized() {
  return detail::ThreadsExec::is_initialized();
}

inline void Threads::impl_initialize(InitializationSettings const &settings) {
  detail::ThreadsExec::initialize(
      settings.has_num_threads() ? settings.get_num_threads() : -1);
}

inline void Threads::impl_finalize() { detail::ThreadsExec::finalize(); }

inline void Threads::print_configuration(std::ostream &os, bool verbose) const {
  os << "Host Parallel Execution Space:\n";
  os << "  FLARE_ENABLE_THREADS: yes\n";

  os << "\nThreads Runtime Configuration:\n";
  detail::ThreadsExec::print_configuration(os, verbose);
}

inline void Threads::impl_static_fence(const std::string &name) {
  detail::ThreadsExec::internal_fence(name, detail::fence_is_static::yes);
}
} /* namespace flare */

#endif  // FLARE_BACKEND_THREADS_THREADS_EXEC_H_
