...
struct Sentinel {
  ~Sentinel() {
    if (s_thread_pool_size[0] || s_thread_pool_size[1] ||
        ...
        s_threads_exec[0]) {
      std::cerr << "ERROR : Process exiting while flare::Threads is still "
                   "initialized"
                << std::endl;
    }
  }
...
ThreadsExec::ThreadsExec()
  ...
  if (&s_threads_process != this) {
    ...
    const int entry = reinterpret_cast<size_t>(s_current_function_arg) <
                          ...
                          : size_t(flare::hwloc::bind_this_thread(
    ...
    if (entry < s_thread_pool_size[0] &&
      ...
      const std::pair<unsigned, unsigned> coord =
          flare::hwloc::get_this_thread_coordinate();
...
ThreadsExec::~ThreadsExec() {
  ...
  using Record = flare::detail::SharedAllocationRecord<flare::HostSpace, void>;
...
ThreadsExec *ThreadsExec::get_thread(const int init_thread_rank) {
  ...
  if (nullptr == th || th->m_pool_rank != init_thread_rank) {
    ...
    msg << "flare::detail::ThreadsExec::get_thread ERROR : "
...
void ThreadsExec::execute_sleep(ThreadsExec &exec, const void *) {
  ...
  for (int i = 0; i < n; ++i) {
    detail::spinwait_while_equal<int>(
...
void ThreadsExec::fence() { internal_fence(detail::fence_is_static::yes); }
void ThreadsExec::fence(const std::string &name) {
  internal_fence(name, detail::fence_is_static::yes);
}

void ThreadsExec::internal_fence(detail::fence_is_static is_static) {
  internal_fence((is_static == detail::fence_is_static::no)
                     ? "flare::ThreadsExec::fence: Unnamed Instance Fence"
                     : "flare::ThreadsExec::fence: Unnamed Static Fence",
                 is_static);
}

// Wait for root thread to become inactive
void ThreadsExec::internal_fence(const std::string &name,
                                 detail::fence_is_static is_static) {
  const auto &fence_lam = [&]() {
    if (s_thread_pool_size[0]) {
      // Wait for the root thread to complete:
      detail::spinwait_while_equal<int>(s_threads_exec[0]->m_pool_state,
                                      ThreadsExec::Active);
    }
  ...
  if (is_static == detail::fence_is_static::yes) {
    flare::Tools::experimental::detail::profile_fence_event<flare::Threads>(
        name,
...
void ThreadsExec::execute_resize_scratch_in_serial() {
  ...
  auto deallocate_scratch_memory = [](ThreadsExec &exec) {
    if (exec.m_scratch) {
      using Record =
          flare::detail::SharedAllocationRecord<flare::HostSpace, void>;
  ...
  if (s_threads_process.m_scratch_thread_end) {
    // Allocate tracked memory:
    {
      using Record =
          flare::detail::SharedAllocationRecord<flare::HostSpace, void>;
      Record *const r =
          Record::allocate(flare::HostSpace(), "flare::thread_scratch",
...
void *ThreadsExec::resize_scratch(size_t reduce_size, size_t thread_size) {
  enum { ALIGN_MASK = flare::detail::MEMORY_ALIGNMENT - 1 };
...
void ThreadsExec::print_configuration(std::ostream &s, const bool detail) {
  ...
  const unsigned numa_count     = flare::hwloc::get_available_numa_count();
  const unsigned cores_per_numa = flare::hwloc::get_available_cores_per_numa();
  ...
  s << " FLARE_ENABLE_THREADS";
#endif
#if defined(FLARE_ENABLE_HWLOC)
...
void ThreadsExec::initialize(int thread_count_arg) {
  ...
  if (!is_initialized) {
    ...
    const bool hwloc_avail = flare::hwloc::available();
    const bool hwloc_can_bind =
        hwloc_avail && flare::hwloc::can_bind_threads();

    if (thread_count == 0) {
      thread_count = hwloc_avail
                         ? flare::hwloc::get_available_numa_count() *
    ...
    const unsigned thread_spawn_begin = hwloc::thread_mapping(
        "flare::Threads::initialize", allow_asynchronous_threadpool,
    ...
    if (!thread_spawn_failed) {
      ...
      // occurred
      if (hwloc_can_bind) {
        flare::hwloc::bind_this_thread(proc_coord);
      }
  ...
  if (is_initialized || thread_spawn_failed) {
    ...
    msg << "flare::Threads::initialize ERROR";
  ...
  if (flare::show_warnings() &&
    ...
    std::cerr << "flare::Threads::initialize WARNING: You are likely "
...
void ThreadsExec::finalize() {
  ...
  if (flare::hwloc::can_bind_threads()) {
    flare::hwloc::unbind_this_thread();
  }
...
void Threads::fence(const std::string &name) const {
  detail::ThreadsExec::internal_fence(name, detail::fence_is_static::no);
}
...
int Threads::impl_thread_pool_rank_host() {
  ...
  int i                     = 0;
  while ((i < detail::s_thread_pool_size[0]) && (pid != detail::s_threads_pid[i])) {
    ++i;
  }
  return i;
}
...
int Threads::impl_thread_pool_size(int depth) {
  return detail::s_thread_pool_size[depth];
}
