...
#ifndef FLARE_BACKEND_CUDA_CUDA_REDUCE_SCAN_H_
#define FLARE_BACKEND_CUDA_CUDA_REDUCE_SCAN_H_
...
#include <flare/backend/cuda/cuda_vectorization.h>
 ...
 */
    ...
    __device__ bool cuda_inter_block_reduction(
        ...
        if (id < 32) {
            ...
            if (id == 0) count = flare::atomic_fetch_add(m_scratch_flags, 1);
            count = flare::shfl(count, 0, 32);
            ...
            if (count == gridDim.x - 1) {
                ...
                if (int(blockDim.x * blockDim.y) > 1) {
                    value_type tmp = flare::shfl_down(value, 1, 32);
                    if (id + 1 < int(gridDim.x)) reducer.join(&value, &tmp);
                }
                unsigned int mask = __activemask();
                __syncwarp(mask);
                if (int(blockDim.x * blockDim.y) > 2) {
                    value_type tmp = flare::shfl_down(value, 2, 32);
                    if (id + 2 < int(gridDim.x)) reducer.join(&value, &tmp);
                }
                __syncwarp(mask);
                if (int(blockDim.x * blockDim.y) > 4) {
                    value_type tmp = flare::shfl_down(value, 4, 32);
                    if (id + 4 < int(gridDim.x)) reducer.join(&value, &tmp);
                }
                __syncwarp(mask);
                if (int(blockDim.x * blockDim.y) > 8) {
                    value_type tmp = flare::shfl_down(value, 8, 32);
                    if (id + 8 < int(gridDim.x)) reducer.join(&value, &tmp);
                }
                __syncwarp(mask);
                if (int(blockDim.x * blockDim.y) > 16) {
                    value_type tmp = flare::shfl_down(value, 16, 32);
    ...
    struct CudaReductionsFunctor<FunctorType, false, true> {
        ...
        __device__ static inline void scalar_intra_warp_reduction(
                ...
                const bool skip_vector,  // Skip threads if flare vector lanes are not
            ...
            for (int delta = skip_vector ? blockDim.x : 1; delta < width; delta *= 2) {
                Scalar tmp = flare::shfl_down(value, delta, width, mask);
                functor.join(&value, &tmp);
            }

            detail::in_place_shfl(result, value, 0, width, mask);
        }
        ...
        __device__ static inline void scalar_intra_block_reduction(
            ...
            // Warp Level Reduction, ignoring flare vector entries
        ...
        __device__ static inline bool scalar_inter_block_reduction(
            ...
            if (threadIdx.x + threadIdx.y == 0) {
                num_teams_done =
                        flare::atomic_fetch_add(global_flags, static_cast<unsigned int>(1)) +
                        1;
            }
 ...
 */
    ...
    __device__ void cuda_intra_block_reduce_scan(
        ...
        const unsigned not_less_power_of_two =
                (1 << (detail::int_log2(blockDim.y - 1) + 1));
 ...
 */
    ...
    __device__ bool cuda_single_inter_block_reduce_scan(
        ...
        if (!DoScan && !std::is_pointer<typename FunctorType::reference_type>::value)
            return flare::detail::CudaReductionsFunctor<
...
// Size in bytes required for inter block reduce or scan
    ...
    cuda_single_inter_block_reduce_scan_shmem(const FunctorType &functor,
        ...
        using Analysis =
                detail::FunctorAnalysis<detail::FunctorPatternInterface::SCAN,
    ...
    inline void check_reduced_tensor_shmem_size(const Policy &policy,
        ...
        if (reqShmemSize > maxShmemPerBlock) {
            flare::detail::throw_runtime_exception(
                    "flare::detail::ParallelReduce< Cuda > requested too much L0 scratch "
                    "memory");
        }
    }
...
#endif   // FLARE_BACKEND_CUDA_CUDA_REDUCE_SCAN_H_
