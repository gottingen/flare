...
#ifndef FLARE_BACKEND_CUDA_CUDA_KERNEL_LAUNCH_H_
#define FLARE_BACKEND_CUDA_CUDA_KERNEL_LAUNCH_H_
...
__device__ __constant__ unsigned long flare_impl_cuda_constant_memory_buffer
[flare::detail::CudaTraits::ConstantMemoryUsage / sizeof(unsigned long)];

template<typename T>
inline __device__ T *flare_impl_cuda_shared_memory() {
    extern __shared__ flare::CudaSpace::size_type sh[];
    return (T *) sh;
}
...
// mechanisms
    ...
    struct CudaParallelLaunchKernelFunc<
            DriverType, flare::LaunchBounds<MaxThreadsPerBlock, MinBlocksPerSM>,
            experimental::CudaLaunchMechanism::LocalMemory> {
    ...
    template<class DriverType>
    struct CudaParallelLaunchKernelFunc<
            DriverType, flare::LaunchBounds<0, 0>,
            experimental::CudaLaunchMechanism::LocalMemory> {
    ...
    struct CudaParallelLaunchKernelInvoker<
        ...
        static_assert(sizeof(DriverType) < CudaTraits::KernelArgumentLimit,
                      "flare Error: Requested CudaLaunchLocalMemory with a Functor "
        ...
        inline static void create_parallel_launch_graph_node(
                ...
                CudaInternal const *cuda_instance) {
            //----------------------------------------
            auto const &graph = detail::get_cuda_graph_from_kernel(driver);
            FLARE_EXPECTS(bool(graph));
            auto &graph_node = detail::get_cuda_graph_node_from_kernel(driver);
            // Expect node not yet initialized
            FLARE_EXPECTS(!bool(graph_node));

            if (!detail::is_empty_launch(grid, block)) {
                detail::check_shmem_request(cuda_instance, shmem);
                ...
                experimental_contains_desired_occupancy) {
                    ...
                    detail::configure_shmem_preference<DriverType, LaunchBounds>(
...
//------------------------------------------------------------------------------
    ...
    struct CudaParallelLaunchKernelInvoker<
        ...
        static void invoke_kernel(DriverType const &driver, dim3 const &grid,
            ...
            FLARE_IMPL_CUDA_SAFE_CALL((cuda_instance->cuda_memcpy_async_wrapper(
        ...
        inline static void create_parallel_launch_graph_node(
            ...
            if (!detail::is_empty_launch(grid, block)) {
                ...
                auto *driver_ptr = detail::allocate_driver_storage_for_kernel(driver);
...
//------------------------------------------------------------------------------
    ...
    struct CudaParallelLaunchKernelInvoker<
        ...
        static_assert(sizeof(DriverType) < CudaTraits::ConstantMemoryUsage,
                      "flare Error: Requested CudaLaunchConstantMemory with a "
        ...
        static void invoke_kernel(DriverType const &driver, dim3 const &grid,
            ...
            FLARE_IMPL_CUDA_SAFE_CALL((cuda_instance->cuda_event_synchronize_wrapper(
            ...
            FLARE_IMPL_CUDA_SAFE_CALL(
                    (cuda_instance->cuda_memcpy_to_symbol_async_wrapper(
                            flare_impl_cuda_constant_memory_buffer, staging,
            ...
            if (!detail::is_empty_launch(grid, block)) {
                ...
                experimental_contains_desired_occupancy) {
                    int desired_occupancy =
                            driver.get_policy().impl_get_desired_occupancy().value();
                    size_t block_size = block.x * block.y * block.z;
                    detail::configure_shmem_preference<
                            DriverType,
                            flare::LaunchBounds<MaxThreadsPerBlock, MinBlocksPerSM>>(
                            base_t::get_kernel_func(), cuda_instance->m_deviceProp, block_size,
                            shmem, desired_occupancy);
                }

                flare::ensure_cuda_lock_arrays_on_device();
                ...
                cuda_instance->fence(
                    "flare::detail::launch_kernel: Debug Only Check for Execution Error");
#endif
            }
        }
...
#endif  // FLARE_BACKEND_CUDA_CUDA_KERNEL_LAUNCH_H_
