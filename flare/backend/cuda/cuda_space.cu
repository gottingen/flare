...
#error CUDART_VERSION undefined!
    ...
    if (arg_alloc_size >= memory_threshold_g) {
      ...
      FLARE_IMPL_CUDA_SAFE_CALL(
          (detail::CudaInternal::singleton().cuda_free_async_wrapper(
              arg_alloc_ptr)));
      detail::cuda_device_synchronize(
...
template class SharedAllocationRecordCommon<flare::CudaSpace>;
template class HostInaccessibleSharedAllocationRecordCommon<flare::CudaSpace>;
template class SharedAllocationRecordCommon<flare::CudaUVMSpace>;
template class SharedAllocationRecordCommon<flare::CudaHostPinnedSpace>;
