...
//
#ifndef FLARE_BACKEND_CUDA_CUDA_VECTORIZATION_H_
#define FLARE_BACKEND_CUDA_CUDA_VECTORIZATION_H_
...
#define FLARE_IMPL_CUDA_MAX_SHFL_SIZEOF sizeof(long long)
#else
#define FLARE_IMPL_CUDA_MAX_SHFL_SIZEOF sizeof(int)
#endif
...
struct in_place_shfl_op {
  // CRTP boilerplate
  __device__ FLARE_IMPL_FORCEINLINE const Derived& self() const noexcept {
  ...
  __device__ inline std::enable_if_t<sizeof(Scalar) <= sizeof(int)> operator()(
    ...
    union conv_type {
      ...
      // active member
      FLARE_FUNCTION conv_type() { conv = 0; }
    };
    conv_type tmp_in;
    tmp_in.orig = in;
    shfl_type tmp_out;
...
// TODO: figure out why 64-bit shfl fails in Clang
  ...
  operator()(Scalar& out, const Scalar& val, int lane_or_delta, int width,
             unsigned mask = shfl_all_mask) const noexcept {
    // TODO DSH shouldn't this be FLARE_IMPL_CUDA_MAX_SHFL_SIZEOF instead of
...
struct in_place_shfl_fn : in_place_shfl_op<in_place_shfl_fn> {
  template <class T>
  __device__ FLARE_IMPL_FORCEINLINE T do_shfl_op(unsigned mask, T& val,
...
template <class... Args>
__device__ FLARE_IMPL_FORCEINLINE void in_place_shfl(Args&&... args) noexcept {
...
template <class... Args>
__device__ FLARE_IMPL_FORCEINLINE void in_place_shfl_up(
    Args&&... args) noexcept {
...
template <class... Args>
__device__ FLARE_IMPL_FORCEINLINE void in_place_shfl_down(
    Args&&... args) noexcept {
...
__device__ inline T shfl(const T& val, const int& srcLane, const int& width,
                         unsigned mask = detail::shfl_all_mask) {
  T rv = {};
  detail::in_place_shfl(rv, val, srcLane, width, mask);
  return rv;
}
...
__device__ inline T shfl_down(const T& val, int delta, int width,
                              unsigned mask = detail::shfl_all_mask) {
  T rv = {};
  detail::in_place_shfl_down(rv, val, delta, width, mask);
  return rv;
}
...
__device__ inline T shfl_up(const T& val, int delta, int width,
                            unsigned mask = detail::shfl_all_mask) {
  T rv = {};
  detail::in_place_shfl_up(rv, val, delta, width, mask);
  return rv;
}

}  // end namespace flare

#undef FLARE_IMPL_CUDA_MAX_SHFL_SIZEOF

#endif  // FLARE_ON_CUDA_DEVICE
#endif  // FLARE_BACKEND_CUDA_CUDA_VECTORIZATION_H_
