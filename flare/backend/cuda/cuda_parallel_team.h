...
#ifndef FLARE_BACKEND_CUDA_CUDA_PARALLEL_TEAM_H_
#define FLARE_BACKEND_CUDA_CUDA_PARALLEL_TEAM_H_
...
#include <flare/core/policy/exp_iterate_tile_gpu.h>
...
class TeamPolicyInternal<flare::Cuda, Properties...>
 ...
 public:
  ...
  int team_size_max(const FunctorType& f, const ParallelForTag&) const {
    using closure_type =
        detail::ParallelFor<FunctorType, TeamPolicy<Properties...>>;
    cudaFuncAttributes attr =
        CudaParallelLaunch<closure_type, typename traits::launch_bounds>::
            get_cuda_func_attributes();
    int block_size =
        flare::detail::cuda_get_max_block_size<FunctorType,
  ...
  inline int team_size_max(const FunctorType& f,
    ...
    using functor_analysis_type =
        detail::FunctorAnalysis<detail::FunctorPatternInterface::REDUCE,
                              TeamPolicyInternal, FunctorType, void>;
    using closure_type = detail::ParallelReduce<
        CombinedFunctorReducer<FunctorType,
                               typename functor_analysis_type::Reducer>,
        TeamPolicy<Properties...>, flare::Cuda>;
  ...
  inline int team_size_max(const FunctorType& f, const ReducerType& /*r*/,
                           const ParallelReduceTag&) const {
    using closure_type =
        detail::ParallelReduce<CombinedFunctorReducer<FunctorType, ReducerType>,
                             TeamPolicy<Properties...>, flare::Cuda>;
    return internal_team_size_max<closure_type>(f);
  }
  ...
  int team_size_recommended(const FunctorType& f, const ParallelForTag&) const {
    ...
    const int block_size =
        flare::detail::cuda_get_opt_block_size<FunctorType,
                                              typename traits::launch_bounds>(
            space().impl_internal_space_instance(), attr, f,
            (size_t)impl_vector_length(),
            (size_t)team_scratch_size(0) + 2 * sizeof(double),
  ...
  inline static int vector_length_max() { return detail::CudaTraits::WarpSize; }
  ...
  TeamPolicyInternal()
      : m_space(typename traits::execution_space()),
        ...
        m_chunk_size(detail::CudaTraits::WarpSize),
        m_tune_team(false),
        m_tune_vector(false) {}
  ...
  TeamPolicyInternal(const execution_space space_, int league_size_,
    ...
    if (league_size_ >= int(detail::cuda_internal_maximum_grid_count()[0]))
    ...
    if (m_team_size * m_vector_length >
        int(detail::CudaTraits::MaxHierarchicalParallelism)) {
      detail::throw_runtime_exception(
          std::string("flare::TeamPolicy< Cuda > the team size is too large. "
  ...
  TeamPolicyInternal(const execution_space space_, int league_size_,
                     int team_size_request, const flare::AUTO_t&)
  ...
  TeamPolicyInternal(int league_size_, const flare::AUTO_t& team_size_request,
                     int vector_length_request = 1)
      : TeamPolicyInternal(typename traits::execution_space(), league_size_,
                           team_size_request, vector_length_request)
  ...
  using member_type = flare::detail::CudaTeamMember;
 ...
 protected:
  ...
  int internal_team_size_common(const FunctorType& f,
    ...
    using Interface =
        typename detail::DeduceFunctorPatternInterface<ClosureType>::type;
    using Analysis =
        detail::FunctorAnalysis<Interface, typename ClosureType::Policy,
                              FunctorType, void>;
...
__device__ inline int64_t cuda_get_scratch_index(Cuda::size_type league_size,
  ...
  if (threadIdx.x == 0 && threadIdx.y == 0) {
    int64_t const wraparound_len = flare::max(
        int64_t(1),
        flare::min(int64_t(league_size),
...
class ParallelFor<FunctorType, flare::TeamPolicy<Properties...>,
 ...
 public:
  ...
  __device__ inline void operator()() const {
    ...
    for (int league_rank = blockIdx.x; league_rank < int_league_size;
      ...
      this->template exec_team<WorkTag>(typename Policy::member_type(
          flare_impl_cuda_shared_memory<void>(), m_shmem_begin, m_shmem_size,
  ...
  inline void execute() const {
    ...
    if (flare::detail::CudaInternal::cuda_use_serial_execution()) {
      grid = dim3(1, 1, 1);
    }
#endif
  ...
  ParallelFor(const FunctorType& arg_functor, const Policy& arg_policy)
    ...
    m_team_size =
        m_team_size >= 0
            ? m_team_size
            : flare::detail::cuda_get_opt_block_size<FunctorType, LaunchBounds>(
    ...
    if (internal_space_instance->m_maxShmemPerBlock < shmem_size_total) {
      ...
      flare::detail::throw_runtime_exception(std::string(
          "flare::detail::ParallelFor< Cuda > insufficient shared memory"));
    }

    if (int(m_team_size) >
        int(flare::detail::cuda_get_max_block_size<FunctorType, LaunchBounds>(
            ...
            arg_policy.impl_vector_length())) {
      flare::detail::throw_runtime_exception(std::string(
          "flare::detail::ParallelFor< Cuda > requested too large team size."));
    }
  }
...
class ParallelReduce<CombinedFunctorReducerType,
                     flare::TeamPolicy<Properties...>, flare::Cuda> {
 public:
  using Policy      = TeamPolicy<Properties...>;
 ...
 public:
  ...
  __device__ inline void run(SHMEMReductionTag&, const int& threadid) const {
    ...
    reference_type value = m_functor_reducer.get_reducer().init(
        flare_impl_cuda_shared_memory<size_type>() +
    ...
    for (int league_rank = blockIdx.x; league_rank < int_league_size;
      ...
      this->template exec_team<WorkTag>(
          Member(flare_impl_cuda_shared_memory<char>() + m_team_begin,
    ...
    if (!zero_length)
      do_final_reduction = cuda_single_inter_block_reduce_scan<false>(
          ...
          flare_impl_cuda_shared_memory<size_type>(), m_scratch_space,
          m_scratch_flags);
    ...
    if (do_final_reduction) {
      ...
      size_type* const shared = flare_impl_cuda_shared_memory<size_type>() +
  ...
  __device__ inline void run(ShflReductionTag, const int& threadid) const {
    ...
    if (int_league_size == 0) {
      ...
      *result = value;
    } else if (detail::cuda_inter_block_reduction(
  ...
  inline void execute() {
    ...
    if (!is_empty_range || need_device_set) {
      ...
      if (is_empty_range
#ifdef FLARE_IMPL_DEBUG_CUDA_SERIAL_EXECUTION
          || flare::detail::CudaInternal::cuda_use_serial_execution()
#endif
      ) {
        block = dim3(1, 1, 1);
        grid  = dim3(1, 1, 1);
      ...
      if (!m_result_ptr_device_accessible) {
        m_policy.space().fence(
            "flare::detail::ParallelReduce<Cuda, TeamPolicy>::execute: Result "
  ...
  ParallelReduce(const CombinedFunctorReducerType& arg_functor_reducer,
    ...
    if ((arg_policy.impl_vector_length() > 1) && !UseShflReduction)
      detail::throw_runtime_exception(
          "flare::parallel_reduce with a TeamPolicy using a vector length of "
          "greater than 1 is not currently supported for CUDA for dynamic "
          "sized reduction types.");

    if ((m_team_size < 32) && !UseShflReduction)
      detail::throw_runtime_exception(
          "flare::parallel_reduce with a TeamPolicy using a team_size smaller "
    ...
    if (!flare::detail::is_integral_power_of_two(m_team_size) &&
        !UseShflReduction) {
      flare::detail::throw_runtime_exception(
          std::string("flare::detail::ParallelReduce< Cuda > bad team size"));
    }

    if (internal_space_instance->m_maxShmemPerBlock < shmem_size_total) {
      flare::detail::throw_runtime_exception(
          std::string("flare::detail::ParallelReduce< Cuda > requested too much "
                      "L0 scratch memory"));
    }
    ...
    if (int(m_team_size) >
        arg_policy.team_size_max(m_functor_reducer.get_functor(),
                                 ...
                                 ParallelReduceTag())) {
      flare::detail::throw_runtime_exception(
          std::string("flare::detail::ParallelReduce< Cuda > requested too "
                      "large team size."));
    }
  }
...
#endif  // FLARE_BACKEND_CUDA_CUDA_PARALLEL_TEAM_H_
