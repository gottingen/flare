...
#ifndef FLARE_BACKEND_CUDA_CUDA_PARALLEL_MDRANGE_H_
#define FLARE_BACKEND_CUDA_CUDA_PARALLEL_MDRANGE_H_
...
int max_tile_size_product_helper(const Policy& pol, const LaunchBounds&) {
  ...
  int const max_warps_per_sm_registers =
      flare::detail::cuda_max_warps_per_sm_registers(prop, attr);
  ...
  return std::min(
      max_threads_per_sm,
      static_cast<int>(flare::detail::CudaTraits::MaxHierarchicalParallelism));
}

template <class FunctorType, class... Traits>
class ParallelFor<FunctorType, flare::MDRangePolicy<Traits...>, flare::Cuda> {
 public:
  using Policy       = flare::MDRangePolicy<Traits...>;
 ...
 public:
  ...
  inline __device__ void operator()() const {
    flare::detail::DeviceIterateTile<Policy::rank, Policy, FunctorType,
  ...
  inline void execute() const {
    ...
    if (RP::rank == 2) {
      ...
      FLARE_ASSERT(block.x > 0);
      FLARE_ASSERT(block.y > 0);
      const dim3 grid(
      ...
      flare::abort("flare::MDRange Error: Exceeded rank bounds with Cuda\n");
    }
...
class ParallelReduce<CombinedFunctorReducerType,
                     flare::MDRangePolicy<Traits...>, flare::Cuda> {
 public:
  using Policy      = flare::MDRangePolicy<Traits...>;
 ...
 public:
  ...
  // smaller than int32_t (flare::Cuda::size_type)
  ...
  using DeviceIteratePattern = typename flare::detail::Reduce::DeviceIterateTile<
 ...
 public:
  ...
  inline __device__ void exec_range(reference_type update) const {
    flare::detail::Reduce::DeviceIterateTile<Policy::rank, Policy, FunctorType,
  ...
  inline __device__ void operator()() const {
...

    {
      reference_type value =
          m_functor_reducer.get_reducer().init(reinterpret_cast<pointer_type>(
              flare_impl_cuda_shared_memory<word_size_type>() +
    ...
    if (cuda_single_inter_block_reduce_scan<false>(
            ...
            flare_impl_cuda_shared_memory<word_size_type>(), m_scratch_space,
            m_scratch_flags)) {
    ...
    while (
        ...
        (n >
         static_cast<unsigned>(
             flare::detail::cuda_get_max_block_size<FunctorType, LaunchBounds>(
        ...
        if (m_result_ptr) {
          if (m_unified_space) {
            m_policy.space().fence(
                "flare::detail::ParallelReduce<Cuda, MDRangePolicy>::execute: "
...
#endif  // FLARE_BACKEND_CUDA_CUDA_PARALLEL_MDRANGE_H_
