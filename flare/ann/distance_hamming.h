...
#ifndef FLARE_ANN_DISTANCE_HAMMING_H_
#define FLARE_ANN_DISTANCE_HAMMING_H_

#include <flare/ann/distance_hamming_impl.h>

namespace flare::ann {

    /// \brief Return the hamming distance of the two vectors x and y.
    /// d = sum((X(i) ^ Y(i))
    ...
    template<class XVector, class execution_space,
            typename std::enable_if<flare::is_execution_space_v<execution_space>,
                    int>::type = 0>
    double distance_hamming(const execution_space &space, const XVector &x, const XVector &y, bool batch = true) {
        static_assert(
                flare::is_execution_space<execution_space>::value,
                "flare::ann::distance_hamming: execution_space must be a flare::execution_space.");
        static_assert(flare::is_tensor<XVector>::value,
                      "flare::ann::distance_hamming: XVector must be a flare::Tensor.");
        static_assert(XVector::rank == 1,
                      "flare::ann::distance_hamming: "
        ...
        using RVector_Internal =
                flare::Tensor<double, default_layout, flare::HostSpace,
                        flare::MemoryTraits<flare::Unmanaged> >;
        double result;
        RVector_Internal R = RVector_Internal(&result);
        if (simd_traits<XVector, execution_space>::is_batch_available && batch) {
            flare::ann::detail::DistanceHamming<execution_space, RVector_Internal, XVector_Internal>::batch_distance(space, R, x,
                                                                                                          y);
        } else {
            flare::ann::detail::DistanceHamming<execution_space, RVector_Internal, XVector_Internal>::distance(space, R, x,
                                                                                                          y);
        }
        space.fence();
        return result;
    ...
    double distance_hamming(const XVector &x, const XVector &y, bool batch = true) {
        return distance_hamming(typename XVector::execution_space{}, x, y, batch);
    }

}  // namespace flare::ann

#endif  // FLARE_ANN_DISTANCE_HAMMING_H_
