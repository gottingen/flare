...
#ifndef FLARE_ANN_NORM_L1_H_
#define FLARE_ANN_NORM_L1_H_

#include <flare/ann/norm_l1_impl.h>
...
namespace flare::ann {
    ...
    norm_l1(const execution_space &space, const XVector &x, bool batch = true) {
        static_assert(
        ...
        if (simd_traits<XVector, execution_space>::is_batch_available && batch) {
            flare::ann::detail::NormL1<execution_space, RVector_Internal, XVector_Internal>::batch_norm(space, R, X);
        } else {
            flare::ann::detail::NormL1<execution_space, RVector_Internal, XVector_Internal>::norm(space, R, X);
        }
        space.fence();
        return result;
    }
    ...
    typename simd_traits<XVector>::mag_type norm_l1(const XVector &x, bool batch = true) {
        return norm_l1(typename XVector::execution_space{}, x,  batch);
    }
    ...
    void normalize_l1(const execution_space& space, const RMV& R, const XMV& X, typename RMV::non_const_value_type norm, bool batch = true) {
    ...
    /// \brief R(i,j) = normalize_l1(X(i,j))
    ///
    /// Non-blocking function to replace each entry in R with the absolute value
    ...
    void normalize_l1(const RMV& R, const XMV& X, typename RMV::non_const_value_type norm, bool batch = true) {
        normalize_l1(typename RMV::execution_space{}, R, X, norm,batch);
    }
    ...
    /// \param X [in] tensor of type XMV.
    template <class RMV, class XMV>
    void normalize_l1(const RMV& R, const XMV& X, bool batch = true) {
        auto norm = norm_l1(X, batch);
        normalize_l1(typename RMV::execution_space{}, R, X, norm, batch);
    }
}  // namespace flare::ann

#endif  // FLARE_ANN_NORM_L1_H_
