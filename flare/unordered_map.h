...
/// \brief Declaration and definition of flare::UnorderedMap.
///
/// This header file declares and defines flare::UnorderedMap and its
/// related nonmember functions.

#ifndef FLARE_UNORDERED_MAP_H_
#define FLARE_UNORDERED_MAP_H_
...
#include <flare/containers/unordered_map_impl.h>
...
///
/// \tparam Device The flare Device type.
///
...
template <typename Key, typename Value,
          typename Device  = flare::DefaultExecutionSpace,
...
class UnorderedMap {
 ...
 public:
  ...
  using histogram_type = detail::UnorderedMapHistogram<const_map_type>;
  //@}
 ...
 public:
  ...
  UnorderedMap(size_type capacity_hint = 0, hasher_type hasher = hasher_type(),
    ...
    flare::deep_copy(m_hash_lists, invalid_index);
    flare::deep_copy(m_next_index, invalid_index);
  }
  ...
  void clear() {
    ...
    flare::deep_copy(m_next_index, invalid_index);
    {
      ...
      flare::deep_copy(m_keys, tmp);
    }
    flare::deep_copy(m_scalars, 0);
    m_size() = 0;
  }
  ...
  bool rehash(size_type requested_capacity, bool bounded_insert) {
    ...
    if (curr_size) {
      ...
      detail::UnorderedMapRehash<insertable_map_type> f(tmp, *this);
      f.apply();
    }
  ...
  bool begin_erase() {
    ...
    if (is_insertable_map && result) {
      execution_space().fence(
          "flare::UnorderedMap::begin_erase: fence before setting erasable "
          "flag");
  ...
  bool end_erase() {
    ...
    if (is_insertable_map && result) {
      execution_space().fence(
          "flare::UnorderedMap::end_erase: fence before erasing");
      detail::UnorderedMapErase<declared_map_type> f(*this);
      f.apply();
      execution_space().fence(
          "flare::UnorderedMap::end_erase: fence after erasing");
  ...
  ///                       flare::UnorderedMapInsertOpTypes for more ops.
  template <typename InsertOpType = default_op_type>
  FLARE_INLINE_FUNCTION insert_result
...
#pragma noprefetch
    ...
    while (not_done) {
      ...
      FLARE_NONTEMPORAL_PREFETCH_LOAD(
...
#pragma noprefetch
      ...
      if (curr != invalid_index) {
        ...
        if (free_existing) {
          ...
          if (!m_available_indexes.reset(new_index)) {
            flare::printf("Unable to free existing\n");
          }
        }
      ...
      // If the thread has claimed an entry try to insert now.
        ...
        if (new_index == invalid_index) {
          ...
          if (!found && ++find_attempts >= max_attempts) {
            ...
            // Set key and value
            FLARE_NONTEMPORAL_PREFETCH_STORE(&m_keys[new_index]);
            m_keys[new_index] = k;

            if (!is_set) {
              FLARE_NONTEMPORAL_PREFETCH_STORE(&m_values[new_index]);
  ...
  size_type find(const key_type &k) const {
    ...
    FLARE_NONTEMPORAL_PREFETCH_LOAD(&m_keys[curr != invalid_index ? curr : 0]);
  ...
  UnorderedMap(
      ...
      std::enable_if_t<
          detail::UnorderedMapCanAssign<declared_key_type, declared_value_type,
                                      SKey, SValue>::value,
          int> = 0)
  ...
  template <typename SKey, typename SValue>
  std::enable_if_t<
      detail::UnorderedMapCanAssign<declared_key_type, declared_value_type, SKey,
                                  SValue>::value,
      declared_map_type &>
  ...
  create_copy_tensor(
    ...
    if (m_hash_lists.data() != src.m_hash_lists.data()) {
      ...
      using raw_deep_copy =
          flare::detail::DeepCopy<typename device_type::memory_space,
 ...
 private:  // private member functions
  ...
  void set_flag(int flag) const {
    ...
    flare::fence(
        "flare::UnorderedMap::set_flag: fence after copying flag from "
        "HostSpace");
  }
  ...
  void reset_flag(int flag) const {
    ...
    flare::fence(
        "flare::UnorderedMap::reset_flag: fence after copying flag from "
        "HostSpace");
  }

  bool get_flag(int flag) const {
    using raw_deep_copy =
        flare::detail::DeepCopy<flare::HostSpace,
                               typename device_type::memory_space>;
    int result = false;
    raw_deep_copy(&result, m_scalars.data() + flag, sizeof(int));
    flare::fence(
        "flare::UnorderedMap::get_flag: fence after copy to return value in "
        "HostSpace");
    return result;
  }
 ...
 private:  // private members
  ...
  friend struct detail::UnorderedMapErase;

  template <typename UMap>
  friend struct detail::UnorderedMapHistogram;

  template <typename UMap>
  friend struct detail::UnorderedMapPrint;
};
