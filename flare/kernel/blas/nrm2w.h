...
#ifndef FLARE_KERNEL_BLAS_NRM2W_H_
#define FLARE_KERNEL_BLAS_NRM2W_H_
...
namespace flare::blas {
    ...
    nrm2w(const execution_space& space, const XVector& x, const XVector& w,
          typename std::enable_if<
                  flare::is_execution_space<execution_space>::value, int>::type = 0) {
        static_assert(flare::is_execution_space<execution_space>::value,
                      "flare::blas::nrm2w: execution_space must be a valid"
                      " flare execution space.");
        static_assert(flare::is_tensor<XVector>::value,
                      "flare::blas::nrm2w: XVector must be a flare::Tensor.");
        static_assert(
                flare::SpaceAccessibility<execution_space,
                        typename XVector::memory_space>::accessible,
                "flare::blas::nrm2w: XVector must be accessible from execution_space");
        ...
        using RVector_Internal =
                flare::Tensor<mag_type, layout_t, flare::HostSpace,
...
/// \param W [in] Input Tensor (rank 1 or 2).
    ...
    void nrm2w(const execution_space& space, const RV& R, const XMV& X,
        ...
        static_assert(
                ...
                "flare::blas::nrm2w: XMV must be accessible from execution_space");
        static_assert(std::is_same<typename RV::value_type,
                              typename RV::non_const_value_type>::value,
                      "flare::blas::nrm2w: R is const.  "
...
#endif  // FLARE_KERNEL_BLAS_NRM2W_H_
