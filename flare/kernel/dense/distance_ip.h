...
#ifndef FLARE_KERNEL_DENSE_DISTANCE_IP_H_
#define FLARE_KERNEL_DENSE_DISTANCE_IP_H_
...
namespace flare::kernel::dense {
    ...
    struct DistanceIPFunctor {
        ...
        DistanceIPFunctor(const XV &x, const XV &y) : m_x(x), m_y(y) {
            static_assert(flare::is_tensor<RV>::value,
                          "flare::ann::detail::DistanceIPFunctor: "
            ...
            static_assert(std::is_same<typename RV::value_type,
                          ...
                          "flare::ann::detail::DistanceIPFunctor: R is const.  "
                          "It must be nonconst, because it is an output argument "
                          "(we have to be able to write to its entries).");

            static_assert(RV::rank == 0 && XV::rank == 1,
                          "flare::blas::detail::DistanceIPFunctor: "
    ...
    void DistanceIPInvoke(const execution_space &space, const RV &r, const XV &X, const XV &Y) {
        const SizeType numRows = static_cast<SizeType>(X.extent(0));
        flare::RangePolicy<execution_space, SizeType> policy(space, 0, numRows);

        typedef DistanceIPFunctor<RV, XV, SizeType> functor_type;
        functor_type op(X, Y);
...
#endif  // FLARE_KERNEL_DENSE_DISTANCE_IP_H_
