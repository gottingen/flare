...
#ifndef FLARE_CORE_COMMON_UTILITY_H_
...
namespace flare::detail {
        ...
        ExecSpaceType get_exec_space_type() {
            return flare_get_exec_space_type<ExecutionSpace>();
        }
        ...
        void get_suggested_vector_size(int &suggested_vector_size_, idx nr, idx nnz) {
            suggested_vector_size_ = flare_get_suggested_vector_size(
            ...
            if (flare_is_gpu_exec_space<execution_space>()) {
                team_policy_t temp =
                        team_policy_t(1, 1, vector_size)
                                .set_scratch_size(0, flare::PerTeam(sharedPerTeam),
                                                  flare::PerThread(sharedPerThread));
                return temp.team_size_recommended(f, ParallelTag());
            } else
                return 1;
        }
        ...
        struct FillSymmetricEdges {
            ...
            void operator()(const team_member &teamMember) const {
                ...
                flare::parallel_for(
                        flare::ThreadVectorRange(teamMember, row_end - row_begin), [&](idx i) {
            ...
            void operator()(const team_member &teamMember /*, idx &nnz*/) const {
                ...
                flare::parallel_for(
                        flare::ThreadVectorRange(teamMember, row_end - row_begin), [&](idx i) {
                            ...
                            if (colIndex < num_rows) {
                                if (colIndex < ii) {
                                    flare::UnorderedMapInsertResult r =
                                            umap.insert(flare::pair<idx, idx>(colIndex, ii));
                                    if (r.success()) {
                                        flare::atomic_fetch_add(&(pre_pps(ii)), atomic_incr_type(1));

                                        flare::atomic_fetch_add(&(pre_pps(colIndex)),
                                                                 atomic_incr_type(1));
                                    }
                                } else if (colIndex > ii) {
                                    flare::UnorderedMapInsertResult r =
                                            umap.insert(flare::pair<idx, idx>(ii, colIndex));
                                    if (r.success()) {
                                        flare::atomic_fetch_add(&(pre_pps(colIndex)),
                                                                 atomic_incr_type(1));
            ...
            void operator()(const team_member_t &teamMember) const {
                ...
                flare::parallel_for(
                        flare::ThreadVectorRange(teamMember, row_end - row_begin), [&](idx i) {
                            ...
                            if (colIndex < num_rows) {
                                if (colIndex < ii) {
                                    if (umap.insert(flare::pair<idx, idx>(colIndex, ii)).success()) {
                                        idx cAdjInd = flare::atomic_fetch_add(&(pre_pps(colIndex)),
                                                                               atomic_incr_type(1));
                                        idx iAdjInd = flare::atomic_fetch_add(&(pre_pps(ii)),
                                                                               atomic_incr_type(1));
                                        sym_adj[cAdjInd] = ii;
                                        sym_adj[iAdjInd] = colIndex;
                                    }
                                } else if (colIndex > ii) {
                                    if (umap.insert(flare::pair<idx, idx>(ii, colIndex)).success()) {
                                        idx cAdjInd = flare::atomic_fetch_add(&(pre_pps(colIndex)),
                                                                               atomic_incr_type(1));
                                        idx iAdjInd = flare::atomic_fetch_add(&(pre_pps(ii)),
                                                                               atomic_incr_type(1));
                ...
                flare::parallel_for(
                        flare::ThreadVectorRange(teamMember, row_end - row_begin), [&](idx i) {
                            ...
                            if (colIndex < num_rows) {
                                if (colIndex < ii) {
                                    if (umap.insert(flare::pair<idx, idx>(colIndex, ii)).success()) {
                                        idx cAdjInd      = flare::atomic_fetch_add(&(pps(colIndex)),
                                                                                    atomic_incr_type(1));
                                    ...
                                    if (umap.insert(flare::pair<idx, idx>(ii, colIndex)).success()) {
                                        idx cAdjInd =
                                                flare::atomic_fetch_add(&(pps(ii)), atomic_incr_type(1));
                                        sym_src[cAdjInd] = ii;
        ...
        struct Reverse_Map_Init {
            ...
            void operator()(const size_t &ii) const {
                ...
                flare::atomic_fetch_add(&(reverse_map_xadj(fm)), atomic_incr_type(1));
            }
        };
        ...
        struct Fill_Reverse_Map {
            ...
            void operator()(const size_t &ii) const {
                ...
                const reverse_type future_index = flare::atomic_fetch_add(
        ...
        void a_times_x_plus_b(typename in_array_t::value_type num_elements,
                              ...
                              scalar_2 b) {
            flare_a_times_x_plus_b<out_array_t, in_array_t, scalar_1, scalar_2, MyExecSpace>(
        ...
        void linear_init(typename array_type::value_type num_elements, array_type arr) {
            ...
            flare::parallel_for("flare::detail::LinearInit",
        ...
        void remove_zeros_in_xadj_vector(
                ...
                forward_array_type arr) {
            typedef flare::RangePolicy<MyExecSpace> my_exec_space;
            flare::parallel_scan("flare::detail::RemoveZerosInXadjVector",
                                  my_exec_space(0, num_elements),
 ...
 */
        ...
        void create_reverse_map(
            ...
            if (num_reverse_elements < MINIMUM_TO_ATOMIC) {
                ...
                flare::parallel_for("flare::detail::ReverseMapScaleInit",
                ...
                Fill_Reverse_Scale_Map<forward_array_type, reverse_array_type> frm(
                        ...
                        division_shift_for_bucket);
                flare::parallel_for("flare::detail::FillReverseMap",
                                     range_policy_t(my_exec_space, 0, num_forward_elements),
                                     frm);
                my_exec_space.fence();
            } else
                // atomic implementation.
            {
                ...
                flare::parallel_for("flare::detail::ReverseMapInit",
                                     range_policy_t(my_exec_space, 0, num_forward_elements),
                                     rmi);
                my_exec_space.fence();
                ...
                flare::deep_copy(my_exec_space, tmp_color_xadj, reverse_map_xadj);
                my_exec_space.fence();
        ...
        void permute_vector(MyExecSpace my_exec_space,
                            ...
                            value_array_type &old_vector,
                            out_value_array_type &new_vector) {
            using range_policy_t = flare::RangePolicy<MyExecSpace>;

            flare::parallel_for(
                    "flare::detail::PermuteVector",
...
// zeroed, when really it's always the whole thing.
        ...
        void zero_vector(ExecSpaceIn &exec_space_in,
            ...
            flare::deep_copy(exec_space_in, vector,
                              flare::ArithTraits<val_type>::zero());
            exec_space_in.fence();
        }
        ...
        void symmetrize_and_get_lower_diagonal_edge_list(
            ...
            typedef flare::TeamPolicy<MyExecSpace> team_policy;
            ...
            idx num_symmetric_edges = 0;
            {
                ...
                flare::parallel_for(
                        "flare::detail::SymmetrizeAndGetLowerDiagonalEdgeList::S0", pol,
            ...
            */
            ...
            MyExecSpace().fence();
            {
                ...
                team_policy pol((num_rows_to_symmetrize + teamSizeMax - 1) / teamSizeMax,
                                teamSizeMax, vector_size);
                flare::parallel_for(
                        "flare::detail::SymmetrizeAndGetLowerDiagonalEdgeList::S1", pol,
                        FSCH);
                MyExecSpace().fence();
            }
        }
        ...
        void symmetrize_graph_symbolic_hashmap(
            ...
            idx num_symmetric_edges = 0;
            {
                ...
                team_policy pol((num_rows_to_symmetrize + teamSizeMax - 1) / teamSizeMax,
                                teamSizeMax, vector_size);
                flare::parallel_for(
                        "flare::detail::SymmetrizeGraphSymbolicHashMap::S0", pol,
                        fse /*, num_symmetric_edges*/);
                MyExecSpace().fence();
            }
            ...
            flare::deep_copy(sym_xadj, pre_pps_);
            {
                ...
                team_policy pol((num_rows_to_symmetrize + teamSizeMax - 1) / teamSizeMax,
                                teamSizeMax, vector_size);
                flare::parallel_for(
                        "flare::detail::SymmetrizeGraphSymbolicHashMap::S1", pol, FSCH);
                MyExecSpace().fence();
            }
        ...
        void copy_vector(size_t num_elements, from_vector from, to_vector to) {
            flare_copy_vector<from_vector, to_vector, MyExecSpace>(num_elements, from, to);
        }
...
// view has num_rows+1 elements.
        ...
        struct InitScalar {
            ...
            void operator()(const team_member_t &teamMember) const {
                ...
                const nnz_lno_t team_row_end = FLARE_MACRO_MIN(
        ...
        struct array_sum_reduce {
            ...
            static constexpr int N_internal =
                    ((N == 3 || N == 5 || N == 7) &&
                     std::is_same<scalar_t, flare::experimental::half_t>::value &&
                     sizeof(flare::experimental::half_t) == 2)
                    ? (N + 1)
                    : N;
            ...
            FLARE_INLINE_FUNCTION  // add operator
            array_sum_reduce &
        ...
        FLARE_INLINE_FUNCTION T *alignPtr(InPtr p) {
...
namespace flare {
    template <typename scalar_t, int N>
    struct reduction_identity<flare::detail::array_sum_reduce<scalar_t, N>> {
        typedef flare::detail::array_sum_reduce<scalar_t, N> T;
        FLARE_FORCEINLINE_FUNCTION static T sum() {
...
#endif  // FLARE_CORE_COMMON_UTILITY_H_
