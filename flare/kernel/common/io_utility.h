...
#ifndef FLARE_CORE_COMMON_IO_UTILITY_H_
#define FLARE_CORE_COMMON_IO_UTILITY_H_
...
namespace flare::detail {
        ...
        // Get the interval for flare::fill_random
        ...
        inline void getRandomBounds(double mag, Scalar &start, Scalar &end) {
            start = -mag * flare::ArithTraits<Scalar>::one();
            end   = mag * flare::ArithTraits<Scalar>::one();
        }

        template <>
        inline void getRandomBounds(double mag, flare::complex<float> &start,
                                    flare::complex<float> &end) {
            start = flare::complex<float>(-mag, -mag);
            end   = flare::complex<float>(mag, mag);
        }

        template <>
        inline void getRandomBounds(double mag, flare::complex<double> &start,
                                    flare::complex<double> &end) {
            start = flare::complex<double>(-mag, -mag);
            end   = flare::complex<double>(mag, mag);
        }
        ...
        struct Edge {
            ...
            // From MTGL
            ////////////////////////////////////////////////////////////////////////////////
        inline size_t flare_get_file_size(const char *file) {
  ...
  int retval = _stat(file, &stat_buf);
        ...
        void buildEdgeListFromBinSrcTarg_undirected(const char *fnameSrc,
                                                    ...
                                                    lno_t **dst) {
            size_t srcFileSize = flare_get_file_size(fnameSrc);
            size_t trgFileSize = flare_get_file_size(fnameTarg);
            // test these values
...
#endif  // FLARE_CORE_COMMON_IO_UTILITY_H_
