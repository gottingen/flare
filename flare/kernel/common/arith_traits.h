...
namespace {  // anonymous
    ...
    FLARE_FORCEINLINE_FUNCTION IntType intPowImpl(const IntType x,
                                                   const IntType y) {
    ...
    struct integer_abs {
        static FLARE_INLINE_FUNCTION T abs(const T& val);
    };

    template <class T>
    struct integer_abs<T, true> {
        static FLARE_INLINE_FUNCTION T abs(const T& x) { return x < 0 ? -x : x; }
    };

    template <class T>
    struct integer_abs<T, false> {
        static FLARE_INLINE_FUNCTION T abs(const T& x) { return x; }
    };
    ...
    /// not throw an exception in that case.
    template <class IntType>
    FLARE_FORCEINLINE_FUNCTION IntType intPowUnsigned(const IntType x,
                                                       const IntType y) {
        // It's not entirely clear what to return if x and y are both zero.
        // In the case of floating-point numbers, 0^0 is NaN.  Here, though,
        // I think it's safe to return 0.
...
// Macro to automate the wrapping of flare Mathematical Functions
// in the ArithTraits struct for real floating point types, hopefully
// this can be expanded to flare::half_t and flare::bhalf_t
#define FLARE_ARITH_TRAITS_REAL_FP(FUNC_QUAL)                           \
  ...
  static FUNC_QUAL val_type min() {                                            \
    return flare::experimental::finite_min<val_type>::value;                  \
  }                                                                            \
  static FUNC_QUAL val_type max() {                                            \
    return flare::experimental::finite_max<val_type>::value;                  \
  }                                                                            \
  static FUNC_QUAL val_type infinity() {                                       \
    return flare::experimental::infinity<val_type>::value;                    \
  }                                                                            \
  static FUNC_QUAL val_type nan() {                                            \
    return flare::experimental::quiet_NaN<val_type>::value;                   \
  }                                                                            \
  static FUNC_QUAL mag_type epsilon() {                                        \
    return flare::experimental::epsilon<val_type>::value;                     \
  }                                                                            \
  static FUNC_QUAL mag_type sfmin() {                                          \
    return flare::experimental::norm_min<val_type>::value;                    \
  }                                                                            \
  static FUNC_QUAL int base() {                                                \
    return flare::experimental::radix<val_type>::value;                       \
  }                                                                            \
  static FUNC_QUAL mag_type prec() {                                           \
    return epsilon() * static_cast<mag_type>(base());                          \
  }                                                                            \
  static FUNC_QUAL int t() {                                                   \
    return flare::experimental::digits<val_type>::value;                      \
  }                                                                            \
  static FUNC_QUAL mag_type rnd() { return one(); }                            \
  static FUNC_QUAL int emin() {                                                \
    return flare::experimental::min_exponent<val_type>::value;                \
  }                                                                            \
  static FUNC_QUAL mag_type rmin() {                                           \
    return flare::experimental::norm_min<val_type>::value;                    \
  }                                                                            \
  static FUNC_QUAL int emax() {                                                \
    return flare::experimental::max_exponent<val_type>::value;                \
  }                                                                            \
  static FUNC_QUAL mag_type rmax() {                                           \
    return flare::experimental::finite_max<val_type>::value;                  \
  }                                                                            \
                                                                               \
  static FUNC_QUAL bool isInf(const val_type x) { return flare::isinf(x); }   \
  static FUNC_QUAL bool isNan(const val_type x) { return flare::isnan(x); }   \
  static FUNC_QUAL mag_type abs(const val_type x) { return flare::abs(x); }   \
  static FUNC_QUAL mag_type real(const val_type x) { return flare::real(x); } \
  static FUNC_QUAL mag_type imag(const val_type x) { return flare::imag(x); } \
  static FUNC_QUAL val_type conj(const val_type x) { return x; }               \
  static FUNC_QUAL val_type pow(const val_type x, const val_type y) {          \
    return flare::pow(x, y);                                                  \
  }                                                                            \
  static FUNC_QUAL val_type sqrt(const val_type x) { return flare::sqrt(x); } \
  static FUNC_QUAL val_type cbrt(const val_type x) { return flare::cbrt(x); } \
  static FUNC_QUAL val_type exp(const val_type x) { return flare::exp(x); }   \
  static FUNC_QUAL val_type log(const val_type x) { return flare::log(x); }   \
  static FUNC_QUAL val_type log10(const val_type x) {                          \
    return flare::log10(x);                                                   \
  }                                                                            \
  static FUNC_QUAL val_type sin(const val_type x) { return flare::sin(x); }   \
  static FUNC_QUAL val_type cos(const val_type x) { return flare::cos(x); }   \
  static FUNC_QUAL val_type tan(const val_type x) { return flare::tan(x); }   \
  static FUNC_QUAL val_type sinh(const val_type x) { return flare::sinh(x); } \
  static FUNC_QUAL val_type cosh(const val_type x) { return flare::cosh(x); } \
  static FUNC_QUAL val_type tanh(const val_type x) { return flare::tanh(x); } \
  static FUNC_QUAL val_type asin(const val_type x) { return flare::asin(x); } \
  static FUNC_QUAL val_type acos(const val_type x) { return flare::acos(x); } \
  static FUNC_QUAL val_type atan(const val_type x) { return flare::atan(x); } \
                                                                               \
...
#define FLARE_ARITH_TRAITS_CMPLX_FP(FUNC_QUAL)                          \
                                                                               \
  ...
  using halfPrecision =                                                        \
      ::flare::complex<ArithTraits<mag_type>::halfPrecision>;                 \
  using doublePrecision =                                                      \
      ::flare::complex<ArithTraits<mag_type>::doublePrecision>;               \
                                                                               \
...
#define FLARE_ARITH_TRAITS_INTEGRAL()                                  \
                                                                              \
  static constexpr bool is_specialized = true;                                \
  ...
  static constexpr bool hasMachineParameters = false;                         \
                                                                              \
  static FLARE_FUNCTION val_type zero() { return static_cast<val_type>(0); } \
  static FLARE_FUNCTION val_type one() { return static_cast<val_type>(1); }  \
  static FLARE_FUNCTION val_type min() {                                     \
    return flare::experimental::finite_min<val_type>::value;                 \
  }                                                                           \
  static FLARE_FUNCTION val_type max() {                                     \
    return flare::experimental::finite_max<val_type>::value;                 \
  }                                                                           \
  static FLARE_FUNCTION val_type infinity() {                                \
    return static_cast<val_type>(0);                                          \
  }                                                                           \
  static FLARE_FUNCTION val_type nan() {                                     \
    return flare_kernel_is_nan<val_type>();                                      \
  }                                                                           \
  static FLARE_FUNCTION bool isInf(const val_type) { return false; }         \
  static FLARE_FUNCTION bool isNan(const val_type) { return false; }         \
  static FLARE_FUNCTION mag_type abs(const val_type x) {                     \
    return flare_kernels_abs(x);                                               \
  }                                                                           \
  static FLARE_FUNCTION mag_type real(const val_type x) {                    \
    return flare::real(x);                                                   \
  }                                                                           \
  static FLARE_FUNCTION mag_type imag(const val_type) { return zero(); }     \
  static FLARE_FUNCTION val_type conj(const val_type x) { return x; }        \
  static FLARE_FUNCTION val_type pow(const val_type x, const val_type y) {   \
    return flare::pow(x, y);                                                 \
  }                                                                           \
  static FLARE_FUNCTION val_type sqrt(const val_type x) {                    \
    return static_cast<val_type>(flare::sqrt(abs(x)));                       \
  }                                                                           \
  static FLARE_FUNCTION val_type cbrt(const val_type x) {                    \
    return static_cast<val_type>(flare::cbrt(abs(x)));                       \
  }                                                                           \
  static FLARE_FUNCTION val_type exp(const val_type x) {                     \
    return static_cast<val_type>(flare::exp(abs(x)));                        \
  }                                                                           \
  static FLARE_FUNCTION val_type log(const val_type x) {                     \
    return static_cast<val_type>(flare::log(abs(x)));                        \
  }                                                                           \
  static FLARE_FUNCTION val_type log10(const val_type x) {                   \
    return static_cast<val_type>(flare::log10(abs(x)));                      \
  }                                                                           \
  static FLARE_FUNCTION mag_type epsilon() { return zero(); }                \
  static FLARE_FUNCTION magnitudeType magnitude(const val_type x) {          \
    return abs(x);                                                            \
  }                                                                           \
  static FLARE_FUNCTION val_type conjugate(const val_type x) {               \
    return conj(x);                                                           \
  }                                                                           \
  static FLARE_FUNCTION bool isnaninf(const val_type) { return false; }      \
  static FLARE_FUNCTION val_type squareroot(const val_type x) {              \
    return sqrt(x);                                                           \
  }
    ...
    /// using the FLARE_FORCEINLINE_FUNCTION macro.  All class methods must be
    ...
    ///
    /// \section ArithTraits_compat Compatibility
    ///
    ...
    ///
    /// \section ArithTraits_unsupp Unsupported types on CUDA devices
    ///
    ...
    ///
    /// \section ArithTraits_whyNotC99 What about C99 integer types?
    ///
    ...
    ///
    /// \section ArithTraits_impl Implementation notes
    ///
    ...
    class ArithTraits {
    public:
        /// \brief A type that acts like T and works with flare.
        ///
        /// This is usually just an alias for T.  However, some types T do
        /// not work well with flare.  In that case, we use a mostly
        /// equivalent type here.  For example, ArithTraits<std::complex<R>
        /// >::val_type is flare::complex<R>.
        using val_type = T;
        ...
        static FLARE_FUNCTION bool isInf(const T& x);
        ...
        /// function), because CUDA appears to implement that function using
        /// a macro, rather than using a function (as C++11 requires).
        static FLARE_FUNCTION bool isNan(const T& x);

        //! The absolute value (magnitude) of x.
        static FLARE_FUNCTION mag_type abs(const T& x);

        //! The zero value of T; the arithmetic identity.
        static FLARE_FUNCTION T zero();

        //! The one value of T; the multiplicative identity.
        static FLARE_FUNCTION T one();
        ...
        /// ArithTraits<T>::has_infinity == true. Provides same
        ...
        static FLARE_FUNCTION T infinity();
        ...
        static FLARE_FUNCTION T min();

        //! The maximum possible value of T.
        static FLARE_FUNCTION T max();

        /// \brief The real part of x.
        ///
        /// If \c is_complex is false, then this just returns x.
        static FLARE_FUNCTION mag_type real(const T& x);

        /// \brief The imaginary part of x.
        ///
        /// If \c is_complex is false, then this just returns zero().
        static FLARE_FUNCTION mag_type imag(const T&);

        /// \brief The complex conjugate of x.
        ///
        /// If \c is_complex is false, then this just returns x.
        static FLARE_FUNCTION T conj(const T&);

        //! x raised to the power y.
        static FLARE_FUNCTION T pow(const T& x, const T& y);
        ...
        static FLARE_FUNCTION T sqrt(const T& x);
        ...
        static FLARE_FUNCTION T cbrt(const T& x);
        ...
        ///
        static FLARE_FUNCTION T exp(const T& x);
        ...
        static FLARE_FUNCTION T log(const T& x);
        ...
        static FLARE_FUNCTION T log10(const T& x);
        ...
        ///
        static FLARE_FUNCTION T sin(const T& x);

        /// \brief The cos function of x
        ///
        static FLARE_FUNCTION T cos(const T& x);

        /// \brief The tan function of x
        ///
        static FLARE_FUNCTION T tan(const T& x);

        /// \brief The sin hyperbolic function of x
        ///
        static FLARE_FUNCTION T sinh(const T& x);

        /// \brief The cos hyperbolic function of x
        ///
        static FLARE_FUNCTION T cosh(const T& x);

        /// \brief The tan hyperbolic function of x
        ///
        static FLARE_FUNCTION T tanh(const T& x);

        /// \brief The asin function of x
        ///
        static FLARE_FUNCTION T asin(const T& x);

        /// \brief The acos function of x
        ///
        static FLARE_FUNCTION T acos(const T& x);

        /// \brief The atan function of x
        ///
        static FLARE_FUNCTION T atan(const T& x);
        ...
        static FLARE_FUNCTION mag_type epsilon();
        ...
        static FLARE_FUNCTION T squareroot(const T& x);
        //@}
    };

// Since flare::experimental::half_t falls back to float, only define
// ArithTraits if half_t is a backend specialization
#if defined(FLARE_HALF_T_IS_FLOAT) && !FLARE_HALF_T_IS_FLOAT
    template <>
class ArithTraits<flare::experimental::half_t> {
 public:
  using val_type = flare::experimental::half_t;
  ...
  static constexpr bool has_infinity = true;
  static FLARE_FUNCTION val_type infinity() {
    return flare::experimental::cast_to_half(
        flare::experimental::infinity<float>::value);
  }

  static FLARE_FUNCTION bool isInf(const val_type x) {
#ifndef __CUDA_ARCH__
    using std::isinf;
#endif
    return isinf(flare::experimental::cast_from_half<float>(x));
  }
  static FLARE_FUNCTION bool isNan(const val_type x) {
#ifndef __CUDA_ARCH__
    using std::isnan;
#endif
    return isnan(flare::experimental::cast_from_half<float>(x));
  }
  static FLARE_FUNCTION mag_type abs(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::abs(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type zero() {
    return flare::experimental::cast_to_half(0.0);
  }
  static FLARE_FUNCTION val_type one() {
    return flare::experimental::cast_to_half(1.0);
  }
  static FLARE_FUNCTION val_type min() {
    return flare::experimental::cast_to_half(-FLARE_IMPL_FP16_MAX);
  }
  static FLARE_FUNCTION val_type max() {
    return flare::experimental::cast_to_half(FLARE_IMPL_FP16_MAX);
  }
  static FLARE_FUNCTION mag_type real(const val_type x) { return x; }
  static FLARE_FUNCTION mag_type imag(const val_type) { return zero(); }
  static FLARE_FUNCTION val_type conj(const val_type x) { return x; }
  static FLARE_FUNCTION val_type pow(const val_type x, const val_type y) {
    return flare::experimental::cast_to_half(
        flare::pow(flare::experimental::cast_from_half<float>(x),
                    flare::experimental::cast_from_half<float>(y)));
  }
  static FLARE_FUNCTION val_type sqrt(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::sqrt(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type cbrt(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::cbrt(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type exp(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::exp(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type log(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::log(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type log10(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::log10(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type sin(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::sin(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type cos(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::cos(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type tan(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::tan(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type sinh(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::sinh(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type cosh(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::cosh(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type tanh(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::tanh(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type asin(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::asin(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type acos(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::acos(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION val_type atan(const val_type x) {
    return flare::experimental::cast_to_half(
        flare::atan(flare::experimental::cast_from_half<float>(x)));
  }
  static FLARE_FUNCTION mag_type epsilon() {
    return flare::experimental::cast_to_half(FLARE_IMPL_FP16_EPSILON);
  }
  ...
  static FLARE_FUNCTION bool isnaninf(const val_type x) {
    return isNan(x) || isInf(x);
  }
  static FLARE_FUNCTION magnitudeType magnitude(const val_type x) {
    return abs(x);
  ...
  static FLARE_FUNCTION val_type nan() {
    return flare::experimental::cast_to_half(
        flare::experimental::quiet_NaN<float>::value);
  }
  static FLARE_FUNCTION mag_type eps() { return epsilon(); }
  static FLARE_FUNCTION mag_type sfmin() {
    return flare::experimental::cast_to_half(FLARE_IMPL_FP16_MIN);
  }
  static FLARE_FUNCTION int base() { return FLARE_IMPL_FP16_RADIX; }
  // Use float to allow running on both host and device
  static FLARE_FUNCTION float prec() {
    float e = FLARE_IMPL_FP16_EPSILON;
    float b = (float)base();
    float r = e * b;
    return r;
  }
  static FLARE_FUNCTION int t() { return FLARE_IMPL_FP16_MANT_DIG; }
  static FLARE_FUNCTION mag_type rnd() { return one(); }
  static FLARE_FUNCTION int emin() { return FLARE_IMPL_FP16_MIN_EXP; }
  static FLARE_FUNCTION mag_type rmin() {
    return flare::experimental::cast_to_half(FLARE_IMPL_FP16_MIN);
  }
  static FLARE_FUNCTION int emax() { return FLARE_IMPL_FP16_MAX_EXP; }
  static FLARE_FUNCTION mag_type rmax() {
    return flare::experimental::cast_to_half(FLARE_IMPL_FP16_MAX);
  }
};
#endif  // FLARE_HALF_T_IS_FLOAT && FLARE_ENABLE_CUDA_HALF

// Since flare::experimental::bhalf_t falls back to float, only define
// ArithTraits if bhalf_t is a backend specialization
#if defined(FLARE_BHALF_T_IS_FLOAT) && !FLARE_BHALF_T_IS_FLOAT
    template <>
class ArithTraits<flare::experimental::bhalf_t> {
 public:
  using val_type = flare::experimental::bhalf_t;
  using mag_type = val_type;
  ...
  static constexpr bool has_infinity = true;
  static FLARE_FUNCTION val_type infinity() {
    return flare::experimental::cast_to_bhalf(
        flare::experimental::infinity<float>::value);
  }

  static FLARE_FUNCTION bool isInf(const val_type x) {
    return flare::isinf(flare::experimental::cast_from_bhalf<float>(x));
  }
  static FLARE_FUNCTION bool isNan(const val_type x) {
    return flare::isnan(flare::experimental::cast_from_bhalf<float>(x));
  }
  static FLARE_FUNCTION mag_type abs(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::abs(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type zero() {
    return flare::experimental::cast_to_bhalf(0.0F);
  }
  static FLARE_FUNCTION val_type one() {
    return flare::experimental::cast_to_bhalf(1.0F);
  }
  static FLARE_FUNCTION val_type min() {
    return flare::experimental::cast_to_bhalf(-FLARE_IMPL_BF16_MAX);
  }
  static FLARE_FUNCTION val_type max() {
    return flare::experimental::cast_to_bhalf(FLARE_IMPL_BF16_MAX);
  }
  static FLARE_FUNCTION mag_type real(const val_type x) { return x; }
  static FLARE_FUNCTION mag_type imag(const val_type) {
    return flare::experimental::cast_to_bhalf(0.0F);
  }
  static FLARE_FUNCTION val_type conj(const val_type x) { return x; }
  static FLARE_FUNCTION val_type pow(const val_type x, const val_type y) {
    return flare::experimental::cast_to_bhalf(
        flare::pow(flare::experimental::cast_from_bhalf<float>(x),
                    flare::experimental::cast_from_bhalf<float>(y)));
  }
  static FLARE_FUNCTION val_type sqrt(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::sqrt(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type cbrt(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::cbrt(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type exp(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::exp(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type log(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::log(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type log10(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::log10(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type sin(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::sin(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type cos(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::cos(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type tan(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::tan(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type sinh(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::sinh(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type cosh(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::cosh(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type tanh(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::tanh(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type asin(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::asin(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type acos(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::acos(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION val_type atan(const val_type x) {
    return flare::experimental::cast_to_bhalf(
        flare::atan(flare::experimental::cast_from_bhalf<float>(x)));
  }
  static FLARE_FUNCTION mag_type epsilon() {
    // return ::pow(2, -FLARE_IMPL_BF16_SIGNIFICAND_BITS);
    return flare::experimental::cast_to_bhalf(FLARE_IMPL_BF16_EPSILON);
  }
  // Backwards compatibility with Teuchos::ScalarTraits.
  using magnitudeType = mag_type;
  ...
  static FLARE_FUNCTION mag_type eps() { return epsilon(); }
  static FLARE_FUNCTION mag_type sfmin() {
    return flare::experimental::cast_to_bhalf(FLARE_IMPL_BF16_MIN);
  }
  static FLARE_FUNCTION int base() { return FLARE_IMPL_BF16_RADIX; }
  // Use float to allow running on both host and device
  static FLARE_FUNCTION float prec() {
    float e = FLARE_IMPL_BF16_EPSILON;
    float b = (float)base();
    float r = e * b;
    return r;
  }
  static FLARE_FUNCTION int t() { return FLARE_IMPL_BF16_MANT_DIG; }
  static FLARE_FUNCTION mag_type rnd() { return one(); }
  static FLARE_FUNCTION int emin() { return FLARE_IMPL_BF16_MIN_EXP; }
  static FLARE_FUNCTION mag_type rmin() {
    return flare::experimental::cast_to_bhalf(FLARE_IMPL_BF16_MIN);
  }
  static FLARE_FUNCTION int emax() { return FLARE_IMPL_BF16_MAX_EXP; }
  static FLARE_FUNCTION mag_type rmax() {
    return flare::experimental::cast_to_bhalf(FLARE_IMPL_BF16_MAX);
  }
};
    ...
    public:
        ...
        // Backwards compatibility with Teuchos::ScalarTraits.
        using magnitudeType   = mag_type;
        using halfPrecision   = float;  // Should we switch to flare::half_t
        using doublePrecision = double;
        ...
        FLARE_ARITH_TRAITS_REAL_FP(FLARE_FUNCTION)
    };
...
#endif  // FLARE_ENABLE_LIBQUADMATH

    template <>
    class ArithTraits< ::flare::complex<float> > {
    public:
        using val_type = ::flare::complex<float>;
        using mag_type = float;

        static std::string name() { return "flare::complex<float>"; }

        FLARE_ARITH_TRAITS_CMPLX_FP(FLARE_FUNCTION)
    };

    template <>
    class ArithTraits< ::flare::complex<double> > {
    public:
        using val_type = ::flare::complex<double>;
        using mag_type = double;

        static std::string name() { return "flare::complex<double>"; }
...
/// long double.
    ...
    public:
    //! flare internally replaces std::complex with flare::complex.
    using val_type = ::flare::complex<RealFloatType>;
...
#ifdef FLARE_ACTIVE_EXECUTION_MEMORY_SPACE_HOST
        using std::isinf;
#endif
...
struct [[deprecated]] ArithTraits<dd_real> {
  ...
  static FLARE_FUNCTION val_type sin(const val_type x) { return ::sin(x); }
  static FLARE_FUNCTION val_type cos(const val_type x) { return ::cos(x); }
  static FLARE_FUNCTION val_type tan(const val_type x) {
    return std::tan(x);
  }
  static FLARE_FUNCTION val_type sinh(const val_type x) { return ::sinh(x); }
  static FLARE_FUNCTION val_type cosh(const val_type x) { return ::cosh(x); }
  static FLARE_FUNCTION val_type tanh(const val_type x) { return ::tanh(x); }
  static FLARE_FUNCTION val_type asin(const val_type x) {
    return ::asin(x);
  }
  static FLARE_FUNCTION val_type acos(const val_type x) {
...
namespace flare::details {
    template <typename T>
    using ArithTraits [[deprecated("Use flare::ArithTraits instead")]] =
            ::flare::ArithTraits<T>;
