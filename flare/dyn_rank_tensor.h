...
/// \file dyn_rank_tensor.h
/// \brief Declaration and definition of flare::DynRankTensor.
///
/// This header file declares and defines flare::DynRankTensor and its
/// related nonmember functions.

#ifndef FLARE_DYN_RANK_TENSOR_H_
#define FLARE_DYN_RANK_TENSOR_H_
...
class DynRankTensor;  // forward declare

namespace detail {

template <typename Specialize>
struct DynRankDimTraits {
  enum : size_t { unspecified = FLARE_INVALID_INDEX };

  // Compute the rank of the tensor from the nonzero dimension arguments.
  ...
  // Compute the rank of the tensor from the nonzero layout arguments.
  template <typename Layout>
  FLARE_INLINE_FUNCTION static size_t computeRank(const Layout& layout) {
  ...
  FLARE_INLINE_FUNCTION static size_t computeRank(
      const flare::detail::TensorCtorProp<P...>& /* prop */,
      const Layout& layout) {
    return computeRank(layout);
  }

  // Create the layout for the rank-7 tensor.
  // Non-strided Layout
  template <typename Layout>
  FLARE_INLINE_FUNCTION static std::enable_if_t<
      (std::is_same<Layout, flare::LayoutRight>::value ||
       std::is_same<Layout, flare::LayoutLeft>::value),
      Layout>
  ...
  template <typename Layout>
  FLARE_INLINE_FUNCTION static std::enable_if_t<
      (std::is_same<Layout, flare::LayoutStride>::value), Layout>
  createLayout(const Layout& layout) {
    return Layout(layout.dimension[0] != unspecified ? layout.dimension[0] : 1,
                  layout.stride[0],
                  layout.dimension[1] != unspecified ? layout.dimension[1] : 1,
  ...
  createLayout(const flare::detail::TensorCtorProp<P...>& /* prop */,
               const typename Traits::array_layout& layout) {
    return createLayout(layout);
  }

  // Create a tensor from the given dimension arguments.
  // This is only necessary because the shmem constructor doesn't take a layout.
  //   NDE shmem Tensor's are not compatible with the added tensor_alloc value_type
  //   / fad_dim deduction functionality
  template <typename TensorType, typename TensorArg>
  static TensorType createTensor(const TensorArg& arg, const size_t N0,
                             const size_t N1, const size_t N2, const size_t N3,
                             const size_t N4, const size_t N5, const size_t N6,
                             const size_t N7) {
    return TensorType(arg, N0 != unspecified ? N0 : 1, N1 != unspecified ? N1 : 1,
...
reconstructLayout(const Layout& layout, iType dynrank) {
  return Layout(dynrank > 0 ? layout.dimension[0] : FLARE_INVALID_INDEX,
                dynrank > 1 ? layout.dimension[1] : FLARE_INVALID_INDEX,
                dynrank > 2 ? layout.dimension[2] : FLARE_INVALID_INDEX,
                dynrank > 3 ? layout.dimension[3] : FLARE_INVALID_INDEX,
                dynrank > 4 ? layout.dimension[4] : FLARE_INVALID_INDEX,
                dynrank > 5 ? layout.dimension[5] : FLARE_INVALID_INDEX,
                dynrank > 6 ? layout.dimension[6] : FLARE_INVALID_INDEX,
                dynrank > 7 ? layout.dimension[7] : FLARE_INVALID_INDEX);
}

// LayoutStride
template <typename Layout, typename iType>
FLARE_INLINE_FUNCTION static std::enable_if_t<
    (std::is_same<Layout, flare::LayoutStride>::value) &&
        std::is_integral<iType>::value,
    Layout>
reconstructLayout(const Layout& layout, iType dynrank) {
  return Layout(dynrank > 0 ? layout.dimension[0] : FLARE_INVALID_INDEX,
...
// TensorMapping; additional checks for extra arguments beyond rank are 0
template <unsigned, typename iType0, class MapType>
FLARE_INLINE_FUNCTION bool dyn_rank_tensor_verify_operator_bounds(
...
FLARE_INLINE_FUNCTION bool dyn_rank_tensor_verify_operator_bounds(
  ...
  if (static_cast<iType0>(R) < rank) {
    return (size_t(i) < map.extent(R)) &&
           dyn_rank_tensor_verify_operator_bounds<R + 1>(rank, map, args...);
  } else if (i != 0) {
    flare::printf(
        "DynRankTensor Debug Bounds Checking Error: at rank %u\n  Extra "
...
inline void dyn_rank_tensor_error_operator_bounds(char*, int, const MapType&) {}

template <unsigned R, class MapType, class iType, class... Args>
inline void dyn_rank_tensor_error_operator_bounds(char* buf, int len,
                                                const MapType& map,
                                                const iType& i, Args... args) {
  const int n = snprintf(
      buf, len, " %ld < %ld %c", static_cast<unsigned long>(i),
      static_cast<unsigned long>(map.extent(R)), (sizeof...(Args) ? ',' : ')'));
  dyn_rank_tensor_error_operator_bounds<R + 1>(buf + n, len - n, map, args...);
}
...
template <typename MemorySpace, typename iType0, typename iType1, class MapType,
          class... Args>
FLARE_INLINE_FUNCTION void dyn_rank_tensor_verify_operator_bounds(
    const iType0& op_rank, const iType1& rank,
    const flare::detail::SharedAllocationTracker& tracker, const MapType& map,
    Args... args) {
  if (static_cast<iType0>(rank) > op_rank) {
    flare::abort(
        "DynRankTensor Bounds Checking Error: Need at least rank arguments to "
        "the operator()");
  }

  if (!dyn_rank_tensor_verify_operator_bounds<0>(rank, map, args...)) {
    FLARE_IF_ON_HOST(
        (enum {LEN = 1024}; char buffer[LEN];
         const std::string label = tracker.template get_label<MemorySpace>();
         int n = snprintf(buffer, LEN, "DynRankTensor bounds error of tensor %s (",
                          label.c_str());
         dyn_rank_tensor_error_operator_bounds<0>(buffer + n, LEN - n, map,
                                                args...);
         flare::detail::throw_runtime_exception(std::string(buffer));))

    FLARE_IF_ON_DEVICE(
        ((void)tracker; flare::abort("DynRankTensor bounds error");))
  }
}

/** \brief  Assign compatible default mappings */
struct TensorToDynRankTensorTag {};
...
class TensorMapping<
                      ...
                      (std::is_same<typename DstTraits::array_layout,
                       ...
                       ((std::is_same<typename DstTraits::array_layout,
                        ...
                        (std::is_same<typename SrcTraits::array_layout,
                         ...
                         std::is_same<typename SrcTraits::array_layout,
                                      flare::LayoutStride>::value)))),
                     flare::detail::TensorToDynRankTensorTag>> {
 private:
  enum {
 ...
 public:
  ...
  using DstType = TensorMapping<DstTraits, typename DstTraits::specialize>;
  using SrcType = TensorMapping<SrcTraits, typename SrcTraits::specialize>;

  template <typename DT, typename... DP, typename ST, typename... SP>
  FLARE_INLINE_FUNCTION static void assign(
      flare::DynRankTensor<DT, DP...>& dst, const flare::Tensor<ST, SP...>& src) {
    static_assert(
        is_assignable_value_type,
...
/* \class DynRankTensor
 * \brief Container that creates a flare tensor with rank determined at runtime.
 *   Essentially this is a rank 7 tensor
 *
 *   Changes from Tensor
 *   1. The rank of the DynRankTensor is returned by the method rank()
 *   2. Max rank of a DynRankTensor is 7
 *   3. subtensor called with 'subtensor(...)' or 'subdynranktensor(...)' (backward
 * compatibility)
 *   4. Every subtensor is returned with LayoutStride
 *   5. Copy and Copy-Assign Tensor to DynRankTensor
 *   6. deep_copy between Tensors and DynRankTensors
 *   7. rank( tensor ); returns the rank of Tensor or DynRankTensor
 *
 */

template <class>
struct is_dyn_rank_tensor : public std::false_type {};

template <class D, class... P>
struct is_dyn_rank_tensor<flare::DynRankTensor<D, P...>> : public std::true_type {
};

template <class T>
inline constexpr bool is_dyn_rank_tensor_v = is_dyn_rank_tensor<T>::value;

template <typename DataType, class... Properties>
class DynRankTensor : public TensorTraits<DataType, Properties...> {
  static_assert(!std::is_array<DataType>::value &&
                    !std::is_pointer<DataType>::value,
                "Cannot template DynRankTensor with array or pointer datatype - "
                "must be pod");

 private:
  template <class, class...>
  friend class DynRankTensor;
  template <class, class...>
  friend class flare::detail::TensorMapping;

 public:
  using drvtraits = TensorTraits<DataType, Properties...>;

  using tensor_type = Tensor<DataType*******, Properties...>;
 ...
 public:
  FLARE_INLINE_FUNCTION
  tensor_type& DownCast() const { return (tensor_type&)(*this); }
  FLARE_INLINE_FUNCTION
  const tensor_type& ConstDownCast() const { return (const tensor_type&)(*this); }
  ...
  using array_type = DynRankTensor<
  ...
  FLARE_INLINE_FUNCTION constexpr typename traits::array_layout layout() const;
...
#define FLARE_IMPL_TENSOR_OPERATOR_VERIFY(ARG)                             \
  flare::detail::runtime_check_memory_access_violation<                    \
      typename traits::memory_space>(                                     \
      "flare::DynRankTensor ERROR: attempt to access inaccessible memory " \
      "space");                                                           \
  flare::detail::dyn_rank_tensor_verify_operator_bounds<                     \
      typename traits::memory_space>                                      \
      ARG;
...
#define FLARE_IMPL_TENSOR_OPERATOR_VERIFY(ARG)                             \
 ...
 public:
  ...
  reference_type operator()() const {
    FLARE_IMPL_TENSOR_OPERATOR_VERIFY((0, this->rank(), m_track, m_map))
  ...
  operator[](const iType& i0) const {
    ...
    // elements in the allocation FLARE_IMPL_TENSOR_OPERATOR_VERIFY( (1 ,
  ...
  operator[](const iType& i0) const {
    ...
    using tmp_tensor_type = flare::Tensor<
        DataType*, typename traits::array_layout, typename traits::device_type,
        flare::MemoryTraits<traits::memory_traits::is_unmanaged |
                             traits::memory_traits::is_random_access |
                             traits::memory_traits::is_atomic>>;
    tmp_tensor_type rankone_tensor(this->data(), bytes, dim_scalar);
    return rankone_tensor(i0);
  }
  ...
  FLARE_INLINE_FUNCTION
      std::enable_if_t<(std::is_void<typename traits::specialize>::value &&
                       ...
                       reference_type>
      operator()(const iType& i0) const {
    FLARE_IMPL_TENSOR_OPERATOR_VERIFY((1, this->rank(), m_track, m_map, i0))
  ...
  operator()(const iType0& i0, const iType1& i1) const {
    FLARE_IMPL_TENSOR_OPERATOR_VERIFY((2, this->rank(), m_track, m_map, i0, i1))
  ...
  operator()(const iType0& i0, const iType1& i1, const iType2& i2) const {
    FLARE_IMPL_TENSOR_OPERATOR_VERIFY(
...
#undef FLARE_IMPL_TENSOR_OPERATOR_VERIFY
  ...
  ~DynRankTensor() = default;

  FLARE_INLINE_FUNCTION
  DynRankTensor() : m_track(), m_map(), m_rank() {}  // Default ctor

  FLARE_INLINE_FUNCTION
  DynRankTensor(const DynRankTensor& rhs)
      : m_track(rhs.m_track), m_map(rhs.m_map), m_rank(rhs.m_rank) {}

  FLARE_INLINE_FUNCTION
  DynRankTensor(DynRankTensor&& rhs)
      : m_track(rhs.m_track), m_map(rhs.m_map), m_rank(rhs.m_rank) {}

  FLARE_INLINE_FUNCTION
  DynRankTensor& operator=(const DynRankTensor& rhs) {
    m_track = rhs.m_track;
    m_map   = rhs.m_map;
    m_rank  = rhs.m_rank;
    return *this;
  ...
  FLARE_INLINE_FUNCTION
  DynRankTensor& operator=(DynRankTensor&& rhs) {
    m_track = rhs.m_track;
    m_map   = rhs.m_map;
    m_rank  = rhs.m_rank;
    return *this;
  ...
  FLARE_INLINE_FUNCTION DynRankTensor(const DynRankTensor<RT, RP...>& rhs)
      : m_track(rhs.m_track, traits::is_managed), m_map(), m_rank(rhs.m_rank) {
    using SrcTraits = typename DynRankTensor<RT, RP...>::traits;
    using Mapping   = flare::detail::TensorMapping<traits, SrcTraits,
                                              typename traits::specialize>;
    static_assert(Mapping::is_assignable,
                  "Incompatible DynRankTensor copy construction");
    Mapping::assign(m_map, rhs.m_map, rhs.m_track);
  }

  template <class RT, class... RP>
  FLARE_INLINE_FUNCTION DynRankTensor& operator=(
      const DynRankTensor<RT, RP...>& rhs) {
    using SrcTraits = typename DynRankTensor<RT, RP...>::traits;
    using Mapping   = flare::detail::TensorMapping<traits, SrcTraits,
                                              typename traits::specialize>;
    static_assert(Mapping::is_assignable,
  ...
  template <class RT, class... RP>
  FLARE_INLINE_FUNCTION DynRankTensor(const Tensor<RT, RP...>& rhs)
      : m_track(), m_map(), m_rank(Tensor<RT, RP...>::rank()) {
    using SrcTraits = typename Tensor<RT, RP...>::traits;
    using Mapping =
        flare::detail::TensorMapping<traits, SrcTraits,
                                  flare::detail::TensorToDynRankTensorTag>;
    static_assert(Mapping::is_assignable,
                  "Incompatible Tensor to DynRankTensor copy construction");
    Mapping::assign(*this, rhs);
  }

  template <class RT, class... RP>
  FLARE_INLINE_FUNCTION DynRankTensor& operator=(const Tensor<RT, RP...>& rhs) {
    using SrcTraits = typename Tensor<RT, RP...>::traits;
    using Mapping =
        flare::detail::TensorMapping<traits, SrcTraits,
                                  flare::detail::TensorToDynRankTensorTag>;
    static_assert(Mapping::is_assignable,
                  "Incompatible Tensor to DynRankTensor copy assignment");
    Mapping::assign(*this, rhs);
    return *this;
  }
  ...
  // unused arg_layout dimensions must be set to FLARE_INVALID_INDEX so that
  // rank deduction can properly take place
  template <class... P>
  explicit inline DynRankTensor(
      const flare::detail::TensorCtorProp<P...>& arg_prop,
      std::enable_if_t<!flare::detail::TensorCtorProp<P...>::has_pointer,
                       typename traits::array_layout> const& arg_layout)
      : m_track(),
        m_map(),
        m_rank(detail::DynRankDimTraits<typename traits::specialize>::
    ...
    if (alloc_prop::initialize &&
      ...
      flare::detail::throw_runtime_exception(
          "Constructing DynRankTensor and initializing data with uninitialized "
          "execution space");
    }

    flare::detail::SharedAllocationRecord<>* record = m_map.allocate_shared(
        prop_copy,
        detail::DynRankDimTraits<typename traits::specialize>::
            template createLayout<traits, P...>(arg_prop, arg_layout),
        detail::TensorCtorProp<P...>::has_execution_space);
  ...
  // Wrappers
  template <class... P>
  explicit FLARE_INLINE_FUNCTION DynRankTensor(
      const flare::detail::TensorCtorProp<P...>& arg_prop,
      std::enable_if_t<flare::detail::TensorCtorProp<P...>::has_pointer,
                       typename traits::array_layout> const& arg_layout)
    ...
    static_assert(
        ...
        "Constructing DynRankTensor to wrap user memory must supply matching "
        "pointer type");
  }
  ...
  explicit inline DynRankTensor(
      const flare::detail::TensorCtorProp<P...>& arg_prop,
      std::enable_if_t<!flare::detail::TensorCtorProp<P...>::has_pointer,
                       size_t> const arg_N0 = FLARE_INVALID_INDEX,
      ...
      : DynRankTensor(arg_prop, typename traits::array_layout(
  ...
  explicit inline DynRankTensor(
      ...
      std::enable_if_t<flare::detail::is_tensor_label<Label>::value,
                       typename traits::array_layout> const& arg_layout)
      : DynRankTensor(flare::detail::TensorCtorProp<std::string>(arg_label),
                    arg_layout) {}
  ...
  explicit FLARE_INLINE_FUNCTION DynRankTensor(
      pointer_type arg_ptr, const size_t arg_N0 = FLARE_INVALID_INDEX,
      const size_t arg_N1 = FLARE_INVALID_INDEX,
      const size_t arg_N2 = FLARE_INVALID_INDEX,
      const size_t arg_N3 = FLARE_INVALID_INDEX,
      const size_t arg_N4 = FLARE_INVALID_INDEX,
      ...
      const size_t arg_N6 = FLARE_INVALID_INDEX,
      const size_t arg_N7 = FLARE_INVALID_INDEX)
      : DynRankTensor(flare::detail::TensorCtorProp<pointer_type>(arg_ptr), arg_N0,
                    arg_N1, arg_N2, arg_N3, arg_N4, arg_N5, arg_N6, arg_N7) {}

  explicit FLARE_INLINE_FUNCTION DynRankTensor(
      pointer_type arg_ptr, typename traits::array_layout& arg_layout)
      : DynRankTensor(flare::detail::TensorCtorProp<pointer_type>(arg_ptr),
                    arg_layout) {}

  //----------------------------------------
  // Shared scratch memory constructor

  static inline size_t shmem_size(const size_t arg_N0 = FLARE_INVALID_INDEX,
                                  const size_t arg_N1 = FLARE_INVALID_INDEX,
                                  const size_t arg_N2 = FLARE_INVALID_INDEX,
                                  const size_t arg_N3 = FLARE_INVALID_INDEX,
                                  const size_t arg_N4 = FLARE_INVALID_INDEX,
    ...
    const size_t num_passed_args =
        (arg_N0 != FLARE_INVALID_INDEX) + (arg_N1 != FLARE_INVALID_INDEX) +
        (arg_N2 != FLARE_INVALID_INDEX) + (arg_N3 != FLARE_INVALID_INDEX) +
        (arg_N4 != FLARE_INVALID_INDEX) + (arg_N5 != FLARE_INVALID_INDEX) +
        (arg_N6 != FLARE_INVALID_INDEX) + (arg_N7 != FLARE_INVALID_INDEX);
  ...
  explicit FLARE_INLINE_FUNCTION DynRankTensor(
      ...
      : DynRankTensor(
            flare::detail::TensorCtorProp<pointer_type>(
  ...
  explicit FLARE_INLINE_FUNCTION DynRankTensor(
      const typename traits::execution_space::scratch_memory_space& arg_space,
      const size_t arg_N0 = FLARE_INVALID_INDEX,
      const size_t arg_N1 = FLARE_INVALID_INDEX,
      const size_t arg_N2 = FLARE_INVALID_INDEX,
      const size_t arg_N3 = FLARE_INVALID_INDEX,
      const size_t arg_N4 = FLARE_INVALID_INDEX,
...
FLARE_INLINE_FUNCTION constexpr unsigned rank(
    const DynRankTensor<D, P...>& DRV) {
  return DRV.rank();
}  // needed for transition to common constexpr method in tensor and dynranktensor
   // to return rank
...
class TensorMapping<
    std::enable_if_t<(std::is_void<typename SrcTraits::specialize>::value &&
                      (std::is_same<typename SrcTraits::array_layout,
                       ...
                       std::is_same<typename SrcTraits::array_layout,
                                    flare::LayoutStride>::value)),
                     flare::detail::DynRankSubtensorTag>,
    SrcTraits, Args...> {
 private:
  enum {
    RZ = false,
 ...
 public:
  ...
  struct ExtentGenerator {
    FLARE_INLINE_FUNCTION
    static SubTensorExtents<7, rank> generator(
      ...
      return SubTensorExtents<7, rank>(dim, arg0, arg1, arg2, arg3, arg4, arg5,
                                     arg6);
    }
  };

  using ret_type = flare::DynRankTensor<value_type, array_layout,
                                       typename SrcTraits::device_type,
                                       typename SrcTraits::memory_traits>;

  template <typename T, class... P>
  FLARE_INLINE_FUNCTION static ret_type subtensor(
      const unsigned src_rank, flare::DynRankTensor<T, P...> const& src,
      Args... args) {
    using DstType = TensorMapping<traits_type, typename traits_type::specialize>;

    using DstDimType = std::conditional_t<
        (rank == 0), TensorDimension<>,
        std::conditional_t<
            (rank == 1), TensorDimension<0>,
            std::conditional_t<
                (rank == 2), TensorDimension<0, 0>,
                std::conditional_t<
                    (rank == 3), TensorDimension<0, 0, 0>,
                    std::conditional_t<
                        (rank == 4), TensorDimension<0, 0, 0, 0>,
                        std::conditional_t<
                            (rank == 5), TensorDimension<0, 0, 0, 0, 0>,
                            std::conditional_t<
                                (rank == 6), TensorDimension<0, 0, 0, 0, 0, 0>,
                                TensorDimension<0, 0, 0, 0, 0, 0, 0>>>>>>>>;

    using dst_offset_type = TensorOffset<DstDimType, flare::LayoutStride>;
    using dst_handle_type = typename DstType::handle_type;

    ret_type dst;

    const SubTensorExtents<7, rank> extents = ExtentGenerator<Args...>::generator(
...
using Subdynranktensor =
    typename flare::detail::TensorMapping<flare::detail::DynRankSubtensorTag, V,
                                       Args...>::ret_type;

template <class D, class... P, class... Args>
FLARE_INLINE_FUNCTION Subdynranktensor<TensorTraits<D*******, P...>, Args...>
subdynranktensor(const flare::DynRankTensor<D, P...>& src, Args... args) {
  if (src.rank() > sizeof...(Args))  // allow sizeof...(Args) >= src.rank(),
                                     // ignore the remaining args
  {
    flare::abort(
        "subdynranktensor: num of args must be >= rank of the source "
        "DynRankTensor");
  }

  using metafcn =
      flare::detail::TensorMapping<flare::detail::DynRankSubtensorTag,
                                flare::TensorTraits<D*******, P...>, Args...>;

  return metafcn::subtensor(src.rank(), src, args...);
}

// Wrapper to allow subtensor function name
template <class D, class... P, class... Args>
FLARE_INLINE_FUNCTION Subdynranktensor<TensorTraits<D*******, P...>, Args...>
subtensor(const flare::DynRankTensor<D, P...>& src, Args... args) {
  return subdynranktensor(src, args...);
}
...
FLARE_INLINE_FUNCTION bool operator==(const DynRankTensor<LT, LP...>& lhs,
                                       const DynRankTensor<RT, RP...>& rhs) {
...
template <class OutputTensor, class Enable = void>
struct DynRankTensorFill {
  using const_value_type = typename OutputTensor::traits::const_value_type;
  ...
  DynRankTensorFill(const OutputTensor& arg_out, const_value_type& arg_in)
      : output(arg_out), input(arg_in) {
    using execution_space = typename OutputTensor::execution_space;
    using Policy          = flare::RangePolicy<execution_space>;

    flare::parallel_for("flare::DynRankTensorFill", Policy(0, output.extent(0)),
                         *this);
  }
};

template <class OutputTensor>
struct DynRankTensorFill<OutputTensor, std::enable_if_t<OutputTensor::rank == 0>> {
  DynRankTensorFill(const OutputTensor& dst,
                  const typename OutputTensor::const_value_type& src) {
    flare::detail::DeepCopy<typename OutputTensor::memory_space,
                           flare::HostSpace>(
        dst.data(), &src, sizeof(typename OutputTensor::const_value_type));
  }
};

template <class OutputTensor, class InputTensor,
          class ExecSpace = typename OutputTensor::execution_space>
struct DynRankTensorRemap {
  const OutputTensor output;
  ...
  DynRankTensorRemap(const ExecSpace& exec_space, const OutputTensor& arg_out,
                   const InputTensor& arg_in)
      : output(arg_out),
        input(arg_in),
    ...
    using Policy = flare::RangePolicy<ExecSpace>;

    flare::parallel_for("flare::DynRankTensorRemap", Policy(exec_space, 0, n0),
                         *this);
  }

  DynRankTensorRemap(const OutputTensor& arg_out, const InputTensor& arg_in)
      : output(arg_out),
        input(arg_in),
        n0(std::min((size_t)arg_out.extent(0), (size_t)arg_in.extent(0))),
        n1(std::min((size_t)arg_out.extent(1), (size_t)arg_in.extent(1))),
    ...
    flare::parallel_for("flare::DynRankTensorRemap", Policy(0, n0), *this);
  }
...
/* \brief Returns a Tensor of the requested rank, aliasing the
   underlying memory, to facilitate implementation of deep_copy() and
   other routines that are defined on Tensor */
template <unsigned N, typename T, typename... Args>
FLARE_FUNCTION auto as_tensor_of_rank_n(
    DynRankTensor<T, Args...> v,
    typename std::enable_if<std::is_same<
        typename TensorTraits<T, Args...>::specialize, void>::value>::type* =
        nullptr) {
  if (v.rank() != N) {
    FLARE_IF_ON_HOST(
        const std::string message =
            "Converting DynRankTensor of rank " + std::to_string(v.rank()) +
            " to a Tensor of mis-matched rank " + std::to_string(N) + "!";
        flare::abort(message.c_str());)
    FLARE_IF_ON_DEVICE(
        flare::abort("Converting DynRankTensor to a Tensor of mis-matched rank!");)
  }

  return Tensor<typename RankDataType<T, N>::type, Args...>(
      v.data(), v.impl_map().layout());
}

template <typename Function, typename... Args>
void apply_to_tensor_of_static_rank(Function&& f, DynRankTensor<Args...> a) {
  switch (rank(a)) {
    case 0: f(as_tensor_of_rank_n<0>(a)); break;
    case 1: f(as_tensor_of_rank_n<1>(a)); break;
    case 2: f(as_tensor_of_rank_n<2>(a)); break;
    case 3: f(as_tensor_of_rank_n<3>(a)); break;
    case 4: f(as_tensor_of_rank_n<4>(a)); break;
    case 5: f(as_tensor_of_rank_n<5>(a)); break;
    case 6: f(as_tensor_of_rank_n<6>(a)); break;
    case 7: f(as_tensor_of_rank_n<7>(a)); break;
    default:
      FLARE_IF_ON_HOST(
          flare::abort(
              std::string(
                  "Trying to apply a function to a tensor of unexpected rank " +
...
template <typename D, class... P>
FLARE_INLINE_FUNCTION constexpr auto DynRankTensor<D, P...>::layout() const ->
    typename traits::array_layout {
  switch (rank()) {
    case 0: return detail::as_tensor_of_rank_n<0>(*this).layout();
    case 1: return detail::as_tensor_of_rank_n<1>(*this).layout();
    case 2: return detail::as_tensor_of_rank_n<2>(*this).layout();
    case 3: return detail::as_tensor_of_rank_n<3>(*this).layout();
    case 4: return detail::as_tensor_of_rank_n<4>(*this).layout();
    case 5: return detail::as_tensor_of_rank_n<5>(*this).layout();
    case 6: return detail::as_tensor_of_rank_n<6>(*this).layout();
    case 7: return detail::as_tensor_of_rank_n<7>(*this).layout();
    default:
      FLARE_IF_ON_HOST(
          flare::abort(
              std::string(
                  "Calling DynRankTensor::layout on DRV of unexpected rank " +
                  std::to_string(rank()))
                  .c_str());)
      FLARE_IF_ON_DEVICE(
          flare::abort(
...
inline void deep_copy(
    const ExecSpace& e, const DynRankTensor<DT, DP...>& dst,
  ...
  detail::apply_to_tensor_of_static_rank(
      [=](auto tensor) { deep_copy(e, tensor, value); }, dst);
}

template <class DT, class... DP>
inline void deep_copy(
    const DynRankTensor<DT, DP...>& dst,
    typename TensorTraits<DT, DP...>::const_value_type& value,
    std::enable_if_t<std::is_same<typename TensorTraits<DT, DP...>::specialize,
                                  void>::value>* = nullptr) {
  detail::apply_to_tensor_of_static_rank([=](auto tensor) { deep_copy(tensor, value); },
                                     dst);
}
...
inline void deep_copy(
    ...
    const DynRankTensor<ST, SP...>& src,
    std::enable_if_t<std::is_same<typename TensorTraits<ST, SP...>::specialize,
                                  void>::value>* = 0) {
  deep_copy(e, dst, detail::as_tensor_of_rank_n<0>(src));
}
...
inline void deep_copy(
    ...
    std::enable_if_t<std::is_same<typename TensorTraits<ST, SP...>::specialize,
                                  void>::value>* = 0) {
  deep_copy(dst, detail::as_tensor_of_rank_n<0>(src));
}
 ...
 *
 * A rank mismatch will error out in the attempt to convert to a Tensor
 */
...
inline void deep_copy(
    ...
    std::enable_if_t<
        (std::is_void<typename DstType::traits::specialize>::value &&
         ...
         (flare::is_dyn_rank_tensor<DstType>::value ||
          flare::is_dyn_rank_tensor<SrcType>::value))>* = nullptr) {
  static_assert(
  ...
  switch (rank(dst)) {
    case 0:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<0>(dst),
                detail::as_tensor_of_rank_n<0>(src));
      break;
    case 1:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<1>(dst),
                detail::as_tensor_of_rank_n<1>(src));
      break;
    case 2:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<2>(dst),
                detail::as_tensor_of_rank_n<2>(src));
      break;
    case 3:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<3>(dst),
                detail::as_tensor_of_rank_n<3>(src));
      break;
    case 4:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<4>(dst),
                detail::as_tensor_of_rank_n<4>(src));
      break;
    case 5:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<5>(dst),
                detail::as_tensor_of_rank_n<5>(src));
      break;
    case 6:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<6>(dst),
                detail::as_tensor_of_rank_n<6>(src));
      break;
    case 7:
      deep_copy(exec_space, detail::as_tensor_of_rank_n<7>(dst),
                detail::as_tensor_of_rank_n<7>(src));
      break;
    default:
      flare::detail::throw_runtime_exception(
          "Calling DynRankTensor deep_copy with a tensor of unexpected rank " +
...
inline void deep_copy(
  ...
  switch (rank(dst)) {
    case 0:
      deep_copy(detail::as_tensor_of_rank_n<0>(dst),
                detail::as_tensor_of_rank_n<0>(src));
      break;
    case 1:
      deep_copy(detail::as_tensor_of_rank_n<1>(dst),
                detail::as_tensor_of_rank_n<1>(src));
      break;
    case 2:
      deep_copy(detail::as_tensor_of_rank_n<2>(dst),
                detail::as_tensor_of_rank_n<2>(src));
      break;
    case 3:
      deep_copy(detail::as_tensor_of_rank_n<3>(dst),
                detail::as_tensor_of_rank_n<3>(src));
      break;
    case 4:
      deep_copy(detail::as_tensor_of_rank_n<4>(dst),
                detail::as_tensor_of_rank_n<4>(src));
      break;
    case 5:
      deep_copy(detail::as_tensor_of_rank_n<5>(dst),
                detail::as_tensor_of_rank_n<5>(src));
      break;
    case 6:
      deep_copy(detail::as_tensor_of_rank_n<6>(dst),
                detail::as_tensor_of_rank_n<6>(src));
      break;
    case 7:
      deep_copy(detail::as_tensor_of_rank_n<7>(dst),
                detail::as_tensor_of_rank_n<7>(src));
      break;
    default:
      flare::detail::throw_runtime_exception(
...
struct MirrorDRTensorType {
  // The incoming tensor_type
  using src_tensor_type = typename flare::DynRankTensor<T, P...>;
  ...
  using dest_tensor_type = flare::DynRankTensor<data_type, array_layout, Space>;
...
struct MirrorDRVType {
  ...
  using data_type = typename src_tensor_type::non_const_data_type;
  // The destination tensor type if it is not the same memory space
  using tensor_type = flare::DynRankTensor<data_type, array_layout, Space>;
};
...
inline typename DynRankTensor<T, P...>::HostMirror create_mirror(
  ...
  return dst_type(prop_copy, detail::reconstructLayout(src.layout(), src.rank()));
}
...
inline auto create_mirror(
    ...
    std::enable_if_t<detail::TensorCtorProp<TensorCtorArgs...>::has_memory_space>* =
        nullptr) {
  using dst_type = typename detail::MirrorDRVType<
...
inline typename DynRankTensor<T, P...>::HostMirror create_mirror(
    const DynRankTensor<T, P...>& src,
    std::enable_if_t<std::is_same<typename TensorTraits<T, P...>::specialize,
                                  void>::value>* = nullptr) {
  return detail::create_mirror(src, flare::detail::TensorCtorProp<>{});
}

template <class T, class... P>
inline typename DynRankTensor<T, P...>::HostMirror create_mirror(
    flare::detail::WithoutInitializing_t wi, const DynRankTensor<T, P...>& src,
    std::enable_if_t<std::is_same<typename TensorTraits<T, P...>::specialize,
                                  void>::value>* = nullptr) {
...
inline typename DynRankTensor<T, P...>::HostMirror create_mirror(
    ...
    std::enable_if_t<
        ...
        !detail::TensorCtorProp<TensorCtorArgs...>::has_memory_space>* = nullptr) {
...
template <class Space, class T, class... P,
          typename Enable = std::enable_if_t<
              flare::is_space<Space>::value &&
              std::is_void<typename TensorTraits<T, P...>::specialize>::value>>
typename detail::MirrorDRVType<Space, T, P...>::tensor_type create_mirror(
    const Space&, const flare::DynRankTensor<T, P...>& src) {
...
typename detail::MirrorDRVType<Space, T, P...>::tensor_type create_mirror(
    ...
    const flare::DynRankTensor<T, P...>& src,
    std::enable_if_t<std::is_same<typename TensorTraits<T, P...>::specialize,
                                  void>::value>* = nullptr) {
  return detail::create_mirror(
...
inline auto create_mirror(
    ...
    std::enable_if_t<
        std::is_void<typename TensorTraits<T, P...>::specialize>::value &&
        detail::TensorCtorProp<TensorCtorArgs...>::has_memory_space>* = nullptr) {
  using ReturnType = typename detail::MirrorDRVType<
      typename detail::TensorCtorProp<TensorCtorArgs...>::memory_space, T,
      P...>::tensor_type;
  return ReturnType{detail::create_mirror(src, arg_prop)};
}
...
inline std::enable_if_t<
    !detail::TensorCtorProp<TensorCtorArgs...>::has_memory_space &&
        std::is_same<
            typename DynRankTensor<T, P...>::memory_space,
            typename DynRankTensor<T, P...>::HostMirror::memory_space>::value &&
        std::is_same<
            typename DynRankTensor<T, P...>::data_type,
            typename DynRankTensor<T, P...>::HostMirror::data_type>::value,
    typename DynRankTensor<T, P...>::HostMirror>
create_mirror_tensor(const DynRankTensor<T, P...>& src,
                   const typename detail::TensorCtorProp<TensorCtorArgs...>&) {
  return src;
}
...
create_mirror_tensor(
    const DynRankTensor<T, P...>& src,
    const typename detail::TensorCtorProp<TensorCtorArgs...>& arg_prop) {
...
inline std::enable_if_t<
    flare::is_space<
        typename detail::TensorCtorProp<TensorCtorArgs...>::memory_space>::value &&
        detail::MirrorDRTensorType<
            typename detail::TensorCtorProp<TensorCtorArgs...>::memory_space, T,
            P...>::is_same_memspace,
    typename detail::MirrorDRTensorType<
        typename detail::TensorCtorProp<TensorCtorArgs...>::memory_space, T,
        P...>::tensor_type>
create_mirror_tensor(const flare::DynRankTensor<T, P...>& src,
                   const typename detail::TensorCtorProp<TensorCtorArgs...>&) {
  return src;
}
...
inline std::enable_if_t<
    (std::is_same<
         typename DynRankTensor<T, P...>::memory_space,
         typename DynRankTensor<T, P...>::HostMirror::memory_space>::value &&
     std::is_same<typename DynRankTensor<T, P...>::data_type,
                  typename DynRankTensor<T, P...>::HostMirror::data_type>::value),
    typename DynRankTensor<T, P...>::HostMirror>
create_mirror_tensor(const flare::DynRankTensor<T, P...>& src) {
  return src;
...
template <class T, class... P>
inline auto create_mirror_tensor(flare::detail::WithoutInitializing_t wi,
                               const DynRankTensor<T, P...>& src) {
...
inline typename detail::MirrorDRTensorType<Space, T, P...>::tensor_type
create_mirror_tensor(
    const Space&, const flare::DynRankTensor<T, P...>& src,
    std::enable_if_t<
        detail::MirrorDRTensorType<Space, T, P...>::is_same_memspace>* = nullptr) {
  return src;
}
...
inline typename detail::MirrorDRTensorType<Space, T, P...>::tensor_type
create_mirror_tensor(
    const Space& space, const flare::DynRankTensor<T, P...>& src,
    std::enable_if_t<
        !detail::MirrorDRTensorType<Space, T, P...>::is_same_memspace>* = nullptr) {
...
auto create_mirror_tensor_and_copy(
  ...
  using Mirror = typename detail::MirrorDRTensorType<Space, T, P...>::tensor_type;
  ...
  auto mirror = typename Mirror::non_const_type{
      arg_prop_copy, detail::reconstructLayout(src.layout(), src.rank())};
...
template <class Space, class T, class... P>
auto create_mirror_tensor_and_copy(const Space&,
                                 const flare::DynRankTensor<T, P...>& src,
 ...
 * indices. */
template <class... TensorCtorArgs, class T, class... P>
inline void impl_resize(const detail::TensorCtorProp<TensorCtorArgs...>& arg_prop,
                        DynRankTensor<T, P...>& v, const size_t n0,
                        const size_t n1, const size_t n2, const size_t n3,
                        const size_t n4, const size_t n5, const size_t n6,
                        const size_t n7) {
  using drtensor_type      = DynRankTensor<T, P...>;
  using alloc_prop_input = detail::TensorCtorProp<TensorCtorArgs...>;
  ...
  auto prop_copy = detail::with_properties_if_unset(
      arg_prop, v.label(), typename drtensor_type::execution_space{});

  drtensor_type v_resized(prop_copy, n0, n1, n2, n3, n4, n5, n6, n7);

  if constexpr (alloc_prop_input::has_execution_space)
    flare::detail::DynRankTensorRemap<drtensor_type, drtensor_type>(
        detail::get_property<detail::ExecutionSpaceTag>(prop_copy), v_resized, v);
  else {
    flare::detail::DynRankTensorRemap<drtensor_type, drtensor_type>(v_resized, v);
    flare::fence("flare::resize(DynRankTensor)");
  }
  v = v_resized;
}

template <class T, class... P>
inline void resize(DynRankTensor<T, P...>& v,
                   const size_t n0 = FLARE_INVALID_INDEX,
                   const size_t n1 = FLARE_INVALID_INDEX,
                   const size_t n2 = FLARE_INVALID_INDEX,
                   const size_t n3 = FLARE_INVALID_INDEX,
                   const size_t n4 = FLARE_INVALID_INDEX,
                   const size_t n5 = FLARE_INVALID_INDEX,
                   const size_t n6 = FLARE_INVALID_INDEX,
                   const size_t n7 = FLARE_INVALID_INDEX) {
...
inline std::enable_if_t<detail::is_tensor_ctor_property<I>::value> resize(
    const I& arg_prop, DynRankTensor<T, P...>& v,
    const size_t n0 = FLARE_INVALID_INDEX,
    const size_t n1 = FLARE_INVALID_INDEX,
    const size_t n2 = FLARE_INVALID_INDEX,
    const size_t n3 = FLARE_INVALID_INDEX,
 ...
 * indices. */
template <class... TensorCtorArgs, class T, class... P>
inline void impl_realloc(DynRankTensor<T, P...>& v, const size_t n0,
                         const size_t n1, const size_t n2, const size_t n3,
                         const size_t n4, const size_t n5, const size_t n6,
                         const size_t n7,
                         const detail::TensorCtorProp<TensorCtorArgs...>& arg_prop) {
  ...
  v = drtensor_type();  // Deallocate first, if the only tensor to allocation
  v = drtensor_type(arg_prop_copy, n0, n1, n2, n3, n4, n5, n6, n7);
}

template <class T, class... P, class... TensorCtorArgs>
inline void realloc(const detail::TensorCtorProp<TensorCtorArgs...>& arg_prop,
                    DynRankTensor<T, P...>& v,
                    const size_t n0 = FLARE_INVALID_INDEX,
                    const size_t n1 = FLARE_INVALID_INDEX,
                    const size_t n2 = FLARE_INVALID_INDEX,
...
template <class T, class... P>
inline void realloc(DynRankTensor<T, P...>& v,
                    const size_t n0 = FLARE_INVALID_INDEX,
                    const size_t n1 = FLARE_INVALID_INDEX,
                    const size_t n2 = FLARE_INVALID_INDEX,
                    const size_t n3 = FLARE_INVALID_INDEX,
...
#endif  // FLARE_DYN_RANK_TENSOR_H_
