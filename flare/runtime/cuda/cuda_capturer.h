...
//
#ifndef FLARE_RUNTIME_CUDA_CUDA_CAPTURE_H_
#define FLARE_RUNTIME_CUDA_CUDA_CAPTURE_H_

#include <flare/runtime/cuda/cuda_task.h>
#include <flare/runtime/cuda/cuda_optimizer.h>
...
The usage of flare::rt::cudaFlowCapturer is similar to flare::rt::cudaFlow, except users can
call the method flare::rt::cudaFlowCapturer::on to capture a sequence of asynchronous
...
@code{.cpp}
taskflow.emplace([](flare::rt::cudaFlowCapturer& capturer){
...
Similar to flare::rt::cudaFlow, a %cudaFlowCapturer is a task (flare::rt::Task)
created from flare::rt::Taskflow
and will be run by @em one worker thread in the executor.
That is, the callable that describes a %cudaFlowCapturer
will be executed sequentially.
Inside a %cudaFlow capturer task, different GPU tasks (flare::rt::cudaTask) may run
in parallel depending on the selected optimization algorithm.
By default, we use flare::rt::cudaFlowRoundRobinOptimizer to transform a user-level
...
class cudaFlowCapturer {
  ...
  public:
    ...
    can be run by the caller thread using flare::rt::cudaFlowCapturer::run.
    */
    ...
    */
    ...
    The method is similar to flare::rt::cudaFlowCapturer::noop but
    ...
    */
    ...
    a user-described %cudaFlow:
      + flare::rt::cudaFlowSequentialOptimizer
      + flare::rt::cudaFlowRoundRobinOptimizer
      + flare::rt::cudaFlowLinearOptimizer

    By default, flare::rt::cudaFlowCapturer uses the round-robin optimization
...
#endif  // FLARE_RUNTIME_CUDA_CUDA_CAPTURE_H_
