...
//
#ifndef FLARE_RUNTIME_ALGORITHM_PIPELINE_H_
#define FLARE_RUNTIME_ALGORITHM_PIPELINE_H_
...
Pipeflow can only be created privately by the flare::rt::Pipeline and
...
@em parallel direction or @em serial direction (specified by flare::rt::PipeType)
and is coupled with a callable to invoke by the pipeline scheduler.
The callable must take a referenced flare::rt::Pipeflow object in the first argument:

@code{.cpp}
Pipe{PipeType::SERIAL, [](flare::rt::Pipeflow&){}}
@endcode
...
*/
template <typename C = std::function<void(flare::rt::Pipeflow&)>>
class Pipe {
  ...
  */
  ...
  @param d pipe type (flare::rt::PipeType)
  @param callable callable type

  The constructor constructs a pipe with the given direction
  (flare::rt::PipeType::SERIAL or flare::rt::PipeType::PARALLEL) and the given callable.
  The callable must take a referenced flare::rt::Pipeflow object in the first argument.
  ...
  */
  ...
  @param type a flare::rt::PipeType variable
  */
  ...
  @param callable a callable object constructible from std::function<void(flare::rt::Pipeflow&)>
...
flare::rt::Pipeline pipeline(num_lines,
  // first pipe must define a serial direction
  flare::rt::Pipe{flare::rt::PipeType::SERIAL, [&buffer](flare::rt::Pipeflow& pf) {
...
flare::rt::Task task = taskflow.composed_of(pipeline)
                        .name("pipeline");
...
Internally, flare::rt::Pipeline uses std::tuple to store the given sequence of pipes.
...
If applications need to change these pipes, please use flare::rt::ScalablePipeline.
*/
template <typename... Ps>
class Pipeline {
  ...
  */
  ...
  The first pipe must define a serial direction (flare::rt::PipeType::SERIAL)
...
void Pipeline<Ps...>::_build() {
  ...
  for(size_t l = 0; l < num_lines(); l++) {
    ...
    _tasks[l + 1] = fb.emplace([this, l] (flare::rt::Runtime& rt) mutable {
...
<i>pipeline scheduling framework</i> using a module task in a taskflow.
Unlike flare::rt::Pipeline that instantiates all pipes upon the construction time,
flare::rt::ScalablePipeline allows variable assignments of pipes using range iterators.
...
auto pipe_callable = [&buffer] (flare::rt::Pipeflow& pf) mutable {
  switch(pf.pipe()) {
...
for(size_t i=0; i<3; i++) {
  pipes.emplace_back(flare::rt::PipeType::SERIAL, pipe_callable);
}

// create a pipeline of four parallel lines based on the given vector of pipes
flare::rt::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
...
Each pipe has the same type of `%flare::rt::Pipe<%std::function<void(%flare::rt::Pipeflow&)>>`
...
#endif  // FLARE_RUNTIME_ALGORITHM_PIPELINE_H_
