...
//
#ifndef FLARE_RUNTIME_ALGORITHM_PARTITIONER_H_
#define FLARE_RUNTIME_ALGORITHM_PARTITIONER_H_
...
to schedule parallel iterations (e.g., flare::rt::Taskflow::for_each).

An partitioner defines the scheduling method for running parallel algorithms,
such flare::rt::Taskflow::for_each, flare::rt::Taskflow::reduce, and so on.
By default, we provide the following partitioners:

+ flare::rt::GuidedPartitioner to enable guided scheduling algorithm of adaptive chunk size
+ flare::rt::DynamicPartitioner to enable dynamic scheduling algorithm of equal chunk size
+ flare::rt::StaticPartitioner to enable static scheduling algorithm of static chunk size
+ flare::rt::RandomPartitioner to enable random scheduling algorithm of random chunk size
...
iteration, flare::rt::StaticPartitioner can deliver the best performance.
On the other hand, if the work unit per iteration is irregular and unbalanced,
flare::rt::GuidedPartitioner or flare::rt::DynamicPartitioner can outperform flare::rt::StaticPartitioner.
In most situations, flare::rt::GuidedPartitioner can deliver decent performance and
...
Similar to flare::rt::DynamicPartitioner,
...
/**
@brief default partitioner set to flare::rt::GuidedPartitioner
...
A partitioner is a derived type from flare::rt::PartitionerBase.
*/
template <typename C>
inline constexpr bool is_partitioner_v = std::is_base_of<PartitionerBase, C>::value;

}  // namespace flare::rt
#endif  // FLARE_RUNTIME_ALGORITHM_PARTITIONER_H_
