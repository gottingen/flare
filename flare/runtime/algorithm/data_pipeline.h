...
//
#ifndef FLARE_RUNTIME_ALGORITHM_DATA_PIPE_H_
#define FLARE_RUNTIME_ALGORITHM_DATA_PIPE_H_
...
(specified by flare::rt::PipeType) and is associated with a callable to invoke
by the pipeline scheduler.

You need to use the template function, flare::rt::make_data_pipe, to create
a data pipe. The input and output types of a flare::rt::DataPipe should be decayed types
...
@code{.cpp}
flare::rt::make_data_pipe<int, std::string>(
...
of flare::rt::Pipeflow in the second argument to probe the runtime information
for a stage task, such as its line number and token number:

@code{.cpp}
flare::rt::make_data_pipe<int, std::string>(
  flare::rt::PipeType::SERIAL,
  [](int& input, flare::rt::Pipeflow& pf) {
...
class DataPipe {
  ...
  */
  ...
  You should use the helper function, flare::rt::make_data_pipe,
  to create a DataPipe object, especially when you need flare::rt::DataPipe
  ...
  */
  ...
  A data pipe can be either parallel (flare::rt::PipeType::PARALLEL) or serial
  (flare::rt::PipeType::SERIAL).
  */
  PipeType type() const {
    return _type;
...
/**
@brief function to construct a data pipe (flare::rt::DataPipe)

@tparam Input input data type
@tparam Output output data type
@tparam C callable type

flare::rt::make_data_pipe is a helper function to create a data pipe (flare::rt::DataPipe)
in a data-parallel pipeline (flare::rt::DataPipeline).
The first argument specifies the direction of the data pipe,
either flare::rt::PipeType::SERIAL or flare::rt::PipeType::PARALLEL,
...
The callable can additionally take a reference of flare::rt::Pipeflow,
...
Similar to flare::rt::Pipeline, a flare::rt::DataPipeline is a composable graph object
for users to create a <i>data-parallel pipeline scheduling framework</i> 
using a module task in a taskflow.
The only difference is that flare::rt::DataPipeline provides a data abstraction
...
int main() {
  ...
  flare::rt::DataPipeline pl(num_lines,
    flare::rt::make_data_pipe<void, int>(flare::rt::PipeType::SERIAL, [&](flare::rt::Pipeflow& pf) -> int{
      if(pf.token() == 5) {
        pf.stop();
        return 0;
      }
...
class DataPipeline {
  ...
  */
  ...
  std::vector<flare::detail::CachelineAligned<data_t>> _buffer;
...
void DataPipeline<Ps...>::_on_pipe(Pipeflow& pf, Runtime&) {
  ...
  visit_tuple([&](auto&& pipe){
    ...
    if constexpr (std::is_invocable_v<callable_t, Pipeflow&>) {
      // [](flare::rt::Pipeflow&) -> void {}, i.e., we only have one pipe
      if constexpr (std::is_void_v<output_t>) {
        pipe._callable(pf);
      // [](flare::rt::Pipeflow&) -> output_t {}
      } else {
    ...
    // other pipes with pipeflow in the second argument
      ...
      // [](input_t&, flare::rt::Pipeflow&) -> void {}
      if constexpr (std::is_void_v<output_t>) {
        pipe._callable(std::get<input_t>(_buffer[pf._line].data), pf);
      // [](input_t&, flare::rt::Pipeflow&) -> output_t {}
      } else {
        _buffer[pf._line].data = pipe._callable(
...
#endif  // FLARE_RUNTIME_ALGORITHM_DATA_PIPE_H_
