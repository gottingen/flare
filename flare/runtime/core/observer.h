...
//
#ifndef FLARE_RUNTIME_CORE_OBSERVER_H_
#define FLARE_RUNTIME_CORE_OBSERVER_H_

#include <flare/runtime/core/task.h>
#include <flare/runtime/core/worker.h>
...
The flare::rt::ObserverInterface class allows users to define custom methods to monitor
...
flare::rt::ObserverInterface provides immutable wrappers,
flare::rt::WorkerView and flare::rt::TaskView, over workers and tasks.

Please refer to flare::rt::WorkerView and flare::rt::TaskView for details.

Example usage:

@code{.cpp}

struct MyObserver : public flare::rt::ObserverInterface {
  ...
  void on_entry(WorkerView w, flare::rt::TaskView tv) override final {
    std::ostringstream oss;
    oss << "worker " << w.id() << " ready to run " << tv.name() << '\n';
    std::cout << oss.str();
  }

  void on_exit(WorkerView w, flare::rt::TaskView tv) override final {
    std::ostringstream oss;
...
A flare::rt::ChromeObserver inherits flare::rt::ObserverInterface and defines methods to dump
...
// create a custom observer
std::shared_ptr<flare::rt::ChromeObserver> observer = executor.make_observer<flare::rt::ChromeObserver>();
...
A flare::rt::TFProfObserver inherits flare::rt::ObserverInterface and defines methods to dump
the observed thread activities into a format that can be visualized through
@TFProf.
...
// create a custom observer
std::shared_ptr<flare::rt::TFProfObserver> observer = executor.make_observer<flare::rt::TFProfObserver>();
...
// constructor
    inline TFProfManager::TFProfManager() :
            _fpath{flare::detail::get_env(TF_ENABLE_PROFILER)} {
...
#endif  // FLARE_RUNTIME_CORE_OBSERVER_H_
