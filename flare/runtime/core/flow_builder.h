...
//
#ifndef FLARE_RUNTIME_CORE_FLOW_BUILDER_H_
#define FLARE_RUNTIME_CORE_FLOW_BUILDER_H_

#include <flare/runtime/core/task.h>
#include <flare/runtime/algorithm/partitioner.h>
...
from which flare::rt::Taskflow and flare::rt::Subflow are derived.
...
class FlowBuilder {
  ...
  */
  ...
  @return a flare::rt::Task handle

  The following example creates a static task.

  @code{.cpp}
  flare::rt::Task static_task = taskflow.emplace([](){});
  @endcode
  ...
  */
  ...
  @tparam C callable type constructible from std::function<void(flare::rt::Subflow&)>

  @param callable callable to construct a dynamic task

  @return a flare::rt::Task handle

  The following example creates a dynamic task (flare::rt::Subflow)
  that spawns two static tasks.

  @code{.cpp}
  flare::rt::Task dynamic_task = taskflow.emplace([](flare::rt::Subflow& sf){
    flare::rt::Task static_task1 = sf.emplace([](){});
    flare::rt::Task static_task2 = sf.emplace([](){});
  });
  @endcode
  ...
  */
  ...
  @tparam C callable type constructible from
          std::function<flare::detail::SmallVector<int>()>
  ...
  auto [init, cond, branch1, branch2, branch3] = taskflow.emplace(
    [] () { },
    [] () { return flare::detail::SmallVector{0, 2}; },
    [] () { std::cout << "branch1\n"; },
    [] () { std::cout << "branch2\n"; },
    [] () { std::cout << "branch3\n"; }
  );
  ...
  */
  ...
  @code{.cpp}
  flare::rt::Task A = taskflow.emplace([](){ std::cout << "A"; });
  flare::rt::Task B = taskflow.emplace([](){ std::cout << "B"; });
  flare::rt::Task C = taskflow.emplace([](){ std::cout << "C"; });
  flare::rt::Task D = taskflow.emplace([](){ std::cout << "D"; });
  A.precede(B, C, D);
  ...
  */
  ...
  @code{.cpp}
  flare::rt::Taskflow t1, t2;
  t1.emplace([](){ std::cout << "t1"; });

  // t2 is partially composed of t1
  flare::rt::Task comp = t2.composed_of(t1);
  flare::rt::Task init = t2.emplace([](){ std::cout << "t2"; });
  init.precede(comp);
  @endcode
  ...
  type flare::rt::Graph such that it can interact with the executor.
  For example:

  @code{.cpp}
  // custom struct
  struct MyObj {
    flare::rt::Graph graph;
    MyObj() {
      flare::rt::FlowBuilder builder(graph);
      flare::rt::Task task = builder.emplace([](){
        std::cout << "a task\n";  // static task
      });
    }
  ...
  MyObj obj;
  flare::rt::Task comp = taskflow.composed_of(obj);
  @endcode
  ...
  */
  ...
  flare::rt::Task placeholder = taskflow.placeholder();
  ...
  */
  ...
  flare::rt::Task C = taskflow.emplace([](){ std::cout << "C"; });
  flare::rt::Task D = taskflow.emplace([](){ std::cout << "D"; });
  std::vector<flare::rt::Task> tasks {A, B, C, D}
  ...
  */
  ...
  @tparam P partitioner type (default flare::rt::GuidedPartitioner)
  ...
  */
  ...
  @param part partitioning algorithm (default flare::rt::GuidedPartitioner)
...
flare::rt::Subflow is a derived class from flare::rt::Runtime with a specialized mechanism
to manage the execution of a child graph.
By default, a subflow automatically @em joins its parent node.
You may explicitly join or detach a subflow by calling flare::rt::Subflow::join
or flare::rt::Subflow::detach, respectively.
...
flare::rt::Task A = taskflow.emplace([](){}).name("A");
flare::rt::Task C = taskflow.emplace([](){}).name("C");
flare::rt::Task D = taskflow.emplace([](){}).name("D");

// create a subflow graph (dynamic tasking)
flare::rt::Task B = taskflow.emplace([] (flare::rt::Subflow& subflow) {
  flare::rt::Task B1 = subflow.emplace([](){}).name("B1");
  flare::rt::Task B2 = subflow.emplace([](){}).name("B2");
  flare::rt::Task B3 = subflow.emplace([](){}).name("B3");
  B1.precede(B3);
  B2.precede(B3);
}).name("B");
...
#endif  // FLARE_RUNTIME_CORE_FLOW_BUILDER_H_
