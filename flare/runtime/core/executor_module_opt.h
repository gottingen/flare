...
//
#ifndef FLARE_RUNTIME_CORE_EXECUTOR_MODULE_OPT_H_
#define FLARE_RUNTIME_CORE_EXECUTOR_MODULE_OPT_H_
...
class Executor {
  ...
  public:
    ...
    */
    ...
    @return a flare::rt::Future that will holds the result of the execution

    The method creates an asynchronous task to launch the given
    function on the given arguments.
    Unlike std::async, the return here is a @em flare::rt::Future that holds
    ...
    @code{.cpp}
    flare::rt::Future<std::optional<int>> future = executor.async([](){
    ...
    */
    ...
    Unlike std::async, the return here is a flare::rt::Future that holds
    an optional object to the result.
    If the asynchronous task is cancelled before it runs, the return is
    a @c std::nullopt, or the value returned by the callable.

    @code{.cpp}
    flare::rt::Future<std::optional<int>> future = executor.named_async("name", [](){
    ...
    */
    ...
    /**
    @brief similar to flare::rt::Executor::named_async but does not return a future object

    This member function is more efficient than flare::rt::Executor::named_async
    and is encouraged to use when there is no data returned.
...
flare::rt::Future<void> Executor::run_until(Taskflow& f, P&& p, C&& c) {
  ...
  if(empty || p()) {
    ...
    promise.set_value();
    _decrement_topology_and_notify();
    return flare::rt::Future<void>(promise.get_future(), std::monostate{});
  }
...
#endif  // FLARE_RUNTIME_CORE_EXECUTOR_MODULE_OPT_H_
