...
//
#ifndef FLARE_RUNTIME_CORE_TASKFLOW_H_
#define FLARE_RUNTIME_CORE_TASKFLOW_H_

#include <flare/runtime/core/flow_builder.h>
...
dependency between two tasks. A task is one of the following types:
  ...
  2. dynamic task        : the callable constructible from
                           @c std::function<void(flare::rt::Subflow&)>
  3. condition task      : the callable constructible from
                           @c std::function<int()>
  4. multi-condition task: the callable constructible from
                           @c %std::function<flare::detail::SmallVector<int>()>
  5. module task         : the task constructed from flare::rt::Taskflow::composed_of
                           @c std::function<void(flare::rt::Runtime&)>
...
flare::rt::Taskflow taskflow("simple");

flare::rt::Task A = taskflow.emplace([](){ std::cout << "TaskA\n"; });
flare::rt::Task B = taskflow.emplace([](){ std::cout << "TaskB\n"; });
flare::rt::Task C = taskflow.emplace([](){ std::cout << "TaskC\n"; });
flare::rt::Task D = taskflow.emplace([](){ std::cout << "TaskD\n"; });
...
class Taskflow : public FlowBuilder {
  ...
  public:
    ...
    @code{.cpp}
    flare::rt::Taskflow taskflow("My Taskflow");
    ...
    */
    ...
    @code{.cpp}
    flare::rt::Taskflow taskflow1(std::move(taskflow2));
    ...
    */
    ...
    @code{.cpp}
    flare::rt::Task parent = taskflow.emplace([](flare::rt::Subflow sf){
    ...
    */
    ...
    This method is similar to flare::rt::Taskflow::dump(std::ostream& ostream),
    ...
    */
    ...
    flare::rt::Taskflow::num_tasks is zero.
    */
    bool empty() const;
    ...
    */
    ...
    A visitor is a callable that takes an argument of type flare::rt::Task
    and returns nothing. The following example iterates each task in a
    taskflow and prints its name:

    @code{.cpp}
    taskflow.for_each_task([](flare::rt::Task task){
      std::cout << task.name() << '\n';
    });
    @endcode
    */
    ...
    */
    ...
    A graph object (of type flare::rt::Graph) is the ultimate storage for the
...
flare::rt::Future is a derived class from std::future that will eventually hold the
execution result of a submitted taskflow (flare::rt::Executor::run)
In addition to the base methods inherited from std::future,
you can call flare::rt::Future::cancel to cancel the execution of the running taskflow
...
// submit the taskflow
flare::rt::Future fu = executor.run(taskflow);
...
class Future : public std::future<T>  {
  ...
  public:
    ...
    */
    ...
    (non-preemptive).
    You can call flare::rt::Future::wait to wait for the cancellation to complete.
    */
    bool cancel();
...
#endif  // FLARE_RUNTIME_CORE_TASKFLOW_H_
