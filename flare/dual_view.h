...
/// \file dual_view.h
/// \brief Declaration and definition of flare::DualView.
///
/// This header file declares and defines flare::DualView and its
/// related nonmember functions.

#ifndef FLARE_DUAL_VIEW_H_
#define FLARE_DUAL_VIEW_H_
...
/* \class DualView
 * \brief Container to manage mirroring a flare::View that lives
 *   in device memory with a flare::View that lives in host memory.
 *
 ...
 * the underlying flare::View objects.
 *
 * The four template arguments are the same as those of flare::View.
 ...
 *     behavior.  Please see the documentation of flare::View for
...
class DualView : public ViewTraits<DataType, Properties...> {
 ...
 public:
  //! \name Typedefs for device types and various flare::View specializations.
  //@{
  ...
  //! The type of a flare::View on the device.
  using t_dev       = View<typename traits::data_type, Properties...>;

  /// \typedef t_host
  /// \brief The type of a flare::View host mirror of \c t_dev.
 ...
 protected:
  ...
  using t_modified_flags = View<unsigned int[2], LayoutLeft, flare::HostSpace>;
 ...
 public:
  ...
  DualView(const std::string& label,
      ...
      : modified_flags(
            flare::view_alloc(typename t_modified_flags::execution_space{},
  ...
  /// omit the integer arguments that follow.
  template <class... P>
  DualView(const detail::ViewCtorProp<P...>& arg_prop,
    ...
    if constexpr (flare::detail::has_type<detail::WithoutInitializing_t,
                                         P...>::value)
      h_view = flare::create_mirror_view(flare::WithoutInitializing, d_view);
    else
      h_view = flare::create_mirror_view(d_view);
  }
  ...
  DualView(const DualView<DT, DP...>& src, const Arg0& arg0, Args... args)
      : modified_flags(src.modified_flags),
        d_view(flare::subview(src.d_view, arg0, args...)),
        h_view(flare::subview(src.h_view, arg0, args...)) {}
  ...
  ///       flare::DualView<float, flare::LayoutRight, flare::Cuda>;
  ///   dual_view_type DV ("my dual view", 100);
  /// \endcode
  /// If you want to get the CUDA device View, do this:
  /// \code
  ///   typename dual_view_type::t_dev cudaView = DV.view<flare::Cuda> ();
  /// \endcode
  ...
  void impl_report_host_sync() const noexcept {
    if (flare::Tools::experimental::get_callbacks().sync_dual_view !=
        nullptr) {
      flare::Tools::syncDualView(
          h_view.label(),
  ...
  void sync_impl(std::true_type, Args const&... args) {
        ...
        if (std::is_same<typename t_dev::memory_space,
          ...
          if (d_view.data() == h_view.data())
            flare::detail::cuda_prefetch_pointer(
                detail::get_cuda_space(args...), d_view.data(),
    ...
    if constexpr (std::is_same<typename t_host::memory_space,
      ...
      typename t_dev::execution_space().fence(
          "flare::DualView<>::sync: fence after syncing DualView");
  ...
  void impl_report_device_modification() {
    if (flare::Tools::experimental::get_callbacks().modify_dual_view !=
        nullptr) {
      flare::Tools::modifyDualView(
          d_view.label(),
          reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(d_view.data()) -
                                  view_header_size),
          true);
...
#ifdef FLARE_ENABLE_DEBUG_DUALVIEW_MODIFY_CHECK
    if (modified_flags(0) && modified_flags(1)) {
      std::string msg = "flare::DualView::modify ERROR: ";
...
#ifdef FLARE_ENABLE_DEBUG_DUALVIEW_MODIFY_CHECK
      if (modified_flags(0) && modified_flags(1)) {
        std::string msg = "flare::DualView::modify_host ERROR: ";
        msg += "Concurrent modification of host and device views ";
        msg += "in DualView \"";
        msg += d_view.label();
        msg += "\"\n";
...
#ifdef FLARE_ENABLE_DEBUG_DUALVIEW_MODIFY_CHECK
      if (modified_flags(0) && modified_flags(1)) {
        std::string msg = "flare::DualView::modify_device ERROR: ";
        msg += "Concurrent modification of host and device views ";
        msg += "in DualView \"";
        msg += d_view.label();
        msg += "\"\n";
  ...
  void impl_realloc(const size_t n0, const size_t n1, const size_t n2,
    ...
    static_assert(!alloc_prop_input::has_label,
                  "The view constructor arguments passed to flare::realloc "
    ...
    const bool sizeMismatch =
        detail::size_mismatch(h_view, h_view.rank_dynamic, new_extents);

    if (sizeMismatch) {
      ::flare::realloc(arg_prop, d_view, n0, n1, n2, n3, n4, n5, n6, n7);
      if (alloc_prop_input::initialize) {
        h_view = create_mirror_view(typename t_host::memory_space(), d_view);
      } else {
        h_view = create_mirror_view(flare::WithoutInitializing,
      ...
      if constexpr (alloc_prop_input::has_execution_space) {
        ...
        ::flare::deep_copy(exec_space, d_view, typename t_dev::value_type{});
      } else
        ::flare::deep_copy(d_view, typename t_dev::value_type{});
    }
  ...
  template <class... ViewCtorArgs>
  void realloc(const detail::ViewCtorProp<ViewCtorArgs...>& arg_prop,
               const size_t n0 = FLARE_IMPL_CTOR_DEFAULT_ARG,
               const size_t n1 = FLARE_IMPL_CTOR_DEFAULT_ARG,
               const size_t n2 = FLARE_IMPL_CTOR_DEFAULT_ARG,
               const size_t n3 = FLARE_IMPL_CTOR_DEFAULT_ARG,
  ...
  void realloc(const size_t n0 = FLARE_IMPL_CTOR_DEFAULT_ARG,
               ...
               const size_t n6 = FLARE_IMPL_CTOR_DEFAULT_ARG,
               const size_t n7 = FLARE_IMPL_CTOR_DEFAULT_ARG) {
    impl_realloc(n0, n1, n2, n3, n4, n5, n6, n7, detail::ViewCtorProp<>{});
  }

  template <typename I>
  std::enable_if_t<detail::is_view_ctor_property<I>::value> realloc(
      ...
      const size_t n6 = FLARE_IMPL_CTOR_DEFAULT_ARG,
      const size_t n7 = FLARE_IMPL_CTOR_DEFAULT_ARG) {
    impl_realloc(n0, n1, n2, n3, n4, n5, n6, n7, flare::view_alloc(arg_prop));
  }
  ...
  template <class... ViewCtorArgs>
  void impl_resize(const detail::ViewCtorProp<ViewCtorArgs...>& arg_prop,
    ...
    [[maybe_unused]] auto resize_on_device = [&](const auto& properties) {
      /* Resize on Device */
      if (sizeMismatch) {
        ::flare::resize(properties, d_view, n0, n1, n2, n3, n4, n5, n6, n7);
        if (alloc_prop_input::initialize) {
          h_view = create_mirror_view(typename t_host::memory_space(), d_view);
        } else {
          h_view = create_mirror_view(flare::WithoutInitializing,
    ...
    [[maybe_unused]] auto resize_on_host = [&](const auto& properties) {
      /* Resize on Host */
      if (sizeMismatch) {
        ::flare::resize(properties, h_view, n0, n1, n2, n3, n4, n5, n6, n7);
        if (alloc_prop_input::initialize) {
          d_view = create_mirror_view(typename t_dev::memory_space(), h_view);

        } else {
          d_view = create_mirror_view(flare::WithoutInitializing,
  ...
  void resize(const size_t n0 = FLARE_IMPL_CTOR_DEFAULT_ARG,
              ...
              const size_t n6 = FLARE_IMPL_CTOR_DEFAULT_ARG,
              const size_t n7 = FLARE_IMPL_CTOR_DEFAULT_ARG) {
    impl_resize(detail::ViewCtorProp<>{}, n0, n1, n2, n3, n4, n5, n6, n7);
  }
  ...
  template <class I>
  std::enable_if_t<detail::is_view_ctor_property<I>::value> resize(
      const I& arg_prop, const size_t n0 = FLARE_IMPL_CTOR_DEFAULT_ARG,
      const size_t n1 = FLARE_IMPL_CTOR_DEFAULT_ARG,
      const size_t n2 = FLARE_IMPL_CTOR_DEFAULT_ARG,
      const size_t n3 = FLARE_IMPL_CTOR_DEFAULT_ARG,
      ...
      const size_t n6 = FLARE_IMPL_CTOR_DEFAULT_ARG,
      const size_t n7 = FLARE_IMPL_CTOR_DEFAULT_ARG) {
    impl_resize(flare::view_alloc(arg_prop), n0, n1, n2, n3, n4, n5, n6, n7);
  }
  ...
  //! The allocation size (same as flare::View::span).
  FLARE_INLINE_FUNCTION constexpr size_t span() const { return d_view.span(); }
...
//----------------------------------------------------------------------------
//
// Partial specializations of flare::subview() for DualView objects.
//
...
auto subview(const DualView<DataType, Properties...>& src, Args&&... args) {
  // leverage flare::View facilities to deduce the properties of the subview
  using deduce_subview_type =
      decltype(subview(std::declval<View<DataType, Properties...>>(),
                       std::forward<Args>(args)...));
  // map it back to dual view
  return typename detail::V2DV<deduce_subview_type>::type(
...
//
// Partial specialization of flare::deep_copy() for DualView objects.
//
