...
#ifndef FLARE_CORE_COMMON_HOST_BARRIER_H_
#define FLARE_CORE_COMMON_HOST_BARRIER_H_
...
namespace flare::detail {
    ...
    public:
        ...
        static bool split_arrive(int *buffer, const int size, int &step,
            ...
            flare::memory_fence();
            const bool result =
                    flare::atomic_fetch_add(buffer + arrive_idx, 1) == size - 1;

            if (master_wait && result) {
                flare::atomic_fetch_add(buffer + master_idx, 1);
            }
        ...
        static void split_release(int *buffer, const int size, const int /*step*/
            ...
            if (size <= 1) return;
            flare::memory_fence();
            flare::atomic_fetch_sub(buffer + arrive_idx, size);
            flare::atomic_fetch_add(buffer + wait_idx, 1);
        }
    ...
    private:
        ...
        static bool test_equal(int *ptr, int v) noexcept {
            const bool result = flare::atomic_fetch_add(ptr, 0) == v;
            if (result) {
                flare::memory_fence();
            }
            return result;
        ...
        static void wait_until_equal(int *ptr, const int v,
                                     bool active_wait = true) noexcept {
            FLARE_IF_ON_HOST((impl_wait_until_equal_host(ptr, v, active_wait);))

            FLARE_IF_ON_DEVICE(((void) active_wait; while (!test_equal(ptr, v)) {}))
        }
...
#endif  // FLARE_CORE_COMMON_HOST_BARRIER_H_
