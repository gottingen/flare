...
namespace flare::detail {
    ...
    public:
        ...
        enum : int {
            max_pool_members = 1024
        };
        enum : int {
            max_team_members = 64
        };
        enum : int {
            max_pool_rendezvous = HostBarrier::required_buffer_size
        };
        enum : int {
            max_team_rendezvous = HostBarrier::required_buffer_size
        };
    ...
    private:
        ...
        using pair_int_t = flare::pair<int64_t, int64_t>;
...
//----------------------------------------------------------------------------
    ...
    public:
        ...
        FLARE_INLINE_FUNCTION void team_barrier() const noexcept {
            FLARE_IF_ON_HOST(
        ...
        template<typename T>
        FLARE_INLINE_FUNCTION void team_broadcast(T &value,
                                                  const int source_team_rank) const
        noexcept {
            FLARE_IF_ON_HOST((if (1 < m_data.m_team_size) {
                                     ...
                                     flare::detail::atomic_store(shared_value, value,
                                     flare::MemoryOrderRelease());

                                     m_data.team_rendezvous_release();
                                     // This thread released all other threads from 'team_rendezvous'
                                     // with a return value of 'false'
                             } else {
                                     value = flare::detail::atomic_load(shared_value,
                                     flare::MemoryOrderAcquire());
                             }
                             }))

            FLARE_IF_ON_DEVICE(((void) value; (void) source_team_rank; flare::abort(
        ...
        FLARE_INLINE_FUNCTION void team_broadcast(Closure const &f, T &value,
                                                  const int source_team_rank) const
        noexcept {
            FLARE_IF_ON_HOST((
                                     T * const shared_value = (T *) m_data.team_reduce();
            ...
            FLARE_IF_ON_DEVICE(
                    ...
                    flare::abort("HostThreadTeamMember team_broadcast\n");))
        }
        ...
        team_reduce(ReducerType const &reducer,
            ...
            FLARE_IF_ON_DEVICE(((void) reducer; (void) contribution;
                                       flare::abort("HostThreadTeamMember team_reduce\n");))
        }

        //--------------------------------------------------------------------------

        template<typename T>
        FLARE_INLINE_FUNCTION T team_scan(T const &value,
            ...
            FLARE_IF_ON_HOST((
...

                             {
                                     ...
                                     ->team_reduce_local();
                                     prev[0] = flare::atomic_fetch_add(global, ptr[0] + ptr[1]);
                             }
            ...
            FLARE_IF_ON_DEVICE(((void) value; (void) global;
                                       flare::abort("HostThreadTeamMember team_scan\n");
                                       return T();))
        }
    };
...
namespace flare {
    ...
    FLARE_INLINE_FUNCTION detail::TeamThreadRangeBoundariesStruct<iType, Member>
    TeamThreadRange(
            Member const &member, iType count,
            std::enable_if_t<detail::is_thread_team_member<Member>::value> const ** =
            nullptr) {
        return detail::TeamThreadRangeBoundariesStruct<iType, Member>(member, 0, count);
    }
    ...
    TeamThreadRange(
            ...
            std::enable_if_t<detail::is_thread_team_member<Member>::value> const ** =
            nullptr) {
        return detail::TeamThreadRangeBoundariesStruct<
    ...
    template<typename iType, typename Member>
    FLARE_INLINE_FUNCTION detail::ThreadVectorRangeBoundariesStruct<iType, Member>
    ThreadVectorRange(
            Member const &member, iType count,
            std::enable_if_t<detail::is_thread_team_member<Member>::value> const ** =
            nullptr) {
        return detail::ThreadVectorRangeBoundariesStruct<iType, Member>(member, count);
    }
    ...
    ThreadVectorRange(
        ...
        return detail::ThreadVectorRangeBoundariesStruct<iType, Member>(
 ...
 */
    ...
    FLARE_INLINE_FUNCTION void parallel_for(
            detail::TeamThreadRangeBoundariesStruct<iType, Member> const &loop_boundaries,
            Closure const &closure,
            std::enable_if_t<detail::is_host_thread_team_member<Member>::value> const ** =
            nullptr) {
    ...
    template<typename iType, class Closure, class Member>
    FLARE_INLINE_FUNCTION void parallel_for(
            detail::ThreadVectorRangeBoundariesStruct<iType, Member> const &
            loop_boundaries,
            Closure const &closure,
            std::enable_if_t<detail::is_host_thread_team_member<Member>::value> const ** =
            nullptr) {
...
//----------------------------------------------------------------------------
    ...
    FLARE_INLINE_FUNCTION
    std::enable_if_t<flare::is_reducer<Reducer>::value &&
                     detail::is_host_thread_team_member<Member>::value>
    parallel_reduce(detail::TeamThreadRangeBoundariesStruct<iType, Member> const &
    loop_boundaries,
    ...
    FLARE_INLINE_FUNCTION
    std::enable_if_t<!flare::is_reducer<ValueType>::value &&
                     detail::is_host_thread_team_member<Member>::value>
    parallel_reduce(detail::TeamThreadRangeBoundariesStruct<iType, Member> const &
    loop_boundaries,
...
void parallel_reduce
  (
detail::TeamThreadRangeBoundariesStruct<iType,detail::HostThreadTeamMember<Space> >
...
{
  detail::Reducer< ValueType , Joiner > reducer( joiner , & result );
...
//----------------------------------------------------------------------------
    ...
    std::enable_if_t<!flare::is_reducer<ValueType>::value &&
                     detail::is_host_thread_team_member<Member>::value>
    parallel_scan(detail::TeamThreadRangeBoundariesStruct<iType, Member> const &
    loop_boundaries,
                  Closure const &closure, ValueType &return_val) {
        // Extract ValueType from the Closure
        using ClosureValueType = typename flare::detail::FunctorAnalysis<
    ...
    template<typename iType, class Closure, class Member>
    FLARE_INLINE_FUNCTION
    std::enable_if_t<detail::is_host_thread_team_member<Member>::value>
    parallel_scan(detail::TeamThreadRangeBoundariesStruct<iType, Member> const &
    loop_boundaries,
                  Closure const &closure) {
        // Extract ValueType from the closure
        using ValueType = typename flare::detail::FunctorAnalysis<
                flare::detail::FunctorPatternInterface::SCAN, void, Closure,
                void>::value_type;
    ...
    FLARE_INLINE_FUNCTION
    std::enable_if_t<detail::is_host_thread_team_member<Member>::value>
    parallel_scan(detail::ThreadVectorRangeBoundariesStruct<iType, Member> const &
    loop_boundaries,
                  ClosureType const &closure) {
        using value_type = typename flare::detail::FunctorAnalysis<
                detail::FunctorPatternInterface::SCAN, void, ClosureType, void>::value_type;
...
#pragma ivdep
    ...
    std::enable_if_t<flare::is_reducer<ReducerType>::value &&
                     detail::is_host_thread_team_member<Member>::value>
    parallel_scan(const detail::ThreadVectorRangeBoundariesStruct<iType, Member> &
    loop_boundaries,
                  const Lambda &lambda, const ReducerType &reducer) {
...
//----------------------------------------------------------------------------
    ...
    FLARE_INLINE_FUNCTION detail::ThreadSingleStruct<Member> PerTeam(
            Member const &member,
            std::enable_if_t<detail::is_thread_team_member<Member>::value> const ** =
            nullptr) {
        return detail::ThreadSingleStruct<Member>(member);
    }

    template<class Member>
    FLARE_INLINE_FUNCTION detail::VectorSingleStruct<Member> PerThread(
            Member const &member,
            std::enable_if_t<detail::is_thread_team_member<Member>::value> const ** =
            nullptr) {
        return detail::VectorSingleStruct<Member>(member);
    }

    template<class Member, class FunctorType>
    FLARE_INLINE_FUNCTION
    std::enable_if_t<detail::is_host_thread_team_member<Member>::value>
    single(const detail::ThreadSingleStruct<Member> &single,
    ...
    FLARE_INLINE_FUNCTION
    std::enable_if_t<detail::is_host_thread_team_member<Member>::value>
    single(const detail::VectorSingleStruct<Member> &,
           const FunctorType &functor) {
        functor();
    }
    ...
    FLARE_INLINE_FUNCTION
    std::enable_if_t<detail::is_host_thread_team_member<Member>::value>
    single(const detail::VectorSingleStruct<Member> &, const FunctorType &functor,
           ValueType &val) {
        functor(val);
    }
