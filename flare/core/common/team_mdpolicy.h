...
#ifndef FLARE_CORE_COMMON_TEAM_MDPOLICY_H_
#define FLARE_CORE_COMMON_TEAM_MDPOLICY_H_
...
FLARE_INLINE_FUNCTION auto nested_policy(
...
template <typename Rank, int ParThreadNestLevel, int ParVectorNestLevel,
          ...
          typename... Args>
FLARE_INLINE_FUNCTION void nested_loop(
    ...
    TeamMDRangeNestingTracker<Rank, ParThreadNestLevel, ParVectorNestLevel,
                              CurrentNestLevel>,
    Policy const&, Lambda const& lambda, detail::NoReductionTag&&, Args... args) {
  lambda(args...);
}
...
FLARE_INLINE_FUNCTION void md_parallel_impl(TeamMDPolicy const& policy,
  ...
  constexpr auto thread_and_vector =
      ((TeamMDPolicy::par_thread == detail::TeamMDRangeParThread::ParThread) &&
       (TeamMDPolicy::par_vector == detail::TeamMDRangeParVector::ParVector))
          ? detail::TeamMDRangeThreadAndVector::Both
          : detail::TeamMDRangeThreadAndVector::NotBoth;
  constexpr auto begin_rank =
      (iter == Iterate::Right) ? 0 : (total_nest_level - 1);

  using ThreadAndVectorNestLevel =
      detail::ThreadAndVectorNestLevel<Rank, typename TeamHandle::execution_space,
                                     thread_and_vector>;
...
#endif  // FLARE_CORE_COMMON_TEAM_MDPOLICY_H_
