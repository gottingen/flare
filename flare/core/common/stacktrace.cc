...
#include <execinfo.h>
#endif
#ifdef FLARE_IMPL_ENABLE_CXXABI
#include <cxxabi.h>
#endif  // FLARE_ENABLE_CXXABI
...
void flare_terminate_handler() {
  using std::cerr;
  using std::endl;

  cerr << "flare observes that std::terminate has been called.  "
...
void set_flare_terminate_handler(std::function<void()> user_post) {
  user_terminate_handler_post_ = user_post;
  std::set_terminate(flare_terminate_handler);
}
