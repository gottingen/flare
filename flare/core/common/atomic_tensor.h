...
//
#ifndef FLARE_CORE_COMMON_ATOMIC_TENSOR_H_
#define FLARE_CORE_COMMON_ATOMIC_TENSOR_H_
...
namespace flare::detail {
    ...
    struct AtomicTensorConstTag {
    };

    template<class TensorTraits>
    class AtomicDataElement {
    public:
        using value_type = typename TensorTraits::value_type;
        using const_value_type = typename TensorTraits::const_value_type;
        using non_const_value_type = typename TensorTraits::non_const_value_type;
        value_type *const ptr;

        FLARE_INLINE_FUNCTION
        AtomicDataElement(value_type *ptr_, AtomicTensorConstTag) : ptr(ptr_) {}
    ...
    template<class TensorTraits>
    class AtomicTensorDataHandle {
    public:
        typename TensorTraits::value_type *ptr;

        FLARE_INLINE_FUNCTION
        AtomicTensorDataHandle() : ptr(nullptr) {}

        FLARE_INLINE_FUNCTION
        AtomicTensorDataHandle(typename TensorTraits::value_type *ptr_) : ptr(ptr_) {}

        template<class iType>
        FLARE_INLINE_FUNCTION AtomicDataElement<TensorTraits> operator[](
                const iType &i) const {
            return AtomicDataElement<TensorTraits>(ptr + i, AtomicTensorConstTag());
        }

        FLARE_INLINE_FUNCTION
        operator typename TensorTraits::value_type *() const { return ptr; }
    };

}  // namespace flare::detail

#endif  // FLARE_CORE_COMMON_ATOMIC_TENSOR_H_
