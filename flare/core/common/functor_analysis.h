 ...
 */
    ...
    private:
        ...
        struct has_value_type<F,
            ...
            static_assert(!std::is_reference<type>::value &&
                          ...
                          "flare Functor::value_type is T or T[]");
        };
        ...
        struct deduce_value_type<F, REDUCE, void, true> {
            ...
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, A &) const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, M, A &) const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, M, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, M, M, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, M, M, M, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, M, M, M, M, M,
                                                                    A &) const);
        ...
        struct deduce_value_type<F, REDUCE, void, false> {
            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag, M, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag, M, M, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag, M, M, M, M,
                                                                    A &) const);
            ...
            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag const &, M, A &)
            const);

            template<typename M, typename A>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag const &, M, M,
                                                                    A &) const);
        ...
        struct deduce_value_type<F, SCAN, void, true> {
            ...
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(M, A &, I) const);
        ...
        struct deduce_value_type<F, SCAN, void, false> {
            template<typename M, typename A, typename I>
            FLARE_INLINE_FUNCTION static A deduce(void (Functor::*)(WTag, M, A &, I)
            const);
    ...
    public:
        ...
        static_assert(!std::is_const<value_type>::value,
                      "flare functor operator reduce argument cannot be const");
    ...
    private:
        ...
        FLARE_INLINE_FUNCTION static constexpr std::enable_if_t<IsArray,
                unsigned int>
    ...
    public:
        ...
        FLARE_FORCEINLINE_FUNCTION static constexpr unsigned int value_count(
                const Functor &f) {
            return FunctorAnalysis::template get_length<candidate_is_array>(f);
        }

        FLARE_FORCEINLINE_FUNCTION static constexpr unsigned int value_size(
                const Functor &f) {
            return FunctorAnalysis::template get_length<candidate_is_array>(f) *
                   sizeof(ValueType);
        }
    ...
    private:
        ...
        struct has_join_no_tag_function<F, /*is_array*/ false> {
            ...
            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(ref_type,
                                                                    cref_type) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(ref_type, cref_type));

            FLARE_INLINE_FUNCTION static void join(F const *const f, ValueType *dst,
                                                   ValueType const *src) {
                f->join(*dst, *src);
            }
        };
        ...
        template<class F>
        struct FLARE_DEPRECATED_WITH_COMMENT(
        ...
        has_volatile_join_no_tag_function<F, /*is_array*/ false> {
            ...
            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(vref_type,
                                                                    cvref_type) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(vref_type,
                                                                 cvref_type));
        ...
        struct has_join_tag_function<F, /*is_array*/ false> {
            ...
            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(WTag, ref_type,
                                                                    cref_type) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(WTag, ref_type,
                                                                 cref_type));

            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(WTag const &,
                                                                    ref_type,
                                                                    cref_type) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(WTag const &, ref_type,
                                                                 cref_type));
        ...
        has_volatile_join_tag_function<F, /*is_array*/ false> {
            ...
            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(WTag, vref_type,
                                                                    cvref_type) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(WTag, vref_type,
                                                                 cvref_type));
            ...
            FLARE_INLINE_FUNCTION static void enable_if(void (*)(WTag const &,
                                                                 vref_type,
                                                                 cvref_type));
        ...
        struct has_init_no_tag_function<F, /*is_array*/ false> {
            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(ValueType &) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(ValueType &));

            FLARE_INLINE_FUNCTION static void init(F const *const f, ValueType *dst) {
                f->init(*dst);
            }
        };
        ...
        struct has_init_tag_function<F, /*is_array*/ false> {
            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(WTag, ValueType &)
            const);

            FLARE_INLINE_FUNCTION static void enable_if(void (F::*)(WTag const &,
                                                                    ValueType &) const);

            FLARE_INLINE_FUNCTION static void enable_if(void (*)(WTag, ValueType &));
        ...
        struct has_final_no_tag_function<F, /*is_array*/ false> {
            ...
            FLARE_INLINE_FUNCTION static void final(F const *const f, ValueType *dst) {
                f->final(*dst);
            }
        };
    ...
    public:
        ...
        enum {
            has_join_member_function = DeduceJoin<>::value
        };
        enum {
            has_init_member_function = DeduceInit<>::value
        };
        enum {
            has_final_member_function = DeduceFinal<>::value
        };

        static_assert((flare::is_reducer<Functor>::value &&
                       has_join_member_function) ||
                      !flare::is_reducer<Functor>::value,
        ...
        private:
            ...
            FLARE_INLINE_FUNCTION constexpr std::enable_if_t<IsArray, int> len() const
            noexcept {
        ...
        public:
            ...
            FLARE_FUNCTION unsigned int value_size() const {
                return FunctorAnalysis::value_size(m_functor);
            }

            FLARE_FUNCTION unsigned int value_count() const {
                return FunctorAnalysis::value_count(m_functor);
            }

            FLARE_FUNCTION static constexpr unsigned int static_value_size() {
                return StaticValueSize;
            }

            template<bool is_array = candidate_is_array>
            FLARE_INLINE_FUNCTION static std::enable_if_t<is_array, reference_type>
            ...
            FLARE_INLINE_FUNCTION constexpr int length() const noexcept {
            ...
            FLARE_INLINE_FUNCTION reference_type init(ValueType *const dst) const
            noexcept {
            ...
            FLARE_INLINE_FUNCTION explicit constexpr Reducer(
