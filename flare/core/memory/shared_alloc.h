...
#ifndef FLARE_CORE_MEMORY_SHARED_ALLOC_H_
#define FLARE_CORE_MEMORY_SHARED_ALLOC_H_

#include <flare/core/defines.h>
#include <flare/core_fwd.h>
#include <flare/core/common/error.h>  // detail::throw_runtime_exception
...
class SharedAllocationHeader {
 ...
 public:
  ...
  FLARE_INLINE_FUNCTION static const SharedAllocationHeader* get_header(
...
#endif
  static FLARE_FUNCTION int tracking_enabled() {
    FLARE_IF_ON_HOST(return t_tracking_enabled;)
 ...
 private:
  ...
  SharedAllocationRecord(const ExecutionSpace& execution_space,
      ...
      : SharedAllocationRecord<MemorySpace, void>(
            ...
            &flare::detail::deallocate<MemorySpace, DestroyFunctor>),
        m_destroy() {}
  ...
  FLARE_INLINE_FUNCTION static SharedAllocationRecord* allocate(
    ...
    FLARE_IF_ON_HOST(
        (return new SharedAllocationRecord(exec_space, arg_space, arg_label,
                                           arg_alloc);))
    FLARE_IF_ON_DEVICE(((void)exec_space; (void)arg_space; (void)arg_label;
...
#define FLARE_IMPL_SHARED_ALLOCATION_TRACKER_INCREMENT          \
  FLARE_IF_ON_HOST((if (!(m_record_bits & DO_NOT_DEREF_FLAG)) { \
    Record::increment(m_record);                                 \
  }))

#define FLARE_IMPL_SHARED_ALLOCATION_TRACKER_DECREMENT          \
  FLARE_IF_ON_HOST((if (!(m_record_bits & DO_NOT_DEREF_FLAG)) { \
    Record::decrement(m_record);                                 \
  }))

#define FLARE_IMPL_SHARED_ALLOCATION_CARRY_RECORD_BITS(rhs,               \
                                                        override_tracking) \
  ...
  FLARE_INLINE_FUNCTION
  int use_count() const {
    FLARE_IF_ON_HOST((Record* const tmp = reinterpret_cast<Record*>(
                           m_record_bits & ~DO_NOT_DEREF_FLAG);
  ...
  FLARE_INLINE_FUNCTION bool has_record() const {
  ...
  void clear() {
    ...
    FLARE_IMPL_SHARED_ALLOCATION_TRACKER_DECREMENT
  ...
  // Copy:
  FLARE_FORCEINLINE_FUNCTION
  ~SharedAllocationTracker(){FLARE_IMPL_SHARED_ALLOCATION_TRACKER_DECREMENT}

  FLARE_FORCEINLINE_FUNCTION constexpr SharedAllocationTracker()
      : m_record_bits(DO_NOT_DEREF_FLAG) {}
  ...
  SharedAllocationTracker(const SharedAllocationTracker& rhs)
      : m_record_bits(FLARE_IMPL_SHARED_ALLOCATION_CARRY_RECORD_BITS(
            rhs, true)){FLARE_IMPL_SHARED_ALLOCATION_TRACKER_INCREMENT}

        /** \brief  Copy construction may disable tracking. */
        FLARE_FORCEINLINE_FUNCTION SharedAllocationTracker(
            const SharedAllocationTracker& rhs, const bool enable_tracking)
      : m_record_bits(FLARE_IMPL_SHARED_ALLOCATION_CARRY_RECORD_BITS(
            rhs,
            enable_tracking)){FLARE_IMPL_SHARED_ALLOCATION_TRACKER_INCREMENT}
    ...
    // If this is tracking then must decrement
    FLARE_IMPL_SHARED_ALLOCATION_TRACKER_DECREMENT
    m_record_bits = FLARE_IMPL_SHARED_ALLOCATION_CARRY_RECORD_BITS(rhs, true);
    FLARE_IMPL_SHARED_ALLOCATION_TRACKER_INCREMENT
    return *this;
  }

  /*  The following functions (assign_direct and assign_force_disable)
   *  are the result of deconstructing the
   *  FLARE_IMPL_SHARED_ALLOCATION_CARRY_RECORD_BITS macro.  This
  ...
  void assign(const SharedAllocationTracker& rhs, const bool enable_tracking) {
    FLARE_IMPL_SHARED_ALLOCATION_TRACKER_DECREMENT
    m_record_bits =
        FLARE_IMPL_SHARED_ALLOCATION_CARRY_RECORD_BITS(rhs, enable_tracking);
    FLARE_IMPL_SHARED_ALLOCATION_TRACKER_INCREMENT
  }

#undef FLARE_IMPL_SHARED_ALLOCATION_TRACKER_INCREMENT
#undef FLARE_IMPL_SHARED_ALLOCATION_TRACKER_DECREMENT
};

} /* namespace detail */
} /* namespace flare */
#endif  // FLARE_CORE_MEMORY_SHARED_ALLOC_H_
