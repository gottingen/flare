...
bool sentinel() {
  ...
  if (nullptr == s_hwloc_topology) {
    std::cerr << "flare::hwloc ERROR : Called after return from main()"
              << std::endl;
    std::cerr.flush();
  }
...
Sentinel::Sentinel() {
  ...
  if (remove_core_0) {
    ...
    if (hwloc_bitmap_intersects(s_process_binding, core->cpuset)) {
      ...
      if (flare::show_warnings() && !ok) {
        std::cerr << "WARNING: flare::hwloc attempted and failed to move "
  ...
  if (flare::show_warnings() && !symmetric) {
    std::cerr << "flare::hwloc WARNING: Using a symmetric subset of a "
...
bool bind_this_thread(const std::pair<unsigned, unsigned> coord) {
  ...
  std::cout << "flare::bind_this_thread() at ";
...
#define HWLOC_DEBUG_PRINT 0
  ...
  std::cout << "flare::unbind_this_thread() from ";
...
#else /* ! defined( FLARE_ENABLE_HWLOC ) */
