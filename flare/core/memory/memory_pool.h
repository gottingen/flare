...
#ifndef FLARE_CORE_MEMORY_MEMORY_POOL_H_
#define FLARE_CORE_MEMORY_MEMORY_POOL_H_
...
namespace flare {
    ...
    class MemoryPool {
    private:
        using CB = flare::detail::concurrent_bitset;

        enum : uint32_t {
            bits_per_int_lg2 = CB::bits_per_int_lg2
        };
        enum : uint32_t {
            state_shift = CB::state_shift
        };
        enum : uint32_t {
            state_used_mask = CB::state_used_mask
        };
        enum : uint32_t {
            state_header_mask = CB::state_header_mask
        };
        enum : uint32_t {
            max_bit_count_lg2 = CB::max_bit_count_lg2
        };
        enum : uint32_t {
            max_bit_count = CB::max_bit_count
        };

        enum : uint32_t {
            HINT_PER_BLOCK_SIZE = 2
        };
        ...
        enum {
            accessible = flare::detail::MemorySpaceAccess<flare::HostSpace,
                    base_memory_space>::accessible
        };

        using Tracker = flare::detail::SharedAllocationTracker;
        using Record = flare::detail::SharedAllocationRecord<base_memory_space>;
    ...
    public:
        ...
        enum : uint32_t {
            max_superblock_size = 1LU << 31 /* 2 gigabytes */ };
        enum : uint32_t {
            max_block_per_superblock = max_bit_count
        };
        ...
        void get_usage_statistics(usage_statistics &stats) const {
            ...
            if (!accessible) {
                flare::detail::DeepCopy<flare::HostSpace, base_memory_space>(
        ...
        void print_state(std::ostream &s) const {
            ...
            detail::_print_memory_pool_state(s, sb_state_array, m_sb_count, m_sb_size_lg2,
        ...
        FLARE_DEFAULTED_FUNCTION MemoryPool(MemoryPool &&) = default;

        FLARE_DEFAULTED_FUNCTION MemoryPool(const MemoryPool &) = default;

        FLARE_DEFAULTED_FUNCTION MemoryPool &operator=(MemoryPool &&) = default;

        FLARE_DEFAULTED_FUNCTION MemoryPool &operator=(const MemoryPool &) = default;

        FLARE_INLINE_FUNCTION MemoryPool()
                : m_tracker(),
        ...
        MemoryPool(const base_memory_space &memspace,
            ...
            flare::detail::memory_pool_bounds_verification(
            ...
            m_min_block_size_lg2 =
                    flare::detail::integral_power_of_two_that_contains(min_block_alloc_size);

            m_max_block_size_lg2 =
                    flare::detail::integral_power_of_two_that_contains(max_block_alloc_size);

            m_sb_size_lg2 =
                    flare::detail::integral_power_of_two_that_contains(min_superblock_size);
            ...
            Record *rec = Record::allocate(memspace, "flare::MemoryPool", alloc_size);
            ...
            if (!accessible) {
                flare::detail::DeepCopy<base_memory_space, flare::HostSpace>(
    ...
    private:
        ...
        uint32_t get_block_size_lg2(uint32_t n) const noexcept {
            const unsigned i = flare::detail::integral_power_of_two_that_contains(n);
    ...
    public:
        ...
        void *allocate(size_t alloc_size, int32_t attempt_limit = 1) const noexcept {
            if (size_t(1LU << m_max_block_size_lg2) < alloc_size) {
                flare::abort(
                        "flare MemoryPool allocation request exceeded specified maximum "
                        "allocation size");
            }
            ...
            const uint32_t block_id_hint =
                    (uint32_t) (flare::detail::clock_tic()
            ...
            while (attempt_limit) {
                ...
                if (sb_state == (state_header_mask & *sb_state_array)) {
                    ...
                    const flare::pair<int, int> result = CB::acquire_bounded_lg2(
                ...
                if (sb_id < 0) {
                    ...
                    if (0 <= sb_id_empty) {
                        ...
                        update_hint =
                                state_empty == flare::atomic_compare_exchange(
                ...
                if (update_hint) {
                    flare::atomic_compare_exchange(hint_sb_id_ptr, uint32_t(hint_sb_id),
                                                   uint32_t(sb_id));
                }
        ...
        void deallocate(void *p, size_t /* alloc_size */) const noexcept {
            ...
            if (!ok_contains || !ok_block_aligned || !ok_dealloc_once) {
                flare::abort("flare MemoryPool::deallocate given erroneous pointer");
            }
        }
        // end deallocate
        //--------------------------------------------------------------------------
...
#endif  // FLARE_CORE_MEMORY_MEMORY_POOL_H_
