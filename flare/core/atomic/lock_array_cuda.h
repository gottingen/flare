...
#ifndef FLARE_CORE_ATOMIC_LOCK_ARRAY_CUDA_H_
#define FLARE_CORE_ATOMIC_LOCK_ARRAY_CUDA_H_
...
/// The function is templated to make it a weak symbol to deal with flare/RAJA
...
///   snapshotted version while also linking against pure Desul
        ...
        /// whose definition will be in cuda_locks.cu (and whose declaration
        ...
        ///  unit can link to it).
        /// Since the cuda_locks.cu translation unit cannot initialize the
...
#define CUDA_SPACE_ATOMIC_MASK 0x1FFFF
        ...
        __device__ inline bool lock_address_cuda(void *ptr, flare::MemoryScopeDevice) {
            size_t offset = size_t(ptr);
            offset = offset >> 2;
            offset = offset & CUDA_SPACE_ATOMIC_MASK;
            return (0 == atomicExch(&flare::detail::CUDA_SPACE_ATOMIC_LOCKS_DEVICE[offset], 1));
        }

        __device__ inline bool lock_address_cuda(void *ptr, flare::MemoryScopeNode) {
            size_t offset = size_t(ptr);
            offset = offset >> 2;
            offset = offset & CUDA_SPACE_ATOMIC_MASK;
            return (0 == atomicExch(&flare::detail::CUDA_SPACE_ATOMIC_LOCKS_NODE[offset], 1));
        }
...
/// lock_address.
        __device__ inline void unlock_address_cuda(void *ptr, flare::MemoryScopeDevice) {
            size_t offset = size_t(ptr);
            offset = offset >> 2;
            offset = offset & CUDA_SPACE_ATOMIC_MASK;
            atomicExch(&flare::detail::CUDA_SPACE_ATOMIC_LOCKS_DEVICE[offset], 0);
        }

        __device__ inline void unlock_address_cuda(void *ptr, flare::MemoryScopeNode) {
            size_t offset = size_t(ptr);
            offset = offset >> 2;
            offset = offset & CUDA_SPACE_ATOMIC_MASK;
            atomicExch(&flare::detail::CUDA_SPACE_ATOMIC_LOCKS_NODE[offset], 0);
        }

        inline static void copy_cuda_lock_arrays_to_device() {
            static bool once = []() {
...
namespace flare {
    ...
    static inline void ensure_cuda_lock_arrays_on_device() {
        detail::copy_cuda_lock_arrays_to_device();
    }


}  // namespace flare

#endif  // FLARE_CORE_ATOMIC_LOCK_ARRAY_CUDA_H_
