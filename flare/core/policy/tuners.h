...
#ifndef FLARE_CORE_POLICY_TUNERS_H_
#define FLARE_CORE_POLICY_TUNERS_H_
...
size_t declare_output_type(const std::string&,
                           flare::Tools::experimental::VariableInfo);
void request_output_values(size_t, size_t,
                           flare::Tools::experimental::VariableValue*);
...
struct GetPoint<PointType,
                std::array<flare::Tools::experimental::VariableValue, X>> {
...
class MultidimensionalSparseTuningProblem {
 public:
  ...
  static constexpr int space_dimensionality =
      detail::get_space_dimensionality<ProblemSpaceInput>::value;
  ...
  using StoredProblemSpace =
      typename detail::MapTypeConverter<ProblemSpaceInput>::type;
  using HierarchyConstructor =
      typename detail::ValueHierarchyConstructor<Container<TemplateArguments...>>;

  using ValueArray = std::array<flare::Tools::experimental::VariableValue,
  ...
  using ExtendedProblemSpace =
      typename detail::MapTypeConverter<extended_map<Key, Value>>::type;
 ...
 public:
  ...
  MultidimensionalSparseTuningProblem(StoredProblemSpace space,
    ...
    for (unsigned long x = 0; x < names.size(); ++x) {
      VariableInfo info;
      info.type = flare::Tools::experimental::ValueType::flare_value_double;
      info.category = flare::Tools::experimental::StatisticalCategory::
          flare_value_interval;
      info.valueQuantity =
          flare::Tools::experimental::CandidateValueType::flare_value_range;
      info.candidates = flare::Tools::experimental::make_candidate_range(
  ...
  auto get_point(Coordinates... coordinates) {
    using ArrayType = std::array<flare::Tools::experimental::VariableValue,
                                 sizeof...(coordinates)>;
    return detail::get_point(
        m_space, ArrayType({flare::Tools::experimental::make_variable_value(
                     0, static_cast<double>(coordinates))...}));
  }

  auto begin() {
    context = flare::Tools::experimental::get_new_context_id();
    ValueArray values;
    for (int x = 0; x < space_dimensionality; ++x) {
      values[x] = flare::Tools::experimental::make_variable_value(
          variable_ids[x], 0.0);
    }
    begin_context(context);
    request_output_values(context, space_dimensionality, values.data());
    return detail::get_point(m_space, values);
  }
...
class TeamSizeTuner : public ExtendableTunerMixin<TeamSizeTuner> {
 ...
 public:
  ...
  TeamSizeTuner(const std::string& name,
    ...
    using PolicyType           = flare::TeamPolicy<Properties...>;
  ...
  void tune(flare::TeamPolicy<Properties...>& policy) {
    if (flare::Tools::experimental::have_tuning_tool()) {
...
struct MDRangeTuner : public ExtendableTunerMixin<MDRangeTuner<MDRangeRank>> {
 private:
  ...
  using SpaceDescription =
      typename detail::n_dimensional_sparse_structure<int, rank>::type;
  using TunerType =
 ...
 public:
  ...
  MDRangeTuner(const std::string& name,
               const flare::MDRangePolicy<Properties...>& policy,
    ...
    detail::fill_tile(desc, max_tile_size);
  ...
  template <typename... Properties>
  void tune(flare::MDRangePolicy<Properties...>& policy) {
    if (flare::Tools::experimental::have_tuning_tool()) {
      auto configuration = tuner.begin();
...
struct CategoricalTuner {
  ...
  CategoricalTuner(std::string name, choice_list m_choices)
    ...
    VariableInfo info;
    info.category      = StatisticalCategory::flare_value_categorical;
    info.valueQuantity = CandidateValueType::flare_value_set;
    info.type          = ValueType::flare_value_int64;
...
#endif  // FLARE_CORE_POLICY_TUNERS_H_
