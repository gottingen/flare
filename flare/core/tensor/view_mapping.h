...
#ifndef FLARE_CORE_TENSOR_VIEW_MAPPING_H_
#define FLARE_CORE_TENSOR_VIEW_MAPPING_H_
...
#include <flare/core/tensor/view_tracker.h>
#include <flare/core/tensor/view_ctor.h>
...
struct variadic_size_t {
  enum : size_t { value = FLARE_INVALID_INDEX };
};
...
#define FLARE_IMPL_VIEW_DIMENSION(R)                                       \
  ...
  struct ViewDimension##R {                                                 \
    ...
    FLARE_INLINE_FUNCTION explicit ViewDimension##R(size_t) {}             \
  ...
  struct ViewDimension##R<0u, RD> {                                         \
    ...
    ViewDimension##R(const ViewDimension##R&) = default;                    \
    ViewDimension##R& operator=(const ViewDimension##R&) = default;         \
    FLARE_INLINE_FUNCTION explicit ViewDimension##R(size_t V) : N##R(V) {} \
  };                                                                        \
  template <unsigned RD>                                                    \
  constexpr size_t ViewDimension##R<0u, RD>::ArgN##R;

FLARE_IMPL_VIEW_DIMENSION(0)
FLARE_IMPL_VIEW_DIMENSION(1)
FLARE_IMPL_VIEW_DIMENSION(2)
FLARE_IMPL_VIEW_DIMENSION(3)
FLARE_IMPL_VIEW_DIMENSION(4)
FLARE_IMPL_VIEW_DIMENSION(5)
FLARE_IMPL_VIEW_DIMENSION(6)
FLARE_IMPL_VIEW_DIMENSION(7)

#undef FLARE_IMPL_VIEW_DIMENSION

// MSVC does not do empty base class optimization by default.
// Per standard it is required for standard layout types
template <size_t... Vals>
struct FLARE_IMPL_ENFORCE_EMPTY_BASE_OPTIMIZATION ViewDimension
...
struct is_integral_extent_type {
  enum : bool { value = std::is_same<T, flare::ALL_t>::value ? 1 : 0 };
};
...
template <int RankDest, int RankSrc, int CurrentArg, class Arg,
          class... SubViewArgs>
struct SubviewLegalArgsCompileTime<flare::LayoutLeft, flare::LayoutLeft,
  ...
  enum {
    value = (((CurrentArg == RankDest - 1) &&
             ...
             ((CurrentArg == 0) &&
              (flare::detail::is_integral_extent_type<Arg>::value))) &&
            (SubviewLegalArgsCompileTime<flare::LayoutLeft, flare::LayoutLeft,
...
template <int RankDest, int RankSrc, int CurrentArg, class Arg,
          class... SubViewArgs>
struct SubviewLegalArgsCompileTime<flare::LayoutRight, flare::LayoutRight,
                                   RankDest, RankSrc, CurrentArg, Arg,
                                   SubViewArgs...> {
  enum {
    value = (((CurrentArg == RankSrc - RankDest) &&
             ...
             ((CurrentArg >= RankSrc - RankDest) &&
              (std::is_same<Arg, flare::ALL_t>::value))) &&
            (SubviewLegalArgsCompileTime<flare::LayoutRight,
...
struct SubviewLegalArgsCompileTime<flare::LayoutStride, flare::LayoutLeft,
...
template <int RankDest, int RankSrc, int CurrentArg, class... SubViewArgs>
struct SubviewLegalArgsCompileTime<flare::LayoutStride, flare::LayoutRight,
                                   RankDest, RankSrc, CurrentArg,
                                   SubViewArgs...> {
  enum : bool { value = true };
};
...
template <int RankDest, int RankSrc, int CurrentArg, class... SubViewArgs>
struct SubviewLegalArgsCompileTime<flare::LayoutStride, flare::LayoutStride,
                                   RankDest, RankSrc, CurrentArg,
                                   SubViewArgs...> {
  enum : bool { value = true };
};
...
struct SubviewExtents {
 private:
  ...
  template <size_t... DimArgs, class... Args>
  void error(char* buf, int buf_len, unsigned domain_rank, unsigned range_rank,
             const ViewDimension<DimArgs...>& dim, flare::ALL_t,
             Args... args) const {
  ...
  template <size_t... DimArgs, class... Args>
  FLARE_FORCEINLINE_FUNCTION void error(const ViewDimension<DimArgs...>& dim,
                                         Args... args) const {
    FLARE_IF_ON_HOST(
        (enum {LEN = 1024}; char buffer[LEN];

         const int n = snprintf(buffer, LEN, "flare::subview bounds error (");
         error(buffer + n, LEN - n, 0, 0, dim, args...);

         flare::detail::throw_runtime_exception(std::string(buffer));))

    FLARE_IF_ON_DEVICE(((void)dim;
                         flare::abort("flare::subview bounds error");
                         [](Args...) {}(args...);))
  }
  ...
  template <size_t... DimArgs, class... Args>
  FLARE_FORCEINLINE_FUNCTION void error(const ViewDimension<DimArgs...>&,
                                         Args...) const {}
 ...
 public:
  template <size_t... DimArgs, class... Args>
  FLARE_INLINE_FUNCTION SubviewExtents(const ViewDimension<DimArgs...>& dim,
                                        Args... args) {
...
struct ViewOffset<
  ...
  FLARE_FUNCTION ViewOffset() : m_dim(dimension_type()) {}
  FLARE_FUNCTION ViewOffset(const ViewOffset& src) { m_dim = src.m_dim; }
  ...
  FLARE_INLINE_FUNCTION constexpr ViewOffset(
      const ViewOffset<DimRHS, flare::LayoutLeft, void>& rhs)
  ...
  template <class DimRHS>
  FLARE_INLINE_FUNCTION constexpr ViewOffset(
      const ViewOffset<DimRHS, flare::LayoutRight, void>& rhs)
  ...
  template <class DimRHS>
  FLARE_INLINE_FUNCTION ViewOffset(
      const ViewOffset<DimRHS, flare::LayoutStride, void>& rhs)
      : m_dim(rhs.m_dim.N0, 0, 0, 0, 0, 0, 0, 0) {
    if (rhs.m_stride.S0 != 1) {
      flare::abort(
          "flare::detail::ViewOffset assignment of LayoutLeft from LayoutStride "
          " requires stride == 1");
    }
  }
  ...
  template <class DimRHS>
  FLARE_INLINE_FUNCTION constexpr ViewOffset(
      const ViewOffset<DimRHS, flare::LayoutLeft, void>&,
...
struct ViewOffset<
 ...
 public:
  ...
  // I changed this.
#ifdef FLARE_IMPL_WINDOWS_CUDA
  FLARE_FUNCTION ViewOffset() : m_dim(dimension_type()), m_stride(0) {}
...
struct ViewOffset<
  ...
  template <class DimRHS>
  FLARE_INLINE_FUNCTION constexpr ViewOffset(
      const ViewOffset<DimRHS, flare::LayoutRight, void>&,
      const SubviewExtents<DimRHS::rank, dimension_type::rank>& sub)
      : m_dim(sub.range_extent(0), 0, 0, 0, 0, 0, 0, 0) {
    static_assert((0 == dimension_type::rank_dynamic) ||
                      (1 == dimension_type::rank &&
                           ...
                           : ((dimension_type::rank == 5)
                                  ...
                                  : ((dimension_type::rank == 6)
                                         ...
                                         : ((dimension_type::rank == 7)
                                                ...
                                                : rhs.m_stride.S7)))))) != 1) {
      flare::abort(
          "flare::detail::ViewOffset assignment of LayoutRight from "
...
template <class Dimension>
struct ViewOffset<Dimension, flare::LayoutStride, void> {
 private:
...
struct ViewDataHandle<
  ...
  using value_type  = typename Traits::value_type;
  using handle_type = typename flare::detail::AtomicViewDataHandle<Traits>;
...
struct ViewDataHandle<
  ...
  static handle_type assign(value_type* arg_data_ptr,
    ...
    if (reinterpret_cast<uintptr_t>(arg_data_ptr) % detail::MEMORY_ALIGNMENT) {
      flare::abort(
          "Assigning NonAligned View or Pointer to flare::View with Aligned "
          "attribute");
    }
    return handle_type(arg_data_ptr);
  }
    ...
    if (detail::is_zero_byte(value)) {
      ...
      if (flare::Profiling::profileLibraryLoaded()) {
        ...
        flare::Profiling::beginParallelFor(
            "flare::View::initialization [" + name + "] via memset",
            flare::Profiling::experimental::device_id(space), &kpID);
      }
      (void)ZeroMemset<
          ExecSpace, flare::View<ValueType*, typename DeviceType::memory_space,
                                  flare::MemoryTraits<flare::Unmanaged>>>(
          space,
          flare::View<ValueType*, typename DeviceType::memory_space,
      ...
      if (default_exec_space)
        space.fence("flare::detail::ViewValueFunctor: View init/destroy fence");
    } else {
#endif
      ...
      if (flare::Profiling::profileLibraryLoaded()) {
        const std::string functor_name =
            (std::is_same_v<Tag, DestroyTag>
                 ? "flare::View::destruction [" + name + "]"
                 : "flare::View::initialization [" + name + "]");
        flare::Profiling::beginParallelFor(
...
#endif
      const flare::detail::ParallelFor<ViewValueFunctor, PolicyType> closure(
          *this, policy);
      closure.execute();
...
// leading to the significant performance issues
  ...
  void parallel_for_implementation() {
    if (!space.in_parallel()) {
      ...
      if (default_exec_space)
        space.fence(
            "flare::detail::ViewValueFunctor: Fence after setting values in "
            "view");
      if (flare::Profiling::profileLibraryLoaded()) {
        flare::Profiling::endParallelFor(kpID);
      }
...
class ViewMapping<
 ...
 public:
  ...
  FLARE_DEFAULTED_FUNCTION ~ViewMapping() = default;
  FLARE_INLINE_FUNCTION ViewMapping() : m_impl_handle(), m_impl_offset() {}

  FLARE_DEFAULTED_FUNCTION ViewMapping(const ViewMapping&) = default;
  FLARE_DEFAULTED_FUNCTION ViewMapping& operator=(const ViewMapping&) =
      default;

  FLARE_DEFAULTED_FUNCTION ViewMapping(ViewMapping&&) = default;
  FLARE_DEFAULTED_FUNCTION ViewMapping& operator=(ViewMapping&&) = default;
  ...
  template <class... P>
  FLARE_INLINE_FUNCTION ViewMapping(
      flare::detail::ViewCtorProp<P...> const& arg_prop,
      typename Traits::array_layout const& arg_layout)
  ...
  flare::detail::SharedAllocationRecord<>* allocate_shared(
    ...
    using alloc_prop = flare::detail::ViewCtorProp<P...>;
    ...
    using value_type = typename Traits::value_type;
    using functor_type =
        ViewValueFunctor<flare::Device<execution_space, memory_space>,
                         value_type>;
    using record_type =
...
class ViewMapping<
 ...
 public:
  ...
  static void assign(DstType& dst, const SrcType& src,
    ...
    dst.m_impl_handle = flare::detail::ViewDataHandle<DstTraits>::assign(
 ...
 public:
  ...
  static void assign(DstType& dst, const SrcType& src,
    ...
    if (!assignable_layout)
      flare::abort("View assignment must have compatible layouts\n");
...
struct SubViewDataTypeImpl<void, ValueType, flare::experimental::Extents<>> {
 ...
 private:
  ...
  using data_type =
      typename SubViewDataType<value_type,
                               typename flare::detail::ParseViewExtents<
...
FLARE_INLINE_FUNCTION bool view_verify_operator_bounds(const MapType&) {
  return true;
}

template <unsigned R, class MapType, class iType, class... Args>
FLARE_INLINE_FUNCTION bool view_verify_operator_bounds(const MapType& map,
                                                        const iType& i,
                                                        Args... args) {
...
struct OperatorBoundsErrorOnDevice<MapType, false> {
  FLARE_INLINE_FUNCTION
  static void run(MapType const&) { flare::abort("View bounds error"); }
};
...
FLARE_INLINE_FUNCTION void view_verify_operator_bounds(
    flare::detail::ViewTracker<ViewType> const& tracker, const MapType& map,
    Args... args) {
  if (!view_verify_operator_bounds<0>(map, args...)) {
    ...
    FLARE_IF_ON_DEVICE((
        ...
        } else { flare::abort("View bounds error"); }))
  }
}
...
struct RuntimeCheckViewMemoryAccessViolation {
  ...
  FLARE_FUNCTION RuntimeCheckViewMemoryAccessViolation(char const* const,
                                                        Track const&,
                                                        Map const&) {}
};
...
struct RuntimeCheckViewMemoryAccessViolation<MemorySpace, AccessSpace, false> {
  template <class Track, class Map>
  FLARE_FUNCTION RuntimeCheckViewMemoryAccessViolation(char const* const msg,
                                                        Track const& track,
                                                        Map const&) {
    char err[256] = "";
    strncat(err, msg, 64);
...
#endif  // FLARE_CORE_TENSOR_VIEW_MAPPING_H_
