...
#ifndef FLARE_CORE_TENSOR_TENSOR_ARRAY_MAPPING_H_
#define FLARE_CORE_TENSOR_TENSOR_ARRAY_MAPPING_H_
...
namespace flare::detail {
    ...
    struct TensorDataAnalysis<DataType, ArrayLayout, flare::Array<V, N, P>> {
    private:
        ...
        static_assert(std::is_same<typename array_analysis::non_const_value_type,
                              flare::Array<V, N, P>>::value,
                      "");
        static_assert(std::is_scalar<V>::value,
                      "Tensor of Array type must be of a scalar type");

    public:
        using specialize = flare::Array<>;
    ...
    public:
        ...
        using scalar_array_type =
                typename TensorDataType_<scalar_type, array_scalar_dimension>::type;
        using const_scalar_array_type =
                typename TensorDataType_<const_scalar_type, array_scalar_dimension>::type;
        using non_const_scalar_array_type =
                typename TensorDataType_<non_const_scalar_type,
                        array_scalar_dimension>::type;
    };

    /** \brief  Tensor mapping for non-specialized data type and standard layout */
    template<class Traits>
    class TensorMapping<Traits, flare::Array<>> {
    private:
        ...
        using contiguous_reference = flare::Array<scalar_type, (~std::size_t(0)),
                flare::Array<>::contiguous>;
        using strided_reference =
                flare::Array<scalar_type, (~std::size_t(0)), flare::Array<>::strided>;
        ...
        enum {
            Array_N = Traits::value_type::size()
        };
        enum {
            Array_S = is_contiguous_reference ? Array_N : 1
        };
    ...
    public:
        ...
        FLARE_FORCEINLINE_FUNCTION reference_type reference(const I0 &i0) const {
    ...
    private:
        ...
        enum {
            MemorySpanSize = sizeof(scalar_type)
        };
    ...
    public:
        ...
        template<class... Args>
        FLARE_INLINE_FUNCTION TensorMapping(pointer_type ptr, Args... args)
                : m_impl_handle(ptr),
        ...
        flare::detail::SharedAllocationRecord<> *allocate_shared(
            ...
            using functor_type =
                    TensorValueFunctor<typename Traits::device_type, scalar_type>;
            using record_type =
            ...
            const size_t alloc_size =
                    ...
                    ~size_t(MemorySpanMask);
            const auto &alloc_name = detail::get_property<detail::LabelTag>(arg_prop);
    ...
    class TensorMapping<
            ...
            std::enable_if_t<(
                    ...
                    std::is_same<typename SrcTraits::specialize, flare::Array<>>::value &&
                    (std::is_same<typename SrcTraits::array_layout,
                     ...
                     std::is_same<typename SrcTraits::array_layout,
                             flare::LayoutStride>::value))>> {
    public:
        // Can only convert to Tensor::array_type
        ...
        using SrcType = TensorMapping<SrcTraits, flare::Array<>>;
    ...
    public:
        ...
        FLARE_INLINE_FUNCTION
        static void assign(TensorMapping<traits_type, void> &dst,
                           TensorMapping<SrcTraits, void> const &src, Args... args) {
            using DstType = TensorMapping<traits_type, void>;
...
#endif  // FLARE_CORE_TENSOR_TENSOR_ARRAY_MAPPING_H_
