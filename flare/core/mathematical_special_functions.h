...
#ifndef FLARE_CORE__MATHEMATICAL_SPECIAL_FUNCTIONS_H_
#define FLARE_CORE__MATHEMATICAL_SPECIAL_FUNCTIONS_H_
...
FLARE_INLINE_FUNCTION RealType expint1(RealType x) {
...
template <class RealType>
FLARE_INLINE_FUNCTION flare::complex<RealType> erf(
    const flare::complex<RealType>& z) {
  ...
  using flare::fabs;
  using flare::sin;
  using flare::experimental::epsilon_v;
  using flare::experimental::infinity_v;
  using flare::numbers::pi_v;

  using CmplxType = flare::complex<RealType>;
  ...
  if (az <= 2.0) {  // Series for abs(z)<=2.0
    ...
    if (xp > 1.0) {
      ...
      if (flare::abs(cz) > 670.0) return CmplxType(inf, inf);
      cz              = -cz;
      RealType er     = cz.real();
      RealType ei     = cz.imag();
      cz              = exp(er) * CmplxType(cos(ei), sin(ei));
      ...
      if (fabs(yp) <
        ...
        for (int i = 1; i <= 35; i++) {
          term  = -term * ak * rcz;
          accum = accum + term;
          if (flare::abs(term) / flare::abs(accum) <= tol) break;
          ak = ak + 2.0;
        }
...
//! for z=cmplx(x,y).
template <class RealType>
FLARE_INLINE_FUNCTION flare::complex<RealType> erfcx(
    const flare::complex<RealType>& z) {
  // This function is a conversion of the corresponding Fortran program written
  // by D.E. Amos, May,1974. D.E. Amos' revisions of Jan 86 incorporated by
  // Ken Damrau on 27-Jan-1986 14:37:13
  ...
  using flare::experimental::epsilon_v;
  using flare::experimental::infinity_v;
  using flare::numbers::inv_sqrtpi_v;
  using flare::numbers::pi_v;
...
//! for real x
template <class RealType>
FLARE_INLINE_FUNCTION RealType erfcx(RealType x) {
  using CmplxType = flare::complex<RealType>;
...
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_j0(const CmplxType& z,
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      for (int k = 1; k <= 12; k++) {  // Calculate (5.2.9)
        cp0 = cp0 + a[k - 1] * flare::pow(z1, -2.0 * k);
      }
      CmplxType cq0 = -0.125 / z1;
      for (int k = 1; k <= 12; k++) {  // Calculate (5.2.10)
        cq0 = cq0 + b[k - 1] * flare::pow(z1, -2.0 * k - 1);
      }
      CmplxType cu = flare::sqrt(r2p / z1);
      cbj0         = cu * (cp0 * flare::cos(ct1) - cq0 * flare::sin(ct1));
    }
  }
  return cbj0;
}
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_y0(const CmplxType& z,
                                               const RealType& joint_val = 25,
                                               const IntType& bw_start   = 70) {
  // This function is converted and modified from the corresponding Fortran
  // program CJYNB in S. Zhang & J. Jin "Computation of Special Functions"
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      CmplxType cu = flare::sqrt(r2p / z1);
      cbj0         = cu * (cp0 * flare::cos(ct1) - cq0 * flare::sin(ct1));
      cby0         = cu * (cp0 * flare::sin(ct1) + cq0 * flare::cos(ct1));
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_j1(const CmplxType& z,
                                               const RealType& joint_val = 25,
                                               const IntType& bw_start   = 70) {
  // This function is converted and modified from the corresponding Fortran
  // program CJYNB in S. Zhang & J. Jin "Computation of Special Functions"
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      for (int k = 1; k <= 12; k++) {  // Calculate (5.2.11)
        cp1 = cp1 + a1[k - 1] * flare::pow(z1, -2.0 * k);
      }
      CmplxType cq1 = 0.375 / z1;
      for (int k = 1; k <= 12; k++) {  // Calculate (5.2.12)
        cq1 = cq1 + b1[k - 1] * flare::pow(z1, -2.0 * k - 1);
      }
      CmplxType cu = flare::sqrt(r2p / z1);
      cbj1         = cu * (cp1 * flare::cos(ct2) - cq1 * flare::sin(ct2));
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_y1(const CmplxType& z,
                                               const RealType& joint_val = 25,
                                               const IntType& bw_start   = 70) {
  // This function is converted and modified from the corresponding Fortran
  // program CJYNB in S. Zhang & J. Jin "Computation of Special Functions"
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      CmplxType cu = flare::sqrt(r2p / z1);
      cbj1         = cu * (cp1 * flare::cos(ct2) - cq1 * flare::sin(ct2));
      cby1         = cu * (cp1 * flare::sin(ct2) + cq1 * flare::cos(ct2));
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_i0(const CmplxType& z,
                                               const RealType& joint_val = 25,
                                               const IntType& bw_start   = 70) {
  // This function is converted and modified from the corresponding Fortran
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      cs0  = flare::exp(z1) / (cbs - cf);
      cbi0 = cbi0 * cs0;
    } else {  // Using asymptotic expansion (6.2.1) for |z|>joint_val
              // (default:25)
      CmplxType ca = flare::exp(z1) / flare::sqrt(2.0 * pi * z1);
      cbi0         = CmplxType(1.0, 0.0);
      CmplxType zr = 1.0 / z1;
      for (int k = 1; k <= 12; k++) {
        cbi0 = cbi0 + a[k - 1] * flare::pow(zr, 1.0 * k);
      }
      cbi0 = ca * cbi0;
    }
  }
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_k0(const CmplxType& z,
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      cs0  = flare::exp(z1) / (cbs - cf);
      cbi0 = cbi0 * cs0;
      cbk0 = -(flare::log(0.5 * z1) + el) * cbi0 + cs0 * csk0;
    } else {  // Using asymptotic expansion (6.2.2) for |z|>joint_val
              // (default:9)
      CmplxType ca0  = flare::sqrt(pi / (2.0 * z1)) * flare::exp(-z1);
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_i1(const CmplxType& z,
                                               const RealType& joint_val = 25,
                                               const IntType& bw_start   = 70) {
  // This function is converted and modified from the corresponding Fortran
  // programs CIKNB and CIK01 in S. Zhang & J. Jin "Computation of Special
  ...
  if (a0 < 1e-100) {  // Treat z=0 as a special case
    ...
    if (a0 <= joint_val) {  // Using backward recurrence for |z|<=joint_val
      ...
      CmplxType zr = 1.0 / z1;
      for (int k = 1; k <= 12; k++) {
        cbi1 = cbi1 + b[k - 1] * flare::pow(zr, 1.0 * k);
      }
      cbi1 = ca * cbi1;
    }
    if (z.real() < 0.0) {  // Apply (6.4.4)
...
//! for a complex argument
template <class CmplxType, class RealType, class IntType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_k1(const CmplxType& z,
                                               const RealType& joint_val = 9,
                                               const IntType& bw_start   = 30) {
  // This function is converted and modified from the corresponding Fortran
  // programs CIKNB and CIK01 in S. Zhang & J. Jin "Computation of Special
...
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_h10(const CmplxType& z) {
  // This function is converted and modified from the corresponding Fortran
...
//! for a complex argument
template <class CmplxType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_h11(const CmplxType& z) {
  // This function is converted and modified from the corresponding Fortran
  // programs CH12N in S. Zhang & J. Jin "Computation of Special Functions"
  //(Wiley, 1996).
  using RealType = typename CmplxType::value_type;
...
//! for a complex argument
template <class CmplxType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_h20(const CmplxType& z) {
  // This function is converted and modified from the corresponding Fortran
  // programs CH12N in S. Zhang & J. Jin "Computation of Special Functions"
  //(Wiley, 1996).
  using RealType = typename CmplxType::value_type;
...
//! for a complex argument
template <class CmplxType>
FLARE_INLINE_FUNCTION CmplxType cyl_bessel_h21(const CmplxType& z) {
  // This function is converted and modified from the corresponding Fortran
  // programs CH12N in S. Zhang & J. Jin "Computation of Special Functions"
  //(Wiley, 1996).
  using RealType = typename CmplxType::value_type;
...
#endif  // FLARE_CORE__MATHEMATICAL_SPECIAL_FUNCTIONS_H_
