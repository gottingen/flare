...
//
#ifndef FLARE_SIMD_TYPES_BATCH_H_
#define FLARE_SIMD_TYPES_BATCH_H_

#include <cassert>
#include <complex>

#include <flare/simd/arch.h>
#include <flare/simd/alignment.h>
...
namespace flare::simd {
    ...
    namespace types {
        ...
        struct integral_only_operators {
            ...
            /** Shorthand for flare::simd::mod() */
            friend batch <T, A> operator%(batch <T, A> const &self, batch <T, A> const &other) noexcept {
                return batch<T, A>(self) %= other;
            }

            /** Shorthand for flare::simd::bitwise_rshift() */
            friend batch <T, A> operator>>(batch <T, A> const &self, batch <T, A> const &other) noexcept {
                return batch<T, A>(self) >>= other;
            }

            /** Shorthand for flare::simd::bitwise_lshift() */
            friend batch <T, A> operator<<(batch <T, A> const &self, batch <T, A> const &other) noexcept {
                return batch<T, A>(self) <<= other;
            }
    ...
    public:
        ...
        template<class U>
        FLARE_NO_DISCARD static batch broadcast(U val) noexcept;
        ...
        template<class U>
        FLARE_NO_DISCARD static batch load_aligned(U const *mem) noexcept;

        template<class U>
        FLARE_NO_DISCARD static batch load_unaligned(U const *mem) noexcept;

        template<class U>
        FLARE_NO_DISCARD static batch load(U const *mem, aligned_mode) noexcept;

        template<class U>
        FLARE_NO_DISCARD static batch load(U const *mem, unaligned_mode) noexcept;

        template<class U, class V>
        FLARE_NO_DISCARD static batch gather(U const *src, batch<V, arch_type> const &index) noexcept;
        ...
        /** Shorthand for flare::simd::add() */
        friend batch operator+(batch const &self, batch const &other) noexcept {
            return batch(self) += other;
        }

        /** Shorthand for flare::simd::sub() */
        friend batch operator-(batch const &self, batch const &other) noexcept {
            return batch(self) -= other;
        }

        /** Shorthand for flare::simd::mul() */
        friend batch operator*(batch const &self, batch const &other) noexcept {
            return batch(self) *= other;
        }

        /** Shorthand for flare::simd::div() */
        friend batch operator/(batch const &self, batch const &other) noexcept {
            return batch(self) /= other;
        }

        /** Shorthand for flare::simd::bitwise_and() */
        friend batch operator&(batch const &self, batch const &other) noexcept {
            return batch(self) &= other;
        }

        /** Shorthand for flare::simd::bitwise_or() */
        friend batch operator|(batch const &self, batch const &other) noexcept {
            return batch(self) |= other;
        }

        /** Shorthand for flare::simd::bitwise_xor() */
        friend batch operator^(batch const &self, batch const &other) noexcept {
            return batch(self) ^= other;
        }

        /** Shorthand for flare::simd::logical_and() */
        friend batch operator&&(batch const &self, batch const &other) noexcept {
            return batch(self).logical_and(other);
        }

        /** Shorthand for flare::simd::logical_or() */
        friend batch operator||(batch const &self, batch const &other) noexcept {
    ...
    public:
        ...
        FLARE_NO_DISCARD static batch_bool load_aligned(bool const *mem) noexcept;

        FLARE_NO_DISCARD static batch_bool load_unaligned(bool const *mem) noexcept;
    ...
    public:
        ...
        // memory operators
        FLARE_NO_DISCARD static batch load_aligned(const T *real_src, const T *imag_src = nullptr) noexcept;

        FLARE_NO_DISCARD static batch load_unaligned(const T *real_src, const T *imag_src = nullptr) noexcept;

        void store_aligned(T *real_dst, T *imag_dst) const noexcept;

        void store_unaligned(T *real_dst, T *imag_dst) const noexcept;

        FLARE_NO_DISCARD static batch load_aligned(const value_type *src) noexcept;

        FLARE_NO_DISCARD static batch load_unaligned(const value_type *src) noexcept;
...
namespace flare::simd {
    ...
    template<class T, class A>
    template<class U>
    FLARE_NO_DISCARD inline batch<T, A> batch<T, A>::broadcast(U val) noexcept {
        detail::static_check_supported_config<T, A>();
    ...
    inline batch<T, A> &types::integral_only_operators<T, A>::operator%=(batch<T, A> const &other) noexcept {
        ::flare::simd::detail::static_check_supported_config<T, A>();
    ...
    template<class T, class A>
    template<class U>
    FLARE_NO_DISCARD inline batch<std::complex<T>, A> batch<std::complex<T>, A>::broadcast(U val) noexcept {
    ...
    namespace details {
        ...
        struct sized_batch<T, N, flare::simd::arch_list<>> {
            using type = void;
        };

        template<typename T, class Arch, bool BatchExists = flare::simd::types::has_simd_register<T, Arch>::value>
        struct batch_trait;

        template<typename T, class Arch>
        struct batch_trait<T, Arch, true> {
            using type = flare::simd::batch<T, Arch>;
            static constexpr std::size_t size = flare::simd::batch<T, Arch>::size;
        };
        ...
        struct sized_batch<T, N, flare::simd::arch_list<Arch, Archs...>> {
            using type = typename std::conditional<
                    batch_trait<T, Arch>::size == N,
                    typename batch_trait<T, Arch>::type,
                    typename sized_batch<T, N, flare::simd::arch_list<Archs...>>::type>::type;
        };
    }
...
#endif  // FLARE_SIMD_TYPES_BATCH_H_
