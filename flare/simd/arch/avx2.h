...
//
#ifndef FLARE_SIMD_ARCH_AVX2_H_
#define FLARE_SIMD_ARCH_AVX2_H_

#include <complex>
#include <type_traits>

#include <flare/simd/types/avx2_register.h>
...
namespace flare::simd {
    ...
    namespace kernel {
        ...
        namespace detail {
            ...
            fast_cast(batch<uint32_t, A> const &v, batch<float, A> const &, requires_arch<avx2>) noexcept {
                ...
                __m256i v_lo = _mm256_and_si256(v,
                                                msk_lo); /* extract the 16 lowest significant bits of self                             */
                __m256i v_hi = _mm256_srli_epi32(v,
                                                 16); /* 16 most significant bits of v                                                 */
                __m256 v_lo_flt = _mm256_cvtepi32_ps(
                        v_lo); /* No rounding                                                                   */
                __m256 v_hi_flt = _mm256_cvtepi32_ps(
                        v_hi); /* No rounding                                                                   */
                v_hi_flt = _mm256_mul_ps(cnst65536f,
                                         v_hi_flt); /* No rounding                                                                   */
                return _mm256_add_ps(v_hi_flt,
        ...
        swizzle(batch<float, A> const &self, batch_constant<batch<uint32_t, A>, V0, V1, V2, V3, V4, V5, V6, V7> mask,
                requires_arch<avx2>) noexcept {
            return _mm256_permutevar8x32_ps(self, (batch<uint32_t, A>)
            mask);
        }
        ...
        swizzle(batch<uint32_t, A> const &self, batch_constant<batch<uint32_t, A>, V0, V1, V2, V3, V4, V5, V6, V7> mask,
                requires_arch<avx2>) noexcept {
            return _mm256_permutevar8x32_epi32(self, (batch<uint32_t, A>)
            mask);
        }
