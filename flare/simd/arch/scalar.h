...
//
#ifndef FLARE_SIMD_ARCH_SCALAR_H_
#define FLARE_SIMD_ARCH_SCALAR_H_
...
namespace flare::simd {
    ...
    namespace detail {
#define FLARE_SIMD_HASSINCOS_TRAIT(func)                                                                                              \
    template <class S>                                                                                                           \
    struct has##func                                                                                                             \
    {                                                                                                                            \
        template <class T>                                                                                                       \
...
#define FLARE_SIMD_HASSINCOS(func, T) has##func<T>::value

        FLARE_SIMD_HASSINCOS_TRAIT(sincos);

        FLARE_SIMD_HASSINCOS_TRAIT(sincosf);

        FLARE_SIMD_HASSINCOS_TRAIT(__sincos);

        FLARE_SIMD_HASSINCOS_TRAIT(__sincosf);

        struct generic_sincosf {
            template<class T>
            typename std::enable_if<FLARE_SIMD_HASSINCOS(sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                sincosf(val, &s, &c);
            }

            template<class T>
            typename std::enable_if<!FLARE_SIMD_HASSINCOS(sincosf, T) && FLARE_SIMD_HASSINCOS(__sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                __sincosf(val, &s, &c);
            }
        ...
        struct generic_sincos {
            template<class T>
            typename std::enable_if<FLARE_SIMD_HASSINCOS(sincos, T), void>::type
            operator()(double val, T &s, T &c) {
                sincos(val, &s, &c);
            }

            template<class T>
            typename std::enable_if<!FLARE_SIMD_HASSINCOS(sincos, T) && FLARE_SIMD_HASSINCOS(__sincos, T), void>::type
            operator()(double val, T &s, T &c) {
                __sincos(val, &s, &c);
            }
...
#undef FLARE_SIMD_HASSINCOS_TRAIT
#undef FLARE_SIMD_HASSINCOS
    }
...
#endif  // FLARE_SIMD_ARCH_SCALAR_H_
