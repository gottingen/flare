...
//
#ifndef FLARE_SIMD_ARCH_AVX512BW_H_
#define FLARE_SIMD_ARCH_AVX512BW_H_

#include <array>
#include <type_traits>

#include <flare/simd/types/avx512bw_register.h>
...
namespace flare::simd {
    ...
    namespace kernel {
        ...
        {
#if defined(FLARE_SIMD_AVX512_SHIFT_INTRINSICS_IMM_ONLY)
        ...
        {
            template <size_t... Is>
            constexpr std::array<uint64_t, sizeof...(Is)> make_slide_perm_hi(::flare::simd::detail::index_sequence<Is...>)
            {
                return { (Is == 0 ? 8 : Is - 1)... };
            }

            template <size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)> make_slide_left_pattern(::flare::simd::detail::index_sequence<Is...>)
            {
                return { (Is >= N ? Is - N : 0)... };
            }
            template <size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)> make_slide_left_mask(::flare::simd::detail::index_sequence<Is...>)
            {
        ...
        inline batch<T, A> slide_left(batch<T, A> const& x, requires_arch<avx512bw>) noexcept
        {
            ...
            if (N & 1)
            {
                ...
                alignas(A::alignment()) auto slide_perm = detail::make_slide_perm_hi(::flare::simd::detail::make_index_sequence<512 / 64>());
            ...
            alignas(A::alignment()) auto slide_pattern = detail::make_slide_left_pattern<N / 2>(::flare::simd::detail::make_index_sequence<512 / 16>());
            alignas(A::alignment()) auto slide_mask = detail::make_slide_left_mask<N / 2>(::flare::simd::detail::make_index_sequence<512 / 16>());
        ...
        {
            template <size_t... Is>
            constexpr std::array<uint64_t, sizeof...(Is)> make_slide_perm_low(::flare::simd::detail::index_sequence<Is...>)
            {
                return { (Is + 1)... };
            }

            template <size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)> make_slide_right_pattern(::flare::simd::detail::index_sequence<Is...>)
            {
                return { (Is < (32 - N) ? Is + N : 0)... };
            }
            template <size_t N, size_t... Is>
            constexpr std::array<uint16_t, sizeof...(Is)> make_slide_right_mask(::flare::simd::detail::index_sequence<Is...>)
            {
        ...
        inline batch<T, A> slide_right(batch<T, A> const& x, requires_arch<avx512bw>) noexcept
        {
            ...
            if (N & 1)
            {
                alignas(A::alignment()) auto slide_perm = detail::make_slide_perm_low(::flare::simd::detail::make_index_sequence<512 / 64>());
            ...
            alignas(A::alignment()) auto slide_pattern = detail::make_slide_right_pattern<N / 2>(::flare::simd::detail::make_index_sequence<512 / 16>());
            alignas(A::alignment()) auto slide_mask = detail::make_slide_right_mask<N / 2>(::flare::simd::detail::make_index_sequence<512 / 16>());
            return _mm512_and_si512(_mm512_permutexvar_epi16(_mm512_load_epi32(slide_pattern.data()), xx), _mm512_load_epi32(slide_mask.data()));
        }
...
#endif  // FLARE_SIMD_ARCH_AVX512BW_H_
