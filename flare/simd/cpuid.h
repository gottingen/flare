...
//
#ifndef FLARE_SIMD_CPUID_H_
#define FLARE_SIMD_CPUID_H_
...
#include <flare/simd/types/all_registers.h>
...
#error "Unsupported configuration"
                ...
                if (sse4_2)
                    best = std::max(best, fma3<flare::simd::sse4_2>::version() * fma3_sse);

                avx = regs1[2] >> 28 & 1;
                best = std::max(best, avx::version() * avx);

                fma3_avx = avx && fma3_sse;
                best = std::max(best, fma3<flare::simd::avx>::version() * fma3_avx);
                ...
                best = std::max(best, fma3<flare::simd::avx2>::version() * fma3_avx2);
