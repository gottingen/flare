...
@{
   ...
   @{
     ...
     @defgroup opencl_mat OpenCL specific functions
        ...
        \brief Accessing Flare's context, queue, and share data with other OpenCL code.

        If your software is using Flare's OpenCL backend, you can also write custom
        kernels and do custom memory operations using native OpenCL commands. The functions
        contained in the \p flycl namespace provide methods to get the context, queue, and
        device(s) that Flare is using as well as convert `cl_mem` handles to
        \ref fly::array objects.

        Please note: the \ref fly::array constructors are not thread safe. You may create and
        upload data to `cl_mem` objects from separate threads, but the thread which
        instantiated Flare must do the `cl_mem` to \ref fly::array conversion.
