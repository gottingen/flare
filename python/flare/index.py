...
class Seq(ct.Structure):
    ...
    def __init__ (self, S):
        ...
        if _is_number(S):
            ...
            raise IndexError("Invalid type while indexing flare.array")
...
class Index(ct.Structure):
    ...
    def __init__ (self, idx):
        ...
        if isinstance(idx, BaseArray):
            ...
            if (idx.type() == Dtype.b8.value):
                safe_call(backend.get().fly_where(c_pointer(arr), idx.arr))
            else:
                safe_call(backend.get().fly_retain_array(c_pointer(arr), idx.arr))
    ...
    def __del__(self):
        if not self.isSeq:
            ...
            backend.get().fly_release_array(arr)
