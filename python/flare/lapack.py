...
def lu(A):
    ...
    U = Array()
    P = Array()
    safe_call(backend.get().fly_lu(c_pointer(L.arr), c_pointer(U.arr), c_pointer(P.arr), A.arr))
    return L,U,P
...
def lu_inplace(A, pivot="lapack"):
    ...
    safe_call(backend.get().fly_lu_inplace(c_pointer(P.arr), A.arr, is_pivot_lapack))
    return P
...
def qr(A):
    ...
    R = Array()
    T = Array()
    safe_call(backend.get().fly_qr(c_pointer(Q.arr), c_pointer(R.arr), c_pointer(T.arr), A.arr))
    return Q,R,T
...
def qr_inplace(A):
    ...
    """
    T = Array()
    safe_call(backend.get().fly_qr_inplace(c_pointer(T.arr), A.arr))
    return T
...
def cholesky(A, is_upper=True):
    ...
    R = Array()
    info = c_int_t(0)
    safe_call(backend.get().fly_cholesky(c_pointer(R.arr), c_pointer(info), A.arr, is_upper))
    return R, info.value
...
def cholesky_inplace(A, is_upper=True):
    ...
    """
    info = c_int_t(0)
    safe_call(backend.get().fly_cholesky_inplace(c_pointer(info), A.arr, is_upper))
    return info.value
...
def solve(A, B, options=MATPROP.NONE):
    ...
    """
    X = Array()
    safe_call(backend.get().fly_solve(c_pointer(X.arr), A.arr, B.arr, options.value))
    return X
...
def solve_lu(A, P, B, options=MATPROP.NONE):
    ...
    """
    X = Array()
    safe_call(backend.get().fly_solve_lu(c_pointer(X.arr), A.arr, P.arr, B.arr, options.value))
    return X
...
def inverse(A, options=MATPROP.NONE):
    ...
    """
    AI = Array()
    safe_call(backend.get().fly_inverse(c_pointer(AI.arr), A.arr, options.value))
    return AI
...
def pinverse(A, tol=1E-6, options=MATPROP.NONE):
    ...
    """
    AI = Array()
    safe_call(backend.get().fly_pinverse(c_pointer(AI.arr), A.arr, c_double_t(tol), options.value))
    return AI
...
def rank(A, tol=1E-5):
    ...
    """
    r = c_uint_t(0)
    safe_call(backend.get().fly_rank(c_pointer(r), A.arr, c_double_t(tol)))
    return r.value
...
def det(A):
    ...
    re = c_double_t(0)
    im = c_double_t(0)
    safe_call(backend.get().fly_det(c_pointer(re), c_pointer(im), A.arr))
    re = re.value
    im = im.value
...
def norm(A, norm_type=NORM.EUCLID, p=1.0, q=1.0):
    ...
    """
    res = c_double_t(0)
    safe_call(backend.get().fly_norm(c_pointer(res), A.arr, norm_type.value,
...
def svd(A):
    ...
    S = Array()
    Vt = Array()
    safe_call(backend.get().fly_svd(c_pointer(U.arr), c_pointer(S.arr), c_pointer(Vt.arr), A.arr))
    return U, S, Vt
...
def svd_inplace(A):
    ...
    S = Array()
    Vt = Array()
    safe_call(backend.get().fly_svd_inplace(c_pointer(U.arr), c_pointer(S.arr), c_pointer(Vt.arr),
                                           A.arr))
    return U, S, Vt
...
def is_lapack_available():
    ...
    """
    res = c_bool_t(False)
    safe_call(backend.get().fly_is_lapack_available(c_pointer(res)))
    return res.value
