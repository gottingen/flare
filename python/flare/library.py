...
class fly_cfloat_t(ct.Structure):
    _fields_ = [("real", ct.c_float), ("imag", ct.c_float)]


class fly_cdouble_t(ct.Structure):
...
def _setup():
    ...
    try:
        FLY_PATH = os.environ['FLY_PATH']
    except KeyError:
        FLY_PATH = None

    FLY_SEARCH_PATH = FLY_PATH
    ...
    if platform_name == 'Windows' or platform_name[:3] == 'CYG':
        ...
        if FLY_SEARCH_PATH is None:
            FLY_SEARCH_PATH = "C:/Program Files/flare/v" + FLARE_VER_MAJOR + "/"
        ...
        if FLY_SEARCH_PATH is None:
            if os.path.exists('/opt/EA'):
                FLY_SEARCH_PATH = '/opt/EA/'
            else:
                FLY_SEARCH_PATH = '/usr/local/'
        ...
        if FLY_SEARCH_PATH is None:
            if os.path.exists('/opt/EA/inf/'):
                FLY_SEARCH_PATH = '/opt/EA/inf/'
            elif os.path.exists('/opt/EA/'):
                FLY_SEARCH_PATH = '/opt/EA/'
            else:
                FLY_SEARCH_PATH = '/usr/local/'
    ...
    return pre, post, FLY_PATH, FLY_SEARCH_PATH, CUDA_FOUND
...
class _clibrary(object):
    ...
    def __libname(self, name, head='fly', ver_major=FLARE_VER_MAJOR):
        post = self.__post.replace(_VER_MAJOR_PLACEHOLDER, ver_major)
        libname = self.__pre + head + name + post

        if self.FLY_PATH:
            if os.path.isdir(self.FLY_PATH + '/lib64'):
                path_search = self.FLY_PATH + '/lib64/'
            else:
                path_search = self.FLY_PATH + '/lib/'
        else:
            if os.path.isdir(self.FLY_SEARCH_PATH + '/lib64'):
                path_search = self.FLY_SEARCH_PATH + '/lib64/'
            else:
                path_search = self.FLY_SEARCH_PATH + '/lib/'
        ...
        path_local = self.FLY_PYMODULE_PATH
        libpaths = [('', libname),
                    (path_site, libname),
                    (path_local, libname)]
        if self.FLY_PATH:  #prefer specified FLY_PATH if exists
    ...
    def __init__(self):
        ...
        more_info_str = "Please look at https://github.com/gottingen/flare/wiki for more information."

        pre, post, FLY_PATH, FLY_SEARCH_PATH, CUDA_FOUND = _setup()

        self.__pre = pre
        self.__post = post
        self.FLY_PATH = FLY_PATH
        self.FLY_SEARCH_PATH = FLY_SEARCH_PATH
        self.CUDA_FOUND = CUDA_FOUND

        # prefer locally packaged flare libraries if they exist
        fly_module = __import__(__name__)
        self.FLY_PYMODULE_PATH = fly_module.__path__[0] + '/' if fly_module.__path__ else None
        ...
        libnames = reversed(self.__libname('theia', head='', ver_major=THEIA_VER_MAJOR))

        try:
            VERBOSE_LOADS = os.environ['FLY_VERBOSE_LOADS'] == '1'
        except KeyError:
            VERBOSE_LOADS = False
        ...
        for name in ('cpu', 'cuda', ''):
            ...
            for libname in libnames:
                try:
                    ...
                    err = clib.fly_randu(c_pointer(out), 4, c_pointer(dims), Dtype.f32.value)
                    if (err == ERR.NONE.value):
                        self.__name = __name
                        clib.fly_release_array(out)
                        if VERBOSE_LOADS:
                            print('Loaded ' + full_libname)
...
def set_backend(name, unsafe=False):
    ...
    if (backend.is_unified()):
        safe_call(backend.get().fly_set_backend(backend.get_id(name)))
    else:
...
def get_backend_id(A):
    ...
    safe_call(backend.get().fly_get_backend_id(c_pointer(backend_id), A.arr))
...
def get_backend_count():
    ...
    """
    count = c_int_t(0)
    safe_call(backend.get().fly_get_backend_count(c_pointer(count)))
    return count.value
...
def get_available_backends():
    ...
    """
    available = c_int_t(0)
    safe_call(backend.get().fly_get_available_backends(c_pointer(available)))
...
def get_active_backend():
    ...
    """
    backend_id = c_int_t(BACKEND.CPU.value)
    safe_call(backend.get().fly_get_active_backend(c_pointer(backend_id)))
    return backend.get_name(backend_id.value)
...
def get_device_id(A):
    ...
    """
    device_id = c_int_t(0)
    safe_call(backend.get().fly_get_device_id(c_pointer(device_id), A.arr))
    return device_id


def get_size_of(dtype):
    """
    Get the size of the type represented by flare.Dtype
    """
    size = c_size_t(0)
    safe_call(backend.get().fly_get_size_of(c_pointer(size), dtype.value))
    return size.value
