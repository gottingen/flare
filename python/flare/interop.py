...
try:
    ...
    def pyopencl_to_fly_array(pycl_arr, copy=True):
        """
        Convert pyopencl.gpuarray to flare.Array
        ...
        if (pycl_arr.flags.f_contiguous):
            ...
            return _cc_to_fly_array(in_ptr, pycl_arr.ndim, in_shape, in_dtype, True, copy)
        else:
            return pyopencl_to_fly_array(pycl_arr.copy())
...
def to_array(in_array, copy = True):
    ...
    if AF_PYOPENCL_FOUND and isinstance(in_array, OpenclArray):
        return pyopencl_to_fly_array(in_array, copy)
