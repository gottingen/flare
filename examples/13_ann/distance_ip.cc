...
#include <cstdio>
#include <flare/ann/distance_ip.h>
...
int main(int argc, char* argv[]) {
    flare::initialize(argc, argv);
    {
        ...
        double dis = flare::ann::distance_ip<tensor_type>(a,b, false);
        printf("Result flare::ann::distance_l1<tensor_type>(a,b, false): %f\n", dis);

        double dis1 = flare::ann::distance_ip<tensor_type>(a,b);
        ...
        auto stats_nor = bencher([&]()
                                 { for(auto i =0; i <  100000; i++) flare::ann::distance_ip<tensor_type>(a,b, false); });
        std::cout << '\n'
                  << "nor " << stats_nor << '\n';
        auto stats_batch = bencher([&]()
                                 { for(auto i =0; i <  100000; i++) flare::ann::distance_ip<tensor_type>(a,b); });
