...
int main(int argc, char* argv[]) {
    flare::initialize(argc, argv);
    {
        ...
        double dis = flare::ann::norm_l1<tensor_type>(a);
        printf("Result flare::ann::distance_l1<tensor_type>(a,b, false): %f\n", dis);

        double dis1 = flare::ann::norm_l1<tensor_type>(a, false);
        printf("Result flare::ann::distance_l1<tensor_type>(a,b, false): %f\n", dis1);

        auto bencher = flare::Benchmarker<>{ 64, std::chrono::seconds { 10 } };
        auto stats_nor = bencher([&]()
                                 { for(auto i =0; i <  100000; i++) flare::ann::norm_l1<tensor_type>(a, false); });
        std::cout << '\n'
                  << "nor " << stats_nor << '\n';
        auto stats_batch = bencher([&]()
                                 { for(auto i =0; i <  100000; i++) flare::ann::norm_l1<tensor_type>(a); });
