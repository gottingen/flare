...
// These two Tensor types are both 2-D arrays of double.  However, they
...
using left_type = flare::Tensor<double **, flare::LayoutLeft>;
using right_type = flare::Tensor<double **, flare::LayoutRight>;
// This is a one-dimensional Tensor, so the layout matters less.
...
// parallel_for functor that fills the given Tensor with some data.  It
// expects to access the Tensor by rows in parallel: each call i of
// operator() accesses a row.
template<class TensorType>
struct init_tensor {
    TensorType a;

    init_tensor(TensorType a_) : a(a_) {}

    using size_type = typename TensorType::size_type;

    FLARE_INLINE_FUNCTION
    void operator()(const typename TensorType::size_type i) const {
...
//
// Since the functor is templated on the TensorTypes itself it doesn't matter what
// there layouts are. That means you can use different layouts on different
// architectures.
template<class TensorType1, class TensorType2>
struct contraction {
    tensor_type a;
    typename TensorType1::const_type v1;
    typename TensorType2::const_type v2;

    contraction(tensor_type a_, TensorType1 v1_, TensorType2 v2_)
            : a(a_), v1(v1_), v2(v2_) {}
    ...
    FLARE_INLINE_FUNCTION
    void operator()(const tensor_type::size_type i) const {
        for (size_type j = 0; j < static_cast<size_type>(a.extent(1)); ++j) {
            a(i) = v1(i, j) * v2(j, i);
        }
    }
...
struct dot {
    ...
    dot(tensor_type a_) : a(a_) {}

    using value_type = double;  // Specify type for reduction target, lsum
    FLARE_INLINE_FUNCTION
    void operator()(const tensor_type::size_type i, double &lsum) const {
        lsum += a(i) * a(i);
    }
};
...
int main(int narg, char *arg[]) {
    ...
    {
        int size = 10000;
        tensor_type a("A", size);

        // Define two tensors with LayoutLeft and LayoutRight.
        left_type l("L", size, 10000);
        right_type r("R", size, 10000);

        // Initialize the data in the tensors.
        flare::parallel_for(size, init_tensor<left_type>(l));
        flare::parallel_for(size, init_tensor<right_type>(r));
        flare::fence();

        // Measure time to execute the contraction kernel when giving it a
        // LayoutLeft tensor for v1 and a LayoutRight tensor for v2. This should be
        ...
        // Measure time to execute the contraction kernel when giving it a
        // LayoutRight tensor for v1 and a LayoutLeft tensor for v2. This should be
