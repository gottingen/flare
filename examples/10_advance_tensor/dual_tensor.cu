// Copyright 2023 The Elastic-AI Authors.
// part of Elastic AI Search
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
//

#include <flare/core.h>
#include <flare/dual_tensor.h>
#include <flare/timer.h>
#include <cstdio>
#include <cstdlib>

// DualTensor helps you manage data and computations that take place on
// two different memory spaces.  Examples include CUDA device memory
// and (CPU) host memory (currently implemented), or Intel Knights
// Landing MCDRAM and DRAM (not yet implemented).  For example, if you
// have ported only some parts of you application to run in CUDA,
// DualTensor can help manage moving data between the parts of your
// application that work best with CUDA, and the parts that work
// better on the CPU.
//
// A DualTensor takes the same template parameters as a Tensor, but
// contains two Tensors: One that lives in the DualTensor's memory space,
// and one that lives in that memory space's host mirror space.  If
// both memory spaces are the same, then the two Tensors just alias one
// another.  This means that you can use DualTensor all the time, even
// when not running in a memory space like CUDA.  DualTensor's
// operations to help you manage memory take almost no time in that
// case.  This makes your code even more performance portable.

using tensor_type = flare::DualTensor<double*>;
using idx_type  = flare::DualTensor<int**>;

template <class ExecutionSpace>
struct localsum {
  // If the functor has a public 'execution_space' alias, that defines
  // the functor's execution space (where it runs in parallel).  This
  // overrides flare' default execution space.
  using execution_space = ExecutionSpace;

  using memory_space = typename flare::detail::if_c<
      std::is_same<ExecutionSpace, flare::DefaultExecutionSpace>::value,
      idx_type::memory_space, idx_type::host_mirror_space>::type;

  // Get the tensor types on the particular device for which the functor
  // is instantiated.
  //
  // "const_data_type" is an alias in Tensor (and DualTensor) which is
  // the const version of the first template parameter of the Tensor.
  // For example, the const_data_type version of double** is const
  // double**.
  flare::Tensor<idx_type::const_data_type, idx_type::array_layout, memory_space>
      idx;
  // "scalar_array_type" is an alias in TensorTraits (and DualTensor) which is the
  // array version of the value(s) stored in the Tensor.
  flare::Tensor<tensor_type::scalar_array_type, tensor_type::array_layout,
               memory_space>
      dest;
  flare::Tensor<tensor_type::const_data_type, tensor_type::array_layout,
               memory_space, flare::MemoryRandomAccess>
      src;

  // Constructor takes DualTensors, synchronizes them to the device,
  // then marks them as modified on the device.
  localsum(idx_type dv_idx, tensor_type dv_dest, tensor_type dv_src) {
    // Extract the tensor on the correct Device (i.e., the correct
    // memory space) from the DualTensor.  DualTensor has a template
    // method, tensor(), which is templated on the memory space.  If the
    // DualTensor has a Tensor from that memory space, tensor() returns the
    // Tensor in that space.
    idx  = dv_idx.tensor<memory_space>();
    dest = dv_dest.template tensor<memory_space>();
    src  = dv_src.template tensor<memory_space>();

    // Synchronize the DualTensor to the correct Device.
    //
    // DualTensor's sync() method is templated on a memory space, and
    // synchronizes the DualTensor in a one-way fashion to that memory
    // space.  "Synchronizing" means copying, from the other memory
    // space to the Device memory space.  sync() does _nothing_ if the
    // Tensors on the two memory spaces are in sync.  DualTensor
    // determines this by the user manually marking one side or the
    // other as modified; see the modify() call below.

    dv_idx.sync<memory_space>();
    dv_dest.template sync<memory_space>();
    dv_src.template sync<memory_space>();

    // Mark dest as modified on Device.
    dv_dest.template modify<memory_space>();
  }

  FLARE_INLINE_FUNCTION
  void operator()(const int i) const {
    double tmp = 0.0;
    for (int j = 0; j < (int)idx.extent(1); ++j) {
      const double val = src(idx(i, j));
      tmp += val * val + 0.5 * (idx.extent(0) * val - idx.extent(1) * val);
    }
    dest(i) += tmp;
  }
};

class ParticleType {
 public:
  double q;
  double m;
  double q_over_m;
  FLARE_INLINE_FUNCTION
  ParticleType(double q_ = -1, double m_ = 1) : q(q_), m(m_), q_over_m(q / m) {}

 protected:
};

using ParticleTypes = flare::DualTensor<ParticleType[10]>;
int main(int narg, char* arg[]) {
  flare::initialize(narg, arg);

  // If Tensor is non-trivial constructible type then add braces so it is out of
  // scope before flare::finalize() call
  {
    ParticleTypes test("Test");
    flare::fence();
    test.h_tensor(0) = ParticleType(-1e4, 1);
    flare::fence();

    int size = 1000000;

    // Create DualTensors. This will allocate on both the device and its
    // host_mirror_device.
    idx_type idx("Idx", size, 64);
    tensor_type dest("Dest", size);
    tensor_type src("Src", size);

    srand(134231);

    // Get a reference to the host tensor of idx directly (equivalent to
    // idx.tensor<idx_type::host_mirror_space>() )
    idx_type::t_host h_idx = idx.h_tensor;
    using size_type        = tensor_type::size_type;
    for (int i = 0; i < size; ++i) {
      for (size_type j = 0; j < static_cast<size_type>(h_idx.extent(1)); ++j) {
        h_idx(i, j) = (size + i + (rand() % 500 - 250)) % size;
      }
    }

    // Mark idx as modified on the host_mirror_space so that a
    // sync to the device will actually move data.  The sync happens in
    // the functor's constructor.
    idx.modify<idx_type::host_mirror_space>();

    // Run on the device.  This will cause a sync of idx to the device,
    // since it was marked as modified on the host.
    flare::Timer timer;
    flare::parallel_for(size,
                         localsum<tensor_type::execution_space>(idx, dest, src));
    flare::fence();
    double sec1_dev = timer.seconds();

    timer.reset();
    flare::parallel_for(size,
                         localsum<tensor_type::execution_space>(idx, dest, src));
    flare::fence();
    double sec2_dev = timer.seconds();

    // Run on the host's default execution space (could be the same as device).
    // This will cause a sync back to the host of dest.  Note that if the Device
    // is CUDA, the data layout will not be optimal on host, so performance is
    // lower than what it would be for a pure host compilation.
    timer.reset();
    flare::parallel_for(
        size, localsum<flare::HostSpace::execution_space>(idx, dest, src));
    flare::fence();
    double sec1_host = timer.seconds();

    timer.reset();
    flare::parallel_for(
        size, localsum<flare::HostSpace::execution_space>(idx, dest, src));
    flare::fence();
    double sec2_host = timer.seconds();

    printf("Device Time with Sync: %f without Sync: %f \n", sec1_dev, sec2_dev);
    printf("Host   Time with Sync: %f without Sync: %f \n", sec1_host,
           sec2_host);
  }

  flare::finalize();
}
