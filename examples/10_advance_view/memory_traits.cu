...
// flare::Views have an MemoryTraits template parameter which
...
// access. The trait flare::Atomic is one of the latter. A view with
...
// flare::RandomAccess means that we expect to use this view
...
using view_type_rnd =
    flare::View<const double*, flare::MemoryTraits<flare::RandomAccess> >;
using idx_type      = flare::View<int**>;
...
int main(int narg, char* arg[]) {
...

  {
    ...
    idx_type_host h_idx = flare::create_mirror_view(idx);
    ...
    flare::Timer time2;
    flare::parallel_for(size, localsum<view_type, view_type>(idx, dest, src));
    flare::fence();
