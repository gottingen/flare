...
void prewitt(array &mag, array &dir, const array &in) {
    ...
    // Find the gradients
    array Gy = fly::convolve(h2d, h1d, in) / 6;
    array Gx = fly::convolve(h1d, h2d, in) / 6;
...
array emboss(const array &input, float azimuth, float elevation, float depth) {
    ...
    array in = input;
    if (in.dims(2) > 1)
        in = colorSpace(input, FLY_GRAY, FLY_RGB);
    else
        in = input;

    // convert angles to radians
    float phi   = elevation * fly::Pi / 180.0f;
    float theta = azimuth * fly::Pi / 180.0f;
...
int main(int argc, char **argv) {
    try {
        ...
        array img1ch = colorSpace(img, FLY_GRAY, FLY_RGB);
        ...
        fly::Window wnd("Image Filters Demo");
