...
struct meanvar_test {
    ...
    meanvar_test(string description, fly_array in, fly_array weights,
                 fly_var_bias bias, int dim,
        ...
        , bias_(bias)
        , dim_(dim) {
        fly_retain_array(&in_, in);
        if (weights) { fly_retain_array(&weights_, weights); }
    ...
    meanvar_test(const meanvar_test<T> &other)
        ...
        , mean_(other.mean_)
        , variance_(other.variance_) {
        if (other.in_) fly_retain_array(&in_, other.in_);
        if (other.weights_) { fly_retain_array(&weights_, other.weights_); }
    }

    ~meanvar_test() {
#ifndef _WIN32
        if (in_) fly_release_array(in_);
        if (weights_) {
            fly_release_array(weights_);
            weights_ = 0;
        }
#endif
    }
...
template<typename T>
fly_dtype meanvar_test<T>::fly_type = dtype_traits<T>::fly_type;
...
class MeanVarTyped : public ::testing::TestWithParam<meanvar_test<T>> {
   public:
    void meanvar_test_function(const meanvar_test<T> &test) {
        ...
        fly_array in = 0;
        ASSERT_SUCCESS(
            fly_cast(&in, test.in_, (fly_dtype)dtype_traits<T>::fly_type));

        EXPECT_EQ(FLY_SUCCESS, fly_meanvar(&mean, &var, in, test.weights_,
        ...
        dim4 outDim(1);
        fly_get_dims(&outDim[0], &outDim[1], &outDim[2], &outDim[3], in);
        outDim[test.dim_] = 1;
        ...
        ASSERT_SUCCESS(fly_release_array(mean));
        ASSERT_SUCCESS(fly_release_array(var));
    }
    ...
    void meanvar_cpp_test_function(const meanvar_test<T> &test) {
        ...
        // Cast to the expected type
        fly_array in_tmp = 0;
        ASSERT_SUCCESS(fly_retain_array(&in_tmp, test.in_));
        array in(in_tmp);
        in = in.as((fly_dtype)dtype_traits<T>::fly_type);

        fly_array weights_tmp = test.weights_;
        if (weights_tmp) {
            ASSERT_SUCCESS(fly_retain_array(&weights_tmp, weights_tmp));
        }
...
meanvar_test<T> meanvar_test_gen(string name, int in_index, int weight_index,
                                 fly_var_bias bias, int dim, int mean_index,
                                 int var_index, test_size size) {
    if (noDoubleTests((fly_dtype)fly::dtype_traits<T>::fly_type) ||
        noDoubleTests((
            fly_dtype)fly::dtype_traits<typename varOutType<T>::type>::fly_type) ||
        noHalfTests((fly_dtype)fly::dtype_traits<T>::fly_type)) {
        meanvar_test<T> out(name);
        return out;
    }

    vector<fly_array> inputs;
    vector<vector<typename varOutType<T>::type>> outputs;
    if (size == MEANVAR_SMALL) {
        vector<fly::dim4> numDims_;
        vector<vector<T>> in_;
        ...
        for (size_t i = 0; i < in_.size(); i++) {
            fly_create_array(&inputs[i], &in_[i].front(), numDims_[i].ndims(),
                            numDims_[i].get(),
        ...
        for (size_t i = 0; i < dimensions.size(); i++) {
            fly_create_array(&inputs[i], &large_.front(), 4,
                            dimensions[i].data(),
                            (fly_dtype)fly::dtype_traits<T>::fly_type);
        }
    ...
    for (auto input : inputs) { fly_release_array(input); }
    return out;
}
...
vector<meanvar_test<T>> small_test_values() {
    ...
    return {
        ...
        meanvar_test_gen<T>(   "Sample1Ddim0",           0,            -1,     FLY_VARIANCE_SAMPLE,     0,           0,          1, MEANVAR_SMALL),
        meanvar_test_gen<T>(   "Sample1Ddim1",           1,            -1,     FLY_VARIANCE_SAMPLE,     1,           0,          1, MEANVAR_SMALL),
        meanvar_test_gen<T>(   "Sample2Ddim0",           2,            -1,     FLY_VARIANCE_SAMPLE,     0,           3,          4, MEANVAR_SMALL),
        meanvar_test_gen<T>(   "Sample2Ddim1",           2,            -1,     FLY_VARIANCE_SAMPLE,     1,           6,          7, MEANVAR_SMALL),

        meanvar_test_gen<T>("Population1Ddim0",          0,            -1, FLY_VARIANCE_POPULATION,     0,           0,          2, MEANVAR_SMALL),
        meanvar_test_gen<T>("Population1Ddim1",          1,            -1, FLY_VARIANCE_POPULATION,     1,           0,          2, MEANVAR_SMALL),
        meanvar_test_gen<T>("Population2Ddim0",          2,            -1, FLY_VARIANCE_POPULATION,     0,           3,          5, MEANVAR_SMALL),
        meanvar_test_gen<T>("Population2Ddim1",          2,            -1, FLY_VARIANCE_POPULATION,     1,           6,          8, MEANVAR_SMALL)};
    // clang-format on
}
...
vector<meanvar_test<T>> large_test_values() {
    return {
        // clang-format off
        //                  |       Name |      in_index | weight_index |                  bias |  dim | mean_index | var_index |
        meanvar_test_gen<T>("Sample1Ddim0",             0,            -1,     FLY_VARIANCE_SAMPLE,     0,           0,          1, MEANVAR_LARGE),
        meanvar_test_gen<T>("Sample1Ddim1",             1,            -1,     FLY_VARIANCE_SAMPLE,     1,           0,          1, MEANVAR_LARGE),
        meanvar_test_gen<T>("Sample1Ddim2",             2,            -1,     FLY_VARIANCE_SAMPLE,     2,           0,          1, MEANVAR_LARGE),
        meanvar_test_gen<T>("Sample2Ddim0",             3,            -1,     FLY_VARIANCE_SAMPLE,     0,           2,          3, MEANVAR_LARGE),
        // TODO(umar) Add additional large tests
        // meanvar_test_gen<T>(    "Sample2Ddim1",           3,            -1, FLY_VARIANCE_SAMPLE,     1,           2,          3, MEANVAR_LARGE),
        // meanvar_test_gen<T>(    "Sample2Ddim1",           2,            -1, FLY_VARIANCE_SAMPLE,     1,           6,          7, MEANVAR_LARGE),
        // clang-format on
    };
}
...
MEANVAR_TEST(ComplexFloat, fly::fly_cfloat)
MEANVAR_TEST(ComplexDouble, fly::fly_cdouble)
