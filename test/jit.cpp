...
TEST(JIT, NonLinearLargeX) {
    ...
    ASSERT_SUCCESS(fly_randu(&r, ndims, rdims, f32));
    ASSERT_SUCCESS(fly_constant(&c, 1, ndims, cdims, f32));
    ASSERT_SUCCESS(fly_eval(c));
    ASSERT_SUCCESS(fly_sub(&s, r, c, true));
    ASSERT_SUCCESS(fly_eval(s));
    ...
    ASSERT_SUCCESS(fly_get_data_ptr(hr.data(), r));
    ASSERT_SUCCESS(fly_get_data_ptr(hc.data(), c));
    ASSERT_SUCCESS(fly_get_data_ptr(hs.data(), s));
    ...
    ASSERT_SUCCESS(fly_release_array(r));
    ASSERT_SUCCESS(fly_release_array(c));
    ASSERT_SUCCESS(fly_release_array(s));
}
...
TEST_P(JIT, Tile) {
    ...
    fly::deviceMemInfo(&alloc_bytes2, &alloc_buffers2, &lock_bytes2,
                      &lock_buffers2);
...
void testTwoLargeNonLinear(const fly_dtype dt) {
    int dimsize = 10;
...
TEST(JIT, getKernelCacheDirectory) {
  size_t length = 0;
  ASSERT_SUCCESS(fly_get_kernel_cache_directory(&length, NULL));

  std::string path;
  path.resize(length);
  ASSERT_SUCCESS(fly_get_kernel_cache_directory(&length, &path.at(0)));
}
...
TEST(JIT, setKernelCacheDirectory) {
  ...
  ASSERT_SUCCESS(fly_get_kernel_cache_directory(&length, &old_path.at(0)));

  // Set cache directory to the new path
  ASSERT_SUCCESS(fly_set_kernel_cache_directory(path.c_str(), false));

  // Get the new path for verification
  size_t new_length = path.size();
  std::string new_path;
  new_path.resize(new_length);
  ASSERT_SUCCESS(fly_get_kernel_cache_directory(&new_length, &new_path.at(0)));

  ASSERT_EQ(path, new_path);
  ASSERT_EQ(path.size(), new_path.size());

  // Reset to the old path
  ASSERT_SUCCESS(fly_set_kernel_cache_directory(old_path.c_str(), false));
}
