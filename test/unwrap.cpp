...
void unwrapTest(string pTestFile, const unsigned resultIdx, const dim_t wx,
    ...
    fly_array outArrayT = 0;
    fly_array outArray2 = 0;

    ASSERT_SUCCESS(fly_create_array(&inArray, &(in[0].front()), idims.ndims(),
                                   idims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));

    ASSERT_SUCCESS(fly_unwrap(&outArray, inArray, wx, wy, sx, sy, px, py, true));
    ASSERT_SUCCESS(
        fly_unwrap(&outArrayT, inArray, wx, wy, sx, sy, px, py, false));
    ASSERT_SUCCESS(fly_transpose(&outArray2, outArrayT, false));
    ...
    ASSERT_SUCCESS(fly_get_data_ptr((void*)&outData[0], outArray));
    ...
    ASSERT_SUCCESS(fly_get_data_ptr((void*)&outData[0], outArray2));
    for (size_t elIter = 0; elIter < nElems; ++elIter) {
        ASSERT_EQ(tests[resultIdx][elIter], outData[elIter])
            << "at: " << elIter << endl;
    }
    ...
    if (outArrayT != 0) fly_release_array(outArrayT);
    if (outArray2 != 0) fly_release_array(outArray2);
}
