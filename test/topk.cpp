...
void topkTest(const int ndims, const dim_t* dims, const unsigned k,
    ...
    fly_dtype dtype = (fly_dtype)dtype_traits<T>::fly_type;

    fly_array input, output, outindex;
    ...
    ASSERT_SUCCESS(fly_create_array(&input, inData.data(), ndims, dims, dtype));
    ASSERT_SUCCESS(fly_topk(&output, &outindex, input, k, dim, order));

    vector<T> hovals(oelems);
    vector<uint> hoidxs(oelems);

    ASSERT_SUCCESS(fly_get_data_ptr((void*)hovals.data(), output));
    ASSERT_SUCCESS(fly_get_data_ptr((void*)hoidxs.data(), outindex));
    ...
    ASSERT_SUCCESS(fly_release_array(input));
    ASSERT_SUCCESS(fly_release_array(output));
    ASSERT_SUCCESS(fly_release_array(outindex));
}

int type_max(fly_dtype type) {
    switch (type) {
...
TYPED_TEST(TopK, Max1D0) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t), 1, 1, 1};
    topkTest<TypeParam>(1, dims, 5, 0, FLY_TOPK_MAX);
}

TYPED_TEST(TopK, Max2D0) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 10, 10, 1, 1};
    topkTest<TypeParam>(2, dims, 3, 0, FLY_TOPK_MAX);
}

TYPED_TEST(TopK, Max3D0) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 100, 10, 10, 1};
    topkTest<TypeParam>(2, dims, 5, 0, FLY_TOPK_MAX);
}
...
TYPED_TEST(TopK, Min1D0) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t), 1, 1, 1};
    topkTest<TypeParam>(1, dims, 5, 0, FLY_TOPK_MIN);
}

TYPED_TEST(TopK, Min2D0) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 10, 10, 1, 1};
    topkTest<TypeParam>(2, dims, 3, 0, FLY_TOPK_MIN);
}

TYPED_TEST(TopK, Min3D0) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 100, 10, 10, 1};
    topkTest<TypeParam>(2, dims, 5, 0, FLY_TOPK_MIN);
}
...
TEST(TopK, ValidationCheck_DimN) {
    ...
    fly_array out, idx, in;
    ASSERT_SUCCESS(fly_randu(&in, 2, dims, f32));
    ASSERT_EQ(FLY_ERR_NOT_SUPPORTED,
              fly_topk(&out, &idx, in, 10, 1, FLY_TOPK_MAX));
...
TEST(TopK, ValidationCheck_DefaultDim) {
    dim_t dims[4] = {10, 10, 1, 1};
    fly_array out, idx, in;
    ASSERT_SUCCESS(fly_randu(&in, 4, dims, f32));
    ASSERT_SUCCESS(fly_topk(&out, &idx, in, 10, -1, FLY_TOPK_MAX));
    ASSERT_SUCCESS(fly_release_array(in));
...
TYPED_TEST(TopK, Max1D0_Stable) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t), 1, 1, 1};
    topkTest<TypeParam>(1, dims, 5, 0, FLY_TOPK_STABLE_MAX);
}

TYPED_TEST(TopK, Max2D0_Stable) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 10, 10, 1, 1};
    topkTest<TypeParam>(2, dims, 3, 0, FLY_TOPK_STABLE_MAX);
}

TYPED_TEST(TopK, Max3D0_Stable) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 100, 10, 10, 1};
    topkTest<TypeParam>(2, dims, 5, 0, FLY_TOPK_STABLE_MAX);
}
...
TYPED_TEST(TopK, Min1D0_Stable) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t), 1, 1, 1};
    topkTest<TypeParam>(1, dims, 5, 0, FLY_TOPK_STABLE_MIN);
}

TYPED_TEST(TopK, Min2D0_Stable) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 10, 10, 1, 1};
    topkTest<TypeParam>(2, dims, 3, 0, FLY_TOPK_STABLE_MIN);
}

TYPED_TEST(TopK, Min3D0_Stable) {
    fly_dtype t    = (fly_dtype)dtype_traits<TypeParam>::fly_type;
    dim_t dims[4] = {type_max(t) / 100, 10, 10, 1};
    topkTest<TypeParam>(2, dims, 5, 0, FLY_TOPK_STABLE_MIN);
}
...
TEST(TopK, ValidationCheck_DimN_Stable) {
    ...
    ASSERT_SUCCESS(fly_randu(&in, 2, dims, f32));
    ASSERT_EQ(FLY_ERR_NOT_SUPPORTED,
              fly_topk(&out, &idx, in, 10, 1, FLY_TOPK_STABLE_MAX));
    ASSERT_SUCCESS(fly_release_array(in));
}
...
TEST(TopK, ValidationCheck_DefaultDim_Stable) {
    ...
    fly_array out, idx, in;
    ASSERT_SUCCESS(fly_randu(&in, 4, dims, f32));
    ASSERT_SUCCESS(fly_topk(&out, &idx, in, 10, -1, FLY_TOPK_STABLE_MAX));
    ASSERT_SUCCESS(fly_release_array(in));
    ASSERT_SUCCESS(fly_release_array(out));
    ASSERT_SUCCESS(fly_release_array(idx));
}
...
ostream& operator<<(ostream& os, const topk_params& param) {
    os << "d0: " << param.d0 << " d1: " << param.d1 << " k:  " << param.k
       << " dim: " << param.dim
       << " order: " << ((param.order == FLY_TOPK_MAX) ? "MAX" : "MIN");
    return os;
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(topk_params{100, 10, 32, 0, FLY_TOPK_MIN},
                      topk_params{100, 10, 64, 0, FLY_TOPK_MIN},
                      topk_params{100, 10, 32, 0, FLY_TOPK_MAX},
                      topk_params{100, 10, 64, 0, FLY_TOPK_MAX},
                      topk_params{100, 10, 5, 0, FLY_TOPK_MIN},
                      topk_params{1000, 10, 5, 0, FLY_TOPK_MIN},
                      topk_params{10000, 10, 5, 0, FLY_TOPK_MIN},
                      topk_params{100, 10, 5, 0, FLY_TOPK_MAX},
                      topk_params{1000, 10, 5, 0, FLY_TOPK_MAX},
                      topk_params{10000, 10, 5, 0, FLY_TOPK_MAX},
                      topk_params{10, 10, 5, 0, FLY_TOPK_MIN},
                      topk_params{10, 100, 5, 0, FLY_TOPK_MIN},
                      topk_params{10, 1000, 5, 0, FLY_TOPK_MIN},
                      topk_params{10, 10000, 5, 0, FLY_TOPK_MIN},
                      topk_params{10, 10, 5, 0, FLY_TOPK_MAX},
                      topk_params{10, 100, 5, 0, FLY_TOPK_MAX},
                      topk_params{10, 1000, 5, 0, FLY_TOPK_MAX},
                      topk_params{10, 10000, 5, 0, FLY_TOPK_MAX},
                      topk_params{1000, 10, 256, 0, FLY_TOPK_MAX}),
    [](const ::testing::TestParamInfo<TopKParams::ParamType> info) {
        ...
        ss << "d0_" << info.param.d0 << "_d1_" << info.param.d1 << "_k_"
           ...
           << ((info.param.order == FLY_TOPK_MAX) ? string("MAX")
                                                 : string("MIN"));
        return ss.str();
    });
...
TEST_P(TopKParams, CPP) {
    ...
    if (order == FLY_TOPK_MIN) { in = -in + (d0 * d1 - 1); }
    array val, idx;
...
TEST(TopK, KGreaterThan256) {
    fly::array a = fly::randu(500);
    fly::array vals, idx;

    int k = 257;
    EXPECT_THROW(topk(vals, idx, a, k), fly::exception)
...
TEST(TopK, DeterministicTiesMin) {
    fly::array a           = fly::constant(1, 500);
    a(fly::seq(0, 499, 2)) = 7;
    fly::array vals_min, idx_min;

    int k = 6;
    topk(vals_min, idx_min, a, k, 0, FLY_TOPK_STABLE_MIN);

    fly::array expected_idx_min   = fly::seq(1, 499, 2);
    fly::array k_expected_idx_min = expected_idx_min(fly::seq(0, k - 1));
...
TEST(TopK, DeterministicTiesMax) {
    fly::array a           = fly::constant(1, 500);
    a(fly::seq(0, 499, 2)) = 7;
    fly::array vals_max, idx_max;

    int k = 6;
    topk(vals_max, idx_max, a, k, 0, FLY_TOPK_STABLE_MAX);

    fly::array expected_idx_max   = fly::seq(0, 499, 2);
    fly::array k_expected_idx_max = expected_idx_max(fly::seq(0, k - 1));
    ASSERT_ARRAYS_EQ(idx_max, k_expected_idx_max.as(u32));
}

TEST(TopK, DeterministicTiesBatchedMin) {
    const int nbatch = 10;
    fly::array a      = fly::constant(1, 500, nbatch, nbatch, nbatch);
    a(fly::seq(0, 499, 2), fly::span, fly::span, fly::span) = 7;
    fly::array vals_min, idx_min;
    ...
    fly::array expected_idx_min = fly::seq(1, 499, 2);
    fly::array k_expected_idx_min =
        fly::tile(expected_idx_min(fly::seq(0, k - 1)),
                 fly::dim4(1, nbatch, nbatch, nbatch));
    ASSERT_ARRAYS_EQ(idx_min, k_expected_idx_min.as(u32));
}
...
TEST(TopK, DeterministicTiesBatchedMax) {
    ...
    fly::array expected_idx_max = fly::seq(0, 499, 2);
    fly::array k_expected_idx_max =
        fly::tile(expected_idx_max(fly::seq(0, k - 1)),
                 fly::dim4(1, nbatch, nbatch, nbatch));
    ASSERT_ARRAYS_EQ(idx_max, k_expected_idx_max.as(u32));
}
