...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    outType output =
        (useDeprecatedAPI ? var<outType>(a, false)
                          : var<outType>(a, FLY_VARIANCE_POPULATION));
    ...
    output = (useDeprecatedAPI ? var<outType>(a, true)
                               : var<outType>(a, FLY_VARIANCE_SAMPLE));
    ...
    output = (useDeprecatedAPI ? var<outType>(b, false)
                               : var<outType>(b, FLY_VARIANCE_POPULATION));
    ...
    output = (useDeprecatedAPI ? var<outType>(b, true)
                               : var<outType>(b, FLY_VARIANCE_SAMPLE));
...
void dimCppSmallTest(const string pFileName,
    ...
    typedef typename varOutType<T>::type outType;
    float tol = 0.001f;
    if ((fly_dtype)fly::dtype_traits<T>::fly_type == f16) { tol = 0.6f; }
    ...
    for (size_t i = 0; i < in.size(); i++) {
        array input(numDims[i], &in[i].front(), flyHost);

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        array bout  = (useDeprecatedAPI ? var(input, true)
                                        : var(input, FLY_VARIANCE_SAMPLE));
        array nbout = (useDeprecatedAPI ? var(input, false)
                                        : var(input, FLY_VARIANCE_POPULATION));

        array bout1 = (useDeprecatedAPI ? var(input, true, 1)
                                        : var(input, FLY_VARIANCE_SAMPLE, 1));
        array nbout1 =
            (useDeprecatedAPI ? var(input, false, 1)
                              : var(input, FLY_VARIANCE_POPULATION, 1));
#pragma GCC diagnostic pop
...
TEST(Var, ISSUE2117) {
    ...
    myArray       = var(myArray, FLY_VARIANCE_SAMPLE, 1);
