...
void fftconvolveTest(string pTestFile, bool expand) {
    ...
    fly_dtype in_type  = (fly_dtype)dtype_traits<T>::fly_type;
    ...
    switch (baseDim) {
        case 1:
            ASSERT_SUCCESS(fly_fft_convolve1(&outArray, signal, filter, mode));
            break;
        case 2:
            ASSERT_SUCCESS(fly_fft_convolve2(&outArray, signal, filter, mode));
            break;
        case 3:
            ASSERT_SUCCESS(fly_fft_convolve3(&outArray, signal, filter, mode));
            break;
    }
    ...
    dim_t out_elems = 0;
    ASSERT_SUCCESS(fly_get_elements(&out_elems, outArray));
...
void fftconvolveTestLarge(int sDim, int fDim, int sBatch, int fBatch,
    ...
    array signal = randu(signalDims, (fly_dtype)dtype_traits<T>::fly_type);
    array filter = randu(filterDims, (fly_dtype)dtype_traits<T>::fly_type);

    array out =
        fftConvolve(signal, filter, expand ? FLY_CONV_EXPAND : FLY_CONV_DEFAULT);
...
TEST(FFTConvolve1, CPP) {
    ...
    array output = fftConvolve1(signal, filter, FLY_CONV_EXPAND);
...
TEST(FFTConvolve2, CPP) {
    ...
    array output = fftConvolve2(signal, filter, FLY_CONV_EXPAND);
...
TEST(FFTConvolve3, CPP) {
    ...
    array output = fftConvolve3(signal, filter, FLY_CONV_EXPAND);
...
TEST(Padding, fftConvolve2) {
    for (int n = 5; n < 32; n++) {
        ...
        array d = convolve2(a, b, FLY_CONV_DEFAULT, FLY_CONV_SPATIAL);
