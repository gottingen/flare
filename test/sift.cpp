...
void siftTest(string pTestFile, unsigned nLayers, float contrastThr,
    ...
    for (size_t testId = 0; testId < testCount; ++testId) {
        ...
        ASSERT_SUCCESS(
            fly_load_image(&inArray_f32, inFiles[testId].c_str(), false));
        ASSERT_SUCCESS(conv_image<T>(&inArray, inArray_f32));

        ASSERT_SUCCESS(fly_sift(&feat, &desc, inArray, nLayers, contrastThr,
                               edgeThr, initSigma, doubleInput, 1.f / 256.f,
                               0.05f));

        dim_t n = 0;
        fly_array x, y, score, orientation, size;

        ASSERT_SUCCESS(fly_get_features_num(&n, feat));
        ASSERT_SUCCESS(fly_get_features_xpos(&x, feat));
        ASSERT_SUCCESS(fly_get_features_ypos(&y, feat));
        ASSERT_SUCCESS(fly_get_features_score(&score, feat));
        ASSERT_SUCCESS(fly_get_features_orientation(&orientation, feat));
        ASSERT_SUCCESS(fly_get_features_size(&size, feat));
        ...
        dim_t descSize;
        dim_t descDims[4];
        ASSERT_SUCCESS(fly_get_elements(&descSize, desc));
        ASSERT_SUCCESS(fly_get_dims(&descDims[0], &descDims[1], &descDims[2],
                                   &descDims[3], desc));
        float* outDesc = new float[descSize];
        ASSERT_SUCCESS(fly_get_data_ptr((void*)outX, x));
        ASSERT_SUCCESS(fly_get_data_ptr((void*)outY, y));
        ASSERT_SUCCESS(fly_get_data_ptr((void*)outScore, score));
        ASSERT_SUCCESS(fly_get_data_ptr((void*)outOrientation, orientation));
        ASSERT_SUCCESS(fly_get_data_ptr((void*)outSize, size));
        ASSERT_SUCCESS(fly_get_data_ptr((void*)outDesc, desc));
        ...
        ASSERT_SUCCESS(fly_release_array(desc));
        ASSERT_SUCCESS(fly_release_features(feat));
