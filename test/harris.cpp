...
void harrisTest(string pTestFile, float sigma, unsigned block_size) {
    ...
    for (size_t testId = 0; testId < testCount; ++testId) {
        ...
        ASSERT_SUCCESS(
            fly_harris(&out, inArray, 500, 1e5f, sigma, block_size, 0.04f));

        dim_t n = 0;
        fly_array x, y, score, orientation, size;

        ASSERT_SUCCESS(fly_get_features_num(&n, out));
        ASSERT_SUCCESS(fly_get_features_xpos(&x, out));
        ASSERT_SUCCESS(fly_get_features_ypos(&y, out));
        ASSERT_SUCCESS(fly_get_features_score(&score, out));
        ASSERT_SUCCESS(fly_get_features_orientation(&orientation, out));
        ASSERT_SUCCESS(fly_get_features_size(&size, out));

        ASSERT_SUCCESS(fly_get_elements(&nElems, x));
        ...
        ASSERT_SUCCESS(fly_get_data_ptr((void *)&outX.front(), x));
        ASSERT_SUCCESS(fly_get_data_ptr((void *)&outY.front(), y));
        ASSERT_SUCCESS(fly_get_data_ptr((void *)&outScore.front(), score));
        ASSERT_SUCCESS(
            fly_get_data_ptr((void *)&outOrientation.front(), orientation));
        ASSERT_SUCCESS(fly_get_data_ptr((void *)&outSize.front(), size));
        ...
        ASSERT_SUCCESS(fly_release_features(out));
    }
}
