...
TYPED_TEST(Array, ConstructorEmptyDim4) {
    ...
    dtype type = (dtype)dtype_traits<TypeParam>::fly_type;
    dim4 dims(3, 3, 3, 3);
    array a(dims, type);
...
TYPED_TEST(Array, ConstructorHostPointer1D) {
    ...
    array a(nelems, &data.front(), flyHost);
    EXPECT_EQ(1u, a.numdims());
...
TYPED_TEST(Array, ConstructorHostPointer2D) {
    ...
    vector<TypeParam> data(nelems, TypeParam(4));
    array a(dim_size, dim_size, &data.front(), flyHost);
...
TYPED_TEST(Array, ConstructorHostPointer3D) {
    ...
    vector<TypeParam> data(nelems, TypeParam(4));
    array a(dim_size, dim_size, dim_size, &data.front(), flyHost);
    EXPECT_EQ(ndims, a.numdims());
    EXPECT_EQ(dim_t(dim_size), a.dims(0));
    EXPECT_EQ(dim_t(dim_size), a.dims(1));
...
TYPED_TEST(Array, ConstructorHostPointer4D) {
    ...
    vector<TypeParam> data(nelems, TypeParam(4));
    array a(dim_size, dim_size, dim_size, dim_size, &data.front(), flyHost);
    EXPECT_EQ(ndims, a.numdims());
    EXPECT_EQ(dim_t(dim_size), a.dims(0));
    EXPECT_EQ(dim_t(dim_size), a.dims(1));
    EXPECT_EQ(dim_t(dim_size), a.dims(2));
...
TEST(Array, CreateHandleInvalidNullDimsPointer) {
    fly_array out = 0;
    EXPECT_EQ(FLY_ERR_ARG, fly_create_handle(&out, 1, NULL, f32));
}
...
TEST(DeviceId, Different) {
...

    {
        ...
        fly_array c;
        fly_err err = fly_matmul(&c, a.get(), b.get(), FLY_MAT_NONE, FLY_MAT_NONE);
        fly::sync();
        ASSERT_EQ(err, FLY_SUCCESS);
    }
...
TEST(Array, ReferenceCount1) {
    ...
    array a = fly::randu(10, 10);
    a.eval();
    fly::sync();
    {
        ASSERT_REF(a, 1) << "After a = randu(10, 10);";

        array b = fly::randu(10, 10);  //(fly::seq(100));
...
TEST(Array, InitializerListFixFLYArray) {
    fly::array a = randu(1);
    fly::array b{a};
...
TEST(Array, InitializerListFixDim4) {
    ...
    fly::array b{dim4(3, 3), data.data()};
    ASSERT_ARRAYS_EQ(constant(3.14, 3, 3), b);
}

TEST(Array, OtherDevice) {
    if (fly::getDeviceCount() == 1) GTEST_SKIP() << "Single device. Skipping";
    fly::setDevice(0);
    fly::info();
    fly::array a = constant(3, 5, 5);
    a.eval();
    fly::setDevice(1);
    fly::info();
    fly::array b = constant(2, 5, 5);
    b.eval();
