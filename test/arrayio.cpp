...
struct type_params {
    ...
    double real;
    double imag;
    type_params(string n, fly_dtype t, double r, double i = 0.)
...
template<typename T>
void checkVals(array arr, double r, double i, fly_dtype t) {
...
TEST_P(ArrayIOType, ReadContent) {
    ...
    switch (arr.type()) {
        ...
        case c32: checkVals<fly::cfloat>(arr, p.real, p.imag, p.type); break;
        case c64: checkVals<fly::cdouble>(arr, p.real, p.imag, p.type); break;
...
TEST(ArrayIO, Save) {
    ...
    saveArray("a", a, "arr.fly");
    saveArray("b", b, "arr.fly", true);

    array aread = readArray("arr.fly", "a");
    array bread = readArray("arr.fly", "b");
