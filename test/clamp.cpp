...
class Clamp : public ::testing::TestWithParam<clamp_params> {
   public:
    void SetUp() {
        ...
        in_.as((dtype)fly::dtype_traits<T>::fly_type).host(&hin[0]);
        lo_.as((dtype)fly::dtype_traits<T>::fly_type).host(&hlo[0]);
        hi_.as((dtype)fly::dtype_traits<T>::fly_type).host(&hhi[0]);
