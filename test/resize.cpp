...
class ResizeI : public ::testing::Test {
   public:
    virtual void SetUp() {
        ...
        subMat1.push_back(fly_make_seq(0, 5, 1));
        subMat1.push_back(fly_make_seq(0, 5, 1));
...
TYPED_TEST(Resize, InvalidDims) {
    ...
    ASSERT_EQ(FLY_ERR_SIZE,
              fly_resize(&outArray, inArray, 0, 0, FLY_INTERP_NEAREST));
...
void resizeTest(string pTestFile, const unsigned resultIdx, const dim_t odim0,
    ...
    ASSERT_SUCCESS(fly_resize(&outArray, inArray, odim0, odim1, method));
...
TYPED_TEST(ResizeI, Resize3CSquareDownLinearSubref) {
    resizeTest<TypeParam>(string(TEST_DIR "/resize/square.test"), 8, 3, 3,
                          FLY_INTERP_BILINEAR, true, &(this->subMat1));
}
...
template<typename T>
void resizeArgsTest(fly_err err, string pTestFile, const dim4 odims,
    ...
    ASSERT_EQ(err, fly_resize(&outArray, inArray, odims[0], odims[1], method));
...
TYPED_TEST(Resize, InvalidArgsDims0) {
    dim4 dims(0, 5, 2, 1);
    resizeArgsTest<TypeParam>(FLY_ERR_SIZE,
...
TYPED_TEST(Resize, InvalidArgsMethod) {
    dim4 dims(10, 10, 1, 1);
    resizeArgsTest<TypeParam>(FLY_ERR_ARG,
                              string(TEST_DIR "/resize/square.test"), dims,
                              FLY_INTERP_CUBIC);
}
