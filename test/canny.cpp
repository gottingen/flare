...
void cannyTest(string pTestFile) {
    ...
    ASSERT_SUCCESS(fly_canny(&outArray, sArray, FLY_CANNY_THRESHOLD_MANUAL,
...
TEST(Canny, DISABLED_Exact) {
    ...
    array out = canny(img, FLY_CANNY_THRESHOLD_AUTO_OTSU, 0.08, 0.32, 3, false);
    array gold =
...
void cannyImageOtsuTest(string pTestFile, bool isColor) {
    ...
    for (size_t testId = 0; testId < testCount; ++testId) {
        ...
        fly_dtype type = (fly_dtype)dtype_traits<T>::fly_type;

        ASSERT_SUCCESS(
            fly_load_image(&_inArray, inFiles[testId].c_str(), isColor));

        ASSERT_SUCCESS(fly_cast(&inArray, _inArray, type));

        ASSERT_SUCCESS(
            fly_load_image_native(&goldArray, outFiles[testId].c_str()));

        ASSERT_SUCCESS(fly_canny(&_outArray, inArray,
                                FLY_CANNY_THRESHOLD_AUTO_OTSU, 0.08, 0.32, 3,
                                false));
        unsigned ndims = 0;
        dim_t dims[4];
        ...
        ASSERT_SUCCESS(fly_constant(&cstArray, 255.0, ndims, dims, f32));

        ASSERT_SUCCESS(fly_mul(&mulArray, cstArray, _outArray, false));
        ASSERT_SUCCESS(fly_cast(&outArray, mulArray, u8));
        ...
        ASSERT_SUCCESS(fly_release_array(mulArray));
...
TEST(CannyEdgeDetector, InvalidSizeArray) {
    ...
    ASSERT_EQ(FLY_ERR_SIZE,
              fly_canny(&outArray, inArray, FLY_CANNY_THRESHOLD_MANUAL, 0.24,
                       0.72, 3, true));
...
void cannyImageOtsuBatchTest(string pTestFile, const dim_t targetBatchCount) {
    ...
    for (size_t testId = 0; testId < testCount; ++testId) {
        ...
        array outIm =
            canny(inputIm, FLY_CANNY_THRESHOLD_AUTO_OTSU, 0.08, 0.32, 3, false);
        outIm *= 255.0;
