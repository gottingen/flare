...
TEST_F(OCLExtContext, PushAndPop) {
    ...
    flycl::addDevice(deviceId, context, queue);
    ASSERT_EQ(true, dCount + 1 == getDeviceCount());

    flycl::deleteDevice(deviceId, context);
...
TEST(OCLCheck, DeviceType) {
    flycl::deviceType devType = flycl::getDeviceType();
    cl_device_type type      = -100;
    clGetDeviceInfo(flycl::getDeviceId(), CL_DEVICE_TYPE, sizeof(cl_device_type),
                    &type, NULL);
    ASSERT_EQ(type, (cl_device_type)devType);
}

TEST(OCLCheck, DevicePlatform) {
    flycl::platform platform = flycl::getPlatform();
    ASSERT_NE(platform, FLYCL_PLATFORM_UNKNOWN);
}
#else
...
TEST(Memory, AfAllocDeviceOpenCL) {
    ...
    void *alloc_ptr;
    ASSERT_SUCCESS(fly_alloc_device(&alloc_ptr, sizeof(float)));
    // fly_alloc_device returns a cl::Buffer object from alloc unfortunately
    cl::Buffer *bptr = static_cast<cl::Buffer *>(alloc_ptr);
    ASSERT_EQ(2, bptr->getInfo<CL_MEM_REFERENCE_COUNT>());

    cl_command_queue queue;
    flycl_get_queue(&queue, true);
    ...
    ASSERT_SUCCESS(fly_free_device(alloc_ptr));
...
TEST(Memory, AfAllocDeviceV2OpenCLC) {
    ...
    void *alloc_ptr;
    ASSERT_SUCCESS(fly_alloc_device_v2(&alloc_ptr, sizeof(float)));
    {
    ...
    ASSERT_SUCCESS(fly_free_device_v2(alloc_ptr));
}
...
TEST(Memory, SNIPPET_AllocOpenCL) {
    ...
    flycl_get_queue(&queue, true);
    cl_context context;
    flycl_get_context(&context, true);
