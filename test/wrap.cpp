...
static void getInput(fly_array *data, const dim_t *dims) {
    float h_data[16] = {10, 20, 20, 30, 30, 40, 40, 50,
                        30, 40, 40, 50, 50, 60, 60, 70};
    ASSERT_SUCCESS(fly_create_array(data, &h_data[0], 2, dims, f32));
}
static void getGold(fly_array *gold, const dim_t *dims) {
    float h_gold[16] = {10, 20, 30, 40, 20, 30, 40, 50,
                        30, 40, 50, 60, 40, 50, 60, 70};
    ASSERT_SUCCESS(fly_create_array(gold, &h_gold[0], 2, dims, f32));
}
...
class WrapCommon : virtual public ::testing::Test {
   protected:
    ...
    virtual void TearDown() {
        if (in_ != 0) fly_release_array(in_);
        if (gold_ != 0) fly_release_array(gold_);
    }
...
class WrapV2 : public WrapCommon {
   protected:
    ...
    void setTestData(float *h_gold, dim4 gold_dims, float *h_in, dim4 in_dims) {
        ...
        ASSERT_SUCCESS(fly_create_array(&this->gold_, &h_gold_cast.front(),
                                       gold_dims.ndims(), gold_dims.get(),
                                       (fly_dtype)dtype_traits<T>::fly_type));
        ASSERT_SUCCESS(fly_create_array(&this->in_, &h_in_cast.front(),
    ...
    void testSpclOutArray(TestOutputArrayType out_array_type) {
        ...
        ASSERT_SUCCESS(fly_wrap_v2(&out, this->in_, 4, 4,  // output dims
                                  2, 2,                   // window size
                                  2, 2,                   // stride
                                  0, 0,                   // padding
                                  true));                 // is_column
    ...
    void releaseArrays() {
        if (this->in_ != 0) { ASSERT_SUCCESS(fly_release_array(this->in_)); }
        if (this->gold_ != 0) { ASSERT_SUCCESS(fly_release_array(this->gold_)); }
    }
};
...
class WrapV2Simple : public WrapV2<T> {
   protected:
    void SetUp() {
        ...
        fly_array tmp_in   = 0;
        fly_array tmp_gold = 0;

        ::getInput(&tmp_in, this->in_dims.get());
        ::getGold(&tmp_gold, this->gold_dims.get());

        fly_dtype dtype = (fly_dtype)dtype_traits<T>::fly_type;
        ASSERT_SUCCESS(fly_cast(&this->in_, tmp_in, dtype));
        ASSERT_SUCCESS(fly_cast(&this->gold_, tmp_gold, dtype));

        ASSERT_SUCCESS(fly_release_array(tmp_in));
        ASSERT_SUCCESS(fly_release_array(tmp_gold));
    }
};
...
TEST_F(WrapNullArgs, NullOutputPtr) {
    fly_array *out_ptr = 0;
    ASSERT_EQ(fly_wrap(out_ptr, this->in_, 4, 4,  // output dims
                      2, 2,                      // window size
                      2, 2,                      // stride
                      0, 0,                      // padding
                      true),                     // is_column
...
TEST_F(WrapNullArgs, NullInputArray) {
    fly_array out = 0;
    ASSERT_EQ(fly_wrap(&out, 0, 4, 4,  // output dims
                      2, 2,           // window size
                      2, 2,           // stride
                      0, 0,           // padding
                      true),          // is_column
...
TEST_F(WrapNullArgs, V2NullOutputPtr) {
    fly_array *out_ptr = 0;
    ASSERT_EQ(fly_wrap_v2(out_ptr, this->in_, 4, 4,  // output dims
                         2, 2,                      // window size
                         2, 2,                      // stride
                         0, 0,                      // padding
                         true),                     // is_column
...
TEST_F(WrapNullArgs, V2NullInputArray) {
    fly_array out = 0;
    ASSERT_EQ(fly_wrap_v2(&out, 0, 4, 4,  // output dims
                         2, 2,           // window size
                         2, 2,           // stride
                         0, 0,           // padding
                         true),          // is_column
...
class WrapArgs {
   public:
    ...
    WrapArgs() : wc_(), sc_(), pc_(), is_column(true), err(fly_err(999)) {}

    WrapArgs(dim_t win_d0, dim_t win_d1, dim_t str_d0, dim_t str_d1,
             dim_t pad_d0, dim_t pad_d1, bool is_col, fly_err err)
        : wc_(win_d0, win_d1)
        , sc_(str_d0, str_d1)
        , pc_(pad_d0, pad_d1)
        , is_column(is_col)
...
TEST_P(WrapAPITest, CheckDifferentWrapArgs) {
    ...
    fly_array out_ = 0;
    fly_err err    = fly_wrap(&out_, in_, in_dims[0], in_dims[1], win_d0, win_d1,
                            str_d0, str_d1, pad_d0, pad_d1, input.is_column);

    ASSERT_EQ(err, input.err);
    if (out_ != 0) fly_release_array(out_);
}

WrapArgs args[] = {
    // clang-format off
    //      | win_dim0 | win_dim1 | str_dim0 | str_dim1 | pad_dim0 | pad_dim1 | is_col |    err    |
    WrapArgs(        2,         2,         2,         2,         0,         0,    true,  FLY_SUCCESS),
    WrapArgs(        2,         2,         2,         2,         0,         0,   false,  FLY_SUCCESS),

    WrapArgs(       -1,         2,         2,         2,         0,         0,    true,  FLY_ERR_ARG),
    WrapArgs(        2,        -1,         2,         2,         0,         0,    true,  FLY_ERR_ARG),
    WrapArgs(       -1,        -1,         2,         2,         0,         0,    true,  FLY_ERR_ARG),

    WrapArgs(        2,         2,        -1,         2,         0,         0,    true,  FLY_ERR_ARG),
    WrapArgs(        2,         2,         2,        -1,         0,         0,    true,  FLY_ERR_ARG),
    WrapArgs(        2,         2,        -1,        -1,         0,         0,    true,  FLY_ERR_ARG),

    WrapArgs(        2,         2,         2,         2,         1,         1,    true,  FLY_ERR_SIZE),
    WrapArgs(        2,         2,         2,         2,        -1,         1,    true,  FLY_ERR_SIZE),
    WrapArgs(        2,         2,         2,         2,         1,        -1,    true,  FLY_ERR_SIZE),
    WrapArgs(        2,         2,         2,         2,        -1,        -1,    true,  FLY_ERR_SIZE),
    // clang-format on
