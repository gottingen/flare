...
double relEps(array in) {
    typedef typename fly::dtype_traits<T>::base_type InBaseType;
    double fixed_eps = eps<T>();
    double calc_eps  = std::numeric_limits<InBaseType>::epsilon() *
                      std::max(in.dims(0), in.dims(1)) * fly::max<double>(in);
...
TYPED_TEST(Pinverse, ApinvA_IsHermitian) {
    ...
    array in = readTestInput<TypeParam>(
        string(TEST_DIR "/pinverse/pinverse10x8.test"));
    array inpinv = pinverse(in);
    array apinva = fly::matmul(inpinv, in);
    array out    = fly::matmul(inpinv, in).H();
...
TEST(Pinverse, SmallSigValExistsFloat) {
    ...
    fly::array vT;
    fly::array sVec;
    fly::svd(u, sVec, vT, in);
    dim_t sSize = sVec.elements();

    sVec(2)         = 1e-12;
    fly::array s     = fly::diag(sVec, 0, false);
    fly::array zeros = fly::constant(0, dim0 > sSize ? dim0 - sSize : sSize,
                                   dim1 > sSize ? dim1 - sSize : sSize);
    s               = fly::join(dim0 > dim1 ? 0 : 1, s, zeros);

    // Make new input array that has a small non-zero value in its SVD sigma
    in           = fly::matmul(u, s, vT);
    array inpinv = pinverse(in);
    array out    = matmul(in, inpinv, in);
...
TEST(Pinverse, C) {
    array in =
        readTestInput<float>(string(TEST_DIR "/pinverse/pinverse10x8.test"));
    fly_array inpinv = 0, identity = 0, out = 0;
    ASSERT_SUCCESS(fly_pinverse(&inpinv, in.get(), 1e-6, FLY_MAT_NONE));
    ASSERT_SUCCESS(
        fly_matmul(&identity, in.get(), inpinv, FLY_MAT_NONE, FLY_MAT_NONE));
    ASSERT_SUCCESS(
        fly_matmul(&out, identity, in.get(), FLY_MAT_NONE, FLY_MAT_NONE));

    ASSERT_ARRAYS_NEAR(in.get(), out, eps<float>());

    ASSERT_SUCCESS(fly_release_array(out));
    ASSERT_SUCCESS(fly_release_array(identity));
    ASSERT_SUCCESS(fly_release_array(inpinv));
}
...
TEST(Pinverse, C_CustomTol) {
    array in =
        readTestInput<float>(string(TEST_DIR "/pinverse/pinverse10x8.test"));
    fly_array inpinv = 0, identity = 0, out = 0;
    ASSERT_SUCCESS(fly_pinverse(&inpinv, in.get(), 1e-12, FLY_MAT_NONE));
    ASSERT_SUCCESS(
        fly_matmul(&identity, in.get(), inpinv, FLY_MAT_NONE, FLY_MAT_NONE));
    ASSERT_SUCCESS(
        fly_matmul(&out, identity, in.get(), FLY_MAT_NONE, FLY_MAT_NONE));
...
TEST(Pinverse, InvalidMatProp) {
    ...
    array out;
    ASSERT_THROW(out = pinverse(in, -1.f, FLY_MAT_SYM), exception);
}
