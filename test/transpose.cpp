...
class Transpose : public ::testing::Test {
   public:
    virtual void SetUp() {
        subMat2D.push_back(fly_make_seq(2, 7, 1));
        subMat2D.push_back(fly_make_seq(2, 7, 1));

        subMat3D.push_back(fly_make_seq(2, 7, 1));
        subMat3D.push_back(fly_make_seq(2, 7, 1));
        subMat3D.push_back(fly_span);
    }
...
void trsTest(string pTestFile, bool isSubRef = false,
    ...
    if (isSubRef) {
        ...
        ASSERT_SUCCESS(fly_transpose(&outArray, subArray, false));
