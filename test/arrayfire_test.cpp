...
using fly::fly_cdouble;
using fly::fly_cfloat;
using std::vector;
...
std::ostream &operator<<(std::ostream &os, fly::Backend bk) {
    switch (bk) {
        case FLY_BACKEND_CPU: os << "FLY_BACKEND_CPU"; break;
        case FLY_BACKEND_CUDA: os << "FLY_BACKEND_CUDA"; break;
        case FLY_BACKEND_OPENCL: os << "FLY_BACKEND_OPENCL"; break;
        case FLY_BACKEND_ONEAPI: os << "FLY_BACKEND_ONEAPI"; break;
        case FLY_BACKEND_DEFAULT: os << "FLY_BACKEND_DEFAULT"; break;
    }
    return os;
}

std::ostream &operator<<(std::ostream &os, fly_err e) {
    return os << fly_err_to_string(e);
}
...
std::string getBackendName() {
    fly::Backend backend = fly::getActiveBackend();
...
::testing::AssertionResult assertArrayEq(std::string aName, std::string bName,
                                         ...
                                         float maxAbsDiff) {
    fly::dtype aType = a.type();
    fly::dtype bType = b.type();
    if (aType != bType)
    ...
    switch (arrDtype) {
        case f32:
            ...
            break;
        case c32:
            return elemWiseEq<fly::cfloat>(aName, bName, a, b, maxAbsDiff);
            break;
        case f64:
            return elemWiseEq<double>(aName, bName, a, b, maxAbsDiff);
            break;
        case c64:
            return elemWiseEq<fly::cdouble>(aName, bName, a, b, maxAbsDiff);
            break;
...
::testing::AssertionResult imageEq(std::string aName, std::string bName,
    ...
    if (NRMSD < maxAbsDiff) {
        ...
        // fly::array img = fly::join(1, a, b);
        // fly::Window win;
        // while (!win.close()) { win.image(img); }
        fly::saveImage(valid_path.c_str(), a.as(f32));
        fly::saveImage(result_path.c_str(), b.as(f32));
        fly::saveImage(diff_path.c_str(), abs(a.as(f32) - b.as(f32)));
...
::testing::AssertionResult assertImageEq(std::string aName, std::string bName,
    ...
    switch (arrDtype) {
        ...
        default: throw(FLY_ERR_NOT_SUPPORTED);
    }
    return ::testing::AssertionSuccess();
}

template<>
float convert(fly::half in) {
    return static_cast<float>(half_float::half(in.data_));
}

template<>
fly_half convert(int in) {
    half_float::half h = half_float::half(in);
    fly_half out;
    memcpy(&out, &h, sizeof(fly_half));
    return out;
}
...
#define INSTANTIATE(Tin, Tout, Tfile)                                  \
    template void readTests<Tin, Tout, Tfile>(                         \
        const std::string &FileName, std::vector<fly::dim4> &inputDims, \
...
INSTANTIATE(fly_cfloat, fly_cfloat, int);
INSTANTIATE(double, double, int);
INSTANTIATE(fly_cdouble, fly_cdouble, int);
...
INSTANTIATE(fly_half, fly_half, int);
...
INSTANTIATE(fly::fly_cfloat, float, float);
INSTANTIATE(fly::fly_cdouble, float, float);
...
INSTANTIATE(fly_cfloat, fly_cfloat, float);
INSTANTIATE(fly_cfloat, fly_cfloat, fly_cfloat);
INSTANTIATE(fly_cdouble, fly_cdouble, fly_cdouble);
INSTANTIATE(double, double, float);
INSTANTIATE(double, double, double);
INSTANTIATE(fly_cdouble, fly_cdouble, float);
...
INSTANTIATE(fly_cdouble, fly_cdouble, double);
INSTANTIATE(double, fly_cdouble, float);
INSTANTIATE(float, fly_cfloat, float);
...
INSTANTIATE(fly_cfloat, fly_cfloat, double);
...
bool noDoubleTests(fly::dtype ty) {
    ...
    int dev                = fly::getDevice();
    bool isDoubleSupported = fly::isDoubleAvailable(dev);
...
bool noHalfTests(fly::dtype ty) {
    ...
    int dev              = fly::getDevice();
    bool isHalfSupported = fly::isHalfAvailable(dev);
...
fly_half abs(fly_half in) {
    ...
    memcpy(static_cast<void *>(&in_), &in, sizeof(fly_half));
    half_float::half out_ = abs(in_);
    fly_half out;
    memcpy(&out, &out_, sizeof(fly_half));
    return out;
}
...
fly_half operator-(fly_half lhs, fly_half rhs) {
    ...
    // casting to void* to avoid class-memaccess warnings on windows
    memcpy(static_cast<void *>(&lhs_), &lhs, sizeof(fly_half));
    memcpy(static_cast<void *>(&rhs_), &rhs, sizeof(fly_half));
    half_float::half out = lhs_ - rhs_;
    fly_half o;
    memcpy(&o, &out, sizeof(fly_half));
    return o;
}

const fly::cfloat &operator+(const fly::cfloat &val) { return val; }

const fly::cdouble &operator+(const fly::cdouble &val) { return val; }

const fly_half &operator+(const fly_half &val) { return val; }
...
void genSubArray(TestOutputArrayInfo *metadata, const unsigned ndims,
    ...
    fly_seq subarr_idxs[4] = {fly_span, fly_span, fly_span, fly_span};
    for (uint i = 0; i < ndims; ++i) {
        fly_seq idx     = {pad_size, pad_size + dims[i] - 1.0, 1.0};
        subarr_idxs[i] = idx;
    }
...
void genReorderedArray(TestOutputArrayInfo *metadata, const unsigned ndims,
    ...
    fly_array reordered = 0;
    ASSERT_SUCCESS(fly_reorder(&reordered, metadata->getOutput(),
...
::testing::AssertionResult testWriteToOutputArray(
    ...
    if (metadata->getOutputArrayType() == NULL_ARRAY) {
        if (out == 0) {
            return ::testing::AssertionFailure()
                   << "Output fly_array " << result_name << " is null";
        }
    ...
    if (metadata->getOutputArrayType() == SUB_ARRAY) {
        ...
        // subarray, the other should have already been injected with the fly_*
...
::testing::AssertionResult assertArrayEq(std::string aName, std::string bName,
                                         const fly_array a, const fly_array b) {
    fly_array aa = 0, bb = 0;
    fly_retain_array(&aa, a);
    fly_retain_array(&bb, b);
    fly::array aaa(aa);
    fly::array bbb(bb);
...
void cleanSlate() {
    ...
    fly::deviceMemInfo(&alloc_bytes, &alloc_buffers, &lock_bytes, &lock_buffers);
    ...
    fly::setMemStepSize(step_bytes);

    ASSERT_EQ(fly::getMemStepSize(), step_bytes);
}
...
INSTANTIATE(float, fly_cfloat);
INSTANTIATE(fly_cfloat, fly_cfloat);
INSTANTIATE(double, double);
INSTANTIATE(double, fly_cdouble);
INSTANTIATE(fly_cdouble, fly_cdouble);
...
TestOutputArrayInfo::TestOutputArrayInfo()
    ...
    for (uint i = 0; i < 4; ++i) { out_subarr_idxs[i] = fly_span; }
}
...
TestOutputArrayInfo::~TestOutputArrayInfo() {
    if (out_subarr) fly_release_array(out_subarr);
    if (out_arr_cpy) fly_release_array(out_arr_cpy);
    if (out_arr) fly_release_array(out_arr);
}

void TestOutputArrayInfo::init(const unsigned ndims, const dim_t *const dims,
                               const fly_dtype ty) {
    ASSERT_SUCCESS(fly_randu(&out_arr, ndims, dims, ty));
}
...
void TestOutputArrayInfo::init(const unsigned ndims, const dim_t *const dims,
    ...
    ASSERT_SUCCESS(fly_copy_array(&out_arr_cpy, out_arr));
    for (uint i = 0; i < ndims; ++i) { out_subarr_idxs[i] = subarr_idxs[i]; }
    out_subarr_ndims = ndims;

    ASSERT_SUCCESS(fly_index(&out_subarr, out_arr, ndims, subarr_idxs));
}
...
void TestOutputArrayInfo::init(double val, const unsigned ndims,
    ...
    switch (ty) {
        case c32:
        case c64:
            fly_constant_complex(&out_arr, val, 0.0, ndims, dims, ty);
            break;
        case s64:
            fly_constant_long(&out_arr, static_cast<intl>(val), ndims, dims);
            break;
        case u64:
            fly_constant_ulong(&out_arr, static_cast<uintl>(val), ndims, dims);
            break;
        default: fly_constant(&out_arr, val, ndims, dims, ty); break;
    }
}
...
fly_array TestOutputArrayInfo::getOutput() {
...
void TestOutputArrayInfo::setOutput(fly_array array) {
    if (out_arr != 0) { ASSERT_SUCCESS(fly_release_array(out_arr)); }
    out_arr = array;
}

fly_array TestOutputArrayInfo::getFullOutput() { return out_arr; }
fly_array TestOutputArrayInfo::getFullOutputCopy() { return out_arr_cpy; }
fly_seq *TestOutputArrayInfo::getSubArrayIdxs() { return &out_subarr_idxs[0]; }
...
::testing::AssertionResult mtxReadSparseMatrix(fly::array &out,
    ...
    if (mm_is_integer(matcode)) {
        return ::testing::AssertionFailure() << "MTX file has integer data. \
                Integer sparse matrices are not supported in Flare yet.\n";
    }
    ...
    if (mm_is_real(matcode)) {
        ...
        out = fly::sparse(M, N, nz, V.data(), I.data(), J.data(), f32,
                         FLY_STORAGE_COO);
    } else if (mm_is_complex(matcode)) {
        std::vector<int> I(nz);
        std::vector<int> J(nz);
        std::vector<fly::cfloat> V(nz);
        ...
        for (int i = 0; i < nz; ++i) {
            ...
            I[i] = r - 1;
            J[i] = c - 1;
            V[i] = fly::cfloat(float(real), float(imag));
        }

        out = fly::sparse(M, N, nz, V.data(), I.data(), J.data(), c32,
                         FLY_STORAGE_COO);
    } else {
        return ::testing::AssertionFailure()
...
fly_err conv_image(fly_array *out, fly_array in) {
    ...
    dim_t d0, d1, d2, d3;
    fly_get_dims(&d0, &d1, &d2, &d3, in);
    fly::dim4 idims(d0, d1, d2, d3);

    dim_t nElems = 0;
    fly_get_elements(&nElems, in);

    float *in_data = new float[nElems];
    fly_get_data_ptr(in_data, in);

    T *out_data = new T[nElems];

    for (int i = 0; i < (int)nElems; i++) out_data[i] = (T)in_data[i];

    fly_create_array(&outArray, out_data, idims.ndims(), idims.get(),
...
#define INSTANTIATE(To) \
    template fly_err conv_image<To>(fly_array * out, fly_array in)
...
INSTANTIATE(char);
INSTANTIATE(short);
INSTANTIATE(fly_cdouble);
INSTANTIATE(fly_cfloat);
...
fly::array cpu_randu(const fly::dim4 dims) {
    typedef typename fly::dtype_traits<T>::base_type BT;

    bool isTypeCplx = is_same_type<T, fly::cfloat>::value ||
                      is_same_type<T, fly::cdouble>::value;
    ...
    return fly::array(dims, (T *)&out[0]);
}

#define INSTANTIATE(To) template fly::array cpu_randu<To>(const fly::dim4 dims)
INSTANTIATE(float);
INSTANTIATE(double);
INSTANTIATE(unsigned char);
INSTANTIATE(half_float::half);
...
template<typename T>
vector<sparseCooValue<T>> toCooVector(const fly::array &arr) {
...
std::string printContext(const std::vector<T> &hGold, std::string goldName,
    ...
    fly::dim4 coords = unravelIdx(idx, arrDims, arrStrides);
    dim_t ctxWidth  = 5;

    // Coordinates that span dim0
    fly::dim4 coordsMinBound = coords;
    coordsMinBound[0]       = 0;
    fly::dim4 coordsMaxBound = coords;
...
std::string printContext(const std::vector<sparseCooValue<T>> &hGold,
                         ...
                         std::string outName, fly::dim4 arrDims,
                         fly::dim4 arrStrides, dim_t idx) {
    std::ostringstream os;
...
::testing::AssertionResult elemWiseEq(std::string aName, std::string bName,
    ...
    if (bItr == b.end()) {
        ...
        fly::dim4 aStrides = calcStrides(aDims);
        fly::dim4 bStrides = calcStrides(bDims);
        fly::dim4 coords   = unravelIdx(idx, bDims, bStrides);
...
template<>
bool absMatch::operator()<fly::fly_cfloat>(const fly::fly_cfloat &lhs,
                                         const fly::fly_cfloat &rhs) const {
    return fly::abs(rhs - lhs) <= diff_;
}

template<>
bool absMatch::operator()<fly::fly_cdouble>(const fly::fly_cdouble &lhs,
                                          const fly::fly_cdouble &rhs) const {
    return fly::abs(rhs - lhs) <= diff_;
}
...
::testing::AssertionResult elemWiseEq(std::string aName, std::string bName,
    ...
    if (aItr == a.end()) {
        ...
        dim_t idx       = std::distance(b.begin(), bItr);
        fly::dim4 coords = unravelIdx(idx, bDims, calcStrides(bDims));
...
::testing::AssertionResult elemWiseEq(std::string aName, std::string bName,
                                      ...
                                      fly::dim4 bDims, float maxAbsDiff,
                                      IntegerTag) {
...
::testing::AssertionResult elemWiseEq(std::string aName, std::string bName,
    ...
    typedef typename cond_type<
        IsFloatingPoint<typename fly::dtype_traits<T>::base_type>::value,
...
::testing::AssertionResult assertArrayEq(std::string aName,
                                         ...
                                         float maxAbsDiff) {
    fly::dtype aDtype = (fly::dtype)fly::dtype_traits<T>::fly_type;
    if (aDtype != b.type()) {
        return ::testing::AssertionFailure()
               << "TYPE MISMATCH:\n"
               << "  Actual: " << bName << "(" << b.type() << ")\n"
...
::testing::AssertionResult assertRefEq(std::string hA_name,
    ...
    int count = 0;
    fly_get_data_ref_count(&count, a.get());
    if (count != expected) {
        std::stringstream ss;
...
#define INSTANTIATE(To)                                                        \
    template std::string printContext(                                         \
        ...
        const std::vector<To> &hOut, std::string outName, fly::dim4 arrDims,    \
        fly::dim4 arrStrides, dim_t idx);                                       \
    template ::testing::AssertionResult assertArrayEq<To>(                     \
        std::string aName, std::string aDimsName, std::string bName,           \
        const std::vector<To> &hA, fly::dim4 aDims, const fly::array &b,         \
        float maxAbsDiff);                                                     \
    template ::testing::AssertionResult assertArrayEq<To>(                     \
        std::string hA_name, std::string aDimsName, std::string bName,         \
        const std::vector<To> &hA, fly::dim4 aDims, const fly_array b);          \
    template ::testing::AssertionResult assertArrayNear<To>(                   \
        std::string hA_name, std::string aDimsName, std::string bName,         \
        std::string maxAbsDiffName, const std::vector<To> &hA, fly::dim4 aDims, \
        const fly_array b, float maxAbsDiff);                                   \
    template ::testing::AssertionResult assertArrayNear<To>(                   \
        std::string hA_name, std::string aDimsName, std::string bName,         \
        std::string maxAbsDiffName, const std::vector<To> &hA, fly::dim4 aDims, \
