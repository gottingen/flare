...
TEST(Array, TestEmptyImage) {
    ...
    fly_array h, hout;
    dim_t ds[1];
    fly_constant(&h, 0, 0, ds, f32);
    fly_histogram(&hout, h, 10, 0.0, 1.0);

    unsigned nd;
    fly_get_numdims(&nd, h);
    ASSERT_EQ(nd, 0u);
    fly_get_numdims(&nd, hout);
    ASSERT_EQ(nd, 0u);
    ASSERT_SUCCESS(fly_release_array(h));
    ASSERT_SUCCESS(fly_release_array(hout));
}
