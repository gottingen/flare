...
array generateArray(int nx, int ny, int nz, int nw) {
    array a = randu(nx, ny, nz, nw, (dtype)dtype_traits<Ti>::fly_type);
    return a;
}

template<>
array generateArray<int>(int nx, int ny, int nz, int nw) {
    array a = (randu(nx, ny, nz, nw, (dtype)dtype_traits<float>::fly_type) * 1e6)
                  .as(s32);
    return a;
}
...
void median_flat(int nx, int ny = 1, int nz = 1, int nw = 1) {
    ...
    To *h_sa = sa.as((fly_dtype)dtype_traits<To>::fly_type).host<To>();
    if (sa.dims(0) % 2 == 1) {
        verify = h_sa[mid - 1];
    } else {
...
void median_test(int nx, int ny = 1, int nz = 1, int nw = 1) {
    ...
    if (sa.dims(dim) % 2 == 1) {
        ...
        sa        = sa.as((fly_dtype)dtype_traits<To>::fly_type);
...
TEST(Median, OneElement) {
    fly::array in = randu(1, f32);

    fly::array out = median(in);
    ASSERT_ARRAYS_EQ(in, out);
}

TEST(Median, TwoElements) {
    fly::array in = randu(2, f32);

    fly::array out  = median(in);
    fly::array gold = mean(in);
