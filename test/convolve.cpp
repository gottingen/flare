...
void convolveTest(string pTestFile, int baseDim, bool expand) {
    ...
    ASSERT_SUCCESS(fly_create_array(&signal, &(in[0].front()), sDims.ndims(),
                                   sDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));
    ASSERT_SUCCESS(fly_create_array(&filter, &(in[1].front()), fDims.ndims(),
                                   fDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));

    fly_conv_mode mode = expand ? FLY_CONV_EXPAND : FLY_CONV_DEFAULT;
    switch (baseDim) {
        case 1:
            ASSERT_SUCCESS(
                fly_convolve1(&outArray, signal, filter, mode, FLY_CONV_AUTO));
            break;
        case 2:
            ASSERT_SUCCESS(
                fly_convolve2(&outArray, signal, filter, mode, FLY_CONV_AUTO));
            break;
        case 3:
            ASSERT_SUCCESS(
                fly_convolve3(&outArray, signal, filter, mode, FLY_CONV_AUTO));
            break;
    }
    ...
    ASSERT_SUCCESS(fly_release_array(signal));
    ASSERT_SUCCESS(fly_release_array(filter));
}
...
void sepConvolveTest(string pTestFile, bool expand) {
    ...
    fly_array signal   = 0;
    fly_array c_filter = 0;
    fly_array r_filter = 0;
    fly_array outArray = 0;

    ASSERT_SUCCESS(fly_create_array(&signal, &(in[0].front()), sDims.ndims(),
                                   sDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));
    ASSERT_SUCCESS(fly_create_array(&c_filter, &(in[1].front()), cfDims.ndims(),
                                   cfDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));
    ASSERT_SUCCESS(fly_create_array(&r_filter, &(in[2].front()), rfDims.ndims(),
                                   rfDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));

    fly_conv_mode mode = expand ? FLY_CONV_EXPAND : FLY_CONV_DEFAULT;
    ASSERT_SUCCESS(
        fly_convolve2_sep(&outArray, c_filter, r_filter, signal, mode));
    ...
    ASSERT_SUCCESS(fly_release_array(outArray));
    ASSERT_SUCCESS(fly_release_array(signal));
    ASSERT_SUCCESS(fly_release_array(c_filter));
    ASSERT_SUCCESS(fly_release_array(r_filter));
}
...
TEST(Convolve, Separable_TypeCheck) {
    ...
    ASSERT_SUCCESS(fly_create_array(&signal, &(in.front()), sDims.ndims(),
                                   sDims.get(),
                                   (fly_dtype)dtype_traits<float>::fly_type));
    ASSERT_SUCCESS(fly_create_array(&c_filter, &(filt.front()), fDims.ndims(),
                                   fDims.get(),
                                   (fly_dtype)dtype_traits<int>::fly_type));
    ASSERT_SUCCESS(fly_create_array(&r_filter, &(filt.front()), fDims.ndims(),
                                   fDims.get(),
                                   (fly_dtype)dtype_traits<int>::fly_type));

    ASSERT_EQ(FLY_ERR_ARG, fly_convolve2_sep(&outArray, c_filter, r_filter,
                                           signal, FLY_CONV_EXPAND));
...
TEST(Convolve1, CPP) {
    ...
    array output = convolve1(signal, filter, FLY_CONV_DEFAULT);
...
TEST(Convolve2, CPP) {
    ...
    array output = convolve2(signal, filter, FLY_CONV_DEFAULT);
...
TEST(Convolve3, CPP) {
    ...
    array output = convolve3(signal, filter, FLY_CONV_DEFAULT);
...
TEST(Convolve, separable_CPP) {
    ...
    array output = convolve(cFilter, rFilter, signal, FLY_CONV_DEFAULT);
...
TEST(ConvolveLargeDim1D, CPP) {
    ...
    array output  = convolve1(signal, identity_filter, FLY_CONV_DEFAULT);
    array output2 = output;
    ASSERT_EQ(largeDim * n, sum<float>(output2));

    signal = constant(1, n, 1, 1, largeDim);

    output = convolve1(signal, identity_filter, FLY_CONV_DEFAULT);
...
TEST(ConvolveLargeDim2D, CPP) {
    ...
    array output = convolve2(signal, identity_filter, FLY_CONV_DEFAULT);
    ASSERT_EQ(largeDim * n * n, sum<float>(output));

    signal = constant(1, n, n, 1, largeDim);

    output = convolve2(signal, identity_filter, FLY_CONV_DEFAULT);
    ASSERT_EQ(largeDim * n * n, sum<float>(output));
}
...
TEST(DISABLED_ConvolveLargeDim3D, CPP) {
    ...
    array output = convolve3(signal, identity_filter, FLY_CONV_DEFAULT);
    ASSERT_EQ(1.f, product<float>(output));

    signal = constant(1, n, n, largeDim);

    output = convolve3(signal, identity_filter, FLY_CONV_EXPAND);
...
TEST(Convolve, CuboidBatchLaunchBugFix) {
    ...
    fly::array signal(sDims, in[0].data());
    fly::array filter(fDims, in[1].data());

    fly::array output = convolve3(signal, filter);
...
void convolve2stridedTest(string pTestFile, dim4 stride, dim4 padding,
    ...
    ASSERT_SUCCESS(fly_convolve2_nn(&convolved, signal, filter, stride.ndims(),
    ...
    ASSERT_SUCCESS(fly_release_array(convolved));
    ASSERT_SUCCESS(fly_release_array(signal));
    ASSERT_SUCCESS(fly_release_array(filter));
}
...
void convolve2GradientTest(string pTestFile, dim4 stride, dim4 padding,
    ...
    ASSERT_SUCCESS(fly_create_array(&convolved, &(currGoldBar.front()),
                                   cDims.ndims(), cDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));

    fly_array incoming_gradient = 0;
    ASSERT_SUCCESS(fly_constant(&incoming_gradient, 1, cDims.ndims(),
                               cDims.get(),
                               (fly_dtype)dtype_traits<T>::fly_type));

    fly_array filter_gradient = 0;
    ASSERT_SUCCESS(fly_convolve2_gradient_nn(
        &filter_gradient, incoming_gradient, signal, filter, convolved,
        stride.ndims(), stride.get(), padding.ndims(), padding.get(),
        dilation.ndims(), dilation.get(), FLY_CONV_GRADIENT_FILTER));

    fly_array data_gradient = 0;
    ASSERT_SUCCESS(fly_convolve2_gradient_nn(
        &data_gradient, incoming_gradient, signal, filter, convolved,
        stride.ndims(), stride.get(), padding.ndims(), padding.get(),
        dilation.ndims(), dilation.get(), FLY_CONV_GRADIENT_DATA));
    ...
    ASSERT_SUCCESS(fly_release_array(incoming_gradient));
    ASSERT_SUCCESS(fly_release_array(convolved));
    ASSERT_SUCCESS(fly_release_array(signal));
    ASSERT_SUCCESS(fly_release_array(filter));
    ASSERT_SUCCESS(fly_release_array(filter_gradient));
    ASSERT_SUCCESS(fly_release_array(data_gradient));
}
