...
// This list does not allow to cleanly add the fly_half/half_float type : at the
...
void meanDimTest(string pFileName, dim_t dim, bool isWeighted = false) {
    ...
    double tol = 1.0e-3;
    if ((fly_dtype)fly::dtype_traits<T>::fly_type == f16) tol = 4.e-3;
    vector<dim4> numDims;
...
void meanAllTest(half_float::half const_value, dim4 dims) {
    ...
    fly_half output2 = mean<fly_half>(a);

    // make sure output2 and output are binary equals. This is necessary
    // because fly_half is not a complete type
    half output2_copy;
    memcpy(static_cast<void*>(&output2_copy), &output2, sizeof(fly_half));
...
TEST(MeanHalf, dim0) {
    ...
    array m16 = fly::mean(in, 0);
    array m32 = fly::mean(in.as(f32), 0);
