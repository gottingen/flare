...
void joinTest(string pTestFile, const unsigned dim, const unsigned in0,
    ...
    fly_array in0Array  = 0;
    fly_array in1Array  = 0;
    fly_array outArray  = 0;
    fly_array tempArray = 0;

    if (isSubRef) {
        ASSERT_SUCCESS(fly_create_array(&tempArray, &(in[in0].front()),
                                       i0dims.ndims(), i0dims.get(),
                                       (fly_dtype)dtype_traits<T>::fly_type));

        ASSERT_SUCCESS(
            fly_index(&in0Array, tempArray, seqv->size(), &seqv->front()));
    } else {
        ASSERT_SUCCESS(fly_create_array(&in0Array, &(in[in0].front()),
                                       i0dims.ndims(), i0dims.get(),
                                       (fly_dtype)dtype_traits<T>::fly_type));
    }

    if (isSubRef) {
        ASSERT_SUCCESS(fly_create_array(&tempArray, &(in[in1].front()),
                                       i1dims.ndims(), i1dims.get(),
                                       (fly_dtype)dtype_traits<T>::fly_type));

        ASSERT_SUCCESS(
            fly_index(&in1Array, tempArray, seqv->size(), &seqv->front()));
    } else {
        ASSERT_SUCCESS(fly_create_array(&in1Array, &(in[in1].front()),
                                       i1dims.ndims(), i1dims.get(),
                                       (fly_dtype)dtype_traits<T>::fly_type));
    }

    ASSERT_SUCCESS(fly_join(&outArray, dim, in0Array, in1Array));
    ...
    if (in0Array != 0) fly_release_array(in0Array);
    if (in1Array != 0) fly_release_array(in1Array);
...
TEST(Join, ManyEmpty) {
    array gold = fly::constant(0, 15, 5);
    array a    = fly::randn(5, 5);
    array e;
    array c  = fly::randn(10, 5);
    array ee = fly::join(0, e, e);
    ASSERT_EQ(ee.elements(), 0);
    array eee = fly::join(0, e, e, e);
    ASSERT_EQ(eee.elements(), 0);

    array eeac                     = fly::join(0, e, e, a, c);
    array eace                     = fly::join(0, e, a, c, e);
    array acee                     = fly::join(0, a, c, e, e);
    gold(fly::seq(0, 4), fly::span)  = a;
    gold(fly::seq(5, 14), fly::span) = c;
