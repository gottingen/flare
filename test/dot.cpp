...
bool isinf(fly::fly_cfloat val) {
    using std::isinf;
    return isinf(val.real) || isinf(val.imag);
}
bool isinf(fly::fly_cdouble val) {
    using std::isinf;
    return isinf(val.real) || isinf(val.imag);
}
...
void dotTest(string pTestFile, const int resultIdx,
             const fly_mat_prop optLhs = FLY_MAT_NONE,
             const fly_mat_prop optRhs = FLY_MAT_NONE) {
    ...
    ASSERT_SUCCESS(fly_create_array(&a, &(in[0].front()), aDims.ndims(),
                                   aDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));
    ASSERT_SUCCESS(fly_create_array(&b, &(in[1].front()), bDims.ndims(),
                                   bDims.get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));

    ASSERT_SUCCESS(fly_dot(&out, a, b, optLhs, optRhs));
...
void dotAllTest(string pTestFile, const int resultIdx,
    ...
    ASSERT_SUCCESS(fly_dot_all(&rval, &ival, a, b, optLhs, optRhs));
...
TEST(DotF, CPP) {
    ...
    array out = dot(a, b, FLY_MAT_CONJ, FLY_MAT_NONE);
...
TEST(DotAllF, CPP) {
    ...
    float out = dot<float>(a, b, FLY_MAT_CONJ, FLY_MAT_NONE);
...
TEST(DotAllCCU, CPP) {
    ...
    cfloat out = dot<cfloat>(a, b, FLY_MAT_CONJ, FLY_MAT_NONE);
...
class Dot : public ::testing::TestWithParam<int> {
   public:
    ...
    void SetUp() {
        ...
        int elems = GetParam();
        array fa  = fly::randu(elems) - 0.5f;
        array fb  = fly::randu(elems) - 0.5f;
