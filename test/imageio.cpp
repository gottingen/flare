...
void loadImageTest(string pTestFile, string pImageFile, const bool isColor) {
    ...
    fly_array imgArray = 0;
    ASSERT_SUCCESS(fly_load_image(&imgArray, pImageFile.c_str(), isColor));

    // Get result
    float* imgData = new float[dims.elements()];
    ASSERT_SUCCESS(fly_get_data_ptr((void*)imgData, imgArray));
    ...
    if (imgArray != 0) fly_release_array(imgArray);
}
...
void loadimageArgsTest(string pImageFile, const bool isColor, fly_err err) {
    IMAGEIO_ENABLED_CHECK();

    fly_array imgArray = 0;

    ASSERT_EQ(err, fly_load_image(&imgArray, pImageFile.c_str(), isColor));
...
using fly::anyTrue;
using fly::deleteImageMem;
using fly::loadImage;
using fly::loadImageMem;
using fly::saveImageMem;
using fly::span;
...
TEST(ImageMem, SaveMemPNG) {
    ...
    void* savedMem = saveImageMem(img, FLY_FIF_PNG);
...
TEST(ImageMem, SaveMemJPG1) {
    ...
    void* savedMem = saveImageMem(img, FLY_FIF_JPEG);
...
TEST(ImageMem, SaveMemBMP) {
    ...
    void* savedMem = saveImageMem(img, FLY_FIF_BMP);
...
void loadImageNativeCPPTest(string pTestFile, string pImageFile) {
    ...
    ASSERT_EQ(img.type(), (fly_dtype)dtype_traits<T>::fly_type);
...
void saveLoadImageNativeCPPTest(dim4 dims) {
    ...
    array input = randu(dims, (fly_dtype)dtype_traits<T>::fly_type);
