...
void nearestNeighbourTest(string pTestFile, int feat_dim,
    ...
    ASSERT_SUCCESS(
        fly_nearest_neighbour(&idx, &dist, query, train, feat_dim, 1, type));
...
TEST(NearestNeighbourSSD, CPP) {
    ...
    array idx, dist;
    nearestNeighbour(idx, dist, query, train, 0, 1, FLY_SSD);
...
TEST(NearestNeighbourSAD, CPP) {
    ...
    array idx, dist;
    nearestNeighbour(idx, dist, query, train, 1, 1, FLY_SAD);
...
TEST(NearestNeighbourSSD, small) {
    ...
    array indices;
    array distances;
    nearestNeighbour(indices, distances, q, t, 0, 1, FLY_SSD);
...
TEST(KNearestNeighbourSSD, small) {
    ...
    const int k = 2;
    nearestNeighbour(indices, actualDistances, q, t, 0, k, FLY_SSD);
...
TEST_P(NearestNeighborsTest, SingleQTests) {
    ...
    nearestNeighbour(indices, distances, query, train, feat_dim, k, FLY_SSD);
...
TEST(KNearestNeighbours, InvalidNegativeK) {
    ...
    array distances;
    int k = -1;
    ASSERT_THROW(nearestNeighbour(indices, distances, q, t, 0, k, FLY_SSD),
                 fly::exception);
}
