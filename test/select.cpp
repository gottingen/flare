...
TEST(Select, InvalidSizeOfAB) {
    ...
    double val = 0;
    dim_t dims = 10;
    ASSERT_SUCCESS(fly_constant(&a, val, 1, &dims, f32));

    dims = 9;
    ASSERT_SUCCESS(fly_constant(&b, val, 1, &dims, f32));

    dims = 10;
    ASSERT_SUCCESS(fly_constant(&cond, val, 1, &dims, b8));

    ASSERT_EQ(FLY_ERR_SIZE, fly_select(&out, cond, a, b));

    char* msg = NULL;
    dim_t len = 0;
    fly_get_last_error(&msg, &len);
    fly_free_host(msg);
    fly_release_array(a);
    fly_release_array(b);
    fly_release_array(cond);
}
