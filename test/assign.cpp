...
class ArrayAssign : public ::testing::Test {
   public:
    virtual void SetUp() {
        subMat1D.push_back(fly_make_seq(5, 20, 1));

        subMat2D.push_back(fly_make_seq(1, 2, 1));
        subMat2D.push_back(fly_make_seq(1, 2, 1));

        subMat3D.push_back(fly_make_seq(3, 4, 1));
        subMat3D.push_back(fly_make_seq(0, 1, 1));
        subMat3D.push_back(fly_make_seq(1, 2, 1));

        subMat4D.push_back(fly_make_seq(3, 4, 1));
        subMat4D.push_back(fly_make_seq(0, 1, 1));
        subMat4D.push_back(fly_make_seq(0, 1, 1));
        subMat4D.push_back(fly_make_seq(1, 2, 1));

        subMat1D_to_2D.push_back(fly_make_seq(1, 2, 1));
        subMat1D_to_2D.push_back(fly_make_seq(1, 1, 1));

        subMat1D_to_3D.push_back(fly_make_seq(5, 20, 1));
        subMat1D_to_3D.push_back(fly_make_seq(1, 1, 1));
        subMat1D_to_3D.push_back(fly_make_seq(2, 2, 1));

        subMat2D_to_3D.push_back(fly_make_seq(3, 4, 1));
        subMat2D_to_3D.push_back(fly_make_seq(0, 1, 1));
        subMat2D_to_3D.push_back(fly_make_seq(1, 1, 1));

        subMat1D_to_4D.push_back(fly_make_seq(3, 4, 1));
        subMat1D_to_4D.push_back(fly_make_seq(0, 0, 1));
        subMat1D_to_4D.push_back(fly_make_seq(0, 0, 1));
        subMat1D_to_4D.push_back(fly_make_seq(1, 1, 1));

        subMat2D_to_4D.push_back(fly_make_seq(3, 4, 1));
        subMat2D_to_4D.push_back(fly_make_seq(0, 1, 1));
        subMat2D_to_4D.push_back(fly_make_seq(0, 0, 1));
        subMat2D_to_4D.push_back(fly_make_seq(1, 1, 1));

        subMat3D_to_4D.push_back(fly_make_seq(3, 4, 1));
        subMat3D_to_4D.push_back(fly_make_seq(0, 1, 1));
        subMat3D_to_4D.push_back(fly_make_seq(0, 1, 1));
        subMat3D_to_4D.push_back(fly_make_seq(1, 1, 1));
    }
    vector<fly_seq> subMat1D;

    vector<fly_seq> subMat2D;
    vector<fly_seq> subMat1D_to_2D;

    vector<fly_seq> subMat3D;
    vector<fly_seq> subMat1D_to_3D;
    vector<fly_seq> subMat2D_to_3D;

    vector<fly_seq> subMat4D;
    vector<fly_seq> subMat1D_to_4D;
    vector<fly_seq> subMat2D_to_4D;
    vector<fly_seq> subMat3D_to_4D;
};
...
void assignTest(string pTestFile, const vector<fly_seq> *seqv) {
    ...
    ASSERT_SUCCESS(fly_create_array(&rhsArray, &(in[0].front()), dims0.ndims(),
                                   dims0.get(),
                                   (fly_dtype)dtype_traits<inType>::fly_type));

    ASSERT_SUCCESS(fly_create_array(&lhsArray, &(in[1].front()), dims1.ndims(),
                                   dims1.get(),
                                   (fly_dtype)dtype_traits<outType>::fly_type));

    ASSERT_SUCCESS(fly_assign_seq(&outArray, lhsArray, seqv->size(),
                                 &seqv->front(), rhsArray));
...
template<typename T>
void assignTestCPP(string pTestFile, const vector<fly_seq> &seqv) {
...
template<typename T>
void assignScalarCPP(string pTestFile, const vector<fly_seq> &seqv) {
    SUPPORTED_TYPE_CHECK(T);
    try {
        vector<dim4> numDims;
        vector<vector<T>> in;
...
TYPED_TEST(ArrayAssign, AssignRowCPP) {
    ...
    array in(dimsize, dimsize, &input.front(), flyHost);
    dim4 size(dimsize, 1, 1, 1);
    array sarr(size, &sq.front(), flyHost);
    array arrIdx(2, &arIdx.front(), flyHost);
...
TYPED_TEST(ArrayAssign, AssignSliceCPP) {
    ...
    array in(dimsize, dimsize, dimsize, &input.front(), flyHost);
...
TEST(ArrayAssign, InvalidArgs) {
    ...
    vector<fly_seq> seqv;
    seqv.push_back(fly_make_seq(5, 14, 1));

    ASSERT_EQ(FLY_ERR_ARG, fly_assign_seq(&outArray, lhsArray, seqv.size(),
                                        &seqv.front(), rhsArray));

    ASSERT_SUCCESS(fly_create_array(&rhsArray, &(in.front()), dims0.ndims(),
                                   dims0.get(),
                                   (fly_dtype)dtype_traits<cfloat>::fly_type));

    ASSERT_EQ(FLY_ERR_ARG, fly_assign_seq(&outArray, lhsArray, seqv.size(),
                                        &seqv.front(), rhsArray));

    ASSERT_SUCCESS(fly_create_array(&lhsArray, &(in.front()), dims1.ndims(),
                                   dims1.get(),
                                   (fly_dtype)dtype_traits<float>::fly_type));

    ASSERT_EQ(FLY_ERR_ARG,
              fly_assign_seq(&outArray, lhsArray, 0, &seqv.front(), rhsArray));

    ASSERT_EQ(FLY_ERR_TYPE, fly_assign_seq(&outArray, lhsArray, seqv.size(),
                                         &seqv.front(), rhsArray));
...
TEST(ArrayAssign, CPP_ASSIGN_TO_INDEXED) {
    ...
    array input(10, 2, &in.front(), flyHost);
...
TEST(Asssign, LinearCPPMaxDim) {
    ...
    array a_copy  = a.copy();
    fly::index idx = array(seq(10, largeDim + 10));
    a(span, idx)  = val;
...
TEST(Asssign, LinearAssignSeq) {
    ...
    fly_array rhs_arr = rhs.get();
    fly_array out_arr;

    ASSERT_SUCCESS(fly_assign_seq(&out_arr, in_arr, 1, &ii.idx.seq, rhs_arr));
...
TEST(Asssign, LinearAssignGenSeq) {
    ...
    ASSERT_SUCCESS(fly_assign_gen(&out_arr, in_arr, 1, &ii, rhs_arr));
...
TEST(Index, ISSUE_2533) {
    ...
    fly::array a = constant(0, 5, 10);
    fly::array b = constant(1, 5, 10);

    a(fly::span, fly::seq(0, 5)) = b(fly::span, fly::seq(0, 5));
