...
#define MINMAXOP(fn, ty)                                         \
    TEST(IndexedReduce, fn##_##ty##_0) {                         \
        ...
        dtype dty    = (dtype)dtype_traits<ty>::fly_type;         \
        const int nx = 10;                                       \
        const int ny = 100;                                      \
        ...
        ASSERT_VEC_ARRAY_EQ(gold, fly::dim4(1, ny), val);         \
        ASSERT_VEC_ARRAY_EQ(igold, fly::dim4(1, ny), val);        \
        fly_free_host(h_in_st);                                   \
        fly_free_host(h_idx);                                     \
    }                                                            \
