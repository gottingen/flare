...
void medfiltTest(string pTestFile, dim_t w_len, dim_t w_wid,
    ...
    ASSERT_SUCCESS(fly_medfilt2(&outArray, inArray, w_len, w_wid, pad));
...
template<typename T>
void medfilt1_Test(string pTestFile, dim_t w_wid, fly_border_type pad) {
    SUPPORTED_TYPE_CHECK(T);
    ...
    ASSERT_SUCCESS(fly_medfilt1(&outArray, inArray, w_wid, pad));
...
void medfiltImageTest(string pTestFile, dim_t w_len, dim_t w_wid) {
    ...
    for (size_t testId = 0; testId < testCount; ++testId) {
        ...
        ASSERT_SUCCESS(
            fly_medfilt2(&outArray, inArray, w_len, w_wid, FLY_PAD_ZERO));
...
void medfiltInputTest(void) {
    ...
    ASSERT_SUCCESS(fly_medfilt2(&outArray, inArray, 1, 1, FLY_PAD_ZERO));

    bool medfilt1;
    ASSERT_SUCCESS(fly_is_vector(&medfilt1, outArray));
...
void medfiltWindowTest(void) {
    ...
    ASSERT_EQ(FLY_ERR_ARG, fly_medfilt2(&outArray, inArray, 3, 5, FLY_PAD_ZERO));
...
void medfilt1d_WindowTest(void) {
    ...
    ASSERT_EQ(FLY_ERR_ARG, fly_medfilt1(&outArray, inArray, -1, FLY_PAD_ZERO));
...
void medfiltPadTest(void) {
    ...
    ASSERT_EQ(FLY_ERR_ARG,
              fly_medfilt2(&outArray, inArray, 3, 3, fly_border_type(3)));

    ASSERT_EQ(FLY_ERR_ARG,
              fly_medfilt2(&outArray, inArray, 3, 3, fly_border_type(-1)));
...
void medfilt1d_PadTest(void) {
    ...
    ASSERT_EQ(FLY_ERR_ARG,
              fly_medfilt1(&outArray, inArray, 3, fly_border_type(3)));

    ASSERT_EQ(FLY_ERR_ARG,
              fly_medfilt1(&outArray, inArray, 3, fly_border_type(-1)));
...
TEST(MedianFilter, CPP) {
    ...
    array output = medfilt(input, w_len, w_wid, FLY_PAD_SYM);
...
TEST(MedianFilter1d, CPP) {
    ...
    dim4 dims = numDims[0];
    array input(dims, &(in[0].front()));
    array output = medfilt1(input, w_wid, FLY_PAD_SYM);
...
TEST(MedianFilter, Docs) {
    ...
    array b = medfilt(a, 3, 3, FLY_PAD_ZERO);
    // fly_print(b);
