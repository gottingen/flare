...
void memAllocArrayScopeTest(int elements) {
    ...
    {
        array a = randu(elements, (fly_dtype)dtype_traits<T>::fly_type);
...
TEST(Memory, Assign2D) {
    ...
    cleanSlate();  // Clean up everything done so far
    {
        array a       = fly::randu(10, 10, f32);
        unsigned hb[] = {3, 5, 6, 8, 9};
        array b(5, hb);
        array c = fly::randu(5, f32);
        deviceMemInfo(&alloc_bytes, &alloc_buffers, &lock_bytes, &lock_buffers);
        a(b) = c;
    }
...
template<typename T>
T *getMemoryManagerPayload(fly_memory_manager manager) {
    void *payloadPtr;
    fly_memory_manager_get_payload(manager, &payloadPtr);
    return (T *)payloadPtr;
}
...
fly_err allocated_fn(fly_memory_manager manager, size_t *out, void *ptr) {
...
fly_err user_lock_fn(fly_memory_manager manager, void *ptr) {
...
fly_err is_user_locked_fn(fly_memory_manager manager, int *out, void *ptr) {
    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
    *out          = payload->locked.find(ptr) != payload->locked.end();
    return FLY_SUCCESS;
}

fly_err unlock_fn(fly_memory_manager manager, void *ptr, int userLock) {
    if (!ptr) { return FLY_SUCCESS; }

    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);

    if (payload->table.find(ptr) == payload->table.end()) {
        return FLY_SUCCESS;  // fast path
    }

    // For testing, treat user-allocated and FLY-allocated memory identically
    if (payload->locked.find(ptr) != payload->locked.end()) {
...
fly_err user_unlock_fn(fly_memory_manager manager, void *ptr) {
    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
    fly_err err    = unlock_fn(manager, ptr, /* user */ 1);
    payload->lockedBytes -= payload->table[ptr];
    return err;
}

fly_err signal_memory_cleanup_fn(fly_memory_manager manager) {
    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
    ...
    for (auto &entry : payload->table) {
        ...
        if (!isUserLocked) {
            ...
            fly_memory_manager_native_free(manager, ptr);
...
fly_err print_info_fn(fly_memory_manager manager, char *c, int b) {
    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
...
fly_err get_memory_pressure_fn(fly_memory_manager manager, float *out) {
    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
...
fly_err jit_tree_exceeds_memory_pressure_fn(fly_memory_manager manager, int *out,
                                           size_t bytes) {
    auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
...
fly_err alloc_fn(fly_memory_manager manager, void **ptr,
    ...
    if (size > 0) {
        ...
        float threshold;
        fly_memory_manager_get_memory_pressure_threshold(manager, &threshold);
        if (pressure >= threshold) { signal_memory_cleanup_fn(manager); }

        if (fly_err err = fly_memory_manager_native_alloc(manager, ptr, size)) {
            return err;
        }
        ...
        // Simple implementation: treat user and FLY allocations the same
        payload->locked.insert(*ptr);
...
void add_memory_management_fn(fly_memory_manager manager, int id) {}

void remove_memory_management_fn(fly_memory_manager manager, int id) {}
...
class MemoryManagerApi : public ::testing::Test {
   public:
    ...
    void SetUp() override {
        ...
        // Set payload_fn
        fly_memory_manager_set_payload(manager, payload.get());

        auto initialize_fn = [](fly_memory_manager manager) {
            auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
            payload->initializeCalledTimes++;
            return FLY_SUCCESS;
        };
        fly_memory_manager_set_initialize_fn(manager, initialize_fn);

        auto shutdown_fn = [](fly_memory_manager manager) {
            auto *payload = getMemoryManagerPayload<E2ETestPayload>(manager);
            payload->shutdownCalledTimes++;
            return FLY_SUCCESS;
        };
        fly_memory_manager_set_shutdown_fn(manager, shutdown_fn);

        // alloc
        fly_memory_manager_set_alloc_fn(manager, alloc_fn);
        fly_memory_manager_set_allocated_fn(manager, allocated_fn);
        fly_memory_manager_set_unlock_fn(manager, unlock_fn);
        // utils
        fly_memory_manager_set_signal_memory_cleanup_fn(
            manager, signal_memory_cleanup_fn);
        fly_memory_manager_set_print_info_fn(manager, print_info_fn);
        // user lock/unlock
        fly_memory_manager_set_user_lock_fn(manager, user_lock_fn);
        fly_memory_manager_set_user_unlock_fn(manager, user_unlock_fn);
        fly_memory_manager_set_is_user_locked_fn(manager, is_user_locked_fn);
        // memory pressure
        fly_memory_manager_set_get_memory_pressure_fn(manager,
                                                     get_memory_pressure_fn);
        fly_memory_manager_set_jit_tree_exceeds_memory_pressure_fn(
            manager, jit_tree_exceeds_memory_pressure_fn);
        // ocl
        fly_memory_manager_set_add_memory_management_fn(
            manager, add_memory_management_fn);
        fly_memory_manager_set_remove_memory_management_fn(
    ...
    void TearDown() override {
        ...
        fly_release_memory_manager(manager);
    }
};
...
TEST_F(MemoryManagerApi, E2ETest2D) {
    ...
    fly::array a = fly::array(aSize, aSize, fly::dtype::f32);
    ASSERT_EQ(payload->table.size(), 1);
    ...
    ASSERT_EQ(payload->lastNdims, 1);
    ASSERT_EQ(payload->lastDims, fly::dim4(aSize * aSize));
}

TEST_F(MemoryManagerApi, E2ETest3D) {
    size_t aSize = 8;

    fly::array a = fly::array(aSize, aSize, aSize, fly::dtype::f32);
    ASSERT_EQ(payload->table.size(), 1);
    ...
    // This behavior will change in the future
    ASSERT_EQ(payload->lastNdims, 1);
    ASSERT_EQ(payload->lastDims, fly::dim4(aSize * aSize * aSize));
}

TEST_F(MemoryManagerApi, E2ETest4D) {
    size_t aSize = 8;

    fly::array a = fly::array(aSize, aSize, aSize, aSize, fly::dtype::f32);
    ASSERT_EQ(payload->table.size(), 1);
    ASSERT_EQ(payload->table[a.device<float>()],
    ...
    // This behavior will change in the future
    ASSERT_EQ(payload->lastNdims, 1);
    ASSERT_EQ(payload->lastDims, fly::dim4(aSize * aSize * aSize * aSize));
    fly::sync();
}
...
TEST_F(MemoryManagerApi, E2ETest4DComplexDouble) {
    ...
    fly::array a = fly::array(aSize, aSize, aSize, aSize, fly::dtype::c64);
    ASSERT_EQ(payload->table.size(), 1);
    ASSERT_EQ(payload->table[a.device<float>()],
...
TEST_F(MemoryManagerApi, E2ETestMultipleAllocations) {
    ...
    fly::array a = fly::array(aSize, fly::dtype::c64);
    ASSERT_EQ(payload->lastElementSize, 16);

    fly::array b = fly::array(aSize, fly::dtype::f64);
...
TEST_F(MemoryManagerApi, OutOfMemory) {
    ...
    try {
        a = fly::randu({N, N, N}, fly::dtype::f32);
        FAIL();
    } catch (fly::exception &ex) {
        ASSERT_EQ(ex.err(), FLY_ERR_NO_MEM);
    } catch (...) { FAIL(); }
}
...
TEST(MemoryManagerE2E, E2ETest) {
    ...
    fly_memory_manager_set_unlock_fn(manager, unlock_fn);
    // utils
    fly_memory_manager_set_signal_memory_cleanup_fn(manager,
    ...
    fly_memory_manager_set_jit_tree_exceeds_memory_pressure_fn(
        manager, jit_tree_exceeds_memory_pressure_fn);
    // ocl
    fly_memory_manager_set_add_memory_management_fn(manager,
    ...
    fly_set_memory_manager(manager);
    {
        ...
        void *a = fly::allocV2(aSize * sizeof(float));
        ASSERT_EQ(payload->table.size(), 1);

        ASSERT_EQ(payload->table[a], aSize * sizeof(float));
        ASSERT_EQ(payload->lastNdims, 1);
        ASSERT_EQ(payload->lastDims, fly::dim4(aSize) * sizeof(float));
        ASSERT_EQ(payload->lastElementSize, 1);

        dim_t bDim = 2;
        auto b     = fly::randu({bDim, bDim});
        ...
        ASSERT_EQ(payload->lastNdims, 1);
        ASSERT_EQ(payload->lastDims, fly::dim4(bDim * b.numdims()));
    ...
    fly::printMemInfo(printInfoMsg.c_str(), printInfoDeviceId);
    ...
    ASSERT_THROW(fly::setMemStepSize(500), fly::exception);
    ASSERT_THROW(fly::getMemStepSize(), fly::exception);

    ASSERT_EQ(payload->table.size(), 0);
    fly_unset_memory_manager();
    fly_release_memory_manager(manager);
    ASSERT_EQ(payload->initializeCalledTimes, 1);
    ASSERT_EQ(payload->shutdownCalledTimes, fly::getDeviceCount());
}

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
TEST(Memory, AfAllocDeviceCPUC) {
    fly_backend active_backend;
    ASSERT_SUCCESS(fly_get_active_backend(&active_backend));

    if (active_backend == FLY_BACKEND_CPU) {
        void *ptr;
...
TEST(Memory, AfAllocDeviceV2CPUC) {
    ...
    if (active_backend == FLY_BACKEND_CPU) {
        void *ptr;
        ASSERT_SUCCESS(fly_alloc_device_v2(&ptr, sizeof(float)));

        // This is the CPU backend so we can assign to the pointer
        *static_cast<float *>(ptr) = 5;
        ASSERT_SUCCESS(fly_free_device_v2(ptr));
    }
}
...
TEST(Memory, SNIPPET_AllocCPU) {
    ...
    if (active_backend == FLY_BACKEND_CPU) {
        ...
        void *ptr   = fly::allocV2(sizeof(float));
