...
void MatMulCheck(string TestFile) {
    ...
    fly_array a, aT, b, bT;
    ASSERT_SUCCESS(fly_create_array(&a, &hData[0].front(), numDims[0].ndims(),
                                   numDims[0].get(),
    ...
    ASSERT_SUCCESS(fly_moddims(&aT, a, atdims.ndims(), atdims.get()));
    ASSERT_SUCCESS(fly_create_array(&b, &hData[1].front(), numDims[1].ndims(),
                                   numDims[1].get(),
                                   (fly_dtype)dtype_traits<T>::fly_type));
    ...
    ASSERT_SUCCESS(fly_moddims(&bT, b, btdims.ndims(), btdims.get()));

    vector<fly_array> out(tests.size(), 0);
    if (isBVector) {
        ASSERT_SUCCESS(fly_matmul(&out[0], aT, b, FLY_MAT_NONE, FLY_MAT_NONE));
        ASSERT_SUCCESS(fly_matmul(&out[1], bT, a, FLY_MAT_NONE, FLY_MAT_NONE));
        ASSERT_SUCCESS(fly_matmul(&out[2], b, a, FLY_MAT_TRANS, FLY_MAT_NONE));
        ASSERT_SUCCESS(fly_matmul(&out[3], bT, aT, FLY_MAT_NONE, FLY_MAT_TRANS));
        ASSERT_SUCCESS(fly_matmul(&out[4], b, aT, FLY_MAT_TRANS, FLY_MAT_TRANS));
    } else {
        ASSERT_SUCCESS(fly_matmul(&out[0], a, b, FLY_MAT_NONE, FLY_MAT_NONE));
        ASSERT_SUCCESS(fly_matmul(&out[1], a, bT, FLY_MAT_NONE, FLY_MAT_TRANS));
        ASSERT_SUCCESS(fly_matmul(&out[2], a, bT, FLY_MAT_TRANS, FLY_MAT_NONE));
        ASSERT_SUCCESS(fly_matmul(&out[3], aT, bT, FLY_MAT_TRANS, FLY_MAT_TRANS));
    }

    for (size_t i = 0; i < tests.size(); i++) {
        dim4 dd;
        dim_t *d = dd.get();
        fly_get_dims(&d[0], &d[1], &d[2], &d[3], out[i]);
        ASSERT_VEC_ARRAY_NEAR(tests[i], dd, out[i], 1e-3);
    }

    ASSERT_SUCCESS(fly_release_array(a));
    ASSERT_SUCCESS(fly_release_array(aT));
    ASSERT_SUCCESS(fly_release_array(b));
    ASSERT_SUCCESS(fly_release_array(bT));

    for (size_t i = 0; i < out.size(); i++) {
        ASSERT_SUCCESS(fly_release_array(out[i]));
    }
}
...
void cppMatMulCheck(string TestFile) {
    ...
    if (isBVector) {
        out[0] = matmul(aT, b, FLY_MAT_NONE, FLY_MAT_NONE);
        out[1] = matmul(bT, a, FLY_MAT_NONE, FLY_MAT_NONE);
        out[2] = matmul(b, a, FLY_MAT_TRANS, FLY_MAT_NONE);
        out[3] = matmul(bT, aT, FLY_MAT_NONE, FLY_MAT_TRANS);
        out[4] = matmul(b, aT, FLY_MAT_TRANS, FLY_MAT_TRANS);
    } else {
        out[0] = matmul(a, b, FLY_MAT_NONE, FLY_MAT_NONE);
        out[1] = matmul(a, bT, FLY_MAT_NONE, FLY_MAT_TRANS);
        out[2] = matmul(a, bT, FLY_MAT_TRANS, FLY_MAT_NONE);
        out[3] = matmul(aT, bT, FLY_MAT_TRANS, FLY_MAT_TRANS);
    }
...
TEST(MatrixMultiply, float) {
    ...
    fly_gemm(&C32, FLY_MAT_NONE, FLY_MAT_NONE, &alpha32, A32.get(), B32.get(),
            &beta32);
    array expected32 = array(3, 3, h_gold);
    ASSERT_ARRAYS_NEAR(expected32, fly::array(C32), 0.0001);
}
...
TEST(MatrixMultiply, half) {
...

    {
        ...
        ASSERT_SUCCESS(fly_gemm(&C16, FLY_MAT_NONE, FLY_MAT_NONE, &alpha16,
...
struct test_params {
    fly_mat_prop opt_lhs;
    fly_mat_prop opt_rhs;
    float *alpha;
    float *h_lhs;
    float *h_rhs;
    float *h_gold;
    ...
    test_params(fly_mat_prop optl, fly_mat_prop optr, float *a, float *l,
...
class Gemm : public ::testing::TestWithParam<test_params> {
   protected:
    ...
    void SetUp() {
        ...
        ASSERT_SUCCESS(fly_create_array(&lhs, params.h_lhs,
                                       params.lhs_dims.ndims(),
                                       params.lhs_dims.get(), f32));
        ASSERT_SUCCESS(fly_create_array(&rhs, params.h_rhs,
                                       params.rhs_dims.ndims(),
                                       params.rhs_dims.get(), f32));

        dim_t gold_dim0 = params.opt_lhs == FLY_MAT_TRANS ? params.lhs_dims[1]
                                                         : params.lhs_dims[0];
        dim_t gold_dim1 = params.opt_rhs == FLY_MAT_TRANS ? params.rhs_dims[0]
                                                         : params.rhs_dims[1];
        ...
        ASSERT_SUCCESS(fly_create_array(&gold, params.h_gold, gold_dims.ndims(),
                                       gold_dims.get(), f32));
    }

    void TearDown() {
        if (gold != 0) { ASSERT_SUCCESS(fly_release_array(gold)); }
        if (rhs != 0) { ASSERT_SUCCESS(fly_release_array(rhs)); }
        if (lhs != 0) { ASSERT_SUCCESS(fly_release_array(lhs)); }
    }
};
...
string out_info(const ::testing::TestParamInfo<Gemm::ParamType> info) {
    ...
    ss << "_";
    ss << (params.opt_lhs == FLY_MAT_TRANS ? "T" : "N");
    ss << (params.opt_rhs == FLY_MAT_TRANS ? "T" : "N");
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs, h_rhs, h_gold, dim4(3, 3), dim4(3, 3), dim4(3, 3), &beta, NULL_ARRAY     ),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs, h_rhs, h_gold, dim4(3, 3), dim4(3, 3), dim4(3, 3), &beta, FULL_ARRAY     ),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs, h_rhs, h_gold, dim4(3, 3), dim4(3, 3), dim4(3, 3), &beta, SUB_ARRAY      ),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs, h_rhs, h_gold, dim4(3, 3), dim4(3, 3), dim4(3, 3), &beta, REORDERED_ARRAY)
        ),
    out_info
    );
// clang-format on
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        //          lhs_opts     rhs_opts     alpha  lhs          rhs    gold          lhs_dims       rhs_dims    out_dims       beta  out_array_type
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs_batch, h_rhs, h_gold_batch, dim4(3, 3, 2), dim4(3, 3), dim4(3, 3, 2), &beta, NULL_ARRAY     ),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs_batch, h_rhs, h_gold_batch, dim4(3, 3, 2), dim4(3, 3), dim4(3, 3, 2), &beta, FULL_ARRAY     ),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs_batch, h_rhs, h_gold_batch, dim4(3, 3, 2), dim4(3, 3), dim4(3, 3, 2), &beta, SUB_ARRAY      ),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_lhs_batch, h_rhs, h_gold_batch, dim4(3, 3, 2), dim4(3, 3), dim4(3, 3, 2), &beta, REORDERED_ARRAY)
        ),
    out_info
    );
// clang-format on
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        //          lhs_opts      rhs_opts      alpha  lhs         rhs         gold       lhs_dims    rhs_dims    out_dims    beta  out_array_type
        test_params(FLY_MAT_NONE,  FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_wide, h_gold_NN, dim4(3, 2), dim4(2, 3), dim4(3, 3), &beta, NULL_ARRAY),
        test_params(FLY_MAT_NONE,  FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_tall, h_gold_NT, dim4(3, 2), dim4(3, 2), dim4(3, 3), &beta, NULL_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_tall, h_gold_TN, dim4(3, 2), dim4(3, 2), dim4(2, 2), &beta, NULL_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_wide, h_gold_TT, dim4(3, 2), dim4(2, 3), dim4(2, 2), &beta, NULL_ARRAY),

        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_half_ones, h_half_ones, h_gold_gemv, dim4(4, 5), dim4(5, 1), dim4(4, 1), &beta, NULL_ARRAY),
        test_params(FLY_MAT_NONE, FLY_MAT_NONE, &alpha, h_half_ones, h_half_ones, h_gold_gemv, dim4(1, 5), dim4(5, 1), dim4(1, 1), &beta, NULL_ARRAY),
        test_params(FLY_MAT_NONE, FLY_MAT_TRANS, &alpha, h_half_ones, h_half_ones, h_gold_gemv, dim4(4, 5), dim4(1, 5), dim4(4, 1), &beta, NULL_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_NONE, &alpha, h_half_ones, h_half_ones, h_gold_gemv, dim4(5, 4), dim4(5, 1), dim4(4, 1), &beta, NULL_ARRAY),

        test_params(FLY_MAT_NONE,  FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_wide, h_gold_NN, dim4(3, 2), dim4(2, 3), dim4(3, 3), &beta, FULL_ARRAY),
        test_params(FLY_MAT_NONE,  FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_tall, h_gold_NT, dim4(3, 2), dim4(3, 2), dim4(3, 3), &beta, FULL_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_tall, h_gold_TN, dim4(3, 2), dim4(3, 2), dim4(2, 2), &beta, FULL_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_wide, h_gold_TT, dim4(3, 2), dim4(2, 3), dim4(2, 2), &beta, FULL_ARRAY),

        test_params(FLY_MAT_NONE,  FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_wide, h_gold_NN, dim4(3, 2), dim4(2, 3), dim4(3, 3), &beta, SUB_ARRAY),
        test_params(FLY_MAT_NONE,  FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_tall, h_gold_NT, dim4(3, 2), dim4(3, 2), dim4(3, 3), &beta, SUB_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_tall, h_gold_TN, dim4(3, 2), dim4(3, 2), dim4(2, 2), &beta, SUB_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_wide, h_gold_TT, dim4(3, 2), dim4(2, 3), dim4(2, 2), &beta, SUB_ARRAY),

        test_params(FLY_MAT_NONE,  FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_wide, h_gold_NN, dim4(3, 2), dim4(2, 3), dim4(3, 3), &beta, REORDERED_ARRAY),
        test_params(FLY_MAT_NONE,  FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_tall, h_gold_NT, dim4(3, 2), dim4(3, 2), dim4(3, 3), &beta, REORDERED_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_NONE,  &alpha, h_lhs_tall, h_rhs_tall, h_gold_TN, dim4(3, 2), dim4(3, 2), dim4(2, 2), &beta, REORDERED_ARRAY),
        test_params(FLY_MAT_TRANS, FLY_MAT_TRANS, &alpha, h_lhs_tall, h_rhs_wide, h_gold_TT, dim4(3, 2), dim4(2, 3), dim4(2, 2), &beta, REORDERED_ARRAY)
        ),
    out_info
    );
// clang-format on
...
TEST_P(Gemm, UsePreallocatedOutArray) {
    test_params params = GetParam();
    ASSERT_SUCCESS(fly_gemm(&out, params.opt_lhs, params.opt_rhs, params.alpha,
                           lhs, rhs, params.beta));
...
TEST(Gemm, DocSnippet) {
    ...
    dim_t adims[] = {5, 3, 2};
    dim_t bdims[] = {3, 5, 2};
    fly_constant(&A, 1, 3, adims, f32);
    fly_constant(&B, 1, 3, bdims, f32);
    ...
    // Undefined behavior!
    // fly_array undef;
    // fly_gemm(&undef, FLY_MAT_NONE, FLY_MAT_NONE, &alpha, a.get(), b.get(),
    // &beta);

    fly_array C = 0;
    fly_gemm(&C, FLY_MAT_NONE, FLY_MAT_NONE, &alpha, A, B, &beta);
    // C =
    //  3.   3.   3.   3.   3.
    //  3.   3.   3.   3.   3.
    //  3.   3.   3.   3.   3.
    ...
    fly_array c1_copy = 0;
    ASSERT_SUCCESS(fly_retain_array(&c1_copy, C));
    fly::array c1(c1_copy);
    fly::array gold1 = fly::constant(3, 5, 5, 2, f32);
    ASSERT_ARRAYS_EQ(gold1, c1);

    //! [ex_fly_gemm_overwrite]
    alpha                = 1.f;
    beta                 = 1.f;
    fly_seq first_slice[] = {fly_span, fly_span, {0., 0., 1.}};
    fly_array Asub, Bsub, Csub;
    fly_index(&Asub, A, 3, first_slice);
    fly_index(&Bsub, B, 3, first_slice);
    fly_index(&Csub, C, 3, first_slice);
    fly_gemm(&Csub, FLY_MAT_NONE, FLY_MAT_NONE, &alpha, Asub, Bsub, &beta);
    // C =
    //  6.   6.   6.   6.   6.
    //  6.   6.   6.   6.   6.
    //  6.   6.   6.   6.   6.
    ...
    fly_array c2_copy = 0;
    ASSERT_SUCCESS(fly_retain_array(&c2_copy, C));
    fly::array c2(c2_copy);
    ...
    fly_release_array(B);
    fly_release_array(C);
    fly_release_array(Asub);
    fly_release_array(Bsub);
    fly_release_array(Csub);
