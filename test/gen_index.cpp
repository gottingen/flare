...
using index_params = std::tuple<index_test, fly_dtype, fly_dtype>;
...
class IndexGeneralizedLegacy : public ::testing::TestWithParam<index_params> {
    void SetUp() {
        ...
        fly_array inTmp = 0;
        ASSERT_SUCCESS(fly_create_array(&inTmp, &(in[0].front()), dims0.ndims(),
                                       dims0.get(), f32));

        ASSERT_SUCCESS(fly_cast(&inArray_, inTmp, get<1>(params)));
        fly_release_array(inTmp);

        fly_array idxTmp = 0;
        ASSERT_SUCCESS(fly_create_array(&idxTmp, &(in[1].front()), dims1.ndims(),
                                       dims1.get(), f32));
        ASSERT_SUCCESS(fly_cast(&idxArray_, idxTmp, get<2>(params)));
        fly_release_array(idxTmp);

        vector<float> hgold = tests[0];
        fly_array goldTmp;
        fly_create_array(&goldTmp, &hgold.front(), get<0>(params).dims_.ndims(),
                        get<0>(params).dims_.get(), f32);
        ASSERT_SUCCESS(fly_cast(&gold_, goldTmp, get<1>(params)));
        fly_release_array(goldTmp);
    }

    void TearDown() {
        if (inArray_) { ASSERT_SUCCESS(fly_release_array(inArray_)); }
        if (idxArray_) { ASSERT_SUCCESS(fly_release_array(idxArray_)); }
        if (gold_) { ASSERT_SUCCESS(fly_release_array(gold_)); }
    }
...
TEST_P(IndexGeneralizedLegacy, SSSA) {
    ...
    fly_index_t indexes[4];
    indexes[0].idx.seq = fly_make_seq(0, 3, 1);
    indexes[1].idx.seq = fly_make_seq(0, 1, 1);
    indexes[2].idx.seq = fly_make_seq(1, 2, 1);
    indexes[3].idx.arr = idxArray_;
    indexes[0].isSeq   = true;
    indexes[1].isSeq   = true;
    indexes[2].isSeq   = true;
    indexes[3].isSeq   = false;
    ASSERT_SUCCESS(fly_index_gen(&outArray, inArray_, 4, indexes));
    ASSERT_ARRAYS_EQ(gold_, outArray);
    fly_release_array(outArray);
}
...
void testGeneralIndexOneArray(string pTestFile, const dim_t ndims,
    ...
    ASSERT_SUCCESS(fly_index_gen(&outArray, inArray, ndims, indexs));
...
TEST(GeneralIndex, ASSS) {
    ...
    indexs[2].idx.seq = fly_span;
    indexs[3].idx.seq = fly_span;
...
TEST(GeneralIndex, SASS) {
    ...
    indexs[0].idx.seq = fly_make_seq(10, 40, 1);
    indexs[0].isSeq   = true;
...
TEST(GeneralIndex, AASS) {
    ...
    ASSERT_SUCCESS(fly_create_array(&idxArray0, &(in[1].front()), dims1.ndims(),
                                   dims1.get(),
                                   (fly_dtype)dtype_traits<float>::fly_type));
    indexs[0].isSeq   = false;
    indexs[0].idx.arr = idxArray0;

    ASSERT_SUCCESS(fly_create_array(&idxArray1, &(in[2].front()), dims2.ndims(),
                                   dims2.get(),
                                   (fly_dtype)dtype_traits<float>::fly_type));
    indexs[1].isSeq   = false;
    indexs[1].idx.arr = idxArray1;

    ASSERT_SUCCESS(fly_index_gen(&outArray, inArray, 2, indexs));
