...
typedef fly_err (*reduceFunc)(fly_array *, const fly_array, const int);

template<typename Ti, typename To, reduceFunc fly_reduce>
void reduceTest(string pTestFile, int off = 0, bool isSubRef = false,
    ...
    if (isSubRef) {
        ASSERT_SUCCESS(
            fly_create_array(&tempArray, &in.front(), dims.ndims(), dims.get(),
                            (fly_dtype)fly::dtype_traits<Ti>::fly_type));
        ASSERT_SUCCESS(
            fly_index(&inArray, tempArray, seqv.size(), &seqv.front()));
        ASSERT_SUCCESS(fly_release_array(tempArray));
    } else {
        ASSERT_SUCCESS(
            fly_create_array(&inArray, &in.front(), dims.ndims(), dims.get(),
                            (fly_dtype)fly::dtype_traits<Ti>::fly_type));
    }
    ...
    for (int d = 0; d < (int)tests.size(); ++d) {
        ...
        // Run sum
        ASSERT_SUCCESS(fly_reduce(&outArray, inArray, d + off));

        fly_dtype t;
        fly_get_type(&t, outArray);
...
template<>
struct promote_type<uchar, fly_sum> {
    typedef uint type;
};
template<>
struct promote_type<char, fly_sum> {
    typedef uint type;
};
template<>
struct promote_type<short, fly_sum> {
    typedef int type;
};
template<>
struct promote_type<ushort, fly_sum> {
    typedef uint type;
};
template<>
struct promote_type<uchar, fly_product> {
    typedef uint type;
};
template<>
struct promote_type<char, fly_product> {
    typedef uint type;
};
template<>
struct promote_type<short, fly_product> {
    typedef int type;
};
template<>
struct promote_type<ushort, fly_product> {
    typedef uint type;
};

#define REDUCE_TESTS(FN)                                                       \
    TYPED_TEST(Reduce, Test_##FN) {                                            \
        reduceTest<TypeParam, typename promote_type<TypeParam, fly_##FN>::type, \
                   fly_##FN>(string(TEST_DIR "/reduce/" #FN ".test"));          \
    }
...
#define REDUCE_TESTS(FN, OT)                          \
    TYPED_TEST(Reduce, Test_##FN) {                   \
        reduceTest<TypeParam, OT, fly_##FN>(           \
...
TEST(Reduce, Test_Reduce_Big0) {
    reduceTest<int, int, fly_sum>(string(TEST_DIR "/reduce/big0.test"), 0);
}

/*
TEST(Reduce,Test_Reduce_Big1)
{
    reduceTest<int, int, fly_sum>(
...
/////////////////////////////////// CPP //////////////////////////////////
//
typedef fly::array (*ReductionOp)(const fly::array &, const int);
...
struct reduce_by_key_params {
    ...
    void *oKeys_;
    void *oVals_;
    fly_dtype kType_, vType_, oType_;
    string testname_;
...
struct reduce_by_key_params_t : public reduce_by_key_params {
    ...
    reduce_by_key_params_t(vector<Tk> ikeys, vector<Tv> ivals, vector<Tk> okeys,
        ...
        reduce_by_key_params::vType_ = (fly_dtype)fly::dtype_traits<Tv>::fly_type;
        reduce_by_key_params::kType_ = (fly_dtype)fly::dtype_traits<Tk>::fly_type;
        reduce_by_key_params::oType_ = (fly_dtype)fly::dtype_traits<To>::fly_type;
        reduce_by_key_params::testname_ = testname_;
    }
    ~reduce_by_key_params_t() {}
};

array ptrToArray(size_t size, void *ptr, fly_dtype type) {
    array res;
    switch (type) {
...
array ptrToArray(fly::dim4 size, void *ptr, fly_dtype type) {
    array res;
    switch (type) {
        case f32: res = array(size, (float *)ptr); break;
        case f64: res = array(size, (double *)ptr); break;
...
string testNameGenerator(
    ...
    fly_dtype kt = info.param->kType_;
    fly_dtype vt = info.param->vType_;
...
void reduce_by_key_test(std::string test_fn) {
    ...
    for (size_t t = 0; t < numDims.size() / 2; ++t) {
        ...
        ASSERT_EQ(
            FLY_SUCCESS,
            fly_create_array(&inKeys, &in_keys.front(), kdim.ndims(), kdim.get(),
                            (fly_dtype)fly::dtype_traits<int>::fly_type));
        ASSERT_EQ(
            FLY_SUCCESS,
            fly_create_array(&inVals, &in_vals.front(), vdim.ndims(), vdim.get(),
        ...
        // Run sum
        ASSERT_EQ(FLY_SUCCESS,
                  fly_sum_by_key(&outKeys, &outVals, inKeys, inVals, 0));

        dim_t ok0, ok1, ok2, ok3;
        dim_t ov0, ov1, ov2, ov3;
        fly_get_dims(&ok0, &ok1, &ok2, &ok3, outKeys);
        fly_get_dims(&ov0, &ov1, &ov2, &ov3, outVals);
        ...
        ASSERT_EQ(FLY_SUCCESS,
                  fly_get_data_ptr((void *)&outKeysVec.front(), outKeys));
        ASSERT_EQ(FLY_SUCCESS,
                  fly_get_data_ptr((void *)&outValsVec.front(), outVals));
        ...
        ASSERT_EQ(FLY_SUCCESS, fly_release_array(outKeys));
        ASSERT_EQ(FLY_SUCCESS, fly_release_array(outVals));
        ASSERT_EQ(FLY_SUCCESS, fly_release_array(inKeys));
        ASSERT_EQ(FLY_SUCCESS, fly_release_array(inVals));
    }
}
...
TEST(ReduceByKey, nDim0ReduceByKey) {
    ...
    const int ntile = 2;
    vals            = tile(vals, fly::dim4(1, ntile, ntile, ntile));
...
TEST(ReduceByKey, nDim1ReduceByKey) {
    ...
    const int ntile = 2;
    vals            = tile(vals, fly::dim4(1, ntile, 1, 1));
    vals            = transpose(vals);
...
TEST(MinMax, MinCplxNaN) {
    ...
    array a = fly::complex(real_wnan, imag_wnan);

    float gold_min_real[] = {-0.5f, 0.2f};
    float gold_min_imag[] = {-0.3f, 0.1f};

    array min_val = fly::min(a);
...
struct ragged_params {
    ...
    int reduceDim_;
    fly_dtype lType_, vType_, oType_;
    string testname_;
...
struct ragged_params_t : public ragged_params {
    ...
    ragged_params_t(size_t reduce_dim_len, int reduce_dim, string testname)
        ...
        ragged_params::lType_        = (fly_dtype)fly::dtype_traits<Tl>::fly_type;
        ragged_params::vType_        = (fly_dtype)fly::dtype_traits<Tv>::fly_type;
        ragged_params::oType_        = (fly_dtype)fly::dtype_traits<To>::fly_type;
...
class RaggedReduceMaxRangeP : public ::testing::TestWithParam<ragged_params *> {
   public:
    ...
    void SetUp() {
        ...
        fly::dim4 rdim(3, 3, 3, 3);
        rdim[dim] = rdim_size;
        vals      = fly::range(rdim, dim, params->vType_);

        rdim[dim]   = 1;
        ragged_lens = fly::range(rdim, (dim > 0) ? 0 : 1, params->lType_) + 1;

        valsReducedGold = fly::range(rdim, (dim > 0) ? 0 : 1, params->oType_);
        idxsReducedGold = fly::range(rdim, (dim > 0) ? 0 : 1, params->lType_);
    }
...
string testNameGeneratorRagged(
    const ::testing::TestParamInfo<typename TestClass::ParamType> info) {
    fly_dtype lt = info.param->lType_;
    fly_dtype vt = info.param->vType_;
...
TEST(ReduceByKey, ISSUE_2955) {
    int N                  = 256;
    fly::array val          = fly::randu(N);
    fly::array key          = fly::range(fly::dim4(N), 0, fly::dtype::s32);
    key(seq(127, fly::end)) = 1;

    fly::array ok, ov;
    fly::sumByKey(ok, ov, key, val);
...
TEST(ReduceByKey, ISSUE_2955_dim) {
    int N                  = 256;
    fly::array val          = fly::randu(8, N);
    fly::array key          = fly::range(fly::dim4(N), 0, fly::dtype::s32);
    key(seq(127, fly::end)) = 1;

    fly::array ok, ov;
    fly::sumByKey(ok, ov, key, val, 1);
    ASSERT_EQ(ok.dims(0), 128);
...
TEST(ReduceByKey, ISSUE_3062) {
    ...
    fly::array ones  = fly::constant(1, N, u32);
    fly::array zeros = fly::constant(0, N, u32);

    fly::array okeys;
    fly::array ovalues;

    fly::sumByKey(okeys, ovalues, zeros, ones);
    ASSERT_EQ(ovalues.scalar<unsigned>(), 129);

    fly::countByKey(okeys, ovalues, zeros, ones);
    ASSERT_EQ(ovalues.scalar<unsigned>(), 129);

    // test reduction on non-zero dimension as well
    ones  = fly::constant(1, 2, N, u32);
    zeros = fly::constant(0, N, u32);

    fly::sumByKey(okeys, ovalues, zeros, ones, 1);
    ASSERT_EQ(ovalues.scalar<unsigned>(), 129);

    fly::countByKey(okeys, ovalues, zeros, ones, 1);
    ASSERT_EQ(ovalues.scalar<unsigned>(), 129);
}

TEST(Reduce, Test_Sum_Global_Array) {
    const int num = 513;
    array a       = fly::randn(num, 2, 33, 4);

    float res         = fly::sum<float>(a);
    array full_reduce = fly::sum<fly::array>(a);
...
TEST(Reduce, Test_Product_Global_Array) {
    const int num = 512;
    array a       = 1 + (0.005 * fly::randn(num, 2, 3, 4));

    float res         = fly::product<float>(a);
    array full_reduce = fly::product<fly::array>(a);
...
TEST(Reduce, Test_Count_Global_Array) {
    ...
    int res       = count<int>(b);
    array res_arr = count<fly::array>(b);
    char *h_b     = b.host<char>();
    unsigned gold = 0;
...
TEST(Reduce, Test_min_Global_Array) {
    ...
    const int num = 10000;
    array a       = fly::randn(num, 2, 3, 4, f64);
    double res    = min<double>(a);
    array res_arr = min<fly::array>(a);
    double *h_a   = a.host<double>();
    double gold   = std::numeric_limits<double>::max();
...
TEST(Reduce, Test_max_Global_Array) {
    const int num = 10000;
    array a       = fly::randn(num, 2, 3, 4);
    float res     = max<float>(a);
    array res_arr = max<fly::array>(a);
    float *h_a    = a.host<float>();
    float gold    = -std::numeric_limits<float>::max();
...
TEST(Reduce, Test_Sum_Global_Array_nanval) {
    ...
    double nanval     = 0.2;
    float res         = fly::sum<float>(a, nanval);
    array full_reduce = fly::sum<fly::array>(a, nanval);
...
TEST(Reduce, nanval_issue_3255) {
    ...
    SUPPORTED_TYPE_CHECK(double);
    char *info_str;
    fly_array ikeys, ivals, okeys, ovals;
    dim_t dims[1] = {8};

    int ikeys_src[8] = {0, 0, 1, 1, 1, 2, 2, 0};
    ASSERT_SUCCESS(fly_create_array(&ikeys, ikeys_src, 1, dims, u32));

    int i;
    for (i = 0; i < 8; i++) {
        double ivals_src[8] = {1, 2, 3, 4, 5, 6, 7, 8};
        ivals_src[i]        = NAN;
        ASSERT_SUCCESS(fly_create_array(&ivals, ivals_src, 1, dims, f64));

        ASSERT_SUCCESS(
            fly_product_by_key_nan(&okeys, &ovals, ikeys, ivals, 0, 1.0));
        fly::array ovals_cpp(ovals);
        ASSERT_FALSE(fly::anyTrue<bool>(fly::isNaN(ovals_cpp)));
        ASSERT_SUCCESS(fly_release_array(okeys));

        ASSERT_SUCCESS(fly_sum_by_key_nan(&okeys, &ovals, ikeys, ivals, 0, 1.0));
        ovals_cpp = fly::array(ovals);

        ASSERT_FALSE(fly::anyTrue<bool>(fly::isNaN(ovals_cpp)));
        ASSERT_SUCCESS(fly_release_array(ivals));
        ASSERT_SUCCESS(fly_release_array(okeys));
    }
    ASSERT_SUCCESS(fly_release_array(ikeys));
}
