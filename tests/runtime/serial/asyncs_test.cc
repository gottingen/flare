...
void subflow_async(size_t W) {
  ...
  auto S1 = taskflow.emplace([&] (flare::rt::Subflow& sf){
    for(int i=0; i<1000; i++) {
      sf.async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    }
  });

  auto S2 = taskflow.emplace([&] (flare::rt::Subflow& sf){
...
void nested_subflow_async(size_t W) {
  ...
  taskflow.emplace([&](flare::rt::Subflow& sf1){

    for(int i=0; i<100; i++) {
      sf1.async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    }

    sf1.emplace([&](flare::rt::Subflow& sf2){
      for(int i=0; i<100; i++) {
      ...
      sf2.emplace([&](flare::rt::Subflow& sf3){
        for(int i=0; i<100; i++) {
          sf3.silent_async(
            "named", [&](){ counter.fetch_add(1, std::memory_order_relaxed); }
          );
...
void runtime_async(size_t W) {
  ...
  auto S1 = taskflow.emplace([&] (flare::rt::Runtime& sf){
    for(int i=0; i<1000; i++) {
      sf.silent_async(
  ...
  auto S2 = taskflow.emplace([&] (flare::rt::Runtime& sf){
  ...
  taskflow.emplace([&] (flare::rt::Runtime& sf){
    sf.silent_async([&](){ counter.fetch_add(1, std::memory_order_relaxed); });
    for(int i=0; i<1000; i++) {
      sf.async(
