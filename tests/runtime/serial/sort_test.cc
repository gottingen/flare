...
TEST_CASE("BubbleSort" * doctest::timeout(300)) {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=10; end <= 1000; end += 200) {
      ...
      flare::rt::Taskflow taskflow("BubbleSort");
      ...
      auto even_phase = taskflow.emplace([&](flare::rt::Subflow& sf){
      ...
      auto odd_phase = taskflow.emplace([&](flare::rt::Subflow& sf) {
...
TEST_CASE("SelectionSort" * doctest::timeout(300)) {
  ...
  std::function<
    void(flare::rt::Subflow& sf, std::vector<int>&, int, int, int&)
  > spawn;
  ...
  spawn = [&] (
    ...
    auto SL = sf.emplace(
      [&spawn, &data, beg, m, l=minl] (flare::rt::Subflow& sf) mutable {
    ...
    auto SR = sf.emplace(
      [&spawn, &data, m, end, r=minr] (flare::rt::Subflow& sf) mutable {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=16; end <= 512; end <<= 1) {
      flare::rt::Taskflow taskflow("SelectionSort");
      ...
      auto argmin = taskflow.emplace(
        [&spawn, &data, &beg, end, &min](flare::rt::Subflow& sf) mutable {
...
TEST_CASE("MergeSort" * doctest::timeout(300)) {
  ...
  std::function<void(flare::rt::Subflow& sf, std::vector<int>&, int, int)> spawn;

  spawn = [&] (flare::rt::Subflow& sf, std::vector<int>& data, int beg, int end) mutable {
    ...
    auto SL = sf.emplace([&spawn, &data, beg, m] (flare::rt::Subflow& sf) {
      spawn(sf, data, beg, m);
    }).name(std::string("[")
          + std::to_string(beg)
          + ':'
    ...
    auto SR = sf.emplace([&spawn, &data, m, end] (flare::rt::Subflow& sf) {
      spawn(sf, data, m, end);
    }).name(std::string("[")
          + std::to_string(m)
          + ':'
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=10; end <= 10000; end = end * 10) {
      flare::rt::Taskflow taskflow("MergeSort");
      ...
      taskflow.emplace([&spawn, &data, end](flare::rt::Subflow& sf){
        spawn(sf, data, 0, end);
      }).name(std::string("[0")
            + ":"
            + std::to_string(end) + ")");
...
TEST_CASE("QuickSort" * doctest::timeout(300)) {
  ...
  std::function<void(flare::rt::Subflow& sf, std::vector<int>&, itr_t, itr_t)> spawn;
  ...
  spawn = [&] (
    ...
    sf.emplace([&spawn, &data, beg, pvt] (flare::rt::Subflow& sf) {
    ...
    sf.emplace([&spawn, &data, pvt, end] (flare::rt::Subflow& sf) {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=16; end <= 16384; end <<= 1) {
      ...
      flare::rt::Taskflow taskflow("QuickSort");
      ...
      taskflow.emplace([&spawn, &data](flare::rt::Subflow& sf){
