...
TEST_CASE("Type" * doctest::timeout(300)) {
  ...
  flare::rt::Taskflow taskflow, taskflow2;

  auto t1 = taskflow.emplace([](){});
  auto t2 = taskflow.emplace([](){ return 1; });
  auto t3 = taskflow.emplace([](flare::rt::Subflow&){ });
  auto t4 = taskflow.composed_of(taskflow2);
  auto t5 = taskflow.emplace([](){ return flare::detail::SmallVector{1, 2}; });
  auto t6 = taskflow.emplace([](flare::rt::Runtime&){});
  auto t7 = taskflow.emplace([](flare::rt::Runtime&){ return 1; });
  auto t8 = taskflow.emplace([](flare::rt::Runtime&){ return flare::detail::SmallVector{1, 2}; });

  REQUIRE(t1.type() == flare::rt::TaskType::STATIC);
  REQUIRE(t2.type() == flare::rt::TaskType::CONDITION);
  REQUIRE(t3.type() == flare::rt::TaskType::DYNAMIC);
  REQUIRE(t4.type() == flare::rt::TaskType::MODULE);
  REQUIRE(t5.type() == flare::rt::TaskType::CONDITION);
  REQUIRE(t6.type() == flare::rt::TaskType::STATIC);
  REQUIRE(t7.type() == flare::rt::TaskType::CONDITION);
  REQUIRE(t8.type() == flare::rt::TaskType::CONDITION);
}
...
TEST_CASE("Builder" * doctest::timeout(300)) {
  ...
  std::vector<flare::rt::Task> silent_tasks;
  std::vector<flare::rt::Task> tasks;
...
TEST_CASE("Iterators" * doctest::timeout(300)) {
  ...
  SUBCASE("Order") {
    ...
    A.for_each_successor([&, i=0] (flare::rt::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == B);
        break;
    ...
    E.for_each_dependent([&, i=0](flare::rt::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == A);
        break;
  ...
  SUBCASE("Generic") {
    ...
    taskflow.for_each_task([&tasks](flare::rt::Task s){
      tasks.push_back(s);
    });
    ...
    A.for_each_successor([B](flare::rt::Task s){ REQUIRE(s==B); });
    B.for_each_dependent([A](flare::rt::Task s){ REQUIRE(s==A); });
    ...
    A.for_each_successor([&](flare::rt::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
    ...
    counter = a = b = c = d = e = 0;
    B.for_each_dependent([&](flare::rt::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
...
void sequential_runs(unsigned W) {
  ...
  SUBCASE("RunWithFuture") {
    ...
    auto B = f.emplace([&](flare::rt::Subflow& subflow){
      count ++;
    ...
    std::list<flare::rt::Future<void>> fu_list;
  ...
  SUBCASE("MultipleRuns") {
    ...
    flare::rt::Taskflow tf1, tf2, tf3, tf4;
...
TEST_CASE("RunAndWait.Simple") {
  ...
  flare::rt::Taskflow taskflow("Demo");
  ...
  flare::rt::Taskflow others;
  flare::rt::Task A = others.emplace([&](){ counter++; });
  flare::rt::Task B = others.emplace([&](){ counter++; });
  A.precede(B);

  // main taskflow
  flare::rt::Task C = taskflow.emplace([&](){
    executor.corun(others);
    REQUIRE(counter == 2);
  });
  flare::rt::Task D = taskflow.emplace([&](){
    executor.corun(others);
    REQUIRE(counter == 4);
  });
  C.precede(D);
...
TEST_CASE("RunAndWait.Complex") {
  ...
  std::array<flare::rt::Taskflow, N> taskflows;
...
void worker_id(unsigned w) {
  ...
  for(int i=0; i<1000; i++) {
    ...
    auto B = taskflow.emplace([&](flare::rt::Subflow& sf){
      auto id = executor.this_worker_id();
      REQUIRE(id>=0);
      REQUIRE(id< w);
      sf.emplace([&](){
...
void parallel_runs(unsigned w) {
  ...
  auto make_taskflow = [&] (flare::rt::Taskflow& tf) {
  ...
  SUBCASE("RunAndWaitForAll") {
    flare::rt::Executor executor(w);
    counter = 0;
    std::vector<std::unique_ptr<flare::rt::Taskflow>> taskflows(32);
    std::atomic<size_t> barrier(0);
    for(int t=0; t<32; t++) {
      threads.emplace_back([&, t=t] () {
        taskflows[t] = std::make_unique<flare::rt::Taskflow>();
...
TEST_CASE("Reduce" * doctest::timeout(300)) {
  ...
  const auto plus_test = [](unsigned num_workers, auto &&data){
    flare::rt::Executor executor(num_workers);
    flare::rt::Taskflow tf;
    int result {0};
...
void observer(unsigned w) {
  ...
  auto observer = executor.make_observer<flare::rt::ChromeObserver>();

  flare::rt::Taskflow taskflowA;
  std::vector<flare::rt::Task> tasks;
  // Static tasking
  for(auto i=0; i < 64; i ++) {
