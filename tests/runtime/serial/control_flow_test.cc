...
TEST_CASE("Cond.Types") {
  ...
  static_assert(flare::rt::is_condition_task_v<decltype(explicit_c)>);
  static_assert(flare::rt::is_condition_task_v<decltype(implicit_c)>);

  REQUIRE(explicit_task.type() == flare::rt::TaskType::CONDITION);
  REQUIRE(implicit_task.type() == flare::rt::TaskType::CONDITION);
}
...
void conditional_spawn(
  ...
  const int max_depth,
  int depth,
  flare::rt::Subflow& subflow
)  {
  if(depth < max_depth) {
    for(int i=0; i<2; i++) {
      auto A = subflow.emplace([&](){ counter++; });
      auto B = subflow.emplace(
        [&, max_depth, depth=depth+1](flare::rt::Subflow& subflow){
...
TEST_CASE("BTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    for(int l=1; l<12; l++) {
      flare::rt::Taskflow flow;
      std::vector<flare::rt::Task> prev_tasks;
...
TEST_CASE("DynamicBTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    ...
    flare::rt::Taskflow flow;
    flow.emplace([&](flare::rt::Subflow& subflow) {
      counter++;
...
void nested_cond(unsigned w) {
  ...
  flare::rt::Taskflow flow;
  auto S = flow.emplace([](){});
  auto A = flow.emplace([&] (flare::rt::Subflow& subflow) mutable {
    //         ___________
    //        |           |
    //        v           |
    //   S -> A -> B -> cond
    ...
    auto B = subflow.emplace([&](flare::rt::Subflow& subflow){
...
void hierarchical_condition(unsigned w) {
  ...
  flare::rt::Executor executor(w);
  flare::rt::Taskflow tf0("c0");
  flare::rt::Taskflow tf1("c1");
  flare::rt::Taskflow tf2("c2");
  flare::rt::Taskflow tf3("top");
  ...
  auto sbf1 = tf3.emplace([&](flare::rt::Subflow& sbf){
...
TEST_CASE("MultiCond.Types") {
  ...
  auto explicit_mc = [](){
    flare::detail::SmallVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> flare::detail::SmallVector<int> {
    return {1, 2, 3, 9};
  };
  ...
  static_assert(flare::rt::is_multi_condition_task_v<decltype(explicit_mc)>);
  static_assert(flare::rt::is_multi_condition_task_v<decltype(implicit_mc)>);
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> flare::detail::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> flare::detail::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> flare::detail::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> flare::detail::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> flare::detail::SmallVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
