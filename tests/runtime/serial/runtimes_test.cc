...
TEST_CASE("Runtime.Basics" * doctest::timeout(300)) {
  ...
  taskflow.emplace([&](flare::rt::Subflow& sf){
    sf.emplace([&](flare::rt::Runtime& rt){
      REQUIRE(&rt.executor() == &executor);
      rt.corun([&](flare::rt::Subflow& sf){
...
TEST_CASE("Runtime.Schedule.ModuleTask" * doctest::timeout(300)) {
  ...
  auto schedule = tf.emplace([&](flare::rt::Runtime& runtime) {
    value++;
...
TEST_CASE("Runtime.ExternalGraph.Simple" * doctest::timeout(300)) {
  ...
  std::vector<flare::rt::Taskflow> graphs(N);
  ...
  for(size_t i=0; i<N; i++) {
    ...
    taskflow.emplace([&res=results[i], &graph=graphs[i]](flare::rt::Runtime& rt)mutable{
      rt.corun(graph);
    });
  }
...
void runtime_subflow(size_t w) {
  ...
  for (size_t subtask = 0; subtask <= subtasks; subtask = subtask == 0 ? subtask + 1 : subtask*2) {
    ...
    std::vector<flare::rt::Task> rts;
    std::atomic<size_t> sums = 0;
    
    for (size_t i = 0; i < runtime_tasks_per_line * lines; ++i) {
      std::string rt_name = "rt-" + std::to_string(i);
      
      rts.emplace_back(taskflow.emplace([&sums, &subtask](flare::rt::Runtime& rt) {
        rt.corun([&sums, &subtask](flare::rt::Subflow& sf) {
...
void pipeline_sp_runtime_subflow(size_t w) {
  ...
  //for (subtask = 0; subtask <= subtasks; subtask = subtask == 0 ? subtask + 1 : subtask*2) {
    ...
    flare::rt::Pipeline pl(
      num_lines, 
      flare::rt::Pipe{
        flare::rt::PipeType::SERIAL, [max_tokens](flare::rt::Pipeflow& pf){
          //std::cout << flare::rt::stringify(pf.token(), '\n');
      ...
      flare::rt::Pipe{
        flare::rt::PipeType::PARALLEL, [subtask, &sums](flare::rt::Pipeflow&, flare::rt::Runtime& rt) {
          rt.corun([subtask, &sums](flare::rt::Subflow& sf) {
...
void pipeline_spspspsp_runtime_subflow(size_t w) {
  ...
  for (size_t subtask = 0; subtask <= subtasks; subtask = subtask == 0 ? subtask + 1 : subtask*2) {
    ...
    flare::rt::Pipeline pl(
      ...
      flare::rt::Pipe{
        flare::rt::PipeType::SERIAL, [subtask, &sums](flare::rt::Pipeflow&, flare::rt::Runtime& rt) {
          rt.corun([subtask, &sums](flare::rt::Subflow& sf) {
            for (size_t i = 0; i < subtask; ++i) {
              sf.emplace([&sums](){
                sums.fetch_add(1, std::memory_order_relaxed);  
...
void pipeline_spspspsp_runtime_irregular_subflow(size_t w) {
  ...
  flare::rt::Pipeline pl(
     ...
     */
    flare::rt::Pipe{
      flare::rt::PipeType::PARALLEL, [&sums](flare::rt::Pipeflow&, flare::rt::Runtime& rt) {
        rt.corun([&sums](flare::rt::Subflow& sf) {
     ...
     */
    flare::rt::Pipe{
      flare::rt::PipeType::SERIAL, [&sums](flare::rt::Pipeflow&, flare::rt::Runtime& rt) {
        rt.corun([&sums](flare::rt::Subflow& sf) {
          auto A = sf.emplace([&sums]() { sums.fetch_add(1, std::memory_order_relaxed); });
          auto B = sf.emplace([&sums]() { sums.fetch_add(1, std::memory_order_relaxed); });
          auto C = sf.emplace([&sums]() { sums.fetch_add(1, std::memory_order_relaxed); });
...
void scalable_pipeline_spspspsp_runtime_subflow(size_t w) {
  ...
  using pipe_t = flare::rt::Pipe<std::function<void(flare::rt::Pipeflow&, flare::rt::Runtime&)>>;
  std::vector<pipe_t> pipes;

  flare::rt::ScalablePipeline<std::vector<pipe_t>::iterator> sp;
  ...
  for (size_t subtask = 0; subtask <= subtasks; subtask = subtask == 0 ? subtask + 1 : subtask*2) {
    ...
    pipes.emplace_back(flare::rt::PipeType::SERIAL, [max_tokens](flare::rt::Pipeflow& pf, flare::rt::Runtime&){
      if (pf.token() == max_tokens) {
        pf.stop();
      }
    });
    ...
    pipes.emplace_back(flare::rt::PipeType::PARALLEL, [subtask, &sums](flare::rt::Pipeflow&, flare::rt::Runtime& rt) {
      rt.corun([subtask, &sums](flare::rt::Subflow& sf) {
        for (size_t i = 0; i < subtask; ++i) {
          sf.emplace([&sums](){
            sums.fetch_add(1, std::memory_order_relaxed);  
    ...
    pipes.emplace_back(flare::rt::PipeType::SERIAL, [subtask, &sums](flare::rt::Pipeflow&, flare::rt::Runtime& rt) {
      rt.corun([subtask, &sums](flare::rt::Subflow& sf) {
        for (size_t i = 0; i < subtask; ++i) {
          sf.emplace([&sums](){
            sums.fetch_add(1, std::memory_order_relaxed);  
