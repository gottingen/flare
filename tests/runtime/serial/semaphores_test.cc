...
void critical_section(size_t W) {
  ...
  flare::rt::CriticalSection section(1);

  int N = 1000;
  int counter = 0;

  for(int i=0; i<N; ++i) {
    flare::rt::Task task = taskflow.emplace([&](){ counter++; })
...
void semaphore(size_t W) {
  ...
  flare::rt::Executor executor(W);
  flare::rt::Taskflow taskflow;
  flare::rt::Semaphore semaphore(1);
...
void overlapped_semaphore(size_t W) {
  ...
  flare::rt::Executor executor(W);
  flare::rt::Taskflow taskflow;
  flare::rt::Semaphore semaphore1(1);
  flare::rt::Semaphore semaphore4(4);
...
void conflict_graph(size_t W) {
  ...
  flare::rt::Executor executor(W);
  flare::rt::Taskflow taskflow;
  flare::rt::Semaphore conflict_AB(1);
  flare::rt::Semaphore conflict_AC(1);

  int counter {0};
  std::mutex mutex;

  flare::rt::Task A = taskflow.emplace([&](){ counter++; });

  // B and C can run together
  flare::rt::Task B = taskflow.emplace([&](){
