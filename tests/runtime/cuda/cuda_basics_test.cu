...
TEST_CASE("Empty" * doctest::timeout(300)) {
  empty<flare::rt::cudaFlow>();
}

TEST_CASE("EmptyCapture" * doctest::timeout(300)) {
  empty<flare::rt::cudaFlowCapturer>();
}
...
void move_semantics() {
  ...
  auto gpu = flare::rt::cuda_malloc_device<int>(N);
  ...
  REQUIRE(rhs.native_executable() != nullptr);
  REQUIRE(flare::rt::cuda_graph_get_num_nodes(rhs.native_graph()) == rhs.num_tasks());
  ...
  REQUIRE(flare::rt::cuda_graph_get_num_nodes(lhs.native_graph()) == lhs.num_tasks());
...
TEST_CASE("cudaFlow.MoveSemantics" * doctest::timeout(300)) {
  move_semantics<flare::rt::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.MoveSemantics" * doctest::timeout(300)) {
  move_semantics<flare::rt::cudaFlowCapturer>();
}
...
void standalone() {
  ...
  std::free(cpu);
  flare::rt::cuda_free(gpu);
}

TEST_CASE("Standalone.cudaFlow") {
  standalone<flare::rt::cudaFlow>();
}

TEST_CASE("Standalone.cudaCapturer") {
  standalone<flare::rt::cudaFlowCapturer>();
}
...
void set() {
  ...
  for(unsigned n=1; n<=123456; n = n*2 + 1) {
    ...
    auto gputask = taskflow.emplace([&]() {
      ...
      REQUIRE(cf.num_tasks() == flare::rt::cuda_graph_get_num_nodes(cf.native_graph()));
    });
...
void bset() {
  ...
  auto gputask = taskflow.emplace([&]() {
    ...
    std::vector<flare::rt::cudaTask> tasks(n+1);
...
TEST_CASE("Memset" * doctest::timeout(300)) {
  memset<flare::rt::cudaFlow>();
}

TEST_CASE("CapturedMemset" * doctest::timeout(300)) {
  memset<flare::rt::cudaFlowCapturer>();
}
...
TEST_CASE("Memset0.i8") {
  memset0<int8_t, flare::rt::cudaFlow>();
}

TEST_CASE("Memset0.i16") {
  memset0<int16_t, flare::rt::cudaFlow>();
}

TEST_CASE("Memset0.i32") {
  memset0<int32_t, flare::rt::cudaFlow>();
}

TEST_CASE("Memset0.f32") {
  memset0<float, flare::rt::cudaFlow>();
}

TEST_CASE("Memset0.f64") {
  memset0<double, flare::rt::cudaFlow>();
}

TEST_CASE("CapturedMemset0.i8") {
  memset0<int8_t, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemset0.i16") {
  memset0<int16_t, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemset0.i32") {
  memset0<int32_t, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemset0.f32") {
  memset0<float, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemset0.f64") {
  memset0<double, flare::rt::cudaFlowCapturer>();
}
...
TEST_CASE("Memcpy.i8") {
  memcpy<int8_t, flare::rt::cudaFlow>();
}

TEST_CASE("Memcpy.i16") {
  memcpy<int16_t, flare::rt::cudaFlow>();
}

TEST_CASE("Memcpy.i32") {
  memcpy<int32_t, flare::rt::cudaFlow>();
}

TEST_CASE("Memcpy.f32") {
  memcpy<float, flare::rt::cudaFlow>();
}

TEST_CASE("Memcpy.f64") {
  memcpy<double, flare::rt::cudaFlow>();
}

TEST_CASE("CapturedMemcpy.i8") {
  memcpy<int8_t, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemcpy.i16") {
  memcpy<int16_t, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemcpy.i32") {
  memcpy<int32_t, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemcpy.f32") {
  memcpy<float, flare::rt::cudaFlowCapturer>();
}

TEST_CASE("CapturedMemcpy.f64") {
  memcpy<double, flare::rt::cudaFlowCapturer>();
}
...
TEST_CASE("NestedRuns" * doctest::timeout(300)) {
  nested_runs<flare::rt::cudaFlow>();
}

TEST_CASE("CapturedNestedRuns" * doctest::timeout(300)) {
  nested_runs<flare::rt::cudaFlowCapturer>();
}
...
void worker_id(unsigned N, unsigned M) {
  ...
  flare::rt::Taskflow taskflow;
  flare::rt::Executor executor(N + M);
  ...
  for(unsigned k=0; k<s; ++k) {
    ...
    auto gputask = taskflow.emplace([&](flare::rt::cudaFlow&) {
      auto id = executor.this_worker_id();
      REQUIRE(id >= 0);
      REQUIRE(id <  N+M);
    });
    ...
    taskflow.emplace([&](flare::rt::cudaFlow&) {
      auto id = executor.this_worker_id();
      REQUIRE(id >= 0);
      REQUIRE(id <  N+M);
    });
    ...
    auto subflow = taskflow.emplace([&](flare::rt::Subflow& sf){
      auto id = executor.this_worker_id();
      REQUIRE(id >= 0);
      REQUIRE(id <  N+M);
      auto t1 = sf.emplace([&](){
      ...
      auto t2 = sf.emplace([&](flare::rt::cudaFlow&){
        auto id = executor.this_worker_id();
        REQUIRE(id >= 0);
        REQUIRE(id <  N+M);
      });
...
void subflow() {
  ...
  auto partask = taskflow.emplace([&](flare::rt::Subflow& sf){
...
TEST_CASE("Subflow" * doctest::timeout(300)) {
  subflow<flare::rt::cudaFlow>();
}

TEST_CASE("CapturedSubflow" * doctest::timeout(300)) {
  subflow<flare::rt::cudaFlowCapturer>();
}
...
void nested_subflow() {
  ...
  auto partask = taskflow.emplace([&](flare::rt::Subflow& sf){
    ...
    auto subtask1 = sf.emplace([&](flare::rt::Subflow& sf) {
      ...
      auto subtask2 = sf.emplace([&](flare::rt::Subflow& sf){
        sf.emplace([&]() {
          F cf;
          dim3 g = {(n+255)/256, 1, 1};
          dim3 b = {256, 1, 1};
...
TEST_CASE("NestedSubflow" * doctest::timeout(300) ) {
  nested_subflow<flare::rt::cudaFlow>();
}

TEST_CASE("CapturedNestedSubflow" * doctest::timeout(300) ) {
  nested_subflow<flare::rt::cudaFlowCapturer>();
}
...
void detached_subflow() {
  ...
  taskflow.emplace([&](flare::rt::Subflow& sf){
...
TEST_CASE("DetachedSubflow" * doctest::timeout(300)) {
  detached_subflow<flare::rt::cudaFlow>();
}

TEST_CASE("CapturedDetachedSubflow" * doctest::timeout(300)) {
  detached_subflow<flare::rt::cudaFlowCapturer>();
}
...
TEST_CASE("Loop" * doctest::timeout(300)) {
  loop<flare::rt::cudaFlow>();
}

TEST_CASE("CapturedLoop" * doctest::timeout(300)) {
  loop<flare::rt::cudaFlowCapturer>();
}
