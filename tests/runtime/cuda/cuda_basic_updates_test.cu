...
TEST_CASE("cudaFlowCapturer.Incrementality") {
  ...
  REQUIRE(cf.native_executable() != nullptr);
  REQUIRE(flare::rt::cuda_graph_get_num_nodes(cf.native_graph()) == cf.num_tasks());
...
TEST_CASE("cudaFlowCapturer.rebind.algorithms") {
  ...
  flare::rt::cudaFlowCapturer capturer;

  auto data = flare::rt::cuda_malloc_shared<int>(10000);
  auto res = flare::rt::cuda_malloc_shared<int>(1);
  ...
  // clear the memory
  flare::rt::cuda_free(data);
  flare::rt::cuda_free(res);
}
