...
template <class TensorTypeData, class TensorTypeTest, class MyItResult,
          class PredicateType>
void verify_data(TensorTypeData tensor_data_h, TensorTypeTest tensor_test,
                 MyItResult my_result, PredicateType pred) {
  // run std::remove_if
  auto std_result =
      std::remove_if(KE::begin(tensor_data_h), KE::end(tensor_data_h), pred);

  // check that returned iterators are correct
  const std::size_t std_diff = std_result - KE::begin(tensor_data_h);
  const std::size_t my_diff  = my_result - KE::begin(tensor_test);
  ...
  for (std::size_t i = 0; i < my_diff; ++i) {
    REQUIRE_EQ(tensor_test_h(i), tensor_data_h[i]);
    // std::cout << "i= " << i << " "
...
void run_single_scenario(const InfoType& scenario_info) {
  ...
  // std::cout << "remove_if: " << name << ", " << tensor_tag_to_string(Tag{})
  ...
  {
    auto tensor = create_tensor<ValueType>(Tag{}, tensor_ext, "remove_if_tensor");
    fill_tensor(tensor, name);
    // make host copy BEFORE running algo
    auto data_h = create_host_space_copy(tensor);
    auto rit    = KE::remove_if(exespace(), KE::begin(tensor), KE::end(tensor),
                             remove_if_even);
    verify_data(data_h, tensor, rit, remove_if_even);
  }

  {
    ...
    // make host copy BEFORE running algo
    auto data_h = create_host_space_copy(tensor);
    auto rit    = KE::remove_if("label", exespace(), KE::begin(tensor),
                             KE::end(tensor), remove_if_even);
    verify_data(data_h, tensor, rit, remove_if_even);
  }

  {
    ...
    // make host copy BEFORE running algo
    auto data_h = create_host_space_copy(tensor);
    auto rit    = KE::remove_if(exespace(), tensor, remove_if_even);
    verify_data(data_h, tensor, rit, remove_if_even);
  }

  {
    ...
    // make host copy BEFORE running algo
    auto data_h = create_host_space_copy(tensor);
    auto rit    = KE::remove_if("label", exespace(), tensor, remove_if_even);
    verify_data(data_h, tensor, rit, remove_if_even);
  }
...
TEST_CASE("std_algorithms_mod_seq_ops, remove_if") {
