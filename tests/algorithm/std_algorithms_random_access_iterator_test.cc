...
struct random_access_iterator_test : std_algorithms_test {
 public:
    random_access_iterator_test() {
        ...
        flare::parallel_for(m_static_tensor.extent(0),
                            AssignIndexFunctor<dyn_tensor_t>(m_dynamic_tensor));

        flare::parallel_for(m_static_tensor.extent(0),
                            AssignIndexFunctor<strided_tensor_t>(m_strided_tensor));
    }
...
TEST_CASE_FIXTURE(random_access_iterator_test, "constructor") {
  ...
  auto it1 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor);
  auto it2 = KE::detail::RandomAccessIterator<dyn_tensor_t>(m_dynamic_tensor);
  auto it3 = KE::detail::RandomAccessIterator<strided_tensor_t>(m_strided_tensor);
  auto it4 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 3);
  auto it5 = KE::detail::RandomAccessIterator<dyn_tensor_t>(m_dynamic_tensor, 3);
  auto it6 = KE::detail::RandomAccessIterator<strided_tensor_t>(m_strided_tensor, 3);
  REQUIRE(true);
}

template <class IteratorType, class ValueType>
void test_random_access_it_verify(IteratorType it, ValueType gold_value) {
  using tensor_t = flare::Tensor<typename IteratorType::value_type>;
  tensor_t checkTensor("checkTensor");
  CopyFromIteratorFunctor<IteratorType, tensor_t> cf(it, checkTensor);
  flare::parallel_for("_std_algo_copy", 1, cf);
  auto v_h =
      flare::create_mirror_tensor_and_copy(flare::HostSpace(), checkTensor);
...
TEST_CASE_FIXTURE(random_access_iterator_test, "dereference") {
  ...
  auto it4 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 3);
  auto it5 = KE::detail::RandomAccessIterator<dyn_tensor_t>(m_dynamic_tensor, 4);
  auto it6 = KE::detail::RandomAccessIterator<strided_tensor_t>(m_strided_tensor, 5);
...
struct CopyFromIteratorUsingSubscriptFunctor {
  ItTypeFrom m_itFrom;
  TensorTypeTo m_tensorTo;

  CopyFromIteratorUsingSubscriptFunctor(const ItTypeFrom itFromIn,
                                        const TensorTypeTo tensorToIn)
      : m_itFrom(itFromIn), m_tensorTo(tensorToIn) {}

  FLARE_INLINE_FUNCTION
  void operator()(int i) const { m_tensorTo(i) = m_itFrom[i]; }
};

template <class IteratorType>
void test_random_access_it_subscript_op_verify(IteratorType it) {
  using value_t = typename IteratorType::value_type;
  using tensor_t  = flare::Tensor<value_t*>;
  tensor_t checkTensor("checkTensor", 3);
  CopyFromIteratorUsingSubscriptFunctor<IteratorType, tensor_t> cf(it, checkTensor);
...
TEST_CASE_FIXTURE(random_access_iterator_test, "operatorsSet1") {
  auto it1 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 3);
  auto it2 = KE::detail::RandomAccessIterator<dyn_tensor_t>(m_dynamic_tensor, 3);
  auto it3 = KE::detail::RandomAccessIterator<strided_tensor_t>(m_strided_tensor, 3);
  ++it1;
  ++it2;
  ++it3;
...
TEST_CASE_FIXTURE(random_access_iterator_test, "operatorsSet4") {
  ...
  auto it4 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 4);
  auto it5 = KE::detail::RandomAccessIterator<dyn_tensor_t>(m_dynamic_tensor, 4);
  auto it6 = KE::detail::RandomAccessIterator<strided_tensor_t>(m_strided_tensor, 4);
  REQUIRE_NE(it1, it4);
  REQUIRE_NE(it2, it5);
  REQUIRE_NE(it3, it6);
  REQUIRE_LT(it1, it4);
  ...
  auto it7 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 3);
  auto it8 = KE::detail::RandomAccessIterator<dyn_tensor_t>(m_dynamic_tensor, 3);
  auto it9 = KE::detail::RandomAccessIterator<strided_tensor_t>(m_strided_tensor, 3);
  REQUIRE_EQ(it1, it7);
  REQUIRE_EQ(it2, it8);
  REQUIRE_EQ(it3, it9);
  REQUIRE_GE(it1, it7);
...
TEST_CASE_FIXTURE(random_access_iterator_test, "assignment_operator") {
  auto it1 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 3);
  auto it2 = KE::detail::RandomAccessIterator<static_tensor_t>(m_static_tensor, 5);
  REQUIRE_NE(it1, it2);
...
TEST_CASE_FIXTURE(random_access_iterator_test, "distance") {
  auto first = KE::begin(m_dynamic_tensor);
  auto last  = KE::end(m_dynamic_tensor);

  REQUIRE_EQ(0, KE::distance(first, first));
  REQUIRE_EQ(1, KE::distance(first, first + 1));
  REQUIRE_EQ(m_dynamic_tensor.extent(0), size_t(KE::distance(first, last)));
}

TEST_CASE_FIXTURE(random_access_iterator_test, "traits_helpers") {
  using T1_t = KE::detail::RandomAccessIterator<static_tensor_t>;
  using T2_t = KE::detail::RandomAccessIterator<dyn_tensor_t>;
  using T3_t = KE::detail::RandomAccessIterator<strided_tensor_t>;
