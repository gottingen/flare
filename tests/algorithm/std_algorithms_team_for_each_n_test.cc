...
template <class DatATensorType, class NTensorType, class UnaryPredType>
struct TestFunctorA {
  DatATensorType m_dataTensor;
  NTensorType m_nTensor;
  int m_apiPick;
  UnaryPredType m_unaryPred;

  TestFunctorA(const DatATensorType dataTensor, const NTensorType nTensor, int apiPick,
  ...
  FLARE_INLINE_FUNCTION void operator()(const MemberType& member) const {
    ...
    const auto n          = m_nTensor(myRowIndex);

    auto myRowTensorFrom = flare::subtensor(m_dataTensor, myRowIndex, flare::ALL());

    switch (m_apiPick) {
      case 0: {
        KE::for_each_n(member, KE::begin(myRowTensorFrom), n, m_unaryPred);
        break;
      }

      case 1: {
        KE::for_each_n(member, myRowTensorFrom, n, m_unaryPred);
        break;
      }
    }
  }
...
void test_A(std::size_t numTeams, std::size_t numCols, int apiId) {
  ...
  flare::Tensor<std::size_t*> nTensor("nTensor", numTeams);
  auto nTensor_h = create_host_space_copy(nTensor);
  using rand_pool =
      flare::Random_XorShift64_Pool<flare::DefaultHostExecutionSpace>;
  rand_pool pool(lowerBound * upperBound);
  flare::fill_random(nTensor_h, pool, 0, numCols);

  flare::deep_copy(nTensor, nTensor_h);
  ...
  TestFunctorA fnc(dataTensor, nTensor, apiId, unaryPred);
  ...
  for (std::size_t i = 0; i < dataTensorAfterOp_h.extent(0); ++i) {
    for (std::size_t j = 0, n = 0; j < dataTensorAfterOp_h.extent(1); ++j, ++n) {
      if (n < nTensor_h(i)) {
        REQUIRE_EQ(dataTensorBeforeOp_h(i, j) + 1, dataTensorAfterOp_h(i, j));
      } else {
        REQUIRE_EQ(dataTensorBeforeOp_h(i, j), dataTensorAfterOp_h(i, j));
      }
    }
  }
}
