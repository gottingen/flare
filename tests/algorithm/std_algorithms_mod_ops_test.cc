...
TEST_CASE("std_algorithms_mod_ops_test, move") {
  MyMovableType a;
  ...
  REQUIRE_EQ(b.m_value, 11);
  REQUIRE_EQ(a.m_value, -2);

  // move assign
  MyMovableType c;
  c = std::move(b);
  REQUIRE_EQ(c.m_value, 11);
...
// swap
// ------------
TEST_CASE("std_algorithms_mod_ops_test, swap") {
  {
    int a = 1;
    int b = 2;
    KE::swap(a, b);
...
void test_iter_swap(ViewType view) {
  ...
  REQUIRE_EQ(a_h(3), value_type(0));
  REQUIRE_EQ(a_h(4), value_type(6));
  REQUIRE_EQ(a_h(5), value_type(5));
  REQUIRE_EQ(a_h(6), value_type(4));
  REQUIRE_EQ(a_h(7), value_type(7));
  REQUIRE_EQ(a_h(8), value_type(8));
