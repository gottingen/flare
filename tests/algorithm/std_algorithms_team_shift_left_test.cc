...
struct TestFunctorA {
  ...
  FLARE_INLINE_FUNCTION void operator()(const MemberType& member) const {
    ...
    if (m_apiPick == 0) {
      auto it = KE::shift_left(member, KE::begin(myRowTensor), KE::end(myRowTensor),
                               m_shift);
      ...
      auto it = KE::shift_left(member, myRowTensor, m_shift);
...
void test_A(std::size_t numTeams, std::size_t numCols, std::size_t shift,
            int apiId) {
  /* description:
     randomly fill a rank-2 tensor and do a team-level KE::shift_left
