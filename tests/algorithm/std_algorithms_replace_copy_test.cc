...
void run_single_scenario(const InfoType& scenario_info) {
  ...
  // std::cout << "replace_copy: " << name << ", " << tensor_tag_to_string(Tag{})
  ...
  {
    auto tensor_from =
        create_tensor<ValueType>(Tag{}, tensor_ext, "replace_copy_from");
    auto tensor_dest =
        create_tensor<ValueType>(Tag{}, tensor_ext, "replace_copy_dest");
    fill_tensor(tensor_from, name);
    auto rit =
        KE::replace_copy(exespace(), KE::cbegin(tensor_from), KE::cend(tensor_from),
...

  {
    ...
    auto tensor_dest =
        create_tensor<ValueType>(Tag{}, tensor_ext, "replace_copy_dest");
    fill_tensor(tensor_from, name);
    auto rit = KE::replace_copy("label", exespace(), KE::cbegin(tensor_from),
...

  {
    ...
    auto tensor_dest =
        create_tensor<ValueType>(Tag{}, tensor_ext, "replace_copy_dest");
    fill_tensor(tensor_from, name);
    auto rit = KE::replace_copy(exespace(), tensor_from, tensor_dest, old_value,
                                new_value);
    verify_data(name, tensor_from, tensor_dest, new_value);
    REQUIRE_EQ(rit, (KE::begin(tensor_dest) + tensor_ext));
  }

  {
    ...
    auto tensor_dest =
        create_tensor<ValueType>(Tag{}, tensor_ext, "replace_copy_dest");
    fill_tensor(tensor_from, name);
    auto rit = KE::replace_copy("label", exespace(), tensor_from, tensor_dest,
                                old_value, new_value);
    verify_data(name, tensor_from, tensor_dest, new_value);
    REQUIRE_EQ(rit, (KE::begin(tensor_dest) + tensor_ext));
  }
