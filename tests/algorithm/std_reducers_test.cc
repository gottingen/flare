...
auto create_host_view_with_reduction_order_indices(
  ...
  using view_t = flare::View<int*, flare::HostSpace>;
...
auto run_min_or_max_test(ViewType view, StdReducersTestEnumOrder enValue) {
  ...
  using reducer_type    = std::conditional_t<
      (flag == 0), flare::MaxFirstLoc<view_value_type, IndexType, ExeSpace>,
      flare::MinFirstLoc<view_value_type, IndexType, ExeSpace> >;
  ...
  using return_type = flare::pair<view_value_type, IndexType>;
...
TEST_CASE("std_algorithms_reducers, max_first_loc") {
  ...
  using view_t                 = flare::View<double*, hostspace>;
...
void run_min_max_test(ViewType view, StdReducersTestEnumOrder enValue,
  ...
  using view_value_type = typename ViewType::value_type;
  using reducer_type =
      flare::MinMaxFirstLastLoc<view_value_type, IndexType, ExeSpace>;
  using reduction_value_type = typename reducer_type::value_type;
...
TEST_CASE("std_algorithms_reducers, min_max_first_last_loc") {
  ...
  using index_type      = int;
  using view_value_type = typename view_t::value_type;
  flare::pair<view_value_type, view_value_type> gold_values = {-1., 2.};
