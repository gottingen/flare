...
struct TestFunctorA {
  ...
  FLARE_INLINE_FUNCTION void operator()(const MemberType& member) const {
    ...
    switch (m_apiPick) {
      ...
      case 1: {
        auto it = KE::find_end(member, myRowTensorFrom, myRowSearchedSeqTensor);
        flare::single(flare::PerTeam(member), [=, *this]() {
          m_distancesTensor(myRowIndex) =
      ...
      case 2: {
        auto it = KE::find_end(member, KE::cbegin(myRowTensorFrom),
                               KE::cend(myRowTensorFrom),
                               KE::cbegin(myRowSearchedSeqTensor),
                               KE::cend(myRowSearchedSeqTensor), m_binaryPred);
        flare::single(flare::PerTeam(member), [=, *this]() {
          m_distancesTensor(myRowIndex) =
              KE::distance(KE::cbegin(myRowTensorFrom), it);
        });
...
void test_A(const bool sequencesExist, std::size_t numTeams,
  ...
  flare::Tensor<ValueType**> searchedSequencesTensor(
      "searchedSequencesTensor", numTeams, halfCols - quarterCols);
