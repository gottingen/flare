...
struct TestFunctorA {
  ...
  FLARE_INLINE_FUNCTION void operator()(const MemberType& member) const {
    ...
    if (m_apiPick == 0) {
      auto itPair = KE::minmax_element(member, KE::cbegin(myRowTensor),
                                       KE::cend(myRowTensor));

      flare::single(flare::PerTeam(member), [=, *this]() {
        m_distancesTensor(myRowIndex, 0) =
            KE::distance(KE::cbegin(myRowTensor), itPair.first);
        m_distancesTensor(myRowIndex, 1) =
            KE::distance(KE::cbegin(myRowTensor), itPair.second);
      });
    }

    else if (m_apiPick == 1) {
      auto itPair = KE::minmax_element(member, myRowTensor);

      flare::single(flare::PerTeam(member), [=, *this]() {
        m_distancesTensor(myRowIndex, 0) =
            KE::distance(KE::begin(myRowTensor), itPair.first);
        m_distancesTensor(myRowIndex, 1) =
            KE::distance(KE::begin(myRowTensor), itPair.second);
      });
    }
    else if (m_apiPick == 2) {
      using value_type = typename TensorType::value_type;
      auto itPair =
          KE::minmax_element(member, KE::cbegin(myRowTensor), KE::cend(myRowTensor),
...
void test_A(std::size_t numTeams, std::size_t numCols, int apiId) {
  /* description:
     team-level KE::minmax_element on a rank-2 tensor where
  ...
  flare::Tensor<std::size_t**> distancesTensor("distancesTensor", numTeams, 2);
  ...
  for (std::size_t i = 0; i < cloneOfDataTensorBeforeOp_h.extent(0); ++i) {
    ...
    REQUIRE_EQ(stdDistance[0], distancesTensor_h(i, 0));
    REQUIRE_EQ(stdDistance[1], distancesTensor_h(i, 1));
  }
