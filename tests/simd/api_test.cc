...
struct simd_api_test
{
    ...
    using int8_vector_type = std::vector<int8_t, flare::simd::default_allocator<int8_t>>;
    using uint8_vector_type = std::vector<uint8_t, flare::simd::default_allocator<uint8_t>>;
    using int16_vector_type = std::vector<int16_t, flare::simd::default_allocator<int16_t>>;
    using uint16_vector_type = std::vector<uint16_t, flare::simd::default_allocator<uint16_t>>;
    using int32_vector_type = std::vector<int32_t, flare::simd::default_allocator<int32_t>>;
    using uint32_vector_type = std::vector<uint32_t, flare::simd::default_allocator<uint32_t>>;
    using int64_vector_type = std::vector<int64_t, flare::simd::default_allocator<int64_t>>;
    using uint64_vector_type = std::vector<uint64_t, flare::simd::default_allocator<uint64_t>>;
    using float_vector_type = std::vector<float, flare::simd::default_allocator<float>>;
    using double_vector_type = std::vector<double, flare::simd::default_allocator<double>>;
...
private:
    ...
    void test_load_impl(const V& v, const std::string& name)
    {
        ...
        b = batch_type::load(v.data(), flare::simd::unaligned_mode());
        INFO(name, " unaligned");
        CHECK_BATCH_EQ(b, expected);

        b = batch_type::load(v.data(), flare::simd::aligned_mode());
        INFO(name, " aligned");
        CHECK_BATCH_EQ(b, expected);
    }
    ...
    {
        batch_type b = batch_type::load(v.data(), flare::simd::aligned_mode());
        V res(size);
    ...
    void test_set_impl(const std::string& name)
    {
        ...
        batch_type res = flare::simd::broadcast<value_type>(v);
        INFO(name);
