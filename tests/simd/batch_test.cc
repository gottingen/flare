...
struct batch_test {
    ...
    void test_load_store() const {
        ...
        alignas(flare::simd::default_arch::alignment()) array_type arhs(this->rhs);
        alignas(flare::simd::default_arch::alignment()) array_type ares;
    ...
    void test_arithmetic() const {
        ...
        // batch * scalar
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           std::bind(std::multiplies<value_type>(), _1, scalar));
            batch_type lres = batch_lhs() * scalar;
            INFO("batch * scalar");
            CHECK_BATCH_EQ(lres, expected);
            batch_type rres = scalar * batch_lhs();
        ...
        // batch / scalar
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           std::bind(std::divides<value_type>(), _1, scalar));
            batch_type lres = batch_lhs() / scalar;
            INFO("batch / scalar");
            CHECK_BATCH_EQ(lres, expected);
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           std::bind(std::divides<value_type>(), scalar, _1));
            batch_type rres = scalar / batch_lhs();
            INFO("scalar / batch");
            CHECK_BATCH_EQ(rres, expected);
        }
    ...
    void test_incr_decr() const {
        ...
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), flare::simd::incr<value_type>);
            batch_type res = flare::simd::incr(batch_lhs());
            INFO("incr(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
        ...
        // incr_if
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           [](value_type v) { return v > 1 ? v + 1 : v; });
            batch_type res = flare::simd::incr_if(batch_lhs(), batch_lhs() > value_type(1));
            INFO("incr_if(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
        ...
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(), flare::simd::decr<value_type>);
            batch_type res = flare::simd::decr(batch_lhs());
            INFO("decr(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
        ...
        // decr_if
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           [](value_type v) { return v > 1 ? v - 1 : v; });
            batch_type res = flare::simd::decr_if(batch_lhs(), batch_lhs() > value_type(1));
            INFO("decr_if(batch)");
            CHECK_BATCH_EQ(res, expected);
        }
    }
    ...
    void test_saturated_arithmetic() const {
        ...
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), rhs.cbegin(), expected.begin(), flare::simd::sadd<value_type>);
            batch_type res = flare::simd::sadd(batch_lhs(), batch_rhs());
        ...
        // batch - batch
        {
            ...
            std::transform(lhs.cbegin(), lhs.cend(), rhs.cbegin(), expected.begin(),
                           [](value_type x, value_type y) { return flare::simd::ssub(x, y); });
            batch_type res = flare::simd::ssub(batch_lhs(), batch_rhs());
    ...
    void test_logical() const {
        ...
        // batch && scalar
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           std::bind(std::logical_and<value_type>(), _1, scalar));
            batch_type lres = batch_lhs() && scalar;
            INFO("batch && scalar");
            CHECK_BATCH_EQ(lres, expected);
            batch_type rres = scalar && batch_lhs();
        ...
        // batch || scalar
        {
            array_type expected;
            std::transform(lhs.cbegin(), lhs.cend(), expected.begin(),
                           std::bind(std::logical_or<value_type>(), _1, scalar));
            batch_type lres = batch_lhs() || scalar;
            INFO("batch || scalar");
            CHECK_BATCH_EQ(lres, expected);
            batch_type rres = scalar || batch_lhs();
    ...
    void test_min_max() const {
        ...
        // min limit case
        {
            ...
            batch_type res = flare::simd::min(batch_type(std::numeric_limits<value_type>::min()), batch_rhs());
            INFO("min limit");
            CHECK_BATCH_EQ(res, expected);
        }
        // fmin
        ...
        // max limit case
        {
            ...
            batch_type res = flare::simd::max(batch_type(std::numeric_limits<value_type>::max()), batch_rhs());
            INFO("max limit");
            CHECK_BATCH_EQ(res, expected);
        }
        // fmax
    ...
    void test_fused_operations() const {
        // fma
        {
            ...
            batch_type res = flare::simd::fma(batch_lhs(), batch_rhs(), batch_rhs());
            INFO("fma");
            CHECK_BATCH_EQ(res, expected);
        }
        // fms
    ...
    template<size_t N>
    typename std::enable_if<4 <= N, void>::type
    test_generic_horizontal_operations(std::integral_constant<size_t, N>) const {
        // reduce generic
        {
            value_type expected = std::accumulate(lhs.cbegin(), lhs.cend(), value_type(1),
                                                  std::multiplies<value_type>());
            value_type res = reduce(flare::simd::mul<typename B::value_type, typename B::arch_type>, batch_lhs());
            INFO("generic reduce");
            CHECK_SCALAR_EQ(res, expected);
        }
    }
...
private:
    ...
    flare::simd::enable_integral_t<T, void> init_operands() {
        for (size_t i = 0; i < size; ++i) {
    ...
    template<class T = value_type>
    flare::simd::enable_floating_point_t<T, void> init_operands() {
        for (size_t i = 0; i < size; ++i) {
