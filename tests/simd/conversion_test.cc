...
struct conversion_test
{
    ...
    using int32_batch = flare::simd::batch<int32_t>;
    using int64_batch = flare::simd::batch<int64_t>;
    using float_batch = flare::simd::batch<float>;
    using double_batch = flare::simd::batch<double>;

    using uint8_batch = flare::simd::batch<uint8_t>;
    using uint16_batch = flare::simd::batch<uint16_t>;
    using uint32_batch = flare::simd::batch<uint32_t>;
    using uint64_batch = flare::simd::batch<uint64_t>;

    using int32_vector = std::vector<int32_t, flare::simd::default_allocator<int32_t>>;
    using int64_vector = std::vector<int64_t, flare::simd::default_allocator<int64_t>>;
    using float_vector = std::vector<float, flare::simd::default_allocator<float>>;
    using double_vector = std::vector<double, flare::simd::default_allocator<double>>;

    using uint8_vector = std::vector<uint8_t, flare::simd::aligned_allocator<uint8_t, A>>;
    ...
    void test_u8_casting()
    {
        ...
        {
            uint16_batch ui16casting = flare::simd::bitwise_cast<uint16_t>(ui8tmp);
            uint8_batch ui8casting = flare::simd::bitwise_cast<uint8_t>(ui16casting);
            ui8casting.store_aligned(ui8vres.data());
            INFO("u8_to_16");
            CHECK_VECTOR_EQ(ui8vres, ui8res);
        }
        {
            uint32_batch ui32casting = flare::simd::bitwise_cast<uint32_t>(ui8tmp);
            uint8_batch ui8casting = flare::simd::bitwise_cast<uint8_t>(ui32casting);
            ui8casting.store_aligned(ui8vres.data());
            INFO("u8_to_32");
            CHECK_VECTOR_EQ(ui8vres, ui8res);
        }
        {
            uint64_batch ui64casting = flare::simd::bitwise_cast<uint64_t>(ui8tmp);
            uint8_batch ui8casting = flare::simd::bitwise_cast<uint8_t>(ui64casting);
            ui8casting.store_aligned(ui8vres.data());
            INFO("u8_to_64");
            CHECK_VECTOR_EQ(ui8vres, ui8res);
        }
...
struct sign_conversion_test
{
    ...
    void test_to_signed()
    {
        ...
        flare::simd::batch<unsigned_type> unsigned_batch(unsigned_value);
        auto signed_batch = flare::simd::batch_cast<signed_type>(unsigned_batch);
    ...
    void test_to_unsigned()
    {
        ...
        flare::simd::batch<signed_type> signed_batch(signed_value);
        auto unsigned_batch = flare::simd::batch_cast<unsigned_type>(signed_batch);
        CHECK_EQ(signed_batch.get(0), signed_value);
        CHECK_EQ(unsigned_batch.get(0), unsigned_value);
    }
};
