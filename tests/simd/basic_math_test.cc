...
namespace detail {
    ...
    struct infinity_tester
    {
        static void test_isfinite()
        {
            T input(1);
            INFO("input: ", input);
            CHECK_UNARY(flare::simd::all(flare::simd::isfinite(input)));
        }
        ...
        static void test_isinf()
        {
            T input(1);
            INFO("input: ", input);
            CHECK_FALSE(flare::simd::any(flare::simd::isinf(input)));
        }
    };
    ...
    struct infinity_tester<T, false>
    {
        static void test_isfinite()
        {
            T input = flare::simd::infinity<T>();
            CHECK_FALSE(flare::simd::any(flare::simd::isfinite(input)));
        }
        static void test_isinf()
        {
            T input = flare::simd::infinity<T>();
            CHECK_UNARY(flare::simd::all(flare::simd::isinf(input)));
        }
    };
}
...
struct basic_math_test
{
    ...
    void test_fmod() const
    {
        ...
        std::transform(
            ...
            { return std::fmod(l, r); });
        batch_type res = flare::simd::fmod(batch_lhs(), batch_rhs());
    ...
    void test_remainder() const
    {
        ...
        batch_type res = flare::simd::remainder(batch_lhs(), batch_rhs());
        CHECK_BATCH_EQ(res, expected);
    }
    ...
    void test_fdim() const
    {
        ...
        std::transform(lhs.cbegin(), lhs.cend(), rhs.cbegin(), expected.begin(),
                       [](const value_type& l, const value_type& r)
                       { return std::fdim(l, r); });
        batch_type res = flare::simd::fdim(batch_lhs(), batch_rhs());
        CHECK_BATCH_EQ(res, expected);
    }
    ...
    void test_clip()
    {
        ...
        batch_type res = flare::simd::clip(batch_clip_input(), batch_type(clip_lo), batch_type(clip_hi));
        CHECK_BATCH_EQ(res, expected);
    }
    ...
    void test_nextafter()
    {
        ...
        batch_type res = flare::simd::nextafter(batch_from_input(), batch_rhs());
        CHECK_BATCH_EQ(res, expected);
    }
