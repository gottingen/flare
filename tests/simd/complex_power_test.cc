    ...
    complex_power_test() {
        ...
        for (size_t i = 0; i < nb_input; ++i) {
            real_value_type real =
                    (real_value_type(i) / 4 + real_value_type(1.2) * std::sqrt(real_value_type(i + 0.25))) / 100;
            real_value_type imag =
                    (real_value_type(i) / 7 + real_value_type(1.7) * std::sqrt(real_value_type(i + 0.37))) / 100;
...
#if (FLARE_SIMD_X86_INSTR_SET >= FLARE_SIMD_X86_AVX512_VERSION) || (FLARE_SIMD_ARM_INSTR_SET >= FLARE_SIMD_ARM7_NEON_VERSION)
#if DEBUG_ACCURACY
        test_pow_impl();
#endif
#else
