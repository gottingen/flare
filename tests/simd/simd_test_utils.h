...
//
#ifndef SIMD_TEST_UTILS_H_
#define SIMD_TEST_UTILS_H_
...
struct precision_t {
#if defined(__apple_build_version__) && (FLARE_SIMD_ENABLE_SSE4_1 || FLARE_SIMD_ENABLE_NEON64)
...
namespace detail {
    ...
    bool expect_batch_near(const ::flare::simd::batch<T, A> &lhs, const std::array<T, N> &rhs) {
        std::array<T, N> tmp;
        lhs.store_unaligned(tmp.data());
    ...
    template<class T, size_t N, class A>
    bool expect_batch_near(const std::array<T, N> &lhs, const ::flare::simd::batch<T, A> &rhs) {
        std::array<T, N> tmp;
    ...
    template<class T, class A>
    bool expect_batch_near(const ::flare::simd::batch<T, A> &lhs, const ::flare::simd::batch<T, A> &rhs) {
        constexpr auto N = flare::simd::batch<T, A>::size;
        std::array<T, N> tmp;
        lhs.store_unaligned(tmp.data());
    ...
    template<class T, size_t N, class A>
    bool expect_batch_near(const ::flare::simd::batch_bool<T, A> &lhs, const std::array<bool, N> &rhs) {
    ...
    template<class T, size_t N, class A>
    bool expect_batch_near(const std::array<bool, N> &lhs, const ::flare::simd::batch_bool<T, A> &rhs) {
        std::array<bool, N> tmp;
        rhs.store_unaligned(tmp.data());
        return expect_array_near(lhs, tmp);
    }
    ...
    template<class T, class A>
    bool expect_batch_near(const ::flare::simd::batch_bool<T, A> &lhs, const ::flare::simd::batch_bool<T, A> &rhs) {
        constexpr auto N = flare::simd::batch<T, A>::size;
        std::array<bool, N> tmp;
        lhs.store_unaligned(tmp.data());
        return expect_batch_near(tmp, rhs);
...
#define BATCH_INT_TYPES flare::simd::batch<uint8_t>,  \
                        flare::simd::batch<int8_t>,   \
                        flare::simd::batch<uint16_t>, \
                        flare::simd::batch<int16_t>,  \
                        flare::simd::batch<uint32_t>, \
                        flare::simd::batch<int32_t>,  \
                        flare::simd::batch<uint64_t>, \
                        flare::simd::batch<int64_t>

#if FLARE_SIMD_ENABLE_NEON64 || !FLARE_SIMD_ENABLE_NEON
#define BATCH_FLOAT_TYPES flare::simd::batch<float>, flare::simd::batch<double>
#else
#define BATCH_FLOAT_TYPES flare::simd::batch<float>
#endif
#if FLARE_SIMD_ENABLE_NEON64 || !FLARE_SIMD_ENABLE_NEON
#define BATCH_COMPLEX_TYPES flare::simd::batch<std::complex<float>>, flare::simd::batch<std::complex<double>>
#else
#define BATCH_COMPLEX_TYPES flare::simd::batch<std::complex<float>>
#endif

#define BATCH_TYPES BATCH_INT_TYPES, BATCH_FLOAT_TYPES
#define BATCH_MATH_TYPES flare::simd::batch<int32_t>, BATCH_FLOAT_TYPES

#if !FLARE_SIMD_ENABLE_AVX || FLARE_SIMD_ENABLE_AVX2
#define BATCH_SWIZZLE_TAIL , flare::simd::batch<uint32_t>, flare::simd::batch<int32_t>, flare::simd::batch<uint64_t>, flare::simd::batch<int64_t>
#else
...
#define CONVERSION_TYPES conversion_param<sizeof(flare::simd::types::simd_register<int, flare::simd::default_arch>) / sizeof(double), flare::simd::default_arch::alignment()>

#endif  // SIMD_TEST_UTILS_H_
