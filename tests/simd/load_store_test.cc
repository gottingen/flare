...
struct load_store_test
{
    ...
    using index_type = typename flare::simd::as_integer_t<batch_type>;
    template <class T>
    using allocator = flare::simd::default_allocator<T, typename B::arch_type>;
...
private:
#ifdef FLARE_SIMD_ENABLE_SSE2
    struct test_load_as_return_type
    {
        using lower_arch = flare::simd::sse2;
        using expected_batch_type = flare::simd::batch<float, lower_arch>;
        using load_as_return_type = decltype(flare::simd::load_as<float, lower_arch>(std::declval<float*>(), flare::simd::aligned_mode()));
    ...
    void test_load_impl(const V& v, const std::string& name)
    {
        ...
        b = flare::simd::load_as<value_type>(v.data(), flare::simd::unaligned_mode());
        INFO(name, " unaligned (load_as)");
        CHECK_BATCH_EQ(b, expected);

        b = flare::simd::load_as<value_type>(v.data(), flare::simd::aligned_mode());
    ...
    struct test_load_char
    {
        ...
        static_assert(std::is_same<flare::simd::batch<char>, decltype(flare::simd::load_as<char>(std::declval<char*>(), flare::simd::aligned_mode()))>::value,
                      "honor explicit type request");
        static_assert(std::is_same<flare::simd::batch<unsigned char>, decltype(flare::simd::load_as<unsigned char>(std::declval<unsigned char*>(), flare::simd::aligned_mode()))>::value,
                      "honor explicit type request");
        static_assert(std::is_same<flare::simd::batch<signed char>, decltype(flare::simd::load_as<signed char>(std::declval<signed char*>(), flare::simd::aligned_mode()))>::value,
                      "honor explicit type request");
    };
    ...
    void test_store_impl(const V& v, const std::string& name)
    {
        ...
        flare::simd::store_as(res.data(), b, flare::simd::unaligned_mode());
        INFO(name, " unaligned (store_as)");
        CHECK_VECTOR_EQ(res, v);

        flare::simd::store_as(res.data(), b, flare::simd::aligned_mode());
    ...
    {
        std::copy(v.cbegin(), v.cend(), expected.begin());
        index_type index = flare::simd::detail::make_sequence_as_batch<index_type>();
