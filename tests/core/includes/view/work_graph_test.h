...
struct TestWorkGraph {
  ...
  using Values      = flare::View<long*, MemorySpace>;
  ...
  void form_graph() {
    ...
    auto h_row_map = flare::create_mirror_view(m_graph.row_map);
    auto h_entries = flare::create_mirror_view(m_graph.entries);
    auto h_values  = flare::create_mirror_view(m_values);
    h_row_map(0)   = 0;
