 ...
 * (BeginParallelForEvent, PushRegionEvent, etc). We subscribe a flare Tool
...
/**
 * @brief There are an unholy number of begin events in flare, this is a base
...
/**
 * @brief Analogous to BeginOperation, there are a lot of things in flare
...
struct InitEvent : public EventBase {
  ...
  flare::Profiling::FlarePDeviceInfo* device_infos;
  std::string descriptor() const override {
    std::stringstream s;
  ...
  InitEvent(int l, uint64_t v_n, uint32_t n_d_i,
            flare::Profiling::FlarePDeviceInfo* d_i)
      : load_sequence(l),
        version_number(v_n),
...
struct DataEvent : public EventBase {
  using SpaceHandleType = flare::Profiling::SpaceHandle;
...
struct ProvideToolProgrammingInterfaceEvent : public EventBase {
  using Interface = flare::Tools::experimental::ToolProgrammingInterface;
...
struct RequestToolSettingsEvent : public EventBase {
  using Settings = flare::Tools::experimental::ToolSettings;
...
struct RequestOutputValuesEvent : public EventBase {
  size_t context;
  size_t num_inputs;
  std::vector<flare::Tools::experimental::VariableValue> inputs;
  size_t num_outputs;
  std::vector<flare::Tools::experimental::VariableValue> outputs;
  std::string descriptor() const override {
    std::stringstream s;
  ...
  RequestOutputValuesEvent(
      size_t c, size_t n_i,
      std::vector<flare::Tools::experimental::VariableValue> i, size_t n_o,
      std::vector<flare::Tools::experimental::VariableValue> o)
...
struct EndContextEvent : public EventBase {
  size_t context;
  flare::Tools::experimental::VariableValue value;
  std::string descriptor() const override {
    return "ContextEndEvent {" + std::to_string(context) + "}";
  }
  EndContextEvent(size_t c, flare::Tools::experimental::VariableValue v)
      : context(c), value(v) {}
};

struct OptimizationGoalDeclarationEvent : public EventBase {
  size_t context;
  flare::Tools::experimental::OptimizationGoal goal;
  std::string descriptor() const override {
    return "OptimizationGoalDeclarationEvent{" + std::to_string(context) + "}";
  }
  OptimizationGoalDeclarationEvent(
      size_t c, flare::Tools::experimental::OptimizationGoal g)
      : context(c), goal(g) {}
};
 ...
 *                sub-options)
 */
#define FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(name, value, depth)    \
...
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(Kernels, profiling.kernels, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(Regions, profiling.regions, 2);
...
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(ProfileEvents, profiling.profile_events,
                                     2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(Metadata, profiling.metadata, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(Contexts, tuning.contexts, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(TypeDeclarations, tuning.type_declarations,
                                     2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(RequestValues, tuning.request_values, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(Init, infrastructure.init, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(Finalize, infrastructure.finalize, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(ProgrammingInterface,
                                     infrastructure.programming_interface, 2);
FLARE_IMPL_TOOLS_TEST_CONFIG_OPTION(RequestSettings,
...
static void set_tool_events_impl(const ToolValidatorConfiguration& config) {
  flare::Tools::experimental::pause_tools();  // remove all events
  if (config.profiling.kernels) {
    flare::Tools::experimental::set_begin_parallel_for_callback(
        [](const char* n, const uint32_t d, uint64_t* k) {
          *k = ++last_kernel_id;
          found_events.push_back(
              std::make_shared<BeginParallelForEvent>(std::string(n), d, *k));
        });
    flare::Tools::experimental::set_begin_parallel_reduce_callback(
        [](const char* n, const uint32_t d, uint64_t* k) {
          *k = ++last_kernel_id;
          found_events.push_back(std::make_shared<BeginParallelReduceEvent>(
              std::string(n), d, *k));
        });
    flare::Tools::experimental::set_begin_parallel_scan_callback(
        [](const char* n, const uint32_t d, uint64_t* k) {
          *k = ++last_kernel_id;
  ...
  if (config.profiling.fences) {
    ...
    flare::Tools::experimental::set_end_fence_callback([](const uint64_t k) {
      found_events.push_back(std::make_shared<EndFenceEvent>(k));
    });
  }  // profiling.fences
  if (config.profiling.allocs) {
    flare::Tools::experimental::set_allocate_data_callback(
        [](flare::Tools::SpaceHandle handle, const char* name,
    ...
    flare::Tools::experimental::set_deallocate_data_callback(
        [](flare::Tools::SpaceHandle handle, const char* name,
           EventBase::PtrHandle const ptr, const uint64_t size) {
  ...
  if (config.profiling.copies) {
    flare::Tools::experimental::set_begin_deep_copy_callback(
        [](flare::Tools::SpaceHandle dst_handle, const char* dst_name,
           EventBase::PtrHandle dst_ptr, flare::Tools::SpaceHandle src_handle,
  ...
  if (config.profiling.sections) {
    flare::Tools::experimental::set_create_profile_section_callback(
        [](const char* name, uint32_t* id) {
          *id = (++last_section_id);
          found_events.push_back(std::make_shared<CreateProfileSectionEvent>(
              std::string(name), *id));
        });
    flare::Tools::experimental::set_destroy_profile_section_callback(
        [](uint32_t id) {
          found_events.push_back(
              std::make_shared<DestroyProfileSectionEvent>(id));
        });
    flare::Tools::experimental::set_start_profile_section_callback(
        [](uint32_t id) {
          found_events.push_back(
              std::make_shared<StartProfileSectionEvent>(id));
        });
    flare::Tools::experimental::set_stop_profile_section_callback(
        [](uint32_t id) {
          found_events.push_back(std::make_shared<StopProfileSectionEvent>(id));
        });
  }
  if (config.profiling.profile_events) {
    flare::Tools::experimental::set_profile_event_callback(
        [](const char* name) {
          found_events.push_back(
  ...
  if (config.profiling.metadata) {
    flare::Tools::experimental::set_declare_metadata_callback(
  ...
  if (config.tuning.contexts) {
    flare::Tools::experimental::set_begin_context_callback(
        [](const size_t context) {
          found_events.push_back(std::make_shared<BeginContextEvent>(context));
        });
    flare::Tools::experimental::set_end_context_callback(
        [](const size_t context,
           flare::Tools::experimental::VariableValue value) {
          found_events.push_back(
  ...
  if (config.tuning.request_values) {
    flare::Tools::experimental::set_request_output_values_callback(
        [](const size_t context, const size_t num_inputs,
           const flare::Tools::experimental::VariableValue* inputs_in,
           const size_t num_outputs,
           flare::Tools::experimental::VariableValue* outputs_in) {
          std::vector<flare::Tools::experimental::VariableValue> inputs,
              outputs;
  ...
  if (config.infrastructure.request_settings) {
    flare::Tools::experimental::set_request_tool_settings_callback(
        [](const uint32_t num_settings,
           flare::Tools::experimental::ToolSettings* settings) {
