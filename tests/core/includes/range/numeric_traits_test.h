...
#define DEFINE_EXTREMA(T, m, M)                 \
  FLARE_FUNCTION static T min(T) { return m; } \
  FLARE_FUNCTION static T max(T) { return M; }
    ...
    !defined(FLARE_COMPILER_NVHPC)  // 23.7 long double
...
// clang-format off
struct Infinity {
    template<class T> using trait = flare::experimental::infinity<T>;
};
struct Epsilon {
    template<class T> using trait = flare::experimental::epsilon<T>;
};
struct FiniteMin {
    template<class T> using trait = flare::experimental::finite_min<T>;
};
struct FiniteMax {
    template<class T> using trait = flare::experimental::finite_max<T>;
};
struct RoundError {
    template<class T> using trait = flare::experimental::round_error<T>;
};
struct NormMin {
    template<class T> using trait = flare::experimental::norm_min<T>;
};
struct DenormMin {
    template<class T> using trait = flare::experimental::denorm_min<T>;
};
struct Digits {
    template<class T> using trait = flare::experimental::digits<T>;
};
struct Digits10 {
    template<class T> using trait = flare::experimental::digits10<T>;
};
struct MaxDigits10 {
    template<class T> using trait = flare::experimental::max_digits10<T>;
};
struct Radix {
    template<class T> using trait = flare::experimental::radix<T>;
};
struct MinExponent {
    template<class T> using trait = flare::experimental::min_exponent<T>;
};
struct MaxExponent {
    template<class T> using trait = flare::experimental::max_exponent<T>;
};
struct MinExponent10 {
    template<class T> using trait = flare::experimental::min_exponent10<T>;
};
struct MaxExponent10 {
    template<class T> using trait = flare::experimental::max_exponent10<T>;
};
struct QuietNaN {
    template<class T> using trait = flare::experimental::quiet_NaN<T>;
};
struct SignalingNaN {
    template<class T> using trait = flare::experimental::signaling_NaN<T>;
};
// clang-format on
...
struct TestNumericTraits {
    ...
    void run() const {
        int errors = 0;
        flare::parallel_reduce(flare::RangePolicy<Space, Tag>(0, 1), *this,
                               errors);
        REQUIRE_EQ(errors, 0);
        (void) take_address_of(trait<T>::value);  // use on host
    }

    FLARE_FUNCTION void operator()(Infinity, int, int &e) const {
    ...
    FLARE_FUNCTION void operator()(Epsilon, int, int &e) const {
        using flare::experimental::epsilon;
    ...
    FLARE_FUNCTION void operator()(FiniteMin, int, int &e) const {
        using flare::experimental::finite_max;
        using flare::experimental::finite_min;
    ...
    // clang-format off
    FLARE_FUNCTION void operator()(FiniteMax, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(RoundError, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(NormMin, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(DenormMin, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(Digits, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(Digits10, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(MaxDigits10, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(Radix, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(MinExponent, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(MaxExponent, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(MinExponent10, int, int &) const { use_on_device(); }

    FLARE_FUNCTION void operator()(MaxExponent10, int, int &) const { use_on_device(); }
    // clang-format on
    FLARE_FUNCTION void operator()(QuietNaN, int, int &e) const {
#ifndef FLARE_COMPILER_NVHPC  // FIXME_NVHPC 23.7 nan
        using flare::experimental::quiet_NaN;
    ...
    FLARE_FUNCTION void operator()(SignalingNaN, int, int &e) const {
#ifndef FLARE_COMPILER_NVHPC  // FIXME_NVHPC 23.7 nan
        using flare::experimental::signaling_NaN;
...
#if (defined(FLARE_COMPILER_NVCC) && defined(FLARE_ON_CUDA_DEVICE))
template <class Tag>
struct TestNumericTraits<
    flare::Cuda,
    long double, Tag> {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_infinity") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_epsilon") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_round_error") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_norm_min") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_denorm_min") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_finite_min_max") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_digits") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_digits10") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_max_digits10") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_radix") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_min_max_exponent") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_min_max_exponent10") {
...
TEST_CASE("TEST_CATEGORY, numeric_traits_quiet_and_signaling_nan") {
...
#define CHECK_TRAIT_IS_SFINAE_FRIENDLY(TRAIT)                              \
  template <class T>                                                       \
  using TRAIT##_value_t = decltype(flare::experimental::TRAIT<T>::value); \
  template <class T>                                                       \
  using has_##TRAIT = flare::is_detected<TRAIT##_value_t, T>;             \
...
template<class T>
using infinity_value_t = decltype(flare::experimental::infinity<T>::value);

template<class T>
using has_infinity = flare::is_detected<infinity_value_t, T>;

template<class T, std::enable_if_t<has_infinity<T>::value> * = nullptr>
constexpr T legacy_std_numeric_limits_infinity() {
    return flare::experimental::infinity<T>::value;
}
...
TEST_CASE("TEST_CATEGORY, numeric_traits_sfinae_friendly") {
    REQUIRE_EQ(legacy_std_numeric_limits_infinity<int>(), 0);
}
...
#define CHECK_SAME_AS_NUMERIC_LIMITS_MEMBER_CONSTANT(T, TRAIT)           \
  static_assert(AssertIntEquality<flare::experimental::TRAIT<T>::value, \
                                  std::numeric_limits<T>::TRAIT>::value, \
                "")
#define CHECK_SAME_AS_NUMERIC_LIMITS_MEMBER_FUNCTION(T, TRAIT) \
  static_assert(flare::experimental::TRAIT<T>::value ==       \
...
// clang-format off
static_assert(flare::experimental::norm_min<float>::value == std::numeric_limits<float>::min(), "");
static_assert(flare::experimental::norm_min<double>::value == std::numeric_limits<double>::min(), "");
static_assert(flare::experimental::norm_min<long double>::value == std::numeric_limits<long double>::min(), "");
// integer types
static_assert(flare::experimental::finite_min<char>::value == std::numeric_limits<char>::min(), "");
static_assert(flare::experimental::finite_min<signed char>::value == std::numeric_limits<signed char>::min(), "");
static_assert(flare::experimental::finite_min<unsigned char>::value == std::numeric_limits<unsigned char>::min(), "");
static_assert(flare::experimental::finite_min<short>::value == std::numeric_limits<short>::min(), "");
static_assert(flare::experimental::finite_min<unsigned short>::value == std::numeric_limits<unsigned short>::min(), "");
static_assert(flare::experimental::finite_min<int>::value == std::numeric_limits<int>::min(), "");
static_assert(flare::experimental::finite_min<unsigned int>::value == std::numeric_limits<unsigned int>::min(), "");
static_assert(flare::experimental::finite_min<long int>::value == std::numeric_limits<long int>::min(), "");
static_assert(
        flare::experimental::finite_min<unsigned long int>::value == std::numeric_limits<unsigned long int>::min(), "");
static_assert(flare::experimental::finite_min<long long int>::value == std::numeric_limits<long long int>::min(), "");
static_assert(flare::experimental::finite_min<unsigned long long int>::value ==
              std::numeric_limits<unsigned long long int>::min(), "");
static_assert(flare::experimental::finite_max<char>::value == std::numeric_limits<char>::max(), "");
static_assert(flare::experimental::finite_max<signed char>::value == std::numeric_limits<signed char>::max(), "");
static_assert(flare::experimental::finite_max<unsigned char>::value == std::numeric_limits<unsigned char>::max(), "");
static_assert(flare::experimental::finite_max<short>::value == std::numeric_limits<short>::max(), "");
static_assert(flare::experimental::finite_max<unsigned short>::value == std::numeric_limits<unsigned short>::max(), "");
static_assert(flare::experimental::finite_max<int>::value == std::numeric_limits<int>::max(), "");
static_assert(flare::experimental::finite_max<unsigned int>::value == std::numeric_limits<unsigned int>::max(), "");
static_assert(flare::experimental::finite_max<long int>::value == std::numeric_limits<long int>::max(), "");
static_assert(
        flare::experimental::finite_max<unsigned long int>::value == std::numeric_limits<unsigned long int>::max(), "");
static_assert(flare::experimental::finite_max<long long int>::value == std::numeric_limits<long long int>::max(), "");
static_assert(flare::experimental::finite_max<unsigned long long int>::value ==
              std::numeric_limits<unsigned long long int>::max(), "");
// floating point types
static_assert(flare::experimental::finite_min<float>::value == -std::numeric_limits<float>::max(), "");
static_assert(flare::experimental::finite_min<double>::value == -std::numeric_limits<double>::max(), "");
static_assert(flare::experimental::finite_min<long double>::value == -std::numeric_limits<long double>::max(), "");
static_assert(flare::experimental::finite_max<float>::value == std::numeric_limits<float>::max(), "");
static_assert(flare::experimental::finite_max<double>::value == std::numeric_limits<double>::max(), "");
static_assert(flare::experimental::finite_max<long double>::value == std::numeric_limits<long double>::max(), "");
// clang-format on
...
#define CHECK_NAN_SAME_AS_NUMERIC_LIMITS_MEMBER_FUNCTION(T, TRAIT)             \
  static_assert(flare::experimental::TRAIT<T>::value !=                       \
                    flare::experimental::TRAIT<T>::value,                     \
                "");                                                           \
  static_assert(                                                               \
...
#if !defined(FLARE_COMPILER_NVHPC) && (CUDA_VERSION < 11060) && \
    !(defined(FLARE_COMPILER_NVCC) && !defined(FLARE_ON_CUDA_DEVICE))
...
#define CHECK_INSTANTIATED_ON_CV_QUALIFIED_TYPES(T, TRAIT)              \
  static_assert(flare::experimental::TRAIT<T const>::value ==          \
                    flare::experimental::TRAIT<T>::value,              \
                "");                                                    \
  static_assert(flare::experimental::TRAIT<T volatile>::value ==       \
                    flare::experimental::TRAIT<T>::value,              \
                "");                                                    \
  static_assert(flare::experimental::TRAIT<T const volatile>::value == \
                    flare::experimental::TRAIT<T>::value,              \
                "")
