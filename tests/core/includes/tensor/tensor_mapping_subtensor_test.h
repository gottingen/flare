...
namespace Test {
    ...
    struct TestTensorMappingSubtensor {
        ...
        using AT = flare::Tensor<int *, ExecSpace>;
        using ACT = flare::Tensor<const int *, ExecSpace>;
        using AS = flare::Subtensor<AT, range>;

        enum {
            BN0 = 10, BN1 = 11, BN2 = 12
        };
        using BT = flare::Tensor<int ***, ExecSpace>;
        using BS = flare::Subtensor<BT, range, range, range>;

        enum {
            CN0 = 10, CN1 = 11, CN2 = 12
        };
        using CT = flare::Tensor<int ***[13][14], ExecSpace>;
        // changing CS to CTS here because when compiling with nvshmem, there is a
        // define for CS that makes this fail...
        using CTS = flare::Subtensor<CT, range, range, range, int, int>;

        enum {
            DN0 = 10, DN1 = 11, DN2 = 12, DN3 = 13, DN4 = 14
        };
        using DT = flare::Tensor<int ***[DN3][DN4], ExecSpace>;
        using DS = flare::Subtensor<DT, int, range, range, range, int>;

        using DLT = flare::Tensor<int ***[13][14], flare::LayoutLeft, ExecSpace>;
        using DLS1 = flare::Subtensor<DLT, range, int, int, int, int>;

#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1000
        static_assert(
            DLS1::rank == 1 &&
                std::is_same<typename DLS1::array_layout, flare::LayoutLeft>::value,
            "Subtensor layout error for rank 1 subtensor of left-most range of "
            "LayoutLeft");
#endif

        using DRT = flare::Tensor<int ***[13][14], flare::LayoutRight, ExecSpace>;
        using DRS1 = flare::Subtensor<DRT, int, int, int, int, range>;

#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1000
        static_assert(
            DRS1::rank == 1 &&
                std::is_same<typename DRS1::array_layout, flare::LayoutRight>::value,
            "Subtensor layout error for rank 1 subtensor of right-most range of "
            "LayoutRight");
#endif
        ...
        TestTensorMappingSubtensor()
                : Aa("Aa", AN),
                  Ab(flare::subtensor(Aa, std::pair<int, int>(1, AN - 1))),
                  Ac(Aa, std::pair<int, int>(1, AN - 1)),
                  Ba("Ba", BN0, BN1, BN2),
                  Bb(flare::subtensor(Ba, std::pair<int, int>(1, BN0 - 1),
                                    std::pair<int, int>(1, BN1 - 1),
                                    std::pair<int, int>(1, BN2 - 1))),
                  Ca("Ca", CN0, CN1, CN2),
                  Cb(flare::subtensor(Ca, std::pair<int, int>(1, CN0 - 1),
                                    std::pair<int, int>(1, CN1 - 1),
                                    std::pair<int, int>(1, CN2 - 1), 1, 2)),
                  Da("Da", DN0, DN1, DN2),
                  Db(flare::subtensor(Da, 1, std::pair<int, int>(1, DN1 - 1),
                                    std::pair<int, int>(1, DN2 - 1),
                                    std::pair<int, int>(1, DN3 - 1), 2)) {}

        FLARE_INLINE_FUNCTION
        void operator()(const int, long &error_count) const {
            auto Ad = flare::subtensor(Aa, flare::pair<int, int>(1, AN - 1));
        ...
        void run() {
            TestTensorMappingSubtensor<ExecSpace> self;
