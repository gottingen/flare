...
CBLAS_TRANSPOSE
toCblasTranspose(fly_mat_prop opt) {
    CBLAS_TRANSPOSE out = CblasNoTrans;
    switch (opt) {
        case FLY_MAT_NONE: out = CblasNoTrans; break;
        case FLY_MAT_TRANS: out = CblasTrans; break;
        case FLY_MAT_CTRANS: out = CblasConjTrans; break;
...
void gemm(Array<T> &out, fly_mat_prop optLhs, fly_mat_prop optRhs, const T *alpha,
    ...
    auto func = [=](Param<T> output, CParam<T> left, CParam<T> right) {
        ...
        if (output.dims().ndims() <= 2) {
            ...
            // However, for Flare's use case, the group_count=1
...
template<>
void gemm<half>(Array<half> &out, fly_mat_prop optLhs, fly_mat_prop optRhs,
...
Array<T> dot(const Array<T> &lhs, const Array<T> &rhs, fly_mat_prop optLhs,
             fly_mat_prop optRhs) {
    Array<T> out = createEmptyArray<T>(fly::dim4(1));
    if (optLhs == FLY_MAT_CONJ && optRhs == FLY_MAT_CONJ) {
        getQueue().enqueue(kernel::dot<T, false, true>, out, lhs, rhs, optLhs,
                           optRhs);
    } else if (optLhs == FLY_MAT_CONJ && optRhs == FLY_MAT_NONE) {
        getQueue().enqueue(kernel::dot<T, true, false>, out, lhs, rhs, optLhs,
                           optRhs);
    } else if (optLhs == FLY_MAT_NONE && optRhs == FLY_MAT_CONJ) {
...
#define INSTANTIATE_GEMM(TYPE)                                               \
    template void gemm<TYPE>(Array<TYPE> & out, fly_mat_prop optLhs,          \
                             fly_mat_prop optRhs, const TYPE *alphas,         \
