...
template<typename T>
Array<T> createNodeArray(const fly::dim4 &dims, common::Node_ptr node);

template<typename T>
Array<T> createValueArray(const fly::dim4 &dims, const T &value);
...
template<typename T>
Array<T> createHostDataArray(const fly::dim4 &dims, const T *const data);
...
template<typename T>
Array<T> createDeviceDataArray(const fly::dim4 &dims, void *data,
                               bool copy = false);

template<typename T>
Array<T> createStridedArray(fly::dim4 dims, fly::dim4 strides, dim_t offset,
...
template<typename T>
Array<T> createEmptyArray(const fly::dim4 &dims);

template<typename T>
Array<T> createSubArray(const Array<T> &parent,
                        const std::vector<fly_seq> &index, bool copy = true);
...
class Array {
    ...
    explicit Array(const fly::dim4 &dims, T *const in_data, bool is_device,
                   bool copy_device = false);
    Array(const Array<T> &parent, const dim4 &dims, const dim_t &offset,
          const dim4 &stride);
    explicit Array(const fly::dim4 &dims, common::Node_ptr n);
    Array(const fly::dim4 &dims, const fly::dim4 &strides, dim_t offset,
   ...
   public:
    ...
    void resetInfo(const fly::dim4 &dims) { info.resetInfo(dims); }

    // Modifies the dimensions of the array without modifing the underlying
    // data
    void resetDims(const fly::dim4 &dims) { info.resetDims(dims); }
    void modDims(const fly::dim4 &newDims) { info.modDims(newDims); }
    void modStrides(const fly::dim4 &newStrides) { info.modStrides(newStrides); }
    void setId(int id) { info.setId(id); }

#define INFO_FUNC(RET_TYPE, NAME) \
    RET_TYPE NAME() const { return info.NAME(); }

    INFO_FUNC(const fly_dtype &, getType)
    INFO_FUNC(const fly::dim4 &, strides)
    INFO_FUNC(dim_t, elements)
    INFO_FUNC(dim_t, ndims)
    INFO_FUNC(const fly::dim4 &, dims)
...
#undef INFO_IS_FUNC
    ...
    friend Array<T> createValueArray<T>(const fly::dim4 &dims, const T &value);
    friend Array<T> createHostDataArray<T>(const fly::dim4 &dims,
                                           const T *const data);
    friend Array<T> createDeviceDataArray<T>(const fly::dim4 &dims, void *data,
                                             bool copy);
    friend Array<T> createStridedArray<T>(fly::dim4 dims, fly::dim4 strides,
                                          dim_t offset, T *const in_data,
                                          bool is_device);

    friend Array<T> createEmptyArray<T>(const fly::dim4 &dims);
    friend Array<T> createNodeArray<T>(const fly::dim4 &dims,
                                       common::Node_ptr node);

    friend Array<T> createSubArray<T>(const Array<T> &parent,
                                      const std::vector<fly_seq> &index,
                                      bool copy);
