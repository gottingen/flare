...
using flare::cpu::jit::BufferNode;
...
template<typename T>
Array<T>::Array(const fly::dim4 &dims, Node_ptr n)
    : info(getActiveDeviceId(), dims, 0, calcStrides(dims),
           static_cast<fly_dtype>(dtype_traits<T>::fly_type))
    , data()
    , data_dims(dims)
...
void evalMultiple(vector<Array<T> *> array_ptrs) {
    ...
    if (getQueue().is_worker()) {
        FLY_ERROR("Array not evaluated", FLY_ERR_INTERNAL);
    }
...
#define INSTANTIATE(T)                                                        \
    ...
    template Array<T>::Array(const fly::dim4 &dims, T *const in_data,          \
