...
int computeHomography(T* H_ptr, const float* rnd_ptr, const float* x_src_ptr,
    ...
    Array<T> A     = createValueArray<T>(fly::dim4(9, 9), static_cast<T>(0));
    ...
    Array<T> V =
        createValueArray<T>(fly::dim4(Adims[1], Adims[1]), static_cast<T>(0));
    V.eval();
    getQueue().sync();
...
int findBestHomography(Array<T>& bestH, const Array<float>& x_src,
                       ...
                       const float inlier_thr, const fly_homography_type htype) {
    ...
    Array<T> H =
        createValueArray<T>(fly::dim4(9, iterations), static_cast<T>(0));
    H.eval();
    getQueue().sync();

    const fly::dim4& rdims = rnd.dims();
    const fly::dim4& Hdims = H.dims();
    ...
    for (unsigned i = 0; i < iter; i++) {
        ...
        if (htype == FLY_HOMOGRAPHY_RANSAC) {
            int inliers_count = 0;
        ...
        } else if (htype == FLY_HOMOGRAPHY_LMEDS) {
