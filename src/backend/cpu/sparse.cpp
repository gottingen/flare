...
SparseArray<T> sparseConvertDenseToStorage(const Array<T> &in) {
    if (stype == FLY_STORAGE_CSR) {
        uint nNZ = getScalar<uint>(reduce_all<fly_notzero_t, T, uint>(in));
        ...
        auto rowIdx =
            arithOp<int, fly_mod_t>(nonZeroIdx, cnst, nonZeroIdx.dims());
        auto colIdx =
            arithOp<int, fly_div_t>(nonZeroIdx, cnst, nonZeroIdx.dims());
        ...
        FLY_ERROR("CPU Backend only supports Dense to CSR or COO",
...
Array<T> sparseConvertStorageToDense(const SparseArray<T> &in) {
    ...
    if (stype == FLY_STORAGE_CSR) {
        ...
        FLY_ERROR("CPU Backend only supports CSR or COO to Dense",
                 FLY_ERR_NOT_SUPPORTED);
    }
...
SparseArray<T> sparseConvertStorageToStorage(const SparseArray<T> &in) {
    ...
    if (src == FLY_STORAGE_CSR && dest == FLY_STORAGE_COO) {
        converter = kernel::csr2coo<T>;
    } else if (src == FLY_STORAGE_COO && dest == FLY_STORAGE_CSR) {
        converter = kernel::coo2csr<T>;
    } else {
        // Should never come here
        FLY_ERROR("CPU Backend invalid conversion combination",
                 FLY_ERR_NOT_SUPPORTED);
    }
...
#define INSTANTIATE_TO_STORAGE(T, S)                     \
    ...
    sparseConvertStorageToStorage<T, S, FLY_STORAGE_CSR>( \
        const SparseArray<T> &);                         \
    template SparseArray<T>                              \
    sparseConvertStorageToStorage<T, S, FLY_STORAGE_CSC>( \
        const SparseArray<T> &);                         \
    template SparseArray<T>                              \
    sparseConvertStorageToStorage<T, S, FLY_STORAGE_COO>( \
        const SparseArray<T> &);

#define INSTANTIATE_SPARSE(T)                                               \
    template SparseArray<T> sparseConvertDenseToStorage<T, FLY_STORAGE_CSR>( \
        const Array<T> &in);                                                \
    template SparseArray<T> sparseConvertDenseToStorage<T, FLY_STORAGE_CSC>( \
        const Array<T> &in);                                                \
    template SparseArray<T> sparseConvertDenseToStorage<T, FLY_STORAGE_COO>( \
        const Array<T> &in);                                                \
    template Array<T> sparseConvertStorageToDense<T, FLY_STORAGE_CSR>(       \
        const SparseArray<T> &in);                                          \
    template Array<T> sparseConvertStorageToDense<T, FLY_STORAGE_CSC>(       \
        const SparseArray<T> &in);                                          \
    template Array<T> sparseConvertStorageToDense<T, FLY_STORAGE_COO>(       \
        const SparseArray<T> &in);                                          \
                                                                            \
    INSTANTIATE_TO_STORAGE(T, FLY_STORAGE_CSR)                               \
    INSTANTIATE_TO_STORAGE(T, FLY_STORAGE_CSC)                               \
    INSTANTIATE_TO_STORAGE(T, FLY_STORAGE_COO)
