...
using reorderFunc = std::function<void(
    ...
    const dim4 filter_tmp_strides, FLY_BATCH_KIND kind)>;
...
Array<T> fftconvolve(Array<T> const& signal, Array<T> const& filter,
    ...
    array<int, FLY_MAX_DIMS> fftDims{};  // FLY_MAX_DIMS(4) > rank
    ...
    for (int k = rank; k < FLY_MAX_DIMS; k++) {
        sbatch *= sd[k];
        fbatch *= fd[k];
    }
    ...
    auto upstream_dft = [=](Param<convT> packed,
                            const array<int, FLY_MAX_DIMS> fftDims) {
    ...
    if (expand) {
        ...
        if (kind == FLY_BATCH_RHS) {
            for (int i = rank; i < FLY_MAX_DIMS; ++i) { oDims[i] = fd[i]; }
        }
    }
