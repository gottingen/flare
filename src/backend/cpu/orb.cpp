...
unsigned orb(Array<float>& x, Array<float>& y, Array<float>& score,
    ...
    for (unsigned i = 0; i < max_levels; i++) {
        ...
        if (i == 0) {
            ...
            lvl_img =
                resize<T>(prev_img, ldims[0], ldims[1], FLY_INTERP_BILINEAR);
        ...
        fly::dim4 usable_feat_dims(usable_feat);
        Array<float> score_harris = createDeviceDataArray<float>(
            usable_feat_dims, h_score_harris.get());
        Array<float> harris_sorted = createEmptyArray<float>(fly::dim4());
        Array<unsigned> harris_idx = createEmptyArray<unsigned>(fly::dim4());
    ...
    if (total_feat > 0) {
        ...
        const fly::dim4 desc_dims(8, total_feat);
