...
sparse_operation_t toSparseTranspose(fly_mat_prop opt) {
    sparse_operation_t out = SPARSE_OPERATION_NON_TRANSPOSE;
    switch (opt) {
        case FLY_MAT_NONE: out = SPARSE_OPERATION_NON_TRANSPOSE; break;
        case FLY_MAT_TRANS: out = SPARSE_OPERATION_TRANSPOSE; break;
        case FLY_MAT_CTRANS: out = SPARSE_OPERATION_CONJUGATE_TRANSPOSE; break;
...
Array<T> matmul(const common::SparseArray<T> &lhs, const Array<T> &rhs,
    ...
    Array<T> out = createValueArray<T>(fly::dim4(M, N, 1, 1), scalar<T>(0));
