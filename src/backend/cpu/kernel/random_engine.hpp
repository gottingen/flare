...
// Generates rationals in [0, 1)
flare::common::half getHalf01(uint *val, uint index) {
    float v = val[index >> 1U] >> (16U * (index & 1U)) & 0x0000ffff;
    return static_cast<flare::common::half>(
        fmaf(v, unsigned_half_factor, unsigned_half_half_factor));
}

// Generates rationals in (-1, 1]
static flare::common::half getHalfNegative11(uint *val, uint index) {
    float v = val[index >> 1U] >> (16U * (index & 1U)) & 0x0000ffff;
    // Conversion to half adapted from Random123
    constexpr float factor =
    ...
    return static_cast<flare::common::half>(fmaf(v, factor, half_factor));
}
...
template<>
flare::common::half transform<flare::common::half>(uint *val,
                                                           uint index) {
    float v = val[index >> 1U] >> (16U * (index & 1U)) & 0x0000ffff;
    return static_cast<flare::common::half>(
...
void boxMullerTransform(uint val[4], flare::common::half *temp) {
...
void uniformDistributionCBRNG(T *out, size_t elements,
                              fly_random_engine_type type, const uintl seed,
                              uintl counter) {
    switch (type) {
        case FLY_RANDOM_ENGINE_PHILOX_4X32_10:
            philoxUniform(out, elements, seed, counter);
            break;
        case FLY_RANDOM_ENGINE_THREEFRY_2X32_16:
            threefryUniform(out, elements, seed, counter);
            break;
        default:
            FLY_ERROR("Random Engine Type Not Supported", FLY_ERR_NOT_SUPPORTED);
    }
}
