...
struct Interp1<InT, LocT, 1> {
    void operator()(Param<InT> &out, int ooff, CParam<InT> &in, int ioff,
                    LocT x, fly_interp_type method, int batch, bool clamp,
        ...
        int xid   = (method == FLY_INTERP_LOWER ? std::floor(x) : std::round(x));
...
struct Interp1<InT, LocT, 2> {
    void operator()(Param<InT> &out, int ooff, CParam<InT> &in, int ioff,
        ...
        LocT ratio = off_x;
        if (method == FLY_INTERP_LINEAR_COSINE) {
            // Smooth the factional part with cosine
            ratio = (1 - std::cos(ratio * fly::Pi)) / 2;
        }
...
struct Interp1<InT, LocT, 3> {
    void operator()(Param<InT> &out, int ooff, CParam<InT> &in, int ioff,
        ...
        for (int n = 0; n < batch; n++) {
            ...
            bool spline = method == FLY_INTERP_CUBIC_SPLINE;
            outptr[ooff + n * ostrides[batch_dim]] =
...
struct Interp2<InT, LocT, 1> {
    void operator()(Param<InT> &out, int ooff, CParam<InT> &in, int ioff,
                    LocT x, LocT y, fly_interp_type method, int nimages,
        ...
        int xid = (method == FLY_INTERP_LOWER ? std::floor(x) : std::round(x));
        int yid = (method == FLY_INTERP_LOWER ? std::floor(y) : std::round(y));
...
struct Interp2<InT, LocT, 2> {
    void operator()(Param<InT> &out, int ooff, CParam<InT> &in, int ioff,
        ...
        if (method == FLY_INTERP_LINEAR_COSINE ||
            ...
            // Smooth the factional part with cosine
            xratio = (1 - std::cos(xratio * fly::Pi)) / 2;
            yratio = (1 - std::cos(yratio * fly::Pi)) / 2;
        }
