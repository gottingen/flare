...
void one2one_1d(InT *optr, InT const *const iptr, AccT const *const fptr,
                fly::dim4 const &oDims, fly::dim4 const &sDims,
                fly::dim4 const &fDims, fly::dim4 const &sStrides,
                const bool expand) {
...
void one2one_2d(InT *optr, InT const *const iptr, AccT const *const fptr,
                fly::dim4 const &oDims, fly::dim4 const &sDims,
                fly::dim4 const &fDims, fly::dim4 const &oStrides,
                fly::dim4 const &sStrides, fly::dim4 const &fStrides,
                const bool expand) {
...
void convolve_nd(Param<InT> out, CParam<InT> signal, CParam<AccT> filter,
    ...
    fly::dim4 const sDims = signal.dims();
    fly::dim4 const fDims = filter.dims();

    fly::dim4 const oStrides = out.strides();
    fly::dim4 const sStrides = signal.strides();
    fly::dim4 const fStrides = filter.strides();

    dim_t out_step[FLY_MAX_DIMS] = {
        0, 0, 0,
        0}; /* first value is never used, and declared for code simplicity */
    dim_t in_step[FLY_MAX_DIMS] = {
        0, 0, 0,
        0}; /* first value is never used, and declared for code simplicity */
    dim_t filt_step[FLY_MAX_DIMS] = {
        0, 0, 0,
        0}; /* first value is never used, and declared for code simplicity */
    dim_t batch[FLY_MAX_DIMS] = {
        0, 1, 1,
...
void convolve2_separable(InT *optr, InT const *const iptr,
                         AccT const *const fptr, fly::dim4 const &oDims,
                         fly::dim4 const &sDims, fly::dim4 const &orgDims,
                         dim_t fDim, fly::dim4 const &oStrides,
                         fly::dim4 const &sStrides, dim_t fStride) {
    UNUSED(orgDims);
    UNUSED(sStrides);
    UNUSED(fStride);
