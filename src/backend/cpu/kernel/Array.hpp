...
void removeNodeOfOperation(
    std::vector<std::shared_ptr<common::Node>> &node_index_map, fly_op_t op) {
    using flare::common::Node;
...
void evalMultiple(std::vector<Param<T>> arrays,
    ...
    fly::dim4 odims = arrays[0].dims();
    fly::dim4 ostrs = arrays[0].strides();
    ...
    removeNodeOfOperation(node_clones, fly_moddims_t);
