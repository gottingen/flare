...
void packData(Param<To> out, const fly::dim4 od, const fly::dim4 os,
              CParam<Ti> in) {
    To* out_ptr = out.get();

    const fly::dim4 id = in.dims();
    const fly::dim4 is = in.strides();
...
template<typename To, typename Ti>
void padArray(Param<To> out, const fly::dim4 od, const fly::dim4 os,
...
template<typename T>
void complexMultiply(Param<T> packed, const fly::dim4 sig_dims,
                     const fly::dim4 sig_strides, const fly::dim4 fit_dims,
                     const fly::dim4 fit_strides, FLY_BATCH_KIND kind,
                     const dim_t offset) {
    T* out_ptr = packed.get() + (kind == FLY_BATCH_RHS ? offset : 0);
    T* in1_ptr = packed.get();
    T* in2_ptr = packed.get() + offset;

    const fly::dim4& od  = (kind == FLY_BATCH_RHS ? fit_dims : sig_dims);
    const fly::dim4& os  = (kind == FLY_BATCH_RHS ? fit_strides : sig_strides);
    const fly::dim4& i1d = sig_dims;
    const fly::dim4& i2d = fit_dims;
    const fly::dim4& i1s = sig_strides;
    const fly::dim4& i2s = fit_strides;
...
void reorderHelper(To* out_ptr, const fly::dim4& od, const fly::dim4& os,
                   const Ti* in_ptr, const fly::dim4& id, const fly::dim4& is,
                   const fly::dim4& fd, const int half_di0, const int fftScale) {
...
void reorder(Param<T> out, Param<convT> packed, CParam<T> filter,
    ...
    const fly::dim4 out_strides = out.strides();

    const fly::dim4 filter_dims = filter.dims();
