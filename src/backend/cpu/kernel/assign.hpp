...
template<typename T>
void assign(Param<T> out, fly::dim4 dDims, CParam<T> rhs,
            std::vector<bool> const isSeq, std::vector<fly_seq> const seqs,
            std::vector<CParam<uint>> idxArrs) {
    fly::dim4 pDims = out.dims();
    // retrieve dimensions & strides for array to which rhs is being copied to
    fly::dim4 dst_offsets = toOffset(seqs, dDims);
    fly::dim4 dst_strides = toStride(seqs, dDims);
    // retrieve rhs array dimenesions & strides
    fly::dim4 src_dims    = rhs.dims();
    fly::dim4 src_strides = rhs.strides();
    // declare pointers to fly_array index data
