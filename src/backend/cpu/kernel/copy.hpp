...
template<typename T>
void stridedCopy(T* dst, fly::dim4 const& ostrides, T const* src,
                 fly::dim4 const& dims, fly::dim4 const& strides, unsigned dim) {
    if (dim == 0) {
        if (strides[dim] == 1) {
...
void copyElemwise(Param<OutT> dst, CParam<InT> src, OutT default_value,
                  double factor) {
    fly::dim4 src_dims    = src.dims();
    fly::dim4 dst_dims    = dst.dims();
    fly::dim4 src_strides = src.strides();
    fly::dim4 dst_strides = dst.strides();
...
struct CopyImpl<T, T> {
    ...
    static void copy_go(T* dst_ptr, const fly::dim4& dst_strides,
                        const fly::dim4& dst_dims, T const* src_ptr,
                        const fly::dim4& src_strides, const fly::dim4& src_dims,
