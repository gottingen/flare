...
template<typename T, fly_op_t op, fly_storage type>
void sparseArithOpD(Param<T> output, CParam<T> values, CParam<int> rowIdx,
    ...
    std::vector<int> temp;
    if (type == FLY_STORAGE_CSR) {
        temp.resize(values.dims().elements());
        for (int i = 0; i < rowIdx.dims(0) - 1; i++) {
            for (int ii = rPtr[i]; ii < rPtr[i + 1]; ii++) { temp[ii] = i; }
        }
        //} else if(type == FLY_STORAGE_CSC) {   // For future
    }

    const int *xx = (type == FLY_STORAGE_CSR) ? temp.data() : rPtr;
    const int *yy = (type == FLY_STORAGE_CSC) ? temp.data() : cPtr;
