...
Array<T> solveLU(const Array<T> &A, const Array<int> &pivot, const Array<T> &b,
    ...
    auto func = [=](CParam<T> A, Param<T> B, CParam<int> pivot, int N,
                    int NRHS) {
        getrs_func<T>()(FLY_LAPACK_COL_MAJOR, 'N', N, NRHS, A.get(),
...
Array<T> triangleSolve(const Array<T> &A, const Array<T> &b,
    ...
    auto func = [=](const CParam<T> A, Param<T> B, int N, int NRHS,
                    const fly_mat_prop options) {
        trtrs_func<T>()(FLY_LAPACK_COL_MAJOR, options & FLY_MAT_UPPER ? 'U' : 'L',
                        'N',  // transpose flag
                        options & FLY_MAT_DIAG_UNIT ? 'U' : 'N', N, NRHS,
        ...
        for (int j = 0; j < batchz; j++) {
            ...
            if (M == N) {
                ...
                auto func = [](Param<T> A, Param<T> B, Param<int> pivot, int N,
                               int K) {
                    gesv_func<T>()(FLY_LAPACK_COL_MAJOR, N, K, A.get(),
                ...
                auto func = [=](Param<T> A, Param<T> B, int M, int N, int K) {
                    ...
                    gels_func<T>()(FLY_LAPACK_COL_MAJOR, 'N', M, N, K, A.get(),
...
Array<T> solveLU(const Array<T> &A, const Array<int> &pivot, const Array<T> &b,
                 const fly_mat_prop options) {
    FLY_ERROR(
        "This version of Flare was built without linear algebra routines",
