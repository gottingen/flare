...
// Defines type conversions from Flare (OpenCL) to CLBlast (C++ std)
template<typename T>
struct CLBlastType {
    using Type = T;
};
...
template<>
struct CLBlastType<flare::common::half> {
    using Type = cl_half;
};

// Converts a constant from Flare types (OpenCL) to CLBlast types (C++ std)
template<typename T>
...
template<>
cl_half inline toCLBlastConstant(const flare::common::half val) {
    cl_half out;
...
template<>
struct CLBlastBasicType<flare::common::half> {
    using Type = cl_half;
};
template<>
