...
void *Allocator::nativeAlloc(const size_t bytes) {
    ...
    if (err != CL_SUCCESS) {
        auto str = fmt::format("Failed to allocate device memory of size {}",
                               bytesToString(bytes));
        FLY_ERROR(str, FLY_ERR_NO_MEM);
    }

    FLY_TRACE("nativeAlloc: {} {}", bytesToString(bytes), ptr);
    return ptr;
}
...
void Allocator::nativeFree(void *ptr) {
    ...
    if (err != CL_SUCCESS) {
        FLY_ERROR("Failed to release device memory.", FLY_ERR_RUNTIME);
    }
}
...
void *AllocatorPinned::nativeAlloc(const size_t bytes) {
    ...
    auto buf   = clCreateBuffer(getContext()(), CL_MEM_ALLOC_HOST_PTR, bytes,
                                nullptr, &err);
    if (err != CL_SUCCESS) {
        FLY_ERROR("Failed to allocate pinned memory.", FLY_ERR_NO_MEM);
    }
    ...
    ptr = clEnqueueMapBuffer(getQueue()(), buf, CL_TRUE,
                             ...
                             nullptr, &err);
    if (err != CL_SUCCESS) {
        FLY_ERROR("Failed to map pinned memory", FLY_ERR_RUNTIME);
    }
