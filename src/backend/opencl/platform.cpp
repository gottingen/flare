...
// Include this before fly/opencl.h
...
using flare::opencl::Allocator;
using flare::opencl::AllocatorPinned;
...
int getBackend() { return FLY_BACKEND_OPENCL; }
...
flycl::platform getPlatformEnum(Device dev) {
    string pname = getPlatformName(dev);
    if (verify_present(pname, "AMD"))
        return FLYCL_PLATFORM_AMD;
    else if (verify_present(pname, "NVIDIA"))
        return FLYCL_PLATFORM_NVIDIA;
    else if (verify_present(pname, "INTEL"))
        return FLYCL_PLATFORM_INTEL;
    else if (verify_present(pname, "APPLE"))
        return FLYCL_PLATFORM_APPLE;
    else if (verify_present(pname, "BEIGNET"))
        return FLYCL_PLATFORM_BEIGNET;
    else if (verify_present(pname, "POCL"))
        return FLYCL_PLATFORM_POCL;
    return FLYCL_PLATFORM_UNKNOWN;
}

string getDeviceInfo() noexcept {
    ostringstream info;
    info << "Flare v" << FLY_VERSION << " (OpenCL, " << get_system()
         << ", build " << FLY_REVISION << ")\n";
...
flycl::platform getActivePlatformVendor() {
    device_id_t& devId = tlocalActiveDeviceId();
...
vector<Version> getOpenCLCDeviceVersion(const Device& device) {
    ...
    /// The ifdef allows us to support BUILDING Flare with older
...
bool OpenCLCPUOffload(bool forceOffloadOSX) {
    static const bool offloadEnv = getEnvVar("FLY_OPENCL_CPU_OFFLOAD") != "0";
    bool offload                 = false;
    ...
    // irrespective of the FLY_OPENCL_CPU_OFFLOAD value
...
void addDeviceContext(cl_device_id dev, cl_context ctx, cl_command_queue que) {
    ...
    int nDevices = 0;
    {
        ...
        devMngr.mPlatforms.push_back(
            std::make_pair<std::unique_ptr<cl::Platform>, flycl_platform>(
...
void setDeviceContext(cl_device_id dev, cl_context ctx) {
    ...
    FLY_ERROR("No matching device found", FLY_ERR_ARG);
}

void removeDeviceContext(cl_device_id dev, cl_context ctx) {
    if (getDevice()() == dev && getContext()() == ctx) {
        FLY_ERROR("Cannot pop the device currently in use", FLY_ERR_ARG);
    }
    ...
    if (deleteIdx < static_cast<int>(devMngr.mUserDeviceOffset)) {
        FLY_ERROR("Cannot pop Flare internal devices", FLY_ERR_ARG);
    } else if (deleteIdx == -1) {
        FLY_ERROR("No matching device found", FLY_ERR_ARG);
    } else {
...
bool synchronize_calls() {
    static const bool sync = getEnvVar("FLY_SYNCHRONOUS_CALLS") == "1";
    return sync;
}
...
int& getMaxJitSize() {
    ...
    if (length <= 0) {
        string env_var = getEnvVar("FLY_OPENCL_MAX_JIT_LEN");
        if (!env_var.empty()) {
...
MemoryManagerBase& memoryManager() {
    ...
    call_once(flag, [&]() {
        ...
        inst.memManager = make_unique<common::DefaultMemoryManager>(
            ...
            FLY_MEM_DEBUG || FLY_OPENCL_MEM_DEBUG);
...
using namespace flare::opencl;

fly_err flycl_get_device_type(flycl_device_type* res) {
    try {
        *res = static_cast<flycl_device_type>(getActiveDeviceType());
    }
    CATCHALL;
    return FLY_SUCCESS;
}
...
fly_err flycl_get_platform(flycl_platform* res) {
    try {
        *res = static_cast<flycl_platform>(getActivePlatformVendor());
    }
    CATCHALL;
    return FLY_SUCCESS;
}
