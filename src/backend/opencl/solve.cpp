...
#define UNMQR 0  // FIXME: UNMQR == 1 should be faster but does not work
        ...
        T beta         = scalar<T>(0.0);
        gemm<T>(B_new, FLY_MAT_NONE, FLY_MAT_NONE, &alpha, A, B, &beta);
        B = B_new;
#endif
    } else if (M > N) {
        // Least squres for this case is solved using the following
        ...
        if (getActivePlatformVendor() == FLYCL_PLATFORM_NVIDIA) {
...
Array<T> triangleSolve(const Array<T> &A, const Array<T> &b,
    ...
    if (getActivePlatformVendor() == FLYCL_PLATFORM_NVIDIA &&
        ...
        OPENCL_BLAS_CHECK(gpu_blas_trsm(
            ...
            options & FLY_MAT_DIAG_UNIT ? OPENCL_BLAS_UNIT_DIAGONAL
        ...
        OPENCL_BLAS_CHECK(gpu_blas_trsm(
            ...
            options & FLY_MAT_LOWER ? OPENCL_BLAS_TRIANGLE_LOWER
