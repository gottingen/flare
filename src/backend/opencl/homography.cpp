...
int homography(Array<T> &bestH, const Array<float> &x_src,
    ...
    Array<float> err = createEmptyArray<float>(fly::dim4());
    if (htype == FLY_HOMOGRAPHY_LMEDS) {
        ...
        err = createEmptyArray<float>(fly::dim4(1));
    }

    const size_t iter_sz = divup(iter, 256) * 256;

    fly::dim4 rdims(4, iter_sz);
    Array<float> fctr =
        createValueArray<float>(rdims, static_cast<float>(nsamples));
    Array<float> rnd = arithOp<float, fly_mul_t>(initial, fctr, rdims);

    Array<T> tmpH =
        createValueArray<T>(fly::dim4(9, iter_sz), static_cast<T>(0));

    bestH = createValueArray<T>(fly::dim4(3, 3), static_cast<T>(0));
