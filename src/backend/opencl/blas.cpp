...
// Converts an fly_mat_prop options to a transpose type for one of the OpenCL
// BLAS back-ends
OPENCL_BLAS_TRANS_T
toBlasTranspose(fly_mat_prop opt) {
    switch (opt) {
        case FLY_MAT_NONE: return OPENCL_BLAS_NO_TRANS;
        case FLY_MAT_TRANS: return OPENCL_BLAS_TRANS;
        case FLY_MAT_CTRANS: return OPENCL_BLAS_CONJ_TRANS;
        default: FLY_ERROR("INVALID fly_mat_prop", FLY_ERR_ARG);
    }
}

template<typename T>
void gemm_fallback(Array<T> &out, fly_mat_prop optLhs, fly_mat_prop optRhs,
...
template<>
void gemm_fallback<half>(Array<half> & /*out*/, fly_mat_prop /*optLhs*/,
                         fly_mat_prop /*optRhs*/, const half * /*alpha*/,
...
void gemm(Array<T> &out, fly_mat_prop optLhs, fly_mat_prop optRhs, const T *alpha,
    ...
    if (OpenCLCPUOffload(false) &&
        static_cast<fly_dtype>(dtype_traits<T>::fly_type) != f16) {
    ...
    for (int n = 0; n < batchSize; n++) {
        ...
        cl::Event event;
        if (rDims[bColDim] == 1) {
            dim_t incr = (optRhs == FLY_MAT_NONE) ? rStrides[0] : rStrides[1];
...
Array<T> dot(const Array<T> &lhs, const Array<T> &rhs, fly_mat_prop optLhs,
             fly_mat_prop optRhs) {
    const Array<T> lhs_ = (optLhs == FLY_MAT_NONE ? lhs : conj<T>(lhs));
    const Array<T> rhs_ = (optRhs == FLY_MAT_NONE ? rhs : conj<T>(rhs));

    const Array<T> temp = arithOp<T, fly_mul_t>(lhs_, rhs_, lhs_.dims());
