...
template<typename T>
std::array<Kernel, 5> getHomographyKernels(const fly_homography_type htype) {
...
int computeH(Param bestH, Param H, Param err, Param x_src, Param y_src,
    ...
    inliers.info.dims[0]    = (htype == FLY_HOMOGRAPHY_RANSAC)
                                  ? blk_x_eh
    ...
    if (htype == FLY_HOMOGRAPHY_LMEDS) {
        ...
        kernel::reduce<unsigned, unsigned, fly_add_t>(totalInliers, inliers, 0,
                                                     false, 0.0);
    ...
    } else /* if (htype == FLY_HOMOGRAPHY_RANSAC) */ {
        unsigned blockIdx;
        inliersH = kernel::ireduceAll<unsigned, fly_max_t>(&blockIdx, inliers);
