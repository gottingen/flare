...
template<typename T>
void rotate(Param out, const Param in, const float theta, fly_interp_type method,
            int order) {
    ...
    constexpr bool isComplex =
        static_cast<fly_dtype>(dtype_traits<T>::fly_type) == c32 ||
        static_cast<fly_dtype>(dtype_traits<T>::fly_type) == c64;
