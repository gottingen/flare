...
FLY_CONSTEXPR const char *getOpString() {
    switch (op) {
        case fly_add_t: return "ADD";
        case fly_sub_t: return "SUB";
        case fly_mul_t: return "MUL";
        case fly_div_t: return "DIV";
...
void ssArithCSR(Param oVals, Param oColIdx, const Param oRowIdx, const uint M,
    ...
    const T iden_val =
        (op == fly_mul_t || op == fly_div_t ? scalar<T>(1) : scalar<T>(0));
