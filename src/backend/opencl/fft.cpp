...
void computeDims(size_t rdims[FLY_MAX_DIMS], const dim4 &idims) {
...
void fft_inplace(Array<T> &in, const int rank, const bool direction) {
    ...
    size_t tdims[FLY_MAX_DIMS], istrides[FLY_MAX_DIMS];

    computeDims(tdims, in.dims());
    computeDims(istrides, in.strides());

    int batch = 1;
    for (int i = rank; i < FLY_MAX_DIMS; i++) { batch *= tdims[i]; }
...
Array<Tc> fft_r2c(const Array<Tr> &in, const int rank) {
    ...
    verifySupported(rank, in.dims());
    size_t tdims[FLY_MAX_DIMS], istrides[FLY_MAX_DIMS], ostrides[FLY_MAX_DIMS];
