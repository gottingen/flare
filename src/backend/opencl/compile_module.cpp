...
using flare::opencl::getActiveDeviceId;
using flare::opencl::getDevice;
using flare::opencl::Kernel;
using flare::opencl::Module;
using cl::Error;
using cl::Program;
using fmt::format;
using nonstd::span;
...
#define THROW_BUILD_LOG_EXCEPTION(PROG)                              \
    do {                                                             \
        ...
        string info        = getEnvVar("FLY_OPENCL_SHOW_BUILD_INFO"); \
        if (!info.empty() && info != "0") puts(build_error.c_str()); \
        FLY_ERROR(build_error, FLY_ERR_INTERNAL);                      \
    } while (0)
...
string getKernelCacheFilename(const int device, const string &key) {
    auto &dev = flare::opencl::getDevice(device);
    ...
    return "KER" + key + "_CL_" + infix + "_FLY_" +
...
Module compileModule(const string &moduleKey, span<const string> sources,
    ...
    auto program      = flare::opencl::buildProgram(sources, options);
    auto compileEnd   = high_resolution_clock::now();

#ifdef FLY_CACHE_KERNELS_TO_DISK
    const int device             = flare::opencl::getActiveDeviceId();
    const string &cacheDirectory = getCacheDirectory();
    if (!cacheDirectory.empty()) {
        const string cacheFile =
            cacheDirectory + FLY_PATH_SEPARATOR +
        ...
        try {
            ...
            FLY_TRACE(
                ...
                moduleKey,
                flare::opencl::getDevice(device).getInfo<CL_DEVICE_NAME>(),
                e.what());
    ...
    FLY_TRACE("{{ {:<20} : {{ compile:{:>5} ms, {{ {} }}, {} }} }}", moduleKey,
...
Module loadModuleFromDisk(const int device, const string &moduleKey,
    ...
    try {
        ...
        program = Program(flare::opencl::getContext(), {dev}, {clbin});
        program.build();
        ...
        FLY_TRACE("{{{:<20} : IO failure while loading {} for {}; {}}}",
