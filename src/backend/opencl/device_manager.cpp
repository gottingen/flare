...
static flycl::deviceType getDeviceTypeEnum(const Device& dev) {
    return static_cast<flycl::deviceType>(dev.getInfo<CL_DEVICE_TYPE>());
}
...
DeviceManager::DeviceManager()
    ...
    string deviceENV = getEnvVar("FLY_OPENCL_DEVICE_TYPE");
    ...
    FLY_TRACE("Found {} OpenCL platforms", platforms.size());
    ...
    for (auto& platform : platforms) {
        ...
        FLY_TRACE("Found {} devices on platform {}", current_devices.size(),
        ...
        if (!current_devices.empty()) {
            ...
            for (auto& dev : current_devices) {
                ...
                FLY_TRACE("Found device {} on platform {}",
    ...
    FLY_TRACE("Found {} OpenCL devices", nDevices);

    if (nDevices == 0) { FLY_ERROR("No OpenCL devices found", FLY_ERR_RUNTIME); }
    ...
    for (int i = 0; i < nDevices; i++) {
        ...
        try {
            ...
            FLY_TRACE("Error creating context for device {} with error {}\n",
    ...
    deviceENV               = getEnvVar("FLY_OPENCL_DEFAULT_DEVICE");
    if (!deviceENV.empty()) {
    ...
    deviceENV = getEnvVar("FLY_OPENCL_DEFAULT_DEVICE_TYPE");
    if (!default_device_set && !deviceENV.empty()) {
        ...
        if (!default_device_set) {
            FLY_TRACE(
                "FLY_OPENCL_DEFAULT_DEVICE_TYPE={} \
...
void DeviceManager::markDeviceForInterop(const int device,
    ...
    try {
        if (device >= static_cast<int>(mQueues.size()) ||
            ...
            FLY_TRACE("Invalid device (}) passed for CL-GL Interop", device);
