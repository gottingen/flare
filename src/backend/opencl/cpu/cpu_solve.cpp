...
Array<T> solveLU(const Array<T> &A, const Array<int> &pivot, const Array<T> &b,
    ...
    getrs_func<T>()(FLY_LAPACK_COL_MAJOR, 'N', N, NRHS, aPtr.get(),
...
Array<T> triangleSolve(const Array<T> &A, const Array<T> &b,
    ...
    trtrs_func<T>()(FLY_LAPACK_COL_MAJOR, options & FLY_MAT_UPPER ? 'U' : 'L',
                    'N',  // transpose flag
                    options & FLY_MAT_DIAG_UNIT ? 'U' : 'N', N, NRHS, aPtr.get(),
    ...
    for (int i = 0; i < batchw; i++) {
        for (int j = 0; j < batchz; j++) {
            ...
            if (M == N) {
                ...
                gesv_func<T>()(FLY_LAPACK_COL_MAJOR, N, K, pA, A.strides()[1],
                               &pivot.front(), pB, B.strides()[1]);
            } else {
                int sM = a.strides()[1];
                int sN = a.strides()[2] / sM;

                gels_func<T>()(FLY_LAPACK_COL_MAJOR, 'N', M, N, K, pA,
