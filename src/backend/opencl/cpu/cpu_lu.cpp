...
Array<int> lu_inplace(Array<T> &in, const bool convert_pivot) {
    ...
    Array<int> pivot = createEmptyArray<int>(fly::dim4(pivot_dim, 1, 1, 1));
    if (convert_pivot) { pivot = range<int>(fly::dim4(M, 1, 1, 1)); }

    mapped_ptr<T> inPtr   = in.getMappedPtr();
    mapped_ptr<int> piPtr = pivot.getMappedPtr();

    getrf_func<T>()(FLY_LAPACK_COL_MAJOR, M, N, inPtr.get(), in.strides()[1],
                    piPtr.get());
