...
class AfError : public std::logic_error {
   ...
   public:
    AfError(const char* const func, const char* const file, const int line,
            const char* const message, fly_err err,
            boost::stacktrace::stacktrace st);

    AfError(std::string func, std::string file, const int line,
            const std::string& message, fly_err err,
            boost::stacktrace::stacktrace st);
    ...
    AfError(AfError&& other) noexcept
        ...
        , error(std::forward<fly_err>(other.error)) {}
    ...
    fly_err getError() const noexcept;
...
class TypeError : public AfError {
   ...
   public:
    TypeError(const char* const func, const char* const file, const int line,
              const int index, const fly_dtype type,
...
fly_err set_global_error_string(const std::string& msg,
                               fly_err err = FLY_ERR_UNKNOWN);

#define DIM_ASSERT(INDEX, COND)                                          \
    do {                                                                 \
        if ((COND) == false) {                                           \
            throw DimensionError(__FLY_FUNC__, __FLY_FILENAME__, __LINE__, \
                                 INDEX, #COND,                           \
...
#define ARG_ASSERT(INDEX, COND)                                                \
    do {                                                                       \
        if ((COND) == false) {                                                 \
            throw ArgumentError(__FLY_FUNC__, __FLY_FILENAME__, __LINE__, INDEX, \
...
#define TYPE_ERROR(INDEX, type)                                              \
    do {                                                                     \
        throw TypeError(__FLY_FUNC__, __FLY_FILENAME__, __LINE__, INDEX, type, \
                        boost::stacktrace::stacktrace());                    \
    } while (0)

#define FLY_ERROR(MSG, ERR_TYPE)                                              \
    do {                                                                     \
        throw AfError(__FLY_FUNC__, __FLY_FILENAME__, __LINE__, MSG, ERR_TYPE, \
                      boost::stacktrace::stacktrace());                      \
    } while (0)

#define FLY_RETURN_ERROR(MSG, ERR_TYPE)                                       \
    do {                                                                     \
        std::stringstream s;                                                 \
        s << "Error in " << __FLY_FUNC__ << "\n"                              \
          << "In file " << __FLY_FILENAME__ << ":" << __LINE__ << ": " << MSG \
          << "\n"                                                            \
          << boost::stacktrace::stacktrace();                                \
...
#define TYPE_ASSERT(COND)                                       \
    do {                                                        \
        if ((COND) == false) {                                  \
            FLY_ERROR("Type mismatch inputs", FLY_ERR_DIFF_TYPE); \
        }                                                       \
    } while (0)

#define FLY_ASSERT(COND, MESSAGE) assert(MESSAGE&& COND)
...
#define FLY_CHECK(fn)                                                       \
    do {                                                                   \
        ...
        throw AfError(__FLY_FUNC__, __FLY_FILENAME__, __LINE__, "\n", __err, \
                      boost::stacktrace::stacktrace());                    \
    } while (0)
