...
fly_err processException() {
    ...
    try {
        ...
        if (ex.code() == sycl::errc::memory_allocation) {
            err = set_global_error_string(oneapi_err_msg, FLY_ERR_NO_MEM);
        } else {
            err = set_global_error_string(oneapi_err_msg, FLY_ERR_INTERNAL);
        }
        ...
        if (ex.err() == CL_MEM_OBJECT_ALLOCATION_FAILURE) {
            err = set_global_error_string(opencl_err_msg, FLY_ERR_NO_MEM);
        } else {
            err = set_global_error_string(opencl_err_msg, FLY_ERR_INTERNAL);
        }
#endif
