...
#define FLY_CPLUSPLUS_CHANGED
#endif

#include <cuda_fp16.h>

#ifdef FLY_CPLUSPLUS_CHANGED
#undef __cplusplus
#undef FLY_CPLUSPLUS_CHANGED
...
// we do not include the fly/compilers header in nvrtc compilations so
// we are defining the FLY_CONSTEXPR expression here
...
FLY_CONSTEXPR __DH__ native_half_t float2half_impl(float value) {
    return __float2half_rn(value);
}

template<std::float_round_style R = std::round_to_nearest>
FLY_CONSTEXPR __DH__ native_half_t float2half_impl(double value) {
    return __float2half_rn(value);
}
...
template<typename T>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(T value) noexcept;

template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(int value) noexcept {
    return __int2half_rn(value);
}

template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(unsigned value) noexcept {
    return __uint2half_rn(value);
}

template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(long long value) noexcept {
    return __ll2half_rn(value);
}

template<>
FLY_CONSTEXPR __DH__ native_half_t
int2half_impl(unsigned long long value) noexcept {
    return __ull2half_rn(value);
}

template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(short value) noexcept {
    return __short2half_rn(value);
}
template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(unsigned short value) noexcept {
    return __ushort2half_rn(value);
}

template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(char value) noexcept {
    return __ull2half_rn(value);
}
template<>
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(unsigned char value) noexcept {
    return __ull2half_rn(value);
}
...
template<std::float_round_style R = std::round_to_nearest>
FLY_CONSTEXPR native_half_t float2half_impl(float value) {
    return static_cast<native_half_t>(value);
}

template<std::float_round_style R = std::round_to_nearest>
FLY_CONSTEXPR native_half_t float2half_impl(double value) {
    return static_cast<native_half_t>(value);
}
...
FLY_CONSTEXPR native_half_t int2half_impl(T value) noexcept {
    return static_cast<native_half_t>(value);
}
...
FLY_CONSTEXPR __DH__ native_half_t int2half_impl(T value) noexcept {
    ...
    uint16_t bits = S << 15;
    if (value > 0xFFFF) {
        FLY_IF_CONSTEXPR(R == std::round_toward_infinity)
        bits |= (0x7C00 - S);
        else FLY_IF_CONSTEXPR(R == std::round_toward_neg_infinity) bits |=
            (0x7BFF + S);
        ...
        bits |= (exp << 10) + m;
        if (exp > 24) {
            FLY_IF_CONSTEXPR(R == std::round_to_nearest)
            bits += (value >> (exp - 25)) & 1
#if HALF_ROUND_TIES_TO_EVEN
                    & (((((1 << (exp - 25)) - 1) & value) != 0) | bits)
#endif
                ;
            else FLY_IF_CONSTEXPR(R == std::round_toward_infinity) bits +=
...
__DH__ native_half_t float2half_impl(float value) noexcept {
...
#endif
        ;
    else FLY_IF_CONSTEXPR(R == std::round_toward_zero) hbits -=
        ((hbits & 0x7FFF) == 0x7C00) & ~shift_table[bits >> 23];
    else FLY_IF_CONSTEXPR(R == std::round_toward_infinity) hbits +=
        ((((bits & 0x7FFFFF &
    ...
    else FLY_IF_CONSTEXPR(R == std::round_toward_neg_infinity) hbits +=
        ((((bits & 0x7FFFFF &
            ((static_cast<uint32_t>(1) << (shift_table[bits >> 23])) - 1)) !=
           0) |
...
__DH__ native_half_t float2half_impl(double value) {
    ...
    if (exp > 1038) {
        ...
        FLY_IF_CONSTEXPR(R == std::round_toward_neg_infinity)
...
FLY_CONSTEXPR __DH__ native_half_t int2half(T value) noexcept {
...
FLY_CONSTEXPR T half2int(native_half_t value) {
#ifdef __CUDA_ARCH__
    FLY_IF_CONSTEXPR(std::is_same<T, short>::value ||
                    std::is_same<T, char>::value ||
                    std::is_same<T, unsigned char>::value) {
        return __half2short_rn(value);
    }
    else FLY_IF_CONSTEXPR(std::is_same<T, unsigned short>::value) {
        return __half2ushort_rn(value);
    }
    else FLY_IF_CONSTEXPR(std::is_same<T, long long>::value) {
        return __half2ll_rn(value);
    }
    else FLY_IF_CONSTEXPR(std::is_same<T, unsigned long long>::value) {
        return __half2ull_rn(value);
    }
    else FLY_IF_CONSTEXPR(std::is_same<T, int>::value) {
    ...
    if (e < 0x3800) {
        ...
        else FLY_IF_CONSTEXPR(R == std::round_toward_neg_infinity) return -T(
            value > 0x8000);
        return T();
    }
    ...
    if (e < 25) {
        FLY_IF_CONSTEXPR(R == std::round_to_nearest)
        m += (1 << (24 - e)) - (~(m >> (25 - e)) & E);
        else FLY_IF_CONSTEXPR(R == std::round_toward_infinity) m +=
            ((value >> 15) - 1) & ((1 << (25 - e)) - 1U);
        else FLY_IF_CONSTEXPR(R == std::round_toward_neg_infinity) m +=
            -(value >> 15) & ((1 << (25 - e)) - 1U);
        m >>= 25 - e;
    } else
        m <<= e - 25;
...
FLY_CONSTEXPR __DH__ static inline bool operator==(
    flare::common::half lhs, flare::common::half rhs) noexcept;
FLY_CONSTEXPR __DH__ static inline bool operator!=(
    flare::common::half lhs, flare::common::half rhs) noexcept;

__DH__ static inline bool operator<(flare::common::half lhs,
                                    flare::common::half rhs) noexcept;
__DH__ static inline bool operator<(flare::common::half lhs,
                                    float rhs) noexcept;

FLY_CONSTEXPR __DH__ static inline bool isinf(half val) noexcept;

/// Classification implementation.
/// \param arg value to classify
/// \retval true if not a number
/// \retval false else
FLY_CONSTEXPR __DH__ static inline bool isnan(
    flare::common::half val) noexcept;
...
class alignas(2) half {
   ...
   public:
    ...
    FLY_CONSTEXPR __DH__ half(internal::binary_t, uint16_t bits) noexcept
        :
#if defined(__CUDA_ARCH__)
    ...
    template<typename T>
    FLY_CONSTEXPR __DH__ explicit half(T value) noexcept
...
#if defined(__CUDA_ARCH__) || defined(FLY_ONEAPI)
    FLY_CONSTEXPR __DH__ explicit half(native_half_t value) noexcept
        : data_(value) {}

    FLY_CONSTEXPR __DH__ half& operator=(native_half_t value) noexcept {
    ...
    FLY_CONSTEXPR __DH__ explicit operator short() const noexcept {
        return half2int<std::round_indeterminate, true, short>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator long long() const noexcept {
        return half2int<std::round_indeterminate, true, long long int>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator int() const noexcept {
        return half2int<std::round_indeterminate, true, int>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator unsigned() const noexcept {
        return half2int<std::round_indeterminate, true, unsigned>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator unsigned short() const noexcept {
        return half2int<std::round_indeterminate, true, unsigned>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator unsigned long long() const noexcept {
        return half2int<std::round_indeterminate, true, unsigned>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator char() const noexcept {
        return half2int<std::round_indeterminate, true, char>(data_);
    }

    FLY_CONSTEXPR __DH__ explicit operator unsigned char() const noexcept {
        return half2int<std::round_indeterminate, true, unsigned char>(data_);
    }

#if defined(__CUDA_ARCH__) || defined(FLY_ONEAPI)
    FLY_CONSTEXPR __DH__ operator native_half_t() const noexcept {
        return data_;
    };
#endif

    friend FLY_CONSTEXPR __DH__ bool operator==(half lhs, half rhs) noexcept;
    friend FLY_CONSTEXPR __DH__ bool operator!=(half lhs, half rhs) noexcept;
    friend __DH__ bool operator<(flare::common::half lhs,
                                 flare::common::half rhs) noexcept;
    friend __DH__ bool operator<(flare::common::half lhs,
                                 float rhs) noexcept;

    friend FLY_CONSTEXPR __DH__ bool isinf(half val) noexcept;
    friend FLY_CONSTEXPR __DH__ inline bool isnan(half val) noexcept;

    FLY_CONSTEXPR __DH__ flare::common::half operator-() const {
#if __CUDA_ARCH__ >= 530
        return flare::common::half(__hneg(data_));
#elif defined(__CUDA_ARCH__)
        return flare::common::half(-(__half2float(data_)));
#elif defined(FLY_ONEAPI)
        return flare::common::half(-data_);
#else
        return flare::common::half(internal::binary, data_ ^ 0x8000);
#endif
    }
    ...
    FLY_CONSTEXPR static half infinity() {
        half out;
#ifdef __CUDA_ARCH__
...
template<>
class numeric_limits<flare::common::half> : public numeric_limits<float> {
   public:
    ...
    static FLY_CONSTEXPR __DH__ flare::common::half min() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0x0400);
    }

    /// Smallest finite value.
    static FLY_CONSTEXPR __DH__ flare::common::half lowest() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0xFBFF);
    }

    /// Largest finite value.
    static FLY_CONSTEXPR __DH__ flare::common::half max() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0x7BFF);
    }

    /// Difference between one and next representable value.
    static FLY_CONSTEXPR __DH__ flare::common::half epsilon() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0x1400);
    }

    /// Maximum rounding error.
    static FLY_CONSTEXPR __DH__ flare::common::half round_error() noexcept {
        return flare::common::half(
            flare::common::internal::binary,
            (round_style == std::round_to_nearest) ? 0x3800 : 0x3C00);
    }

    /// Positive infinity.
    static FLY_CONSTEXPR __DH__ flare::common::half infinity() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0x7C00);
    }

    /// Quiet NaN.
    static FLY_CONSTEXPR __DH__ flare::common::half quiet_NaN() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0x7FFF);
    }

    /// Signalling NaN.
    static FLY_CONSTEXPR __DH__ flare::common::half
    ...
    static FLY_CONSTEXPR __DH__ flare::common::half denorm_min() noexcept {
        return flare::common::half(flare::common::internal::binary,
                                       0x0001);
    }
};
...
template<>
struct hash<
    flare::common::half>  //: unary_function<flare::common::half,size_t>
{
    /// Type of function argument.
    typedef flare::common::half argument_type;
...
namespace flare {
namespace common {
FLY_CONSTEXPR __DH__ static bool isinf(half val) noexcept {
#if __CUDA_ARCH__ >= 530
...
FLY_CONSTEXPR __DH__ static inline bool isnan(half val) noexcept {
#if __CUDA_ARCH__ >= 530
