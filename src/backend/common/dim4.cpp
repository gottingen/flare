...
#if FLY_COMPILER_CXX_STATIC_ASSERT
static_assert(std::is_standard_layout<dim4>::value,
              "fly::dim4 must be a standard layout type");
#endif
...
bool hasEnd(const fly_seq& seq) { return (seq.begin <= -1 || seq.end <= -1); }

bool isSpan(const fly_seq& seq) {
    return (seq.step == 0 && seq.begin == 1 && seq.end == 1);
}

size_t seqElements(const fly_seq& seq) {
    size_t out = 0;
    if (seq.step > DBL_MIN) {
...
dim_t calcDim(const fly_seq& seq, const dim_t& parentDim) {
    dim_t outDim = 1;
    if (isSpan(seq)) {
        outDim = parentDim;
    } else if (hasEnd(seq)) {
        fly_seq temp = {seq.begin, seq.end, seq.step};
