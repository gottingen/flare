...
namespace std {
template<>
struct hash<flare::common::Node *> {
    /// Calls the getHash function of the Node pointer
    size_t operator()(flare::common::Node *const n) const noexcept;
};
}  // namespace std
...
static const char *getFullName(fly::dtype type) {
    switch (type) {
...
static const char *getShortName(fly::dtype type) {
    switch (type) {
...
class Node {
   ...
   public:
    ...
    Node() = default;
    Node(const fly::dtype type, const int height,
    ...
    virtual fly_op_t getOp() const noexcept { return fly_none_t; }
    ...
    /// Returns the type
    fly::dtype getType() const { return m_type; }
    ...
    virtual size_t getHash() const noexcept {
        ...
        std::hash<fly::dtype> flytype_hash;
        std::hash<int> int_hash;
        const void *ptr = this;
        size_t h =
            ptr_hash(ptr) ^ (flytype_hash(m_type) << 1) ^ (int_hash(m_height));
        return h;
    }
...
#ifdef FLY_CPU
    template<typename U>
    friend void flare::cpu::kernel::evalMultiple(
        std::vector<flare::cpu::Param<U>> arrays,
        std::vector<common::Node_ptr> output_nodes_);

    virtual void setShape(fly::dim4 new_shape) { UNUSED(new_shape); }
...
inline std::array<int, 4> compressArray(ArrayT dims) {
    ...
    do {
        changed = false;
        for (int i = 0; i < FLY_MAX_DIMS - 1; i++) {
...
void removeEmptyDimensions(nonstd::span<ParamT> outputs,
    ...
    applyShifts<dim_t>(shifts, {outStrides, FLY_MAX_DIMS});
        ...
        switch ((*nodeIt)->getNodeType()) {
            case kNodeType::Buffer: {
                ...
                applyShifts<dim_t>(shifts,
                                   {buf->m_param.dims_ptr(), FLY_MAX_DIMS});
                applyShifts<dim_t>(shifts,
                                   {buf->m_param.strides_ptr(), FLY_MAX_DIMS});
            } break;
    ...
    std::for_each(
        std::begin(outputs) + 1, std::end(outputs), [&shifts](ParamT &output) {
            applyShifts<dim_t>(shifts, {output.dims_ptr(), FLY_MAX_DIMS});
            applyShifts<dim_t>(shifts, {output.strides_ptr(), FLY_MAX_DIMS});
        });
}
