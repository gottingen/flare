...
class NaryNode : public Node {
   ...
   public:
    NaryNode(const fly::dtype type, const char *op_str, const int num_children,
             const std::array<common::Node_ptr, Node::kMaxChildren> &&children,
             const fly_op_t op, const int height)
        : common::Node(
              type, height,
              std::forward<
    ...
    fly_op_t getOp() const noexcept final { return m_op; }
...
common::Node_ptr createNaryNode(
    const fly::dim4 &odims, FUNC createNode,
