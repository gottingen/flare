...
class SparseArrayBase {
   ...
   public:
    ...
    SparseArrayBase(const fly::dim4 &_dims, dim_t _nNZ, fly::storage _storage,
                    fly_dtype _type);

    SparseArrayBase(const fly::dim4 &_dims, dim_t _nNZ, int *const _rowIdx,
                    int *const _colIdx, const fly::storage _storage,
                    fly_dtype _type, bool _is_device = false,
                    bool _copy_device = false);

    SparseArrayBase(const fly::dim4 &_dims, const detail::Array<int> &_rowIdx,
...
#undef INSTANTIATE_INFO
    ...
    fly::storage getStorage() const { return stype; }
};
...
class SparseArray {
   private:
    ...
    SparseArray(const fly::dim4 &_dims, dim_t _nNZ, fly::storage _storage);

    explicit SparseArray(const fly::dim4 &_dims, dim_t _nNZ, T *const _values,
                         int *const _rowIdx, int *const _colIdx,
                         const fly::storage _storage, bool _is_device = false,
                         bool _copy_device = false);

    SparseArray(const fly::dim4 &_dims, const detail::Array<T> &_values,
                const detail::Array<int> &_rowIdx,
                const detail::Array<int> &_colIdx, const fly::storage _storage,
                bool _copy = false);
...
#define INSTANTIATE_INFO(return_type, func) \
    ...
    INSTANTIATE_INFO(fly::storage, getStorage)
...
#undef INSTANTIATE_INFO
    ...
    friend SparseArray<T> createEmptySparseArray<T>(const fly::dim4 &_dims,
                                                    dim_t _nNZ,
                                                    const fly::storage _storage);

    friend SparseArray<T> createHostDataSparseArray<T>(
        const fly::dim4 &_dims, const dim_t nNZ, const T *const _values,
    ...
    friend SparseArray<T> createArrayDataSparseArray<T>(
        const fly::dim4 &_dims, const detail::Array<T> &_values,
