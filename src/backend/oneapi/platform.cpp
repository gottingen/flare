...
using flare::oneapi::Allocator;
using flare::oneapi::AllocatorPinned;
...
int getBackend() { return FLY_BACKEND_ONEAPI; }
...
fly_oneapi_platform getPlatformEnum(sycl::device dev) {
    string pname = getPlatformName(dev);
    if (verify_present(pname, "AMD"))
        return FLY_ONEAPI_PLATFORM_AMD;
    else if (verify_present(pname, "NVIDIA"))
        return FLY_ONEAPI_PLATFORM_NVIDIA;
    else if (verify_present(pname, "INTEL"))
        return FLY_ONEAPI_PLATFORM_INTEL;
    else if (verify_present(pname, "APPLE"))
        return FLY_ONEAPI_PLATFORM_APPLE;
    else if (verify_present(pname, "BEIGNET"))
        return FLY_ONEAPI_PLATFORM_BEIGNET;
    else if (verify_present(pname, "POCL"))
        return FLY_ONEAPI_PLATFORM_POCL;
    return FLY_ONEAPI_PLATFORM_UNKNOWN;
}

string getDeviceInfo() noexcept {
    ostringstream info;
    info << "Flare v" << FLY_VERSION << " (oneAPI, " << get_system()
         << ", build " << FLY_REVISION << ")\n";
...
bool OneAPICPUOffload(bool forceOffloadOSX) {
    static const bool offloadEnv = getEnvVar("FLY_ONEAPI_CPU_OFFLOAD") != "0";
    bool offload                 = false;
...
MemoryManagerBase& memoryManager() {
    ...
    call_once(flag, [&]() {
        ...
        inst.memManager = make_unique<common::DefaultMemoryManager>(
            ...
            FLY_MEM_DEBUG || FLY_ONEAPI_MEM_DEBUG);
...
fly_err flycl_get_platform(flycl_platform* res) {
    try {
        *res = static_cast<flycl_platform>(getActivePlatform());
    }
    CATCHALL;
    return FLY_SUCCESS;
}
