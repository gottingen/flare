...
using flare::common::getFuncName;
using flare::common::half;
using flare::common::kNodeType;
using flare::common::ModdimNode;
using flare::common::Node;
using flare::common::Node_ids;
using flare::common::Node_map_t;
using flare::common::Node_ptr;
using flare::common::NodeIterator;
using flare::common::ShiftNodeBase;
using flare::oneapi::getActiveDeviceBaseBuildFlags;
using flare::oneapi::jit::BufferNode;
using flare::oneapi::jit::ShiftNode;
...
cl_kernel getKernel(
    ...
    if (kernel_found) {
        ...
        string jitstr = flare::opencl::getKernelString(
            funcName, full_nodes, full_ids, output_ids, is_linear, false, false,
            ap[0].dims[2] > 1);

        cl_int err;
        vector<const char*> jitsources = {
            {flare::oneapi::opencl::KParam_hpp,
             flare::oneapi::opencl::jit_cl, jitstr.c_str()}};
        vector<size_t> jitsizes = {flare::oneapi::opencl::KParam_hpp_len,
                                   flare::oneapi::opencl::jit_cl_len,
                                   jitstr.size()};
...
void evalNodes(vector<Param<T>>& outputs, const vector<Node*>& output_nodes) {
    ...
    for (const Node* node : full_nodes) {
        ...
        moddimsFound |= (node->getOp() == fly_moddims_t);
    }
    ...
    if (moddimsFound || emptyColumnsFound) {
        ...
        if (moddimsFound) {
            const auto isModdim{[](const Node_ptr& ptr) {
                return ptr->getOp() == fly_moddims_t;
            }};
        ...
        h.host_task([ap, full_nodes, output_ids, full_ids, is_linear, funcName,
            ...
            switch (hh.get_backend()) {
                case backend::opencl: {
                    ...
                    cl_kernel kernel = flare::opencl::getKernel<T>(
