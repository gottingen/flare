...
template<typename Tk, typename To, fly_op_t op>
...
int reduce_by_key_first(Array<Tk> &keys_out, Array<To> &vals_out,
    ...
    do {
        ...
        if (first_pass) {
            ...
            constexpr fly_op_t op2 = (op == fly_notzero_t) ? fly_add_t : op;
...
void reduce_by_key(Array<Tk> &keys_out, Array<To> &vals_out,
    ...
    kdims[0]   = n_reduced;
    odims[dim] = n_reduced;
    std::vector<fly_seq> kindex, vindex;
    for (int i = 0; i < odims.ndims(); ++i) {
        fly_seq sk = {0.0, (double)kdims[i] - 1, 1.0};
        fly_seq sv = {0.0, (double)odims[i] - 1, 1.0};
        kindex.push_back(sk);
        vindex.push_back(sv);
    }
