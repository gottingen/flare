...
static ::oneapi::mkl::transpose toMKLTranspose(fly_mat_prop opt) {
    switch (opt) {
...
Array<T> leastSquares(const Array<T> &a, const Array<T> &b) {
    ...
    if (M < N) {
        ...
        B = (endPadding == NullShape
                 ? copyArray(b)
                 : padArrayBorders(b, NullShape, endPadding, FLY_PAD_ZERO));
        ...
        // TODO: move to helper? trsm<T>(A, B, FLY_MAT_CTRANS, true, true,
        // false);
        ...
        } else if constexpr (common::isComplex(static_cast<fly::dtype>(
                                 dtype_traits<compute_t<T>>::fly_type))) {
            std::int64_t scratchpad_size =
...
Array<T> triangleSolve(const Array<T> &A, const Array<T> &b,
    ...
    compute_t<T> alpha       = scalar<compute_t<T>>(1);
    ::oneapi::mkl::uplo uplo = (options & FLY_MAT_UPPER)
                                   ? ::oneapi::mkl::uplo::upper
                                   : ::oneapi::mkl::uplo::lower;

    ::oneapi::mkl::diag unitdiag = (options & FLY_MAT_DIAG_UNIT)
...
Array<T> solve(const Array<T> &a, const Array<T> &b,
               const fly_mat_prop options) {
    if (options & FLY_MAT_UPPER || options & FLY_MAT_LOWER) {
...
Array<T> solveLU(const Array<T> &A, const Array<int> &pivot, const Array<T> &b,
                 const fly_mat_prop options) {
    FLY_ERROR("Linear Algebra is disabled on OneAPI", FLY_ERR_NOT_CONFIGURED);
}
