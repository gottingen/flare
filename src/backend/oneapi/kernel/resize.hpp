...
template<typename T>
void resize(Param<T> out, const Param<T> in, const fly_interp_type method) {
        ...
        switch (method) {
            case FLY_INTERP_NEAREST:
                h.parallel_for(sycl::nd_range{global, local},
                               resizeCreateKernel<T, FLY_INTERP_NEAREST>(
                                   d_out, out.info, d_in, in.info,
                                   blocksPerMatX, blocksPerMatY, xf, yf));
                break;
            case FLY_INTERP_BILINEAR:
                h.parallel_for(sycl::nd_range{global, local},
                               resizeCreateKernel<T, FLY_INTERP_BILINEAR>(
                                   d_out, out.info, d_in, in.info,
                                   blocksPerMatX, blocksPerMatY, xf, yf));
                break;
            case FLY_INTERP_LOWER:
                h.parallel_for(sycl::nd_range{global, local},
                               resizeCreateKernel<T, FLY_INTERP_LOWER>(
                                   d_out, out.info, d_in, in.info,
                                   blocksPerMatX, blocksPerMatY, xf, yf));
                break;
            default: break;
