    ...
    static int idxByndEdge(const int i, const int lb, const int len) {
        ...
        switch (BType) {
            case FLY_PAD_SYM:
                ...
                break;
            case FLY_PAD_CLAMP_TO_EDGE:
                retVal = sycl::clamp(i - lb, 0, len - 1);
                break;
            case FLY_PAD_PERIODIC: {
                int rem   = (i - lb) % len;
                bool cond = rem < 0;
                retVal    = cond * (rem + len) + (1 - cond) * rem;
            } break;
            default: retVal = 0; break;  // FLY_PAD_ZERO
        }
        return retVal;
    }
        ...
        switch (btype) {
            case FLY_PAD_ZERO:
                h.parallel_for(
                    ...
                    padBordersKernel<T, FLY_PAD_ZERO>(
            ...
            case FLY_PAD_SYM:
                h.parallel_for(
                    sycl::nd_range{global, local},
                    padBordersKernel<T, FLY_PAD_SYM>(
                        oData, out.info, iData, in.info, lBoundPadding[0],
                        lBoundPadding[1], lBoundPadding[2], lBoundPadding[3],
                        groups_x, groups_y));
                break;
            case FLY_PAD_CLAMP_TO_EDGE:
                h.parallel_for(
                    sycl::nd_range{global, local},
                    padBordersKernel<T, FLY_PAD_CLAMP_TO_EDGE>(
                        oData, out.info, iData, in.info, lBoundPadding[0],
                        lBoundPadding[1], lBoundPadding[2], lBoundPadding[3],
                        groups_x, groups_y));
                break;
            case FLY_PAD_PERIODIC:
                h.parallel_for(
                    sycl::nd_range{global, local},
                    padBordersKernel<T, FLY_PAD_PERIODIC>(
                        oData, out.info, iData, in.info, lBoundPadding[0],
                        lBoundPadding[1], lBoundPadding[2], lBoundPadding[3],
                        groups_x, groups_y));
                break;
