...
struct Interp1<Ty, Tp, 1> {
    void operator()(write_accessor<Ty> out, KParam oInfo, int ooff,
                    ...
                    int xdim, fly::interpType method, int batch, bool clamp,
                    int batch_dim = 1) {
        Ty zero = scalar<Ty>(0);
        ...
        int xid = (method == FLY_INTERP_LOWER ? sycl::floor(x) : sycl::round(x));
...
struct Interp2<Ty, Tp, 1> {
    void operator()(write_accessor<Ty> out, KParam oInfo, int ooff,
                    ...
                    int xdim, int ydim, fly::interpType method, int batch,
                    bool clamp, int batch_dim = 2) {
        int xid = (method == FLY_INTERP_LOWER ? sycl::floor(x) : sycl::round(x));
        int yid = (method == FLY_INTERP_LOWER ? sycl::floor(y) : sycl::round(y));
