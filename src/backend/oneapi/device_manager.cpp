...
auto arrayfire_exception_handler(sycl::exception_list exceptions) {
    for (std::exception_ptr const& e : exceptions) {
        try {
            ...
            FLY_ERROR(ex.what(), FLY_ERR_INTERNAL);
        }
    }
}
...
DeviceManager::DeviceManager()
    ...
    FLY_TRACE("Found {} sycl devices", nDevices);

    if (nDevices == 0) { FLY_ERROR("No sycl devices found", FLY_ERR_RUNTIME); }
    ...
    string deviceENV        = getEnvVar("FLY_ONEAPI_DEFAULT_DEVICE");
    ...
    deviceENV = getEnvVar("FLY_ONEAPI_DEFAULT_DEVICE_TYPE");
    if (!default_device_set && !deviceENV.empty()) {
        ...
        for (int i = 0; i < nDevices; i++) {
            if (mDevices[i]->get_info<sycl::info::device::device_type>() ==
                default_device_type) {
                default_device_set = true;
                FLY_TRACE("Setting to first available {}({})", deviceENV, i);
                setActiveContext(i);
                break;
            }
        }
        if (!default_device_set) {
            FLY_TRACE(
                "FLY_ONEAPI_DEFAULT_DEVICE_TYPE={} \
