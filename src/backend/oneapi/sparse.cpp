...
#define P(exp) fly_print_array_gen(#exp, getHandle(exp), 2)
...
SparseArray<T> sparseConvertDenseToCOO(const Array<T> &in) {
    ...
    Array<int> rowIdx =
        arithOp<int, fly_mod_t>(nonZeroIdx, constDim, nonZeroIdx.dims());
    Array<int> colIdx =
        arithOp<int, fly_div_t>(nonZeroIdx, constDim, nonZeroIdx.dims());
...
SparseArray<T> sparseConvertDenseToStorage(const Array<T> &in_) {
    ...
    uint nNZ = getScalar<uint>(reduce_all<fly_notzero_t, T, uint>(in_));
...
Array<T> sparseConvertStorageToDense(const SparseArray<T> &in_) {
    if (stype != FLY_STORAGE_CSR) {
        FLY_ERROR("oneAPI Backend only supports CSR or COO to Dense",
...
SparseArray<T> sparseConvertStorageToStorage(const SparseArray<T> &in) {
    ...
    if (src == FLY_STORAGE_CSR && dest == FLY_STORAGE_COO) {
        ...
        FLY_ERROR("oneAPI Backend invalid conversion combination",
                 FLY_ERR_NOT_SUPPORTED);
    }
...
#define INSTANTIATE_COO_SPECIAL(T)                                 \
    template<>                                                     \
    SparseArray<T> sparseConvertDenseToStorage<T, FLY_STORAGE_COO>( \
        const Array<T> &in) {                                      \
        return sparseConvertDenseToCOO<T>(in);                     \
    }                                                              \
    template<>                                                     \
    Array<T> sparseConvertStorageToDense<T, FLY_STORAGE_COO>(       \
        const SparseArray<T> &in) {                                \
