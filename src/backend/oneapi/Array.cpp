...
template<>
void verifyTypeSupport<flare::common::half>() {
...
template<typename T>
Array<T>::Array(const fly::dim4 &dims, buffer<T> *const mem, size_t offset,
                bool copy)
    : info(getActiveDeviceId(), dims, 0, calcStrides(dims),
           static_cast<fly_dtype>(dtype_traits<T>::fly_type))
...
kJITHeuristics passesJitHeuristics(span<Node *> root_nodes) {
    ...
    // bool isIntel = platform == FLYCL_PLATFORM_INTEL;
...
template<typename T>
Array<T> createSubArray(const Array<T> &parent, const vector<fly_seq> &index,
                        bool copy) {
    parent.eval();
...
#define INSTANTIATE(T)                                                       \
    ...
    template Array<T> createSubArray<T>(                                     \
        const Array<T> &parent, const vector<fly_seq> &index, bool copy);     \
