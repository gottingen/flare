...
static void gemvDispatch(sycl::queue queue, oneapi::mkl::transpose lOpts,
                         ...
                         const T *alpha, const flare::oneapi::Array<T> &lhs,
                         dim_t lStride, const flare::oneapi::Array<T> &x,
                         dim_t incx, const T *beta,
                         flare::oneapi::Array<T> &out, dim_t oInc) {
    using Dt                   = flare::oneapi::data_t<T>;
    const fly::dim4 lStrides    = lhs.strides();
    const fly::dim4 xStrides    = x.strides();
    ...
    if constexpr (!std::is_same_v<T, flare::common::half>) {
...
static void gemmDispatch(sycl::queue queue, oneapi::mkl::transpose lOpts,
                         ...
                         const T *alpha, const flare::oneapi::Array<T> &lhs,
                         dim_t lStride, const flare::oneapi::Array<T> &rhs,
                         dim_t rStride, const T *beta,
                         flare::oneapi::Array<T> &out, dim_t oleading) {
    using Dt                = flare::oneapi::data_t<T>;
    const fly::dim4 lStrides = lhs.strides();

    const fly::dim4 rStrides    = rhs.strides();
    const fly::dim4 oStrides    = out.strides();
    sycl::buffer<Dt, 1> lhsBuf = lhs.template getBufferWithOffset<Dt>();
...
bool isStrideMonotonic(const fly::dim4 &dim) {
...
void gemm(Array<T> &out, fly_mat_prop optLhs, fly_mat_prop optRhs, const T *alpha,
    ...
    const auto aRowDim = (optLhs == FLY_MAT_NONE) ? 0 : 1;
    const auto aColDim = (optLhs == FLY_MAT_NONE) ? 1 : 0;
    const auto bColDim = (optRhs == FLY_MAT_NONE) ? 1 : 0;
    ...
    if (oDims.ndims() <= 2) {  // if non-batched
        if (rhs.dims()[bColDim] == 1) {
            if constexpr (std::is_same_v<T, flare::common::half>) {
                ...
                dim_t incr =
                    (optRhs == FLY_MAT_NONE) ? rStrides[0] : rStrides[1];
...
Array<T> dot(const Array<T> &lhs, const Array<T> &rhs, fly_mat_prop optLhs,
             fly_mat_prop optRhs) {
    auto lhs_ = (optLhs == FLY_MAT_NONE ? lhs : conj<T>(lhs));
    auto rhs_ = (optRhs == FLY_MAT_NONE ? rhs : conj<T>(rhs));
    auto temp = arithOp<T, fly_mul_t>(lhs_, rhs_, lhs_.dims());
    return reduce<fly_add_t, T, T>(temp, 0, false, 0);
}

#define INSTANTIATE_GEMM(TYPE)                                               \
    template void gemm<TYPE>(Array<TYPE> & out, fly_mat_prop optLhs,          \
                             fly_mat_prop optRhs, const TYPE *alpha,          \
...
#define INSTANTIATE_DOT(TYPE)                                                  \
    template Array<TYPE> dot<TYPE>(const Array<TYPE> &lhs,                     \
                                   const Array<TYPE> &rhs, fly_mat_prop optLhs, \
                                   fly_mat_prop optRhs);
