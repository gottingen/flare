...
using flare::common::int_version_to_string;
using flare::common::MemoryManagerBase;
using flare::common::unique_handle;
using flare::cuda::Allocator;
using flare::cuda::AllocatorPinned;
...
unique_handle<cudnnHandle_t> *nnManager(const int deviceId) {
    ...
    call_once(initFlags[deviceId], [handle, &error] {
        ...
        // Not throwing an FLY_ERROR here because we are in a lambda that could
    ...
    if (error) {
        ...
        FLY_ERROR(error_msg, FLY_ERR_RUNTIME);
    }
...
int getBackend() { return FLY_BACKEND_CUDA; }
...
string getDeviceInfo() noexcept {
    ostringstream info;
    info << "Flare v" << FLY_VERSION << " (CUDA, " << get_system()
         << ", build " << FLY_REVISION << ")\n";
...
int &getMaxJitSize() {
    ...
    if (length <= 0) {
        string env_var = getEnvVar("FLY_CUDA_MAX_JIT_LEN");
        if (!env_var.empty()) {
...
MemoryManagerBase &pinnedMemoryManager() {
    ...
    call_once(flag, [&]() {
        ...
        inst.pinnedMemManager = make_unique<common::DefaultMemoryManager>(
            1, common::MAX_BUFFERS, FLY_MEM_DEBUG || FLY_CUDA_MEM_DEBUG);
        // Set the memory manager's device memory manager
...
cudnnHandle_t nnHandle() {
    ...
    if (*handle) {
        ...
        FLY_ERROR("Error Initializing cuDNN\n", FLY_ERR_RUNTIME);
    }
}
#endif
...
fly_err flycu_get_stream(cudaStream_t *stream, int id) {
    try {
        *stream = flare::cuda::getStream(id);
    }
    CATCHALL;
    return FLY_SUCCESS;
}
...
fly_err flycu_get_native_id(int *nativeid, int id) {
    try {
        *nativeid = flare::cuda::getDeviceNativeId(id);
    }
    CATCHALL;
    return FLY_SUCCESS;
}
...
fly_err flycu_set_native_id(int nativeid) {
    try {
        flare::cuda::setDevice(
            flare::cuda::getDeviceIdFromNativeId(nativeid));
    }
    CATCHALL;
    return FLY_SUCCESS;
}
...
fly_err flycu_cublasSetMathMode(cublasMath_t mode) {
    try {
        CUBLAS_CHECK(cublasSetMathMode(flare::cuda::blasHandle(), mode));
    }
    CATCHALL;
    return FLY_SUCCESS;
}
