...
template<fly_flux_function FluxEnum>
__device__ float gradientUpdate(const float mct, const float C, const float S,
    ...
    if (FluxEnum == FLY_FLUX_EXPONENTIAL) {
        cx  = expf((df * df + 0.25f * flypowf(dy + 0.5f * (SE - NE), 2)) * mct);
        cxd = expf((db * db + 0.25f * flypowf(dy + 0.5f * (SW - NW), 2)) * mct);
    } else {
        cx =
            quadratic((df * df + 0.25f * flypowf(dy + 0.5f * (SE - NE), 2)) * mct);
        cxd =
            quadratic((db * db + 0.25f * flypowf(dy + 0.5f * (SW - NW), 2)) * mct);
    }
    delta += (cx * df - cxd * db);
    ...
    if (FluxEnum == FLY_FLUX_EXPONENTIAL) {
        cx  = expf((df * df + 0.25f * flypowf(dx + 0.5f * (SE - SW), 2)) * mct);
        cxd = expf((db * db + 0.25f * flypowf(dx + 0.5f * (NE - NW), 2)) * mct);
    } else {
        cx =
            quadratic((df * df + 0.25f * flypowf(dx + 0.5f * (SE - SW), 2)) * mct);
        cxd =
            quadratic((db * db + 0.25f * flypowf(dx + 0.5f * (NE - NW), 2)) * mct);
    }
    delta += (cx * df - cxd * db);
...
__device__ float curvatureUpdate(const float mct, const float C, const float S,
    ...
    gmsqf = (df * df + 0.25f * flypowf(dy + 0.5f * (SE - NE), 2));
    gmsqb = (db * db + 0.25f * flypowf(dy + 0.5f * (SW - NW), 2));
    ...
    gmsqf = (df * df + 0.25f * flypowf(dx + 0.5f * (SE - SW), 2));
    gmsqb = (db * db + 0.25f * flypowf(dx + 0.5f * (NE - NW), 2));
    gmf   = sqrtf(1.0e-10 + gmsqf);
    gmb   = sqrtf(1.0e-10 + gmsqb);
    ...
    if (delta > 0) {
        prop_grad +=
            (flypowf(fminf(db0, 0.0f), 2.0f) + flypowf(fmaxf(df0, 0.0f), 2.0f));
        prop_grad +=
            (flypowf(fminf(db, 0.0f), 2.0f) + flypowf(fmaxf(df, 0.0f), 2.0f));
    } else {
        prop_grad +=
            (flypowf(fmaxf(db0, 0.0f), 2.0f) + flypowf(fminf(df0, 0.0f), 2.0f));
        prop_grad +=
            (flypowf(fmaxf(db, 0.0f), 2.0f) + flypowf(fminf(df, 0.0f), 2.0f));
    }

    return sqrtf(prop_grad) * delta;
}

template<typename T, fly_flux_function FluxEnum, bool isMCDE>
