...
void memcopy(Param<T> out, CParam<T> in, dim_t indims) {
    ...
    const char *kernelName{th.loop0   ? "flare::cuda::memCopyLoop0"
                           : th.loop2 ? "flare::cuda::memCopyLoop123"
                           : th.loop1 ? th.loop3
                                            ? "flare::cuda::memCopyLoop13"
                                            : "flare::cuda::memCopyLoop1"
                           : th.loop3 ? "flare::cuda::memCopyLoop3"
                                      : "flare::cuda::memCopy"};
...
void copy(Param<outType> dst, CParam<inType> src, dim_t ondims,
    ...
    auto copy{common::getKernel(
        th.loop0                 ? "flare::cuda::scaledCopyLoop0"
        : (th.loop2 || th.loop3) ? "flare::cuda::scaledCopyLoop123"
        : th.loop1               ? "flare::cuda::scaledCopyLoop1"
                                 : "flare::cuda::scaledCopy",
        {{copy_cuh_src}},
