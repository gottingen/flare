...
Array<T> createInitialImage(CParam<T> img, const float init_sigma,
    ...
    if (double_input) {
        resize<T>(init_img, img, FLY_INTERP_BILINEAR);
    ...
    for (unsigned o = 0; o < n_octaves; o++) {
        ...
        for (unsigned l = 0; l < n_layers + 3; l++) {
            ...
            if (o == 0 && l == 0) {
                ...
                resize<T>(tmp_pyr[idx], tmp_pyr[src_idx], FLY_INTERP_BILINEAR);
            } else {
...
void update_permutation(thrust::device_ptr<T>& keys,
                        flare::cuda::ThrustVector<int>& permutation) {
    // temporary storage for keys
    flare::cuda::ThrustVector<T> temp(permutation.size());
...
void apply_permutation(thrust::device_ptr<T>& keys,
    ...
    flare::cuda::ThrustVector<T> temp(keys, keys + permutation.size());
...
void sift(unsigned* out_feat, unsigned* out_dlen, float** d_x, float** d_y,
    ...
    for (unsigned i = 0; i < n_octaves; i++) {
        ...
        flare::cuda::ThrustVector<int> permutation(interp_feat);
