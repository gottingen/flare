...
template<fly::borderType BType>
__device__ int idxByndEdge(const int i, const int lb, const int len) {
    uint retVal;
    switch (BType) {
        case FLY_PAD_SYM: retVal = trimIndex(i - lb, len); break;
        case FLY_PAD_CLAMP_TO_EDGE: retVal = clamp(i - lb, 0, len - 1); break;
...
template<typename T, fly::borderType BType>
