...
struct Interp1<Ty, Tp, xdim, 1> {
    __device__ void operator()(Param<Ty> out, int ooff, CParam<Ty> in, int ioff,
                               Tp x, fly::interpType method, int batch,
        ...
        int xid   = (method == FLY_INTERP_LOWER ? floor(x) : round(x));
...
struct Interp2<Ty, Tp, xdim, ydim, 1> {
    __device__ void operator()(Param<Ty> out, int ooff, CParam<Ty> in, int ioff,
                               Tp x, Tp y, fly::interpType method, int batch,
                               bool clamp, int batch_dim = 2) {
        int xid = (method == FLY_INTERP_LOWER ? floor(x) : round(x));
        int yid = (method == FLY_INTERP_LOWER ? floor(y) : round(y));
