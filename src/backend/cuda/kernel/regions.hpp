...
__device__ inline double fetch<double>(const int n,
                                       flare::cuda::Param<double> equiv_map,
                                       cudaTextureObject_t tex) {
...
__global__ static void initial_label(flare::cuda::Param<T> equiv_map,
                                     flare::cuda::CParam<char> bin) {
...
template<typename T, int n_per_thread>
__global__ static void final_relabel(flare::cuda::Param<T> equiv_map,
                                     flare::cuda::CParam<char> bin,
                                     const T* d_tmp) {
    const int base_x = (blockIdx.x * blockDim.x * n_per_thread) + threadIdx.x;
    const int base_y = (blockIdx.y * blockDim.y * n_per_thread) + threadIdx.y;
...
__device__ __inline__ static T relabel(const T a, const T b) {
    T aa = (a == 0) ? flare::cuda::maxval<T>() : a;
    T bb = (b == 0) ? flare::cuda::maxval<T>() : b;
    return min(aa, bb);
}
...
__global__ static void update_equiv(flare::cuda::Param<T> equiv_map,
...
void regions(flare::cuda::Param<T> out, flare::cuda::CParam<char> in,
             cudaTextureObject_t tex) {
    using flare::cuda::getActiveStream;
    ...
    auto tmp = flare::cuda::memAlloc<T>(size);
    ...
    flare::cuda::ThrustVector<T> labels(num_bins);
