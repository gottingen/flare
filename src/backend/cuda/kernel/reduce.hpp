...
template<typename Ti, typename To, fly_op_t op, uint dim, uint DIMY>
...
template<typename Ti, typename To, fly_op_t op, int dim>
...
void reduce_dim(Param<To> out, CParam<Ti> in, bool change_nan, double nanval) {
    ...
    if (blocks_dim[dim] > 1) {
        ...
        if (op == fly_notzero_t) {
            reduce_dim_launcher<To, To, fly_add_t, dim>(
...
template<typename Ti, typename To, fly_op_t op, uint DIMX>
...
void reduce_all_launcher(Param<To> out, CParam<Ti> in, const uint blocks_x,
    ...
    if (tmp_elements > UINT_MAX) {
        FLY_ERROR("Too many blocks requested (retirementCount == unsigned)",
                 FLY_ERR_RUNTIME);
    }
...
void reduce_first(Param<To> out, CParam<Ti> in, bool change_nan,
    ...
    if (blocks_x > 1) {
        ...
        if (op == fly_notzero_t) {
            reduce_first_launcher<To, To, fly_add_t>(
