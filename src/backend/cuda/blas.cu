...
using flare::common::kernel_type;
using std::is_same;
using std::vector;
...
cublasOperation_t toCblasTranspose(fly_mat_prop opt) {
    cublasOperation_t out = CUBLAS_OP_N;
    switch (opt) {
        case FLY_MAT_NONE: out = CUBLAS_OP_N; break;
        case FLY_MAT_TRANS: out = CUBLAS_OP_T; break;
        case FLY_MAT_CTRANS: out = CUBLAS_OP_C; break;
...
template<typename T>
void trsm(const Array<T> &lhs, Array<T> &rhs, fly_mat_prop trans, bool is_upper,
...
#define INSTANTIATE_TRSM(TYPE)                                               \
    template void trsm<TYPE>(const Array<TYPE> &lhs, Array<TYPE> &rhs,       \
                             fly_mat_prop trans, bool is_upper, bool is_left, \
                             bool is_unit);
