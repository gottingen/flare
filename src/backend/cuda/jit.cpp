...
using flare::common::findModule;
using flare::common::getEnvVar;
using flare::common::getFuncName;
using flare::common::half;
using flare::common::isBufferOrShift;
...
void evalNodes(vector<Param<T>>& outputs, const vector<Node*>& output_nodes) {
    ...
    for_each(
        ...
        [outDims, outStrides](Param<T>& output) {
            assert(equal(output.dims, output.dims + FLY_MAX_DIMS, outDims) &&
                   equal(output.strides, output.strides + FLY_MAX_DIMS,
                         outStrides));
        });
#endif
    ...
    try {
        ...
        const fly::dtype outputType{output_nodes[0]->getType()};
        ...
        if (moddimsFound | emptyColumnsFound) {
            ...
            if (moddimsFound) {
                const auto isModdim{[](const Node_ptr& node) {
                    return node->getOp() == fly_moddims_t;
                }};
