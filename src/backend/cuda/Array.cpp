...
using flare::common::NodeIterator;
...
void verifyTypeSupport() {
    if ((std::is_same<T, double>::value || std::is_same<T, cdouble>::value) &&
        ...
        FLY_ERROR("Double precision not supported", FLY_ERR_NO_DBL);
    } else if (std::is_same<T, common::half>::value &&
               !isHalfSupported(getActiveDeviceId())) {
        FLY_ERROR("Half precision not supported", FLY_ERR_NO_HALF);
    }
}
...
template<typename T>
Array<T>::Array(const fly::dim4 &dims)
...
template<typename T>
Array<T>::Array(const fly::dim4 &dims, const T *const in_data, bool is_device,
                bool copy_device)
    : info(getActiveDeviceId(), dims, 0, calcStrides(dims),
           static_cast<fly_dtype>(dtype_traits<T>::fly_type))
...
Array<T>::Array(Param<T> &tmp, bool owner_)
    : info(getActiveDeviceId(),
           fly::dim4(tmp.dims[0], tmp.dims[1], tmp.dims[2], tmp.dims[3]), 0,
           fly::dim4(tmp.strides[0], tmp.strides[1], tmp.strides[2],
                    tmp.strides[3]),
           static_cast<fly_dtype>(dtype_traits<T>::fly_type))
    , data(tmp.ptr, owner_ ? std::function<void(T *)>(memFree)
                           : std::function<void(T *)>([](T * /*unused*/) {}))
    , data_dims(fly::dim4(tmp.dims[0], tmp.dims[1], tmp.dims[2], tmp.dims[3]))
    , node()
    , owner(owner_) {}

template<typename T>
Array<T>::Array(const fly::dim4 &dims, common::Node_ptr n)
    : info(getActiveDeviceId(), dims, 0, calcStrides(dims),
           static_cast<fly_dtype>(dtype_traits<T>::fly_type))
    , data()
    , data_dims(dims)
...
template<typename T>
Array<T>::Array(const fly::dim4 &dims, const fly::dim4 &strides, dim_t offset_,
...
kJITHeuristics passesJitHeuristics(span<Node *> root_nodes) {
    ...
    if (atHeightLimit || getMemoryPressure() >= getMemoryPressureThreshold()) {
        ...
        if (jitTreeExceedsMemoryPressure(info.total_buffer_size)) {
            FLY_TRACE("JIT tree evaluated because of memory pressure: {}",
...
Array<T> createSubArray(const Array<T> &parent,
                        const std::vector<fly_seq> &index, bool copy) {
    parent.eval();
...
#define INSTANTIATE(T)                                                        \
    ...
    template Array<T> createSubArray<T>(                                      \
        const Array<T> &parent, const std::vector<fly_seq> &index, bool copy); \
    template void destroyArray<T>(Array<T> * A);                              \
    template Array<T> createNodeArray<T>(const dim4 &size,                    \
                                         common::Node_ptr node);              \
    template Array<T>::Array(const fly::dim4 &dims, const fly::dim4 &strides,   \
                             dim_t offset, const T *const in_data,            \
                             bool is_device);                                 \
    template Array<T>::Array(const fly::dim4 &dims, const T *const in_data,    \
