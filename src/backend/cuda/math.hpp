...
template<typename T>
inline T maxval() {
    FLY_IF_CONSTEXPR(std::is_floating_point<T>::value && !fast_math) {
...
/// is used
__device__ inline double flypowf(double x, double y) { return pow(x, y); }
#else
/// The pow function with fast math is constantly wrong with fast math
/// so this function converts the operation to double when fast-math
/// is used
__device__ inline float flypowf(float x, float y) { return powf(x, y); }
#endif

}  // namespace cuda
}  // namespace flare

__SDH__ bool operator==(flare::cuda::cfloat a, flare::cuda::cfloat b) {
    return (a.x == b.x) && (a.y == b.y);
}
__SDH__ bool operator!=(flare::cuda::cfloat a, flare::cuda::cfloat b) {
    return !(a == b);
}
__SDH__ bool operator==(flare::cuda::cdouble a,
                        flare::cuda::cdouble b) {
    return (a.x == b.x) && (a.y == b.y);
}
__SDH__ bool operator!=(flare::cuda::cdouble a,
                        flare::cuda::cdouble b) {
