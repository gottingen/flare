...
using flare::common::makeTempFilename;
using flare::common::removeFile;
using flare::common::renameFile;
using flare::cuda::getComputeCapability;
using flare::cuda::getDeviceProp;
using detail::Module;
using nonstd::span;
...
#define CU_LINK_CHECK(fn)                                               \
    do {                                                                \
        ...
        snprintf(cu_err_msg.data(), cu_err_msg.size(),                  \
                 "CU Link Error %s(%d): %s\n", cu_err_name, (int)(res), \
                 linkError);                                            \
        FLY_ERROR(cu_err_msg.data(), FLY_ERR_INTERNAL);                   \
    } while (0)
...
string getKernelCacheFilename(const int device, const string &key) {
    ...
    return "KER" + key + "_CU_" + computeVersion + "_FLY_" +
           to_string(FLY_API_VERSION_CURRENT) + ".bin";
}
...
Module compileModule(const string &moduleKey, span<const string> sources,
    ...
    if (sourceIsJIT) {
        ...
        constexpr static const char *includeNames[] = {
            ...
            "stdbool.h",       // DUMMY ENTRY TO SATISFY fly/defines.h inclusion
            "stdlib.h",        // DUMMY ENTRY TO SATISFY fly/defines.h inclusion
        ...
        static const array<string, numHeaders> sourceStrings = {{
            ...
            string(""),  // DUMMY ENTRY TO SATISFY fly/defines.h inclusion
            string(""),  // DUMMY ENTRY TO SATISFY fly/defines.h inclusion
            string(""),  // DUMMY ENTRY TO SATISFY cuComplex_h inclusion
...
#ifdef FLY_CACHE_KERNELS_TO_DISK
    // save kernel in cache
    const string &cacheDirectory = getCacheDirectory();
    if (!cacheDirectory.empty()) {
        const string cacheFile = cacheDirectory + FLY_PATH_SEPARATOR +
                                 getKernelCacheFilename(device, moduleKey);
        const string tempFile =
            cacheDirectory + FLY_PATH_SEPARATOR + makeTempFilename();
        try {
            ...
            FLY_TRACE("{{{:<30} : failed saving binary to {} for {}, {}}}",
    ...
    FLY_TRACE("{{ {:<20} : compile:{:>5} ms, link:{:>4} ms, {{ {} }}, {} }}",
             moduleKey,
...
Module loadModuleFromDisk(const int device, const string &moduleKey,
    ...
    try {
        ...
        if (!in) {
            FLY_TRACE("{{{:<20} : Unable to open {} for {}}}", moduleKey,
        ...
        if (recomputedHash != cubinHash) {
            FLY_ERROR("Module on disk seems to be corrupted", FLY_ERR_LOAD_SYM);
        }

        CU_CHECK(cuModuleLoadData(&modOut, cubin.data()));

        FLY_TRACE("{{{:<20} : loaded from {} for {} }}", moduleKey, cacheFile,
                 getDeviceProp(device).name);

        retVal.set(modOut);
    } catch (const std::ios_base::failure &e) {
        FLY_TRACE("{{{:<20} : Unable to read {} for {}}}", moduleKey, cacheFile,
                 getDeviceProp(device).name);
        removeFile(cacheFile);
    } catch (const AfError &e) {
        if (e.getError() == FLY_ERR_LOAD_SYM) {
            FLY_TRACE(
...
flare::cuda::Kernel getKernel(const Module &mod, const string &nameExpr,
