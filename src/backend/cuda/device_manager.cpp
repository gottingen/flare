...
#include <cublas_v2.h>  // needed for fly/cuda.h
#include <driver.h>
        ...
        if (computeCapability.second > minorVersion) {
            ...
            clog::get("platform")
                ->warn(
                    ...
                    "Flare to avoid this warning. Using {}.{} for JIT "
                    "compilation kernels.",
        ...
        if (computeCapability.second > minorVersion) {
            ...
            clog::get("platform")
                ->warn(
                    ...
                    "capability supported by Flare's CUDA runtime({}.{}). "
                    "Please create an issue or a pull request on the Flare "
...
void DeviceManager::resetMemoryManager() {
    ...
    std::unique_ptr<MemoryManagerBase> mgr(
        new common::DefaultMemoryManager(getDeviceCount(), common::MAX_BUFFERS,
                                         FLY_MEM_DEBUG || FLY_CUDA_MEM_DEBUG));
...
void debugRuntimeCheck(clog::logger *logger, int runtime_version,
    ...
    if (runtime_it == end(CudaToDriverVersion)) {
        ...
        const char *err_msg =
            ...
            "or a pull request on the Flare repository to update the "
    ...
    if (driver_it == end(CudaToDriverVersion)) {
        FLY_TRACE(
            ...
            "array. Please create an issue or a pull request on the Flare "
...
void DeviceManager::checkCudaVsDriverVersion() {
    ...
    FLY_TRACE("CUDA Driver supports up to CUDA {} Flare CUDA Runtime {}",
    ...
    if (runtime > driver) {
        ...
        if (runtime_it == end(CudaToDriverVersion)) {
            ...
            char err_msg[] =
                ...
                "issue or a pull request on the Flare repository to "
...
DeviceManager::DeviceManager()
    ...
    try {
        ...
        FLY_TRACE("Found {} CUDA devices", nDevices);
        if (nDevices == 0) {
            FLY_ERROR("No CUDA capable devices found", FLY_ERR_DRIVER);
            return;
        }
        ...
        for (int i = 0; i < nDevices; i++) {
            ...
            if (dev.prop.major < getMinSupportedCompute(cudaMajorVer)) {
                FLY_TRACE("Unsuppored device: {}", dev.prop.name);
                continue;
            } else {
        ...
        // into a more informative Flare exception.
        if (err.getError() == FLY_ERR_INTERNAL) {
            FLY_ERROR(
        ...
        for (int j = 0; j < nDevices; j++) {
            if (i != j) {
                ...
                if (can_access_peer) {
                    ...
                    FLY_TRACE("Peer access enabled for {}({}) and {}({})", i,
    ...
    std::string deviceENV = getEnvVar("FLY_CUDA_DEFAULT_DEVICE");
    FLY_TRACE("FLY_CUDA_DEFAULT_DEVICE: {}", deviceENV);
    if (deviceENV.empty()) {
        ...
        if (def_device < 0 || def_device >= nDevices) {
            getLogger()->warn(
                "FLY_CUDA_DEFAULT_DEVICE({}) out of range. Setting default "
                "device to 0.",
                def_device);
            setActiveDevice(0, cuDevices[0].nativeId);
        } else {
