...
Array<T> generalSolveBatched(const Array<T> &a, const Array<T> &b) {
    ...
    if (M != N) {
        FLY_ERROR("Batched solve requires square matrices", FLY_ERR_ARG);
    }
    ...
    Array<int> pivots = createEmptyArray<int>(fly::dim4(N, batch, 1, 1));
    Array<int> info   = createEmptyArray<int>(fly::dim4(batch, 1, 1, 1));
...
Array<T> leastSquares(const Array<T> &a, const Array<T> &b) {
    ...
    if (M < N) {
        ...
        trsm<T>(A, B, FLY_MAT_CTRANS, true, true, false);
        ...
        B.resetDims(dim4(N, K));
        trsm(A, B, FLY_MAT_NONE, true, true, false);
    }
    return B;
}
...
Array<T> triangleSolve(const Array<T> &A, const Array<T> &b,
    ...
    Array<T> B = copyArray<T>(b);
    trsm(A, B,
         FLY_MAT_NONE,  // transpose flag
         options & FLY_MAT_UPPER ? true : false,
         true,  // is_left
         options & FLY_MAT_DIAG_UNIT ? true : false);
    return B;
}
