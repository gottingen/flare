...
fly_err fly_constant(fly_array *result, const double value, const unsigned ndims,
    ...
    try {
        ...
        if (ndims <= 0) { return fly_create_handle(result, 0, nullptr, type); }
...
static inline fly_array createCplx(dim4 dims, const Ti real, const Ti imag) {
    To cval      = scalar<To, Ti>(real, imag);
    fly_array out = getHandle(createValueArray<To>(dims, cval));
    return out;
}

fly_err fly_constant_complex(fly_array *result, const double real,
                           const double imag, const unsigned ndims,
                           const dim_t *const dims, fly_dtype type) {
    try {
        fly_array out;
        FLY_CHECK(fly_init());
...
fly_err fly_constant_long(fly_array *result, const intl val, const unsigned ndims,
                        const dim_t *const dims) {
    try {
        fly_array out;
        FLY_CHECK(fly_init());

        if (ndims <= 0) { return fly_create_handle(result, 0, nullptr, s64); }
        dim4 d = verifyDims(ndims, dims);
...
fly_err fly_constant_ulong(fly_array *result, const uintl val,
                         const unsigned ndims, const dim_t *const dims) {
    try {
        fly_array out;
        FLY_CHECK(fly_init());

        if (ndims <= 0) { return fly_create_handle(result, 0, nullptr, u64); }
        dim4 d = verifyDims(ndims, dims);
...
template<typename T>
static inline fly_array identity_(const fly::dim4 &dims) {
...
fly_err fly_identity(fly_array *out, const unsigned ndims, const dim_t *const dims,
    ...
    try {
        ...
        if (ndims == 0) { return fly_create_handle(out, 0, nullptr, type); }
...
template<typename T>
static inline fly_array range_(const dim4 &d, const int seq_dim) {
    return getHandle(range<T>(d, seq_dim));
}

// Strong Exception Guarantee
fly_err fly_range(fly_array *result, const unsigned ndims, const dim_t *const dims,
...
template<typename T>
static inline fly_array iota_(const dim4 &dims, const dim4 &tile_dims) {
    return getHandle(iota<T>(dims, tile_dims));
}

// Strong Exception Guarantee
fly_err fly_iota(fly_array *result, const unsigned ndims, const dim_t *const dims,
...
template<typename T>
static inline fly_array diagCreate(const fly_array in, const int num) {
    return getHandle(diagCreate<T>(getArray<T>(in), num));
}

template<typename T>
static inline fly_array diagExtract(const fly_array in, const int num) {
...
template<typename T>
inline fly_array triangle(const fly_array in, const bool is_upper,
...
template<typename T>
inline fly_array pad(const fly_array in, const dim4 &lPad, const dim4 &uPad,
                    const fly::borderType ptype) {
    return getHandle(padArrayBorders<T>(getArray<T>(in), lPad, uPad, ptype));
}

fly_err fly_pad(fly_array *out, const fly_array in, const unsigned begin_ndims,
              const dim_t *const begin_dims, const unsigned end_ndims,
              const dim_t *const end_dims, const fly_border_type pad_type) {
    try {
        ...
        ARG_ASSERT(6, (pad_type >= FLY_PAD_ZERO && pad_type <= FLY_PAD_PERIODIC));
        ...
        for (unsigned i = begin_ndims; i < FLY_MAX_DIMS; i++) { lPad[i] = 0; }
        for (unsigned i = end_ndims; i < FLY_MAX_DIMS; i++) { uPad[i] = 0; }
