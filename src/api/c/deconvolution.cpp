...
Array<T> complexNorm(const Array<CT>& input) {
    ...
    return arithOp<T, fly_mul_t>(mag, mag, input.dims());
}

std::vector<fly_seq> calcPadInfo(dim4& inLPad, dim4& psfLPad, dim4& inUPad,
                                dim4& psfUPad, dim4& odims, dim_t nElems,
                                const dim4& idims, const dim4& fdims) {
    vector<fly_seq> index(4);
...
void richardsonLucy(Array<T>& currentEstimate, const Array<T>& in,
    ...
    for (unsigned i = 0; i < iters; ++i) {
        ...
        auto cmul1 = arithOp<CT, fly_mul_t>(fft1, P, P.dims());
        auto ifft1 = fft_c2r<CT, T>(cmul1, normFactor, odims, BASE_DIM);
        auto div1  = arithOp<T, fly_div_t>(in, ifft1, in.dims());
        auto fft2  = fft_r2c<CT, T>(div1, BASE_DIM);
        auto cmul2 = arithOp<CT, fly_mul_t>(fft2, Pc, Pc.dims());
        auto ifft2 = fft_c2r<CT, T>(cmul2, normFactor, odims, BASE_DIM);

        currentEstimate =
            arithOp<T, fly_mul_t>(currentEstimate, ifft2, ifft2.dims());
    }
}
...
void landweber(Array<T>& currentEstimate, const Array<T>& in,
    ...
    auto prod     = arithOp<T, fly_mul_t>(alpha, Pn, dims);
    auto lhsFac   = arithOp<T, fly_sub_t>(ONE, prod, dims);
    auto lhs      = cast<CT>(lhsFac);
    auto rhsFac   = arithOp<CT, fly_mul_t>(Pc, I, dims);
    auto rhs      = arithOp<CT, fly_mul_t>(rhsFac, alphaC, dims);
    auto iterTemp = I;

    for (unsigned i = 0; i < iters; ++i) {
        auto mul = arithOp<CT, fly_mul_t>(iterTemp, lhs, dims);
        iterTemp = arithOp<CT, fly_add_t>(mul, rhs, dims);
    }
    currentEstimate = fft_c2r<CT, T>(iterTemp, normFactor, odims, BASE_DIM);
}

template<typename InputType, typename RealType = float>
fly_array iterDeconv(const fly_array in, const fly_array ker, const uint iters,
                    const float rfactor, const fly_iterative_deconv_algo algo) {
    using T    = RealType;
    ...
    auto index = calcPadInfo(inLPad, psfLPad, inUPad, psfUPad, odims, nElems,
                             idims, fdims);
    auto paddedIn =
        padArrayBorders<T>(input, inLPad, inUPad, FLY_PAD_CLAMP_TO_EDGE);
    auto paddedPsf = padArrayBorders<T>(psf, psfLPad, psfUPad, FLY_PAD_ZERO);
    ...
    switch (algo) {
        case FLY_ITERATIVE_DECONV_RICHARDSONLUCY:
            richardsonLucy(currentEstimate, paddedIn, P, Pc, iters, normFactor,
                           odims);
            break;
        case FLY_ITERATIVE_DECONV_LANDWEBER:
        default:
...
fly_err fly_iterative_deconv(fly_array* out, const fly_array in, const fly_array ker,
    ...
    try {
        ...
        ARG_ASSERT(6, (algo == FLY_ITERATIVE_DECONV_DEFAULT ||
                       algo == FLY_ITERATIVE_DECONV_LANDWEBER ||
                       algo == FLY_ITERATIVE_DECONV_RICHARDSONLUCY));
        fly_array res   = 0;
        unsigned iters = iterations;
        float rfac     = relax_factor;

        fly_dtype inputType = inputInfo.getType();
        switch (inputType) {
            case f32:
...
Array<CT> denominator(const Array<CT>& I, const Array<CT>& P, const float gamma,
    ...
    using T = typename fly::dtype_traits<CT>::base_type;

    auto RCNST = createValueArray(I.dims(), scalar<T>(gamma));

    if (algo == FLY_INVERSE_DECONV_TIKHONOV) {
        auto normP = complexNorm<T, CT>(P);
        auto denom = arithOp<T, fly_add_t>(normP, RCNST, normP.dims());
        ...
        auto sRes  = arithOp<T, fly_sub_t>(normI, RCNST, normI.dims());
        auto dRes  = arithOp<T, fly_div_t>(RCNST, sRes, RCNST.dims());
        auto normP = complexNorm<T, CT>(P);
        auto denom = arithOp<T, fly_add_t>(normP, dRes, normP.dims());
...
template<typename InputType, typename RealType = float>
fly_array invDeconv(const fly_array in, const fly_array ker, const float gamma,
                   const fly_inverse_deconv_algo algo) {
    using T    = RealType;
    using CT   = typename std::conditional<std::is_same<T, double>::value,
                                         cdouble, cfloat>::type;
    ...
    auto P      = fft_r2c<CT, T>(shiftedPsf, BASE_DIM);
    auto Pc     = conj(P);
    auto numer  = arithOp<CT, fly_mul_t>(I, Pc, I.dims());
    auto denom  = denominator(I, P, gamma, algo);
    auto absVal = detail::abs<T, CT>(denom);
    auto THRESH = createValueArray(I.dims(), scalar<T>(gamma));
    auto cond   = logicOp<T, fly_ge_t>(absVal, THRESH, absVal.dims());
    auto val    = arithOp<CT, fly_div_t>(numer, denom, numer.dims());
...
fly_err fly_inverse_deconv(fly_array* out, const fly_array in, const fly_array psf,
    ...
    try {
        ...
        ARG_ASSERT(5, (algo == FLY_INVERSE_DECONV_DEFAULT ||
                       algo == FLY_INVERSE_DECONV_TIKHONOV));
        fly_array res = 0;
