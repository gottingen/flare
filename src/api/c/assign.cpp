...
using flare::common::if_complex;
using flare::common::if_real;
...
static void assign(Array<Tout>& out, const vector<fly_seq> seqs,
                   const Array<Tin>& in) {
    ...
    if (isVec) {
        ...
        Array<Tin> in_ = in.elements() == 1 ? flare::common::tile(in, oDims)
                                            : modDims(in, oDims);
...
template<typename T>
static if_complex<T> assign(Array<T>& out, const vector<fly_seq> iv,
                            const fly_array& in) {
...
template<typename T>
static if_real<T> assign(Array<T>& out, const vector<fly_seq> iv,
                         const fly_array& in) {
    const ArrayInfo& iInfo = getInfo(in);
    fly_dtype iType         = iInfo.getType();
...
fly_err fly_assign_seq(fly_array* out, const fly_array lhs, const unsigned ndims,
                     const fly_seq* index, const fly_array rhs) {
    try {
        ...
        if (ndims == 1 && ndims != lInfo.ndims()) {
            fly_array tmp_in;
            fly_array tmp_out;
            FLY_CHECK(fly_flat(&tmp_in, lhs));
            FLY_CHECK(fly_assign_seq(&tmp_out, tmp_in, ndims, index, rhs));
            FLY_CHECK(
                fly_moddims(out, tmp_out, lInfo.ndims(), lInfo.dims().get()));
            FLY_CHECK(fly_release_array(tmp_in));
            ...
            if (tmp_in != tmp_out) { FLY_CHECK(fly_release_array(tmp_out)); }
            return FLY_SUCCESS;
        }
        ...
        if (*out != lhs) {
            int count = 0;
            FLY_CHECK(fly_get_data_ref_count(&count, lhs));
            if (count > 1) {
                FLY_CHECK(fly_copy_array(&res, lhs));
            } else {
                res = retain(lhs);
            }
        } else {
        ...
        try {
            if (lhs != rhs) {
                ...
                vector<fly_seq> inSeqs(ndims, fly_span);
...
template<typename T>
inline void genAssign(fly_array& out, const fly_index_t* indexs,
                      const fly_array& rhs) {
...
fly_err fly_assign_gen(fly_array* out, const fly_array lhs, const dim_t ndims,
                     const fly_index_t* indexs, const fly_array rhs_) {
    try {
        ARG_ASSERT(2, (ndims > 0 && ndims <= FLY_MAX_DIMS));
        ARG_ASSERT(3, (indexs != NULL));

        int track = 0;
        vector<fly_seq> seqs(FLY_MAX_DIMS, fly_span);
        ...
        if (track == static_cast<int>(ndims)) {
            // all indexs are sequences, redirecting to fly_assign
            return fly_assign_seq(out, lhs, ndims, seqs.data(), rhs);
        }
        ...
        fly_dtype lhsType       = lInfo.getType();
        fly_dtype rhsType       = rInfo.getType();

        if (rhsDims.ndims() == 0) { return fly_retain_array(out, lhs); }

        if (lhsDims.ndims() == 0) {
            return fly_create_handle(out, 0, nullptr, lhsType);
        }
        ...
        if (ndims == 1 && ndims != static_cast<dim_t>(lInfo.ndims())) {
            ...
            fly_array tmp_out = 0;
            FLY_CHECK(fly_flat(&tmp_in, lhs));
            FLY_CHECK(fly_assign_gen(&tmp_out, tmp_in, ndims, indexs, rhs_));
            FLY_CHECK(
                fly_moddims(out, tmp_out, lInfo.ndims(), lInfo.dims().get()));
            FLY_CHECK(fly_release_array(tmp_in));
            // This can run into a double free issue if tmp_in == tmp_out
        ...
        if (*out != lhs) {
            ...
            FLY_CHECK(fly_get_data_ref_count(&count, lhs));
            if (count > 1) {
                FLY_CHECK(fly_copy_array(&output, lhs));
            } else {
                output = retain(lhs);
            }
        } else {
        ...
        // if fly_array are indexs along any
        ...
        if (isVec) {
            ...
            if (rInfo.elements() == 1) {
                FLY_CHECK(fly_tile(&rhs, rhs_, oDims[0], oDims[1], oDims[2],
                                 oDims[3]));
            } else {
                // If both out and rhs are vectors of equal
                // elements, reshape rhs to out dims
                FLY_CHECK(fly_moddims(&rhs, rhs_, oDims.ndims(), oDims.get()));
            }
        } else {
            for (int i = 0; i < FLY_MAX_DIMS; i++) {
                if (oDims[i] != rhsDims[i]) {
                    FLY_ERROR("Size mismatch between input and output",
                             FLY_ERR_SIZE);
                }
            }
        }
        ...
        try {
            ...
            if (*out != lhs) {
                FLY_CHECK(fly_release_array(output));
                if (isVec) { FLY_CHECK(fly_release_array(rhs)); }
            }
            throw;
        }
        if (isVec) { FLY_CHECK(fly_release_array(rhs)); }
