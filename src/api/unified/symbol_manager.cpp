...
string getBkndLibName(const fly_backend backend) {
    ...
    switch (backend) {
        case FLY_BACKEND_CUDA:
            ...
            break;
        case FLY_BACKEND_OPENCL:
            ret = string(LIB_FLY_BKND_PREFIX) + "flyopencl" + LIB_FLY_BKND_SUFFIX;
            break;
        case FLY_BACKEND_CPU:
            ret = string(LIB_FLY_BKND_PREFIX) + "flycpu" + LIB_FLY_BKND_SUFFIX;
            break;
        case FLY_BACKEND_ONEAPI:
            ret = string(LIB_FLY_BKND_PREFIX) + "flyoneapi" + LIB_FLY_BKND_SUFFIX;
            break;
...
string getBackendDirectoryName(const fly_backend backend) {
    ...
    switch (backend) {
        ...
        case FLY_BACKEND_OPENCL: ret = "opencl"; break;
        case FLY_BACKEND_CPU: ret = "cpu"; break;
        case FLY_BACKEND_ONEAPI: ret = "oneapi"; break;
        default: assert(1 != 1 && "Invalid backend");
    }
    return ret;
}
...
FlySymbolManager::FlySymbolManager()
    ...
    static const fly_backend order[] = {FLY_BACKEND_CUDA, FLY_BACKEND_ONEAPI,
                                       FLY_BACKEND_OPENCL, FLY_BACKEND_CPU};
