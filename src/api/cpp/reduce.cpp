...
array sum(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_sum(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}

array sum(const array &in, const int dim, const double nanval) {
    fly_array out = 0;
    FLY_THROW(fly_sum_nan(&out, in.get(), dim, nanval));
    return array(out);
}

void sumByKey(array &keys_out, array &vals_out, const array &keys,
              const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_sum_by_key(&okeys, &ovals, keys.get(), vals.get(),
...
void sumByKey(array &keys_out, array &vals_out, const array &keys,
    ...
    fly_array okeys, ovals;
    FLY_THROW(
        fly_sum_by_key_nan(&okeys, &ovals, keys.get(), vals.get(), dim, nanval));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
array product(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_product(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}

array product(const array &in, const int dim, const double nanval) {
    fly_array out = 0;
    FLY_THROW(fly_product_nan(&out, in.get(), dim, nanval));
    return array(out);
}
...
void productByKey(array &keys_out, array &vals_out, const array &keys,
                  const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_product_by_key(&okeys, &ovals, keys.get(), vals.get(),
                               getFNSD(dim, vals.dims())));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
void productByKey(array &keys_out, array &vals_out, const array &keys,
                  const array &vals, const int dim, const double nanval) {
    fly_array okeys, ovals;
    FLY_THROW(fly_product_by_key_nan(&okeys, &ovals, keys.get(), vals.get(), dim,
                                   nanval));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
array min(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_min(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}
...
void minByKey(array &keys_out, array &vals_out, const array &keys,
              const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_min_by_key(&okeys, &ovals, keys.get(), vals.get(),
                           getFNSD(dim, vals.dims())));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
array max(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_max(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}
...
void maxByKey(array &keys_out, array &vals_out, const array &keys,
              const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_max_by_key(&okeys, &ovals, keys.get(), vals.get(),
                           getFNSD(dim, vals.dims())));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
void max(array &val, array &idx, const array &in, const array &ragged_len,
         const int dim) {
    fly_array oval, oidx;
    FLY_THROW(fly_max_ragged(&oval, &oidx, in.get(), ragged_len.get(), dim));
    val = array(oval);
    idx = array(oidx);
}
...
array allTrue(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_all_true(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}
...
void allTrueByKey(array &keys_out, array &vals_out, const array &keys,
                  const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_all_true_by_key(&okeys, &ovals, keys.get(), vals.get(),
                                getFNSD(dim, vals.dims())));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
array anyTrue(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_any_true(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}
...
void anyTrueByKey(array &keys_out, array &vals_out, const array &keys,
                  const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_any_true_by_key(&okeys, &ovals, keys.get(), vals.get(),
                                getFNSD(dim, vals.dims())));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
array count(const array &in, const int dim) {
    fly_array out = 0;
    FLY_THROW(fly_count(&out, in.get(), getFNSD(dim, in.dims())));
    return array(out);
}
...
void countByKey(array &keys_out, array &vals_out, const array &keys,
                const array &vals, const int dim) {
    fly_array okeys, ovals;
    FLY_THROW(fly_count_by_key(&okeys, &ovals, keys.get(), vals.get(),
                             getFNSD(dim, vals.dims())));
    keys_out = array(okeys);
    vals_out = array(ovals);
}
...
void min(array &val, array &idx, const array &in, const int dim) {
    fly_array out = 0;
    fly_array loc = 0;
    FLY_THROW(fly_imin(&out, &loc, in.get(), getFNSD(dim, in.dims())));
    val = array(out);
    idx = array(loc);
}
...
void max(array &val, array &idx, const array &in, const int dim) {
    fly_array out = 0;
    fly_array loc = 0;
    FLY_THROW(fly_imax(&out, &loc, in.get(), getFNSD(dim, in.dims())));
    val = array(out);
    idx = array(loc);
}
...
#define INSTANTIATE(fnC, fnCPP)                      \
    ...
    INSTANTIATE_CPLX(fnC, fnCPP, fly_cfloat, float)   \
    INSTANTIATE_CPLX(fnC, fnCPP, fly_cdouble, double)

#define INSTANTIATE_REAL(fnC, fnCPP, T)                   \
    template<>                                            \
    FLY_API T fnCPP(const array &in) {                      \
        double rval, ival;                                \
        FLY_THROW(fly_##fnC##_all(&rval, &ival, in.get())); \
        return (T)(rval);                                 \
    }
...
#define INSTANTIATE_ARRAY(fnC, fnCPP)                   \
    template<>                                          \
    FLY_API fly::array fnCPP(const array &in) {            \
        fly_array out = 0;                               \
        FLY_THROW(fly_##fnC##_all_array(&out, in.get())); \
        return array(out);                              \
    }
...
#define INSTANTIATE_REAL(fnC, fnCPP, T)                           \
    template<>                                                    \
    FLY_API T fnCPP(const array &in, const double nanval) {         \
        double rval, ival;                                        \
        FLY_THROW(fly_##fnC##_all(&rval, &ival, in.get(), nanval)); \
        return (T)(rval);                                         \
    }
...
#define INSTANTIATE_ARRAY(fnC, fnCPP)                             \
    template<>                                                    \
    FLY_API fly::array fnCPP(const array &in, const double nanval) { \
        fly_array out = 0;                                         \
        FLY_THROW(fly_##fnC##_all_array(&out, in.get(), nanval));   \
        return array(out);                                        \
    }
...
#define INSTANTIATE_COMPAT(fnCPP, fnCompat, T) \
    template<>                                 \
    FLY_API T fnCompat(const array &in) {        \
        return fnCPP<T>(in);                   \
    }
...
#define INSTANTIATE(fnCPP, fnCompat)                        \
    ...
    INSTANTIATE_COMPAT(fnCPP, fnCompat, fly_cfloat)          \
    INSTANTIATE_COMPAT(fnCPP, fnCompat, fly_cdouble)         \
...
#define INSTANTIATE_REAL(fn, T)                                \
    template<>                                                 \
    FLY_API void fn(T *val, unsigned *idx, const array &in) {    \
        double rval, ival;                                     \
        FLY_THROW(fly_i##fn##_all(&rval, &ival, idx, in.get())); \
        *val = (T)(rval);                                      \
    }
...
#define INSTANTIATE(fn)                    \
    ...
    INSTANTIATE_CPLX(fn, fly_cfloat, float) \
    INSTANTIATE_CPLX(fn, fly_cdouble, double)
